/*
 * ----------------------------------------------------------------------------
 *  Copyright (c) 2011-2024 Qualcomm Technologies, Inc.  All Rights Reserved.
...
static void * rmt_storage_client_thread(void *data)
{
	...
	while(1) {
		...
		KMSGI("Calling %s",(client->xfer_dir == RMTFS_DIRECTION_WRITE_V01) ?"Write" : "Read");
		KMSGI("offset=%lu", xfer->sector_addr * (off_t)SECTOR_SIZE);
		KMSGI("size=%u", xfer->num_sector * SECTOR_SIZE);
		KMSGI("req_h 0x%x", (uint32_t)req_handle);
		KMSGI("wr_count %" PRIu64 "", write_counter);
		KMSGI("for %s!", client->path);
					...
					struct uio_mmap_info *mmap_info)
{
	...
	if(result != 0) {
		LOGE("Setting up client mmap buffer failed with result: %d\n", result);
		return -1;
	}
...
static int32_t rmts_server_mmap(void)
{
	...
	if(result != 0) {
		LOGE("Shared memory setup failed with result: %d\n", result);
		return -1;
	}
