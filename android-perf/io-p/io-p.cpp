...
int configure_preferred_apps() {
    ...
    int memTotal = -1;
    if (memInfo == NULL) {
        QLOGE(LOG_TAG, "Could not open /proc/meminfo ..\n");
        return -1;
    }
...
static void *uxe_server(void *data)
{
    ...
    /* Main loop */
    for (;;) {
        QLOGI(LOG_TAG, "UXEngine: Waiting on uxe_server_submit_req in uxe_server\n");
        ...
        switch (cmd) {
            case UXENGINE_CMD_PERFLOCK_TRIGGER:
            {
                ...
                {
                    QLOGI(LOG_TAG, "UXEngine is disabled");
                ...
                if(msg->opcode == UXE_TRIGGER || msg->opcode == UXE_ULMK_TRIGGER)
                {
                    ...
                    QLOGI(LOG_TAG, "UXEngine, Received back to home");
                    ...
                    QLOGI(LOG_TAG, "Final OUTPUT: Preferred Apps returned : %d\n", uba_return);

                    for (i = 0; i < uba_return ; i++) {
                        if (final_out[i][0] != 0) {
                            QLOGI(LOG_TAG, "Final OUTPUT: Apps returned: %s\n", final_out[i]);
                    ...
                    QLOGI(LOG_TAG, "UXEngine: Set preferred apps in uxe_server: %s\n", preferred_apps);
                    pthread_cond_signal(&ux_trigger_cond);
                }
                    ...
                    for (int time_slot_iter = 1; time_slot_iter <= TIME_SLOTS; time_slot_iter++) {
                        for(int week_day_iter = 0; week_day_iter < DAY_SLOTS; week_day_iter++) {
                            ...
                            QLOGI(LOG_TAG, "Final OUTPUT:  List of all Preferred Apps returned : %d\n",
                                   uba_return);
                            int j = 0;
                    ...
                    QLOGI(LOG_TAG, "UXEngine: List of all preferred apps in uxe_server: %s\n",
                           preferred_apps);
                    if (msg->opcode == UXE_SPEED_TRIGGER) {
                        ...
                        QLOGI(LOG_TAG, "No. of old PApps to be decompiled: %d\n", iop_return);
                        ...
                        reset_speed_bits();
                        QLOGI(LOG_TAG, "UXEngine: List of all preferred apps for speed/despeed %s\n",
                               preferred_apps);
                    }
                    free(speed_apps);
                    pthread_cond_signal(&ux_trigger_cond);
            ...
            default:
                QLOGI(LOG_TAG, "Unknown command %d", cmd);
        }
        UXEevqueue.GetDataPool().Return(evData);
    }
    QLOGI(LOG_TAG, "UXE server thread exit due to rc=%d", rc);
    return NULL;
}
       ...
       //wait for perflock commands
        ...
        switch (cmd) {
            case UXENGINE_CMD_PERFLOCK_EVENTS:
            {
                if(uxe_prop_disable || !is_boot_complete())
                {
                    QLOGE(LOG_TAG, "UXEngine is disabled");
                    break;
                }
                ...
                if(opcode == UXE_EVENT_BINDAPP)
                {
                    ...
                    QLOGI(LOG_TAG, "UXEngine: Received bindApp duration: %d pkg_name %s tmp_pkg_name: %s\n",
                                                            bindApp_dur, in_pkg_name, tmp_pkg_name);
                    if (bindApp_count) {
                        int lat_thres = 0;
                        get_ux_lat_pkg(in_pkg_name, &ux_lat);
                        QLOGI(LOG_TAG, "UXEngine: Average bindApp before: %d bindApp_count : %d\n",
                        ...
                        QLOGI(LOG_TAG, "UXEngine: Average bindApp: %d for app: %s bindApp_count: %d\n",
                    ...
                    if (!pkg_match)
                    {
                        ...
                        QLOGI(LOG_TAG, "UXEngine: Updating bindApp duration: %d pkg_name %s\n",
                ...
                if (opcode == UXE_EVENT_DISPLAYED_ACT)
                {
                    ...
                    if (!strncmp(in_pkg_name, tmp_pkg_name, PKG_NAME_LEN)) {
                        ...
                        //skip update
                        QLOGI(LOG_TAG, "UXEngine: Skip. Received weird DA: %s\n", in_pkg_name);
                        goto disAct_cleanup;
                    }
                    ...
                    if ((bindApp_dur == 0 && launching) || !pkg_match) {
                        QLOGI(LOG_TAG, "UXEngine: Empty app launch. Just update DA. pkg_name: %s\n",
                              in_pkg_name);
                        bindApp_count = get_total_pkg_bindapp_count(in_pkg_name);
                        get_ux_lat_pkg(in_pkg_name, &ux_lat);
                    ...
                    QLOGI(LOG_TAG, "UXEngine: bindApp duration: %d, DisplayedActivity: %d\n",
                           bindApp_dur, disAct_dur);
                    if (pkg_count - 1) {
                        QLOGI(LOG_TAG, "UXEngine: Average displayed activity before: %d pkg_count :%d\n",
                        ...
                        QLOGI(LOG_TAG, "UXEngine: Average displayed activity after: %d\n",
                               avg_disAct);
                        if(avg_disAct != 0)
                            disAct_dur = avg_disAct;
                    }
                    QLOGI(LOG_TAG, "UXEngine: Updating bindApp & DA duration: %d %d pkg_name %s\n",
                    ...
                    disAct_cleanup:
                        ...
                        launching = false;
                        memset(tmp_pkg_name, 0, PKG_NAME_LEN);
                        QLOGI(LOG_TAG, "UXEngine: Finished ux_lat update & reset \n");
                }

                if(opcode == UXE_EVENT_KILL)
                {
                    QLOGI(LOG_TAG, "UXEngine: Received app no-AM kill: %s\n", in_pkg_name);
                    update_palm_table(in_pkg_name, 1, 0);
                }

                if(opcode == UXE_EVENT_GAME)
                {
                    QLOGI(LOG_TAG, "UXEngine: Received game info - %s\n", in_pkg_name);
                ...
                if(opcode == UXE_EVENT_SUB_LAUNCH)
                {
                    ...
                    QLOGI(LOG_TAG, "UXEngine Updating sub_launch details: \
                ...
                {
                    QLOGI(LOG_TAG, "UXEngine: Received pkg uninstall - %s\n", in_pkg_name);
                    int userId = msg->lat;
                    update_palm_table(in_pkg_name, 1, 0);
                ...
                {
                    QLOGI(LOG_TAG, "UXEngine: Received pkg install - %s\n", in_pkg_name);
                ...
                {
                    QLOGI(LOG_TAG, "UXEngine: Received pkg speed mode add - %s\n", in_pkg_name);
                ...
                {
                    QLOGI(LOG_TAG, "UXEngine: Received pkg speed mode del - %s\n", in_pkg_name);
                    remove_pkg(in_pkg_name);
            ...
            case IOP_CMD_PERFLOCK_IOPREFETCH_START:
            {
                ...
                if(msg->pid < 0)
                {
                    ...
                    launching = true;
                    time(&pkg_info.last_time_launched);
                    QLOGI(LOG_TAG, "UXEngine Updating Details: pkg_name: %s, week_day: %s, \
                           time_slot: %d %s\n",
                           pkg_info.pkg_name, week_day, time_slot, tmp_pkg_name);
                    update_ux_pkg_details(pkg_info, week_day, time_slot, 0);
                    update_palm_table(msg->pkg_name, 0, 1);

                    QLOGI(LOG_TAG, "UXEngine finished ux_pkg_details update \n");
                    free(week_day);
                    is_in_recent_list = false;
                    if(!enable_prefetcher)
                    {
                        QLOGE(LOG_TAG, "io prefetch is disabled");
                        break;
                    }
                    ...
                    for(ittr = 0; ittr < IOP_NO_RECENT_APP; ittr++)
                    {
                        if(0 == strcmp(msg->pkg_name,recent_list[ittr]))
                        {
                            ...
                            QLOGE(LOG_TAG, "is_in_recent_list is TRUE");
                            break;
                        }
                    }
                    ...
                    {
                        QLOGE(LOG_TAG, "io prefetch is deactivate");
                        break;
                    }
                ...
                if(msg->pid > 0)
                {
                    ...
                    if(true == is_in_recent_list)
                    {
                        QLOGE(LOG_TAG, "io prefetch Capture is deactivated ");
                        break;
                    }
                    stop_capture();
            ...
            default:
                QLOGE(LOG_TAG, "Unknown command %d", cmd);
        }
        IOPevqueue.GetDataPool().Return(evData);
    }

    QLOGI(LOG_TAG, "IOP server thread exit due to rc=%d", rc);
    return NULL;
}
...
int iop_server_init() {
    ...
    QLOGI(LOG_TAG, "IOP server starting");
    int ret = configure_preferred_apps();
    if (ret < 0)
        QLOGE(LOG_TAG, "Preferred Apps are not configured at Runtime, using default value = %u", preferred_app_count);
    ...
    for (;;) {
        if(!is_boot_complete()) {
            ...
            if (elapsed_time > 1 * 60 * MICRO_FOR_MILLI) { // if the device is not booted for 1 min smth is wrong
                QLOGE(LOG_TAG, "Device taking longer time for Bootup. Returning..");
                goto error;//return the for loop
            }
            continue;
        }
        QLOGI(LOG_TAG, "Boot_completed. Create IOP DB");
        if (is_db_init == false) {
            if(create_database() == 0) {
                is_db_init = true;
                QLOGI(LOG_TAG, "Successfully Created IOP database");
                break;
            } else {
                loop_count--;
                usleep(50 * MICRO_FOR_MILLI);
                if (loop_count < 0) { //This Check is to avoid creteDB() forever.
                    QLOGE(LOG_TAG, "Error in IOP DB Creation. Returning..");
    ...
    if (uxe_disabled()) {
        ...
        if (uba_rc == 0) {
            QLOGE(LOG_TAG, "Fail to init UBA");
            goto error;
        }
        uxe_init = true;
    }
...
error:
    QLOGE(LOG_TAG, "Unable to create control service (stage=%d, rc1=%d rc2=%d uba_rc=%d)",
...
char * uxe_server_submit_request(iop_msg_t *msg) {
    ...
    QLOGI(LOG_TAG, "UXEngine Entered server submit opcode: %d\n", msg->opcode);
    //boot complete check
    if (NULL == msg) {
    ...
    QLOGI(LOG_TAG, "UXEngine PHAL: Received back-to-home. Cmd: %d\n", cmd);

    evData = UXEevqueue.GetDataPool().Get();
    if ((NULL == evData) || (NULL == evData->mEvData)) {
        QLOGE(LOG_TAG, "UXEngine event data pool ran empty");
        pthread_mutex_unlock(&ux_trigger_mutex);
        return NULL;
    }
    ...
    QLOGI(LOG_TAG, "UXEngine: Waiting for server to populate preferred_apps\n");
    ...
    if (pthread_cond_timedwait(&ux_trigger_cond, &ux_trigger_mutex, &trigger_timeout) != 0) {
        QLOGE(LOG_TAG, "UXEngine: Error no.: %d\n", errno);
        pthread_mutex_unlock(&ux_trigger_mutex);
        if (pa_ready)
    ...
    QLOGI(LOG_TAG, "UXEngine: Server request read string: %s\n", preferred_apps);
