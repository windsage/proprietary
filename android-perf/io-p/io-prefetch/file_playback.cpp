...
static void *iop_fileplayback_run(file_details info, struct stat file_stat, int fd)
{
    ...
    int read_data = 0;
    int ret = 0;
    QLOGI(LOG_TAG, "MINCORE, PLAYBACK %s START, mincore_array: %p",
    ...
    if (mincore_array == NULL) {
        ...
        read_data += len_size;
        if (ret !=0) {
            QLOGE(LOG_TAG, "posix_fadvise failed: %s", strerror(errno));
        }
    ...
    QLOGI(LOG_TAG, "MINCORE, PLAYBACK %s END real read file size = %.4fMB",
...
static void drop_file_cache(int fd, file_details *info)
{
    ...
    if (posix_fadvise(fd, offset, len_of_range, POSIX_FADV_DONTNEED)) {
        QLOGE(LOG_TAG, "ERROR error not able to dorp file cache of file %s", info->file_name);
    }
}
...
static void* start_playback_thread(void *pkg_name_arg)
{
    ...
    QLOGI(LOG_TAG, "pkg_name = %s",pkg_name);
    num_file = get_total_file(pkg_name);

    if( num_file <= 0)
    {
        QLOGI(LOG_TAG, "no file to read get_total_file %d", num_file);
        remove_pkg(pkg_name);
        free(pkg_name);
        ATRACE_END();
        return NULL;
    ...
    {
        QLOGI(LOG_TAG, "Fail to allocate memory");
        remove_pkg(pkg_name);
        free(pkg_name);
        ATRACE_END();
        return NULL;
    ...
    QLOGI(LOG_TAG, "num_of_files = %d",num_of_files);

    if (num_of_files <= 0)
    {
        QLOGI(LOG_TAG, "no file to read get_file_list");
        remove_pkg(pkg_name);
    ...
    for(i = 0; i < num_file;i++)
    {
        ...
        if (file_detail_ptr[i].disabled) {
            QLOGE(LOG_TAG, "skip preload disabled file %d %s", i, file_detail_ptr[i].file_name);
            continue;
        }
        if (file_detail_ptr[i].study_finish && file_detail_ptr[i].file_iop_size < pagesize) {
            ...
            QLOGE(LOG_TAG, "skip preload ZERO needed file %d %s", i, file_detail_ptr[i].file_name);
            continue;
        }
        ...
        if (file_stat.st_mtime != file_detail_ptr[i].file_modify_time) {
            if (strstr(file_detail_ptr[i].file_name,".apk")!=NULL
                    ...
                    strstr(file_detail_ptr[i].file_name, "/system/priv-app/")!=NULL))
                {
                    ...
                    QLOGI(LOG_TAG, "apk file %s modify time changed %d/%ld, \
                    ...
                    QLOGI(LOG_TAG, "file %s modify time changed update db", file_detail_ptr[i].file_name);
        ...
        if (file_detail_ptr[i].study_finish == 0) {
            QLOGI(LOG_TAG, "Drop cache of file %s", file_detail_ptr[i].file_name);
            ...
            QLOGI(LOG_TAG, "io preload of file %s", file_detail_ptr[i].file_name);
        ...
        {
            QLOGI(LOG_TAG, "Max total size limit reached Total = %d",total_data);
    ...
    ATRACE_END(); //end for start_playback_thread enter
    QLOGI(LOG_TAG, " Total Data = %.2f MB", total_data/1024.0/1024);
...
void start_playback(char *pkg_name_arg) {
    char *pkg_name = NULL;
    int len = 0;
    QLOGE(LOG_TAG, " start_playback-1 ");
    ...
    if(len <= 0)
    {
        QLOGW(LOG_TAG, "Incorrect length for pkg_name_arg");
        return;
    }
    ...
    QLOGI(LOG_TAG, "Exit playback");
    return ;
}

void stop_playback()
{
    QLOGI(LOG_TAG, "Stop Playback ENTER");
    pthread_join(start_playback_pthread_id, (void **)NULL);
    QLOGI(LOG_TAG, "Stop Playback EXIT");
}
