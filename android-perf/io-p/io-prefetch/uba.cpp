...
int init_uba()
{
    ...
    if (tracker == NULL) {
        QLOGE(LOG_TAG, "UBA Init Allocation failed.");
        return 0;
    }
    QLOGI(LOG_TAG, "UBA Init Tracker allocated");
    return 1;
}
...
int set_palm_table(char **list, int tb_size, long cur_time, bool launch) {
    ...
    if (cur_tracker == NULL) {
        QLOGE(LOG_TAG, "Set_Palm_Table: No memory to set temp table. Return");
        return 0;
    }

    QLOGI(LOG_TAG, "Set_Palm_Table: Entry, Table size: %d, old table size: %d, Current time: %ld \n", tb_size, old_tbl_size, cur_time);
    for(i = 0; i < tb_size; i++)
    {
         ...
         for(j = 0; j < old_tbl_size; j++)
         {
             if(!strncmp(tracker[j].pkg_name, list[i], PKG_NAME_LEN))
             {
                 ...
                 if(launch || tracker[j].empty || tracker[j].cached) {
                     list[i][0] = 0;
                     list[i][1] = '\0';
                     QLOGI(LOG_TAG, "Set_Palm_Table: Matched empty from previous table. Pkg_name: %s Pkg State--Cached: %d, Empty: %d\n", tracker[j].pkg_name, tracker[j].cached, tracker[j].empty);
                     strlcpy(cur_tracker[i].pkg_name, tracker[j].pkg_name, PKG_NAME_LEN);
                     match++;
                 }
                 else if(tracker[j].do_not_launch) {
                     QLOGI(LOG_TAG, "Set_Palm_Table: Not allowed for launch due to looping: %s\n", tracker[j].pkg_name);
                     list[i][0] = 0;
                     list[i][1] = '\0';
                     ...
                     if ((cur_time - tracker[j].last_empty_time) > empty_restart_threshold) {
                         cur_tracker[i].do_not_launch = false;
                         QLOGE(LOG_TAG, "Set_Palm_Table: Timeout expired. Allowed to start as empty: %s\n", tracker[j].pkg_name);
                     }
                 }
                 else {
                     cur_tracker[i].empty = true;
                     cur_tracker[i].cached = false;
                     QLOGI(LOG_TAG, "Set_Palm_Table: Matched, but app was killed. Resetting. Pkg_name: %s Pkg State--Cached: %d, Empty: %d\n", cur_tracker[i].pkg_name, cur_tracker[i].cached, cur_tracker[i].empty);
                 }
             }
         }
    }
...
void update_palm_table(char *pkg_name, int kill, int launch)
{
    ...
    QLOGI(LOG_TAG, "Enter Update_palm_table. Pkg_name : %s old_tbl_size %d kill: %d launch: %d\n", pkg_name, old_tbl_size, kill, launch);
    for(i = 0 ; i < old_tbl_size ; i++)
    {
        if(!strncmp(tracker[i].pkg_name, pkg_name, PKG_NAME_LEN))
        {
            QLOGI(LOG_TAG, "Update_palm_table. Tracker Pkg_name : %s\n", tracker[i].pkg_name);
            if (kill) {
                ...
                QLOGI(LOG_TAG, "Update_palm_table kill. Pkg_name : %s state-- cached: %d empty: %d\n", tracker[i].pkg_name, tracker[i].cached, tracker[i].empty);
            }
            if (launch) {
                tracker[i].cached = true;
                tracker[i].empty = false;
                tracker[i].hit = true;
                QLOGI(LOG_TAG, "Update_palm_table launched. Pkg_name : %s state-- cached: %d empty: %d\n", tracker[i].pkg_name, tracker[i].cached, tracker[i].empty);
            }
        }
    }
}
...
int get_preferred_apps(char **out_list, int disable_palm, char * all_week_day, int all_time_slot, bool launch)
{
    ...
    QLOGI(LOG_TAG, "Get_Preferred_Apps: Current Time stats: Week_day: %s, Time_slot: %d, Total_pkgs: %d\n", week_day, time_slot, total_pkgs);
    ...
    QLOGI(LOG_TAG, "Get_Preferred_Apps: Get Top pkg list Returned pkgs: %d\n", num_pkgs);
    ...
    for (index = 0 ; (index < preferred_app_count && index < num_pkgs) ; index++) {
         ...
         QLOGI(LOG_TAG, "Get_Preferred_Apps: Accessing cur list: %s pkg_launch_count: %d bindApp_dur: %d disAct_dur%d\n", cur_list[index].pkg_name, cur_list[index].num_launches,
                                                                                                                cur_list[index].bindApp_dur, cur_list[index].disAct_dur);
         strlcpy(out_list[index], cur_list[index].pkg_name, PKG_NAME_LEN);
         QLOGI(LOG_TAG, "Get_Preferred_Apps: Accessing copied list: %s\n", out_list[index]);
         table_size++;
    }

    if (disable_palm == 0) {
        QLOGI(LOG_TAG, "Get_Preferred_Apps: Accessing table_size %d\n", table_size);
        set_palm_table(out_list, table_size, cur_time, launch);
    }
    else if (disable_palm == 1) {
        QLOGI(LOG_TAG, "Get_Preferred_Apps: Return all Fav apps List with size = %d\n", table_size);
    }
...
void compute_time_day_slot(char *week_day, int *time_slot)
{
    ...
    if (tinfo == NULL) {
        QLOGE(LOG_TAG,"compute_time_day_slot error: can't get current weekday and timeslot");
        if (time_slot != NULL)
            *time_slot = -1;
        return;
    }
    int t_hour = tinfo->tm_hour;
    QLOGI(LOG_TAG, "Compute_time_day_slot .. t_hour: %d time: %s\n", t_hour, asctime(tinfo));
    ...
    if (3 < t_hour && t_hour <= 11) {
        ...
        *time_slot = 4;
    QLOGI(LOG_TAG, "Compute_time_day: week_day: %s time_slot: %d\n", week_day, *time_slot);
}
