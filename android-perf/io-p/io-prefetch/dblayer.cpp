...
static int open_db()
{
    ...
    if( db_conn != NULL )
    {
      QLOGI(LOG_TAG,  "Already Opened database connection");
      return 1;
    }
    // open the database connection
    int rc = sqlite3_open(_dbpath, &db_conn );
    QLOGI(LOG_TAG, "sqlite3_open db_conn = %p rc = %d \n",db_conn,rc);

    if( db_conn == NULL )
    {
        QLOGE(LOG_TAG, "unable to open database at this path=\"%s\" with sql err_msg=%s"
    ...
    if ( rc != SQLITE_OK )
    {
        ...
        QLOGE(LOG_TAG, "unable to open database at this path=\"%s\" with sql ret code=%d.", _dbpath, rc);
        return 0;
    }

    QLOGI(LOG_TAG,  "db=%s has been opened successful", _dbpath);
    return 1;
}
...
int create_database()
{
    ...
    if(fd < 0)
    {
        QLOGE(LOG_TAG, "fail to create database");
        return -1;
    }
    ...
    else
    {
        if( db_conn == NULL )
        {
          QLOGE(LOG_TAG, "fail to create DB");
            return -1;
        }
        sem_init(&mutex, 0, 1);
        //create table
        QLOGI(LOG_TAG, "executing Query");
...
static int close_database()
{
  ...
  QLOGI(LOG_TAG,  "db=%s has been closed successfully", _dbpath);
  return 0;
}
...
int get_file_list(char *pkg_name, file_details *file_info_list, int size)
{
    ...
    QLOGI(LOG_TAG, "Query = %s", query_str);
...
int iop_query_exec(const char * query_str)
{
    ...
    {
        QLOGI(LOG_TAG, "queries = %s fail to execute\n",query_str);
    }
...
int update_file_details(char * pkg_name,file_details *file_info[], int size)
{
    ...
    for(i=0;i<size;i++) {
        ...
        if(is_file_present) {
            //Update
            QLOGI(LOG_TAG, "MINCORE update file %s %s study_finish %d, cache_dropped %d",
        ...
        else
        {
            ...
            QLOGI(LOG_TAG, "MINCORE insert new file %s %s iopsize %d filesize %d",
...
int update_pkg_details(pkg_details pkg_info)
{
    ...
    QLOGI(LOG_TAG, "update_pkg_details");
...
int get_top_ux_pkg_list(pkg_ux_top_details *pkg_list, char *week_day, int time_slot, int size, int start)
{
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW)
    {
        ...
        QLOGI(LOG_TAG, "pkg_name = %s timeslot_cnt = %d bindApp_dur = %d disAct_dur = %d not_game = %d\n", pkg_name,
...
int set_ux_lat_app_upd(char *pkg_name)
{
    ...
    snprintf(query_str,sizeof(query_str),UX_LATENCY_QUERY_SET_PKG_UPDATE
                                      ,pkg_name);
    QLOGI(LOG_TAG, "set_ux_lat_db_on_app_update query: %s\n", query_str);
...
int update_ux_lat_details(char *pkg_name, int bindApp_dur, int disAct_dur, int memory, int non_empty)
{
    ...
    QLOGI(LOG_TAG, "Updating ux_lat database: is_pkg_present: %d\n", is_pkg_present);
...
int update_ux_lat_pred_details(char *pkg_name, int succ, int weight)
{
    ...
    QLOGI(LOG_TAG, "update_lat_pred_details");
...
int update_ux_pkg_details(pkg_ux_details pkg_info, char *week_day, int time_slot, int sub)
{
    ...
    QLOGI(LOG_TAG, "Updating_ux_pkg_details database: is_pkg_present: %d\n", is_pkg_present);
    ...
    else
    {
        ...
        switch (time_slot) {
            ...
            case 4:
                ...
                break;
            default:
                QLOGI(LOG_TAG, "Database insert: Erroneous time slot. Skip insert\n");
                goto out;
        }
