    ...
    if (fd != -1) {
        ...
        if (mem != MAP_FAILED) {
            ...
            if (mincore_array == NULL) {
                munmap(mem, len_of_range);
                close(fd);
                QLOGE(LOG_TAG, "malloc mincore array failed");
                ATRACE_END();
                return -1;
            }
            ...
            if (mincore((void *)mem, len_of_range, (unsigned char *)mincore_array)) {
                QLOGE(LOG_TAG, "mincore syscall failed %s", strerror(errno));
                free(mincore_array);
                munmap(mem, len_of_range);
                close(fd);
            ...
            if (munmap(mem, len_of_range)) {
                QLOGE(LOG_TAG, "munmap failed");
            }
            QLOGI(LOG_TAG, "MINCORE: %s mincore result %d of %d in cache",
                    file_name, total_pages_in_core, pages_in_range);
            close(fd);
            ATRACE_END();//update mincore array
        } else {
            QLOGE(LOG_TAG, "MINCORE: mmap failed of file %s", file_name);
            close(fd);
            ATRACE_END();
            return -1;
        }
    } else {
        QLOGE(LOG_TAG, "MINCORE: error open file %s", file_name);
        ATRACE_END();
        return -1;
    }
...
static int find_insert_into_list(char *pkg_name,char *filename, long long size, long file_capture_time) {
    ...
    {
         QLOGE(LOG_TAG, "MAX Num of file reached, can not add more files");
         return -1;
    }
    if(NULL == filename)
    {
        QLOGE(LOG_TAG, "Invalid argument");
        return -1;
    }
    ...
    if (is_file_present && temp_file_info.disabled) {
        QLOGI(LOG_TAG, "skip file %s %s disabled", pkg_name, filename);
        return -1;
    }
    //study finished
    if (is_file_present && (temp_file_info.study_finish != 0 && temp_file_info.cache_dropped!=1)) {
        QLOGI(LOG_TAG, "skip file %s %s already study finished", pkg_name, filename);
        return -1;
    }
    ...
    if (index == total_files) {
        ...
        if(NULL == file_list[index]) {
            QLOGE(LOG_TAG, "Fail to allocate memory");
            return -1;
        }
...
int get_snapshot(char *pkg_name, pid_t pid)
{
    ...
    QLOGI(LOG_TAG, "Dirname = %s MAX_PATH = %d\n", dirname, PATH_MAX);
    dp = opendir(dirname);
    QLOGI(LOG_TAG, "get_snapshot Enter dp = %p",dp);
    QLOGI(LOG_TAG, "dp = %p",dp);
    if (dp != NULL) {
        int i = 0;
        while ((ep = readdir(dp)))
        {
            ...
            {
                need_to_filter = 0;
                QLOGI(LOG_TAG, "file is in filter list %s",linkname);
        ...
        QLOGI(LOG_TAG, "get_snapshot Exit dp = %p",dp);
        closedir(dp);
    }
    ATRACE_END();
    return 0;
...
int capture_pkg_file(char *pkg_name, DIR *dp,char *dir_path,int priority)
{
    ...
    while ((ep = readdir(dp))) {
        ...
        {
            QLOGI(LOG_TAG, "current or previous dir IGNORE");
            continue;
        }
        ...
        if(ep->d_type == DT_DIR)
        {
            ...
            QLOGI(LOG_TAG, "Opening dir new_dp = %s",pathname);
            if (new_dp != NULL) {
        ...
        QLOGI(LOG_TAG, "Opening file = %s",pathname);
        fd = open(pathname, O_RDONLY);
        if(fd < 0)
        {
            QLOGI(LOG_TAG, "fail to open file");
            continue;
        }

        if ( stat( pathname, &f_st ) < 0 ) {
            QLOGI(LOG_TAG, "fail to get file stat");
            close(fd);
            continue;
        }
        ...
        {
            QLOGI(LOG_TAG, "File size is smaller then 4k");
            close(fd);
            continue;
        }
        ...
        if(need_to_filter)
        {
            need_to_filter = 0;
            close(fd);
            QLOGI(LOG_TAG, "file is in filter list %s",pathname);
            continue; // goto  next file
        }
        QLOGI(LOG_TAG, "curTime = %lu, start_capture_time_s = %lu,f_st.st_atime = %lu",time(NULL)
                                ,start_capture_time_s,f_st.st_atime);

        QLOGI(LOG_TAG, "File is accessed %s ",ep->d_name);
        if (S_ISREG(f_st.st_mode))
        {
            QLOGI(LOG_TAG, "inserting pathname = %s,f_st.st_size = %zd, a_time = %d"
...
void get_priv_files(capture_thread_arg * arg_bundle)
{
    ...
    QLOGI(LOG_TAG, "Dir = %s",f_name);
    dp = opendir(f_name);
    QLOGI(LOG_TAG, "First dp = %p",dp);
    if (dp != NULL) {
...
void * capture_thread(void * arg) {
    ...
    QLOGI(LOG_TAG, "pkg_name  = %s",arg_bundle->pkg_name);

    if(polling_enable)
    {
        while (duration_counter < halt_counter) {
            if (halt_thread) goto cleanup;
            QLOGI(LOG_TAG, "Getting snapshot %d\n", arg_bundle->pid);
    ...
    QLOGI(LOG_TAG, "pkg_name = %s total_files = %d ",arg_bundle->pkg_name,total_files);
...
cleanup:
    ...
    QLOGE(LOG_TAG, "# Final entry : pkg_name file_name file_time_stamp filesize file_iop_size");
    for(i = 0; i < total_files; i++) {
        QLOGE(LOG_TAG, "%d. Final entry : %s %s %d %d %d\n",i
    ...
    free(arg_bundle);
    QLOGI(LOG_TAG, "Exit capture_thread");
    return NULL;
}
...
int start_capture(pid_t pid, char *pkg_name, char *code_path, bool ofr) {
    ...
    if ( len <= 0)
    {
        QLOGW(LOG_TAG, "incorrect length for pkg_name");
        return -1;
    }
    ...
    total_files = 0;
    QLOGE(LOG_TAG, " start_capture ENTER ");
