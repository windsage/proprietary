...
struct IopDeathRecipient : virtual public hidl_death_recipient
{
    virtual void serviceDied(uint64_t /*cookie*/, const wp<IBase>& /*who*/) override {
        ...
        gIopHal = NULL;
        QLOGE(LOG_TAG, "IIop serviceDied");
    }
};
                ...
                if (gIopHal != NULL) {
                    ...
                    if (iopDeathRecipient != NULL) {
                        ...
                        if (!linked || !linked.isOk()) {
                            gIopHal = NULL;
                            QLOGE(LOG_TAG, "Unable to link to gIopHal death notifications!");
                        }
                    } else {
                        QLOGE(LOG_TAG, "Failed to allocate iopDeathRecipient");
                        gIopHal = NULL;
                        QLOGE(LOG_TAG, "IIop:: Iop HAL Service is not available.");
                    }
                } else {
                    QLOGE(LOG_TAG, "IIop:: Iop HAL Service is not available.");
                }
...
static int processIntReturn(const Return<int32_t> &intReturn, const char* funcName) {
    ...
    if (!intReturn.isOk()) {
        unique_lock<shared_timed_mutex> write_lock(gIop_lock);
        gIopHal = NULL;
        QLOGE(LOG_TAG, "%s() failed: iop HAL service not available.", funcName);
    } else {
        ret = intReturn;
    }
    return ret;
