...
void Iop::LoadIopLib() {
    ...
    if (!mHandle.is_opened) {
         ...
         if (mHandle.dlhandle == NULL) {
             QLOGE(LOG_TAG, "IOP-HAL %s Failed to (dl)open iopd\n", __func__);
             return;
         }
         ...
         *(void **) (&mHandle.iop_server_init) = dlsym(mHandle.dlhandle, "iop_server_init");
         if ((rc = dlerror()) != NULL) {
             QLOGE(LOG_TAG, "IOP-HAL %s Failed to get iop_server_init\n", rc);
         ...
         *(void **) (&mHandle.iop_server_exit) = dlsym(mHandle.dlhandle, "iop_server_exit");
         if ((rc = dlerror()) != NULL) {
             QLOGE(LOG_TAG, "IOP-HAL %s Failed to get iop_server_exit\n", rc);
             dlclose(mHandle.dlhandle);
             mHandle.dlhandle = NULL;
             return;
         }
         ...
         *(void **) (&mHandle.iop_server_submit_request) = dlsym(mHandle.dlhandle, "iop_server_submit_request");
         if ((rc = dlerror()) != NULL) {
             QLOGE(LOG_TAG, "IOP-HAL %s Failed to get iop_server_submit_request\n", rc);
             dlclose(mHandle.dlhandle);
             mHandle.dlhandle = NULL;
             return;
         }
         ...
         *(void **) (&mHandle.uxe_server_submit_request) = dlsym(mHandle.dlhandle, "uxe_server_submit_request");
         if ((rc = dlerror()) != NULL) {
             QLOGE(LOG_TAG, "IOP-HAL %s Failed to get uxe_server_submit_request\n", __func__);
             dlclose(mHandle.dlhandle);
             mHandle.dlhandle = NULL;
             return;
         }
