...
void __attribute__ ((constructor)) initialize()
{
    ...
    /*
     * Call VendorIGlue constructor to register the module for perf HAL.
     *
     * The lmPerfGlue object is used only to call the VendorIGlue()
     ...
     */
    DEBUGV(LOG_TAG_LMPG,"Calling VendorIGlue()");
    VendorIGlue lmPerfGlue {
        LM_LIB_NAME,
        &asyncsOfInterest[0],
...
void perfmodule_sync_request_ext(int cmd, char **debugInfo) {
    int ret = -1;
    DEBUGV(LOG_TAG_LMPG,"perfmodule_sync_request() hintID:0x%x",cmd);
    AsyncTriggerPayload payload;
    payload.hintID = cmd;
    payload.feedback = debugInfo;
    nsecs_t now = systemTime();
    payload.timeStamp = now;
    ret = LearningModule::getInstance().trigger(payload);
    return;
