...
<Feature>
    ...
    <PipelinedPackages>
        <Package Name="pkg name" NumThreads="n" />
    </PipelinedPackages>
...
FeatureInfo FeatureXML::getFeatureInfo() {
    ...
    if(DynamicLoader::getInstance().mCurrVendor == NULL) {
        DEBUGE(LOG_TAG_FX, "mCurrVendor is Empty!!");
    } else {
        tname = (DynamicLoader::getInstance().mCurrVendor)->getVendorTargetName().c_str();
    }
    ...
    if(mXMLRoot != NULL) {
        ...
        /* XML:
         *        <DependsOn>FearureName</DependsOn>
         */
        currFeatureInfo.dependsOn = getFirstChildText(mXMLRoot, "DependsOn");
        /* XML:
         *        <Meters>
         */
         ...
         *      </PipelinedPackages>
         */
        XMLElement* pipelinedPackages = mXMLRoot->FirstChildElement("PipelinedPackages");
        if (pipelinedPackages != NULL) {
            for (XMLElement* currPackage = pipelinedPackages->FirstChildElement("Package");
                    currPackage != NULL;
                    currPackage = currPackage->NextSiblingElement("Package")) {

                bool validTarget = true;
                tinyxml2::XMLElement *e = currPackage->ToElement();
                if (e) {
                    const char *xmlTarget = e->Attribute("target");
                    if (tname and xmlTarget) {
                        if ((strlen(tname) != strlen(xmlTarget)) ||
                            (strncmp(tname, xmlTarget, strlen(xmlTarget)))) {
                            validTarget = false;
                        }
                    }
                    ...
                    if (validTarget) {
                        ...
                        if (name) {
                            ...
                            int numThreads = 0;
                            int fps = 0;
                            e->QueryIntAttribute("FPS", &fps);
                            e->QueryIntAttribute("NumThreads", &numThreads);
                            currFeatureInfo.pipelinedPackages[pkgName][fps] = numThreads;
                        } // if (name)
                    } // if (validTarget)
                } // if (e)
            } // for loop
        } // if (pipelinedPackages)
    }
