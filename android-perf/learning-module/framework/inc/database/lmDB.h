...
lmdbError LMDatabase<T...>::openLMdb() {
    ...
    if (LMDB_SQL_OK != (sqlErr = LMDB_SQL_OPEN_FN(dbname, &lmdb_conn, sqlFlg, NULL))) {
        QLOGE(LOG_TAG_LMDB, "ERROR:Db=%s sql open failed, err=%d\n", dbname, sqlErr);
        closeLMdb();
...
lmdbError LMDatabase<T...>::createLMdbTables() {
    ...
    try {
        ...
        QLOGE(LOG_TAG_LMDB, "Caught exception: %s in %s",e.what(), __func__);
    } catch (...) {
        QLOGE(LOG_TAG_LMDB, "Caught Exception in %s",__func__);
    }
    return LMDB_NO_ERROR;
}
...
lmdbError LMDatabase<T...>::closeLMdb() {
    ...
    if (lmdb_conn) {
        ...
        if (LMDB_SQL_OK != (sqlErr=LMDB_SQL_CLOSE_FN(lmdb_conn))) {
            QLOGE(LOG_TAG_LMDB, "ERROR:Db=%s sql close failed, err=%d\n", lmdbFile.c_str(), sqlErr);
            return LMDB_SQL_ERROR;
        }
    }
    return LMDB_NO_ERROR;
    ...
    try {
        if (lmdb_conn) {
            ...
            if (reqCount > RESTRICT_MAX_READALLOC_REQCOUNT ) {
                ...
                QLOGW(LOG_TAG_LMDB, "Dbt=%s Restrict reqCnt to %zu\n", tblNm, reqCount);
            } else if (reqCount == 0) {
                reqCount = RESTRICT_MAX_READALLOC_REQCOUNT;
            }
        ...
        QLOGE(LOG_TAG_LMDB, "Error in std::string operations in %s", __func__);
    }
    return node;
}
#ifdef LMDB_DEBUG_SQL_SHADOW_RD
    ...
    try {
        if (lmdb_conn) {
            ...
            if (Count == 0) {
                ...
                QLOGW(LOG_TAG_LMDB, "Dbt=%s Restrict Cnt to %zu\n", tblNm, Count);
            }
        ...
        QLOGE(LOG_TAG_LMDB, "Caught exception: %s",e.what());
    } catch (...) {
        QLOGE(LOG_TAG_LMDB, "Error in std::string operations");
    }
    return node;
}
#ifdef LMDB_DEBUG_SQL_SHADOW_COL_LIST
