 ...
 */
#define SPCOM_MAX_REQUEST_SIZE         SPCOM_MAX_MESSAGE_SIZE
#define SPCOM_MAX_RESPONSE_SIZE        SPCOM_MAX_MESSAGE_SIZE
 ...
 */
typedef enum {
    SP_HEALTH_ARI_LIFECYCLE_STATE = 0,
...
/**
 * spcom_is_app_loaded_by_uuid() - check if the SP App is already loaded by the app uuid.
 *
 * This shall be useful to check if a 3rd party app was loaded in UEFI stage because the previous is_app_loaded will not work.
 *
 * @uuid:   the first 4 bytes of the uuid of the app to be queried (little endian).
 *
 * @note: This API is available only on HLOS.
 *
 * return: true if loaded,false otherwise.
 */
bool spcom_is_app_loaded_by_uuid(uint32_t uuid);
...
/**
 * spcom_sp_sysdata_read_ext()
 *
 * Read SP System data buffer.
 *
 * @sysdata_id: Predefined system data id
 * @sysdata_arg1: First argument
 * @sysdata_arg1: Second argument
 * @sysdata_buf: sysdata buffer to be written to
 * @sysdata_size: Sysdata buffer size
 * @preconnected_client: client to be used for communicate with SP
 *
 * return: value indicating success(0) or failure(!=0)
 */
int spcom_sp_sysdata_read_ext(
        sp_sysdata_id_enum sysdata_id,
        uint32_t sysdata_arg1,
        uint32_t sysdata_arg2,
        uint8_t* sysdata_buf,
        uint32_t sysdata_size,
        struct spcom_client* preconnected_client);

/**
 * spcom_sp_sysparam_read_ext()
 *
 ...
 * Once a channel is created, changing its shareable attribute
 * is not supported, even after unregistration.
 *
...
/**
 * spcom_ion_alloc_ex - allocate shared buffer for SPU and other subsystem
 *
 * @size - allocation size.
 * @fd_ptr[out] - return the buffer file descriptor.
 * @subsys - subsystem that SPU shares a buffer with - HLOS/TZ/Modem
 *
 * return:
 *   HLOS-SP: return virtual buffer pointer on success, NULL on failure.
 *   TZ-SP + Modem-SP: return NULL, as HLOS should not access the buffer.
 */
void* spcom_ion_alloc_ex(uint32_t size, int *fd_ptr, sp_alloc_subsys_t subsys);
...
/**
 * spcom_ion_free_ex() - Free the ION buffer.
 *
 * @vbuf - virtual buffer pointer.
 * @size - allocation size.
 * @fd - ION buffer file descriptor.
 ...
 * return: none.
 */
void spcom_ion_free_ex(void *vbuf, int size, int fd, sp_alloc_subsys_t subsys);
...
/**
 * spcom_set_ssr_state() - set SPSS SSR state (disabled/enabled).
 *
 * In case IAR is active and new SPSS FW was pushed to the device,
 * SPSS SSR shall be disalbed, to avoid Cryptographic Erase.
 * When disabled, the device should crash on SPU reset (SSR).
 *
 * @disable_ssr: True if caller wishes to disable SPSS SSR, false if caller
 *               wishes to enable it.
 *
 * return: 0 on success, negative error code on failure (see errno.h)
 */
int spcom_set_ssr_state(bool disable_ssr);

/**
 * spcom_sync_start() - Start sync HLOS-SP shared dmabuf
 *
 * Call this API BEFORE any read/write sequence to shared buffer.
 *
 * @dma_buf_fd: dma buffer file descriptor returned by spcom_ion_alloc()
 *
 * return: 0 on success, negative error code on failure (see errno.h)
 */
int spcom_sync_start(int dma_buf_fd);

/**
 * spcom_sync_end() - End sync HLOS-SP shared dmabuf
 *
 * Call this API AFTER any read/write sequence to shared buffer, before used by SP.
 *
 * @dma_buf_fd: dma buffer file descriptor returned by spcom_ion_alloc()
 *
 * return: 0 on success, negative error code on failure (see errno.h)
 */
int spcom_sync_end(int dma_buf_fd);
