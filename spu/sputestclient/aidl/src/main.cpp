...
int32_t spcomClientTestFlow()
{
    ...
    uint32_t uuid = *(uint32_t *)spuSrvAppUUID;
    err = SPUTestFw::loadApp(spuSrvAppFullPath, spuSrvAppName, uuid);
...
int32_t spcomServerTestFlow()
{
    ...
    uint32_t uuid = *(uint32_t *)spuCliAppUUID;
    err = SPUTestFw::loadApp(spuCliAppFullPath, spuCliAppName, uuid);
    ALOGI("loadApp() returned [%d]", err);
    if (err && err != ALREADY_LOADED) {
...
int32_t ssrFlow()
{
    std::shared_ptr<SPUNotifier> notifier = SPUTestFw::getNotifier();
    if (!notifier)
        return FRAMEWORK_ERROR;
