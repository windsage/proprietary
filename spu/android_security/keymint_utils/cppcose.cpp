...
getSenderPubKeyFromCoseEncrypt(const cppbor::Item* coseEncrypt) {
    ...
    if (key_id && key_id->asBstr()) {
        auto pair = std::make_pair(pubkey->asBstr()->value(), key_id->asBstr()->value());
        return pair;
    }

    // If no key ID, just return an empty vector.
    auto pair = std::make_pair(pubkey->asBstr()->value(), bytevec{});
    return pair;
}


ErrMsgOr<bytevec> decryptCoseEncrypt(const bytevec& key,
