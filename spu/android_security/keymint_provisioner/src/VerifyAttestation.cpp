...
bool VerifyAttestation::verify_attestation_record(
    ...
    if (!cert.get()) {
        cerr << "parse_cert_blob failed" << endl;
        return false;
    }

    ASN1_OCTET_STRING* attest_rec = get_attestation_record(cert.get());
    if (!attest_rec) {
        cerr << "get_attestation_record failed" << endl;
        return false;
    }
    ...
    if (ret) {
        cerr << "parse_attestation_record failed: " << ret << endl;
        return false;
    }

    if (expected_attestation_version != (att_attestation_version * 100)) {
        cerr << "att_attestation_version failed: " << att_attestation_version
             << ", expected_attestation_version: " << expected_attestation_version << endl;
        return false;
    }

    if (expected_keymaster_version != (att_keymaster_version / 100)) {
        cerr << "att_keymaster_version failed: " << att_keymaster_version
    ...
    if (provision_device_id) {
        ...
        expected_tee_enforced.push_back(TAG_ATTESTATION_ID_SERIAL, SerialNum.c_str(),
                                        SerialNum.size());
        for (size_t i = 0; i< Imei.dev_id_count && i < KM_MAX_SUBS; i++) {
             size_t bytes_len = Imei.dev_id[i].size();
             if (bytes_len) {
                 const void *bytes = Imei.dev_id[i].c_str();
                 if (i == 0)
                     expected_tee_enforced.push_back(TAG_ATTESTATION_ID_IMEI, bytes, bytes_len);
                 else
                     expected_tee_enforced.push_back(TAG_ATTESTATION_ID_SECOND_IMEI, bytes, bytes_len);
             }
        }
        for (size_t i = 0; i< Meid.dev_id_count && i < KM_MAX_SUBS; i++) {
    ...
    if (expected_sw_enforced != att_sw_enforced) {
        cerr << "att_sw_enforced" << endl;
        cerr << att_sw_enforced << endl;
        cerr << "expected_sw_enforced" << endl;
        cerr << expected_sw_enforced << endl;
        cerr << "SW enforced failed" << endl;
        return false;
    }
    ...
    if (expected_tee_enforced != att_tee_enforced) {
        cerr << "TEE enforced failed" << endl;
        cerr << "att_tee_enforced" << endl;
        cerr << att_tee_enforced << endl;
        cerr << "expected_tee_enforced" << endl;
        cerr << expected_tee_enforced << endl;
        return false;
    }
    ...
    if (provision_device_id) {
        ...
        for (size_t i = 0; i< Imei.dev_id_count && i < KM_MAX_SUBS; i++) {
             ...
             if (bytes_len) {
                 const void *bytes = Imei.dev_id[i].c_str();
                 if (i == 0)
                   attest_params.push_back(TAG_ATTESTATION_ID_IMEI, bytes, bytes_len);
                 else
                   attest_params.push_back(TAG_ATTESTATION_ID_SECOND_IMEI, bytes, bytes_len);
             }
        }
...
keymaster_error_t VerifyAttestation::Attestation() {
    ...
    if ((cert_chain.entry_count != 3) && (cert_chain.entry_count != 4)) {
        cerr << "entry_count failed " << cert_chain.entry_count << endl;
        return KM_ERROR_VERIFICATION_FAILED;
    }

    if (!verify_chain(cert_chain)) {
        cerr << "verify_chain failed " << endl;
        return KM_ERROR_VERIFICATION_FAILED;
    }
    ...
    if (!verify_attestation_record("challenge", sw_enforced(), hw_enforced(),
        ...
        cerr << "verify_attestation_record failed" << endl;
        return KM_ERROR_VERIFICATION_FAILED;
    }
...
keymaster_error_t VerifyAttestation::RsaAttestation() {
    ...
    ret = GenerateKey(AuthorizationSetBuilder()
                              ...
                              .Padding(KM_PAD_NONE));
    if (ret) {
        cerr << "RSA GenerateKey failed with ret " << ret << endl;
        return ret;
    }
...
keymaster_error_t VerifyAttestation::EcAttestation() {
    ...
    ret = GenerateKey(AuthorizationSetBuilder()
                              .EcdsaSigningKey(KM_EC_CURVE_P_256)
                              .Digest(KM_DIGEST_SHA_2_256)
                              .Authorization(TAG_INCLUDE_UNIQUE_ID));
    if (ret) {
        cerr << "ECC GenerateKey failed with ret " << ret << endl;
        return ret;
    }
