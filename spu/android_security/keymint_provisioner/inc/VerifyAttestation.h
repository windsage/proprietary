...
#ifndef SPU_KEYMINT_PROVISIONER_VERIFYATTESTATION_H_
#define SPU_KEYMINT_PROVISIONER_VERIFYATTESTATION_H_
...
class VerifyAttestation {
  public:
    VerifyAttestation(bool provision_device_id, string Brand, string Device, string Product,
                      string SerialNum, string Manufacturer, string Model, keymaster_device_id_t Imei, keymaster_device_id_t Meid)
        : device(new KeymasterHalDevice(keymaster::KM_SECURITY_LEVEL_STRONGBOX)),
          ...
          Imei(Imei),
          Meid(Meid),
          level(keymaster::KM_SECURITY_LEVEL_STRONGBOX){
  ...
  private:
    ...
    std::string Model;
    keymaster_device_id_t Imei;
    keymaster_device_id_t Meid;
    keymaster_security_level_t level;
};

#endif /* SPU_KEYMINT_PROVISIONER_VERIFYATTESTATION_H_ */
