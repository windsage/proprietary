...
using namespace ::vendor::qti::hardware::secureprocessor::common::aidl;
using ::ndk::ScopedAStatus;

#define VALID_SESSION(id) (((id) > 0) && ((id) < MAX_SESSION) && (sConfig_[(id)].active))
...
{
    std::vector<uint8_t> outConfig = {0};
 ...
 * buffers are expected to be prepared using SecureProcessorConfigAidl common
 * helper class.
 *
 * @param sessionId Session identifier.
 *
                                          ...
                                          std::vector<uint8_t> &outConfig)
{
    ...
    SecureProcessorConfigAidl *cfgOutWrapper = nullptr;
    uint32_t numEntries = 0;
    // Convert AIDL config to config wrapper class
    SecureProcessorConfigAidl *cfgInWrapper =
        new SecureProcessorConfigAidl((void *)inConfig.data());
    ...
    if ((operation & SET_CONFIG_MASK) == GET_CONFIG) {
        // Allocate the SecureProcessorConfigAidl class for outConfig and
        // it's associated config buffer with requested number of entries
        cfgOutWrapper = new SecureProcessorConfigAidl(numEntries);
...
exit:
    ...
    if ((operation & 0x1) == GET_CONFIG) {
        // Populate outConfig AIDL vector
        uint32_t size = cfgOutWrapper->getSize();
        const uint8_t *dummy = (uint8_t *)cfgOutWrapper->releaseAndGetBuffer();
        outConfig.assign(dummy, dummy + size);
        int sizetemp = outConfig.size();
                                             ...
                                             ConfigEntry *entry)
{
    ...
    switch (tag) {
        case (uint32_t)ConfigTag::SECURE_PROCESSOR_SESSION_CONFIG_NUM_SENSOR: {
            T_CHECK_ERR(sConfig_[sessionId].state ==
                            device::SECURE_PROCESSOR_STATE_INIT,
                        ErrorCode::SECURE_PROCESSOR_FAIL);
        ...
        case (uint32_t)
            ...
            if (0 != memcmp((void *)sConfig_[sessionId].taName.c_str(),
                ...
                memcpy_s((void *)(sConfig_[sessionId].taName.data()),
        ...
        case SECURE_PROCESSOR_CUSTOM_CONFIG_USECASE_LICENSE: {
            ...
            memcpy_s((void *)(sConfig_[sessionId].license), MAX_CERT_SIZE,
                     value, size);
            LOG_MSG("TAG: 0x%x, VALUE = %s", tag, value);
            ret = ErrorCode::SECURE_PROCESSOR_OK;
 ...
 * @param sessionId Session identifier.
 *
 * @param cfgWrapper SecureProcessorConfigAidl helper class.
 *
 ...
 *
 */
ErrorCode SecureProcessor::_createSessionCfg(
    uint32_t sessionId, SecureProcessorConfigAidl *cfgWrapper,
    ConfigEntry *entry)
{
    ErrorCode ret = ErrorCode::SECURE_PROCESSOR_FAIL;
    uint32_t tag = entry->tag;
 ...
 * using SecureProcessorConfigAidl common helper class.
 *
 * @param sessionId Session identifier.
 *
...
SecureProcessor::~SecureProcessor()
{
    ...
    for (uint32_t sessionId = 1; sessionId < MAX_SESSION; sessionId++) {
        if (sConfig_[sessionId].active) {
            ErrorCode ret = ErrorCode::SECURE_PROCESSOR_FAIL;
            stopSession(sessionId, &ret);
            deleteSession(sessionId, &ret);
        }
    }
}
 ...
 *
 */
ScopedAStatus SecureProcessor::createSession(sessionIDout *out_sessionId,
                                             ErrorCode *_aidl_return)
{
    ...
    for (sessionId = 1; sessionId < MAX_SESSION; sessionId++) {
        ...
        sConfig_[sessionId].state = device::SECURE_PROCESSOR_STATE_INIT;
        ...
        memcpy_s(sConfig_[sessionId].license, MAX_CERT_SIZE, gLicense,
                 sizeof(gLicense));
        break;
    }
    ret = ErrorCode::SECURE_PROCESSOR_OK;
...
exit:
    *_aidl_return = ret;
    out_sessionId->sID = (int)sessionId;
 ...
 * SecureProcessorConfigAidl common helper class.
 *
 * @param in_sessionId Session identifier.
 *
 * @param inConfig Input configuration buffer.
 *
 * @return status Return status of this operation:
...
ScopedAStatus SecureProcessor::setConfig(int32_t in_sessionId,
                                         const std::vector<uint8_t> &inConfig,
                                         ErrorCode *_aidl_return)
{
    std::lock_guard<std::mutex> lock(mLock_);
    ...
    T_CHECK_ERR(VALID_SESSION(in_sessionId),
                ErrorCode::SECURE_PROCESSOR_BAD_VAL);

    ret = _processConfig(in_sessionId, SET_SESSION_CONFIG, inConfig);
...
ScopedAStatus SecureProcessor::getConfig(int32_t in_sessionId,
                                         const std::vector<uint8_t> &inConfig,
                                         std::vector<uint8_t> *outConfig,
                                         ErrorCode *_aidl_return)
{
    ErrorCode ret = ErrorCode::SECURE_PROCESSOR_FAIL;
    std::vector<uint8_t> outConfigBuf;
    std::lock_guard<std::mutex> lock(mLock_);

    if (!VALID_SESSION(in_sessionId)) {
        LOG_MSG("unexpected sessionId[%d]", in_sessionId);
        *_aidl_return = ErrorCode::SECURE_PROCESSOR_BAD_VAL;
        *outConfig = outConfigBuf;
        return ndk::ScopedAStatus::ok();
    }

    ret = _processConfig(in_sessionId, GET_SESSION_CONFIG, inConfig,
                         outConfigBuf);
...
ScopedAStatus SecureProcessor::startSession(int32_t in_sessionId,
                                            ErrorCode *_aidl_return)
{
    ErrorCode ret = ErrorCode::SECURE_PROCESSOR_FAIL;
    ...
    T_CHECK_ERR(VALID_SESSION(in_sessionId),
                ErrorCode::SECURE_PROCESSOR_BAD_VAL);
    T_CHECK_ERR(
        sConfig_[in_sessionId].state == device::SECURE_PROCESSOR_STATE_INIT,
        ErrorCode::SECURE_PROCESSOR_FAIL);

    // Process any session specific configs as necessary
    // Set License should be first
    T_CHECK_ERR(sConfig_[in_sessionId].license_size < MAX_CERT_SIZE,
                (ErrorCode)SECURE_PROCESSOR_LICENSE_ERROR);

    certSize = sConfig_[in_sessionId].license_size;
    memcpy_s(cert, MAX_CERT_SIZE, sConfig_[in_sessionId].license, certSize);

    T_CHECK_ERR(mSeccam2App->initialize(sConfig_[in_sessionId].numSensor,
                                        (uint8_t *)cert, certSize) == Object_OK,
                ErrorCode::SECURE_PROCESSOR_FAIL);

    // Set State RUNNING
    sConfig_[in_sessionId].state = SECURE_PROCESSOR_STATE_RUNNING;
 ...
 * @param sessionId Session identifier.
 *
 * @param in_image Image handle for secure image data buffer.
 *
 * @param inConfig Input configuration buffer.
 *
 * @return status Return status of this operation:
...
ScopedAStatus SecureProcessor::processImage(
    int32_t in_sessionId, const NativeHandle &in_image,
    const std::vector<uint8_t> &inConfig, std::vector<uint8_t> *outConfig,
    ErrorCode *_aidl_return)
{
    ErrorCode ret = ErrorCode::SECURE_PROCESSOR_FAIL;
    std::vector<uint8_t> outConfigBuf;
    ...
    if (sConfig_[in_sessionId].state != SECURE_PROCESSOR_STATE_RUNNING) {
    ...
    _resetImgCfg(in_sessionId);

    T_CALL(_processConfig(in_sessionId, SET_IMAGE_CONFIG, inConfig));

    // Process image on requested secure destination w/ requested inCfg
    T_CALL(_processImage(in_sessionId, in_image));
...
ScopedAStatus SecureProcessor::stopSession(int32_t in_sessionId,
                                           ErrorCode *_aidl_return)
{
    ErrorCode ret = ErrorCode::SECURE_PROCESSOR_FAIL;
    std::lock_guard<std::mutex> lock(mLock_);
    ...
    T_CHECK_ERR(VALID_SESSION(in_sessionId),
                ErrorCode::SECURE_PROCESSOR_BAD_VAL);
    T_CHECK_ERR(sConfig_[in_sessionId].state == SECURE_PROCESSOR_STATE_RUNNING,
                ErrorCode::SECURE_PROCESSOR_FAIL);
...
ScopedAStatus SecureProcessor::deleteSession(int32_t in_sessionId,
                                             ErrorCode *_aidl_return)
{
    ErrorCode ret = ErrorCode::SECURE_PROCESSOR_FAIL;
    std::lock_guard<std::mutex> lock(mLock_);
    ...
    std::memset(&sConfig_[in_sessionId], 0, sizeof(sConfig_[0]));
