...
CHIPSET_64BIT_TAS_ONLY = ["taro", "kalama", "pineapple"]
...
SUPPORTED_CRYPTO_MAP = {
    ...
    "GPCE" : ["AES", "CMAC", "HMAC", "SHA", "TDES", "XTS", "CCM", "GCM"],
}

DEVICE_BINARY_NAME = "acvp_test"
DEVICE_BINARY_PATH = "/vendor/bin/acvp_test"
DEVICE_DATA_PATH = "/data/acvp"
DEVICE_BINARY_PATH_LE = "/usr/bin/acvp"
DEVICE_DATA_PATH_LE = "/firmware/image/acvp"
...
FIPSTESTAPP_PATH = ["/vendor/firmware_mnt/image/fipstestapp.b00",
                    "/vendor/firmware_mnt/image/fipstestapp.mbn"]
FIPSTESTAPP64_PATH = ["/vendor/firmware_mnt/image/fipstestapp64.b00",
                      "/vendor/firmware_mnt/image/fipstestapp64.mbn"]

chipset_name = ""

#Default to LA path. LE path will be updated if selected.
device_data_path = DEVICE_DATA_PATH
device_bin_path = DEVICE_BINARY_PATH
...
def parse_command_line_options():
    ...
    parser.add_option("-q", "--qseecom", dest="use_qseecom", action='store_true', help="Use Qseecom for Comm")
    ...
    parser.add_option("-d", "--devicepath", dest="lepath", action='store_true', help="Use device path for LE Targets")
...
def prepare_device(options):
    global device_data_path, device_bin_path

    if options.lepath:
        device_data_path = DEVICE_DATA_PATH_LE
        device_bin_path = DEVICE_BINARY_PATH_LE
    ...
    if options.push:
        ...
        if path:
            run_shell_command("adb push {0} {1}".format(path, device_bin_path))
            run_shell_command("adb shell chmod 777 {0}".format(device_bin_path))
...
def check_for_binary(name, path):
    ...
    list_check = isinstance(path, list)
    ...
    for path in path_list:
        file_exist = check_for_binary_exist(name, path)
...
def check_for_binary_exist(name, path):
...
def sanity_check_device(options, engines):
    ...
    check_for_binary(DEVICE_BINARY_NAME, device_bin_path)
...
def run_acvp_tests(engine, cryptos, options):
    ...
    qseecom_arg = 1 if options.use_qseecom else 0
    eng_dir = ENGINE_TO_DIRECTORY_MAP[engine]
    ...
    for (folder, prefix) in folders:
        ...
        command = "adb shell {0} {1} {2} {3} {4} {5} {6}".format(device_bin_path, engine, req_file, resp_file, crypto, mct_arg, qseecom_arg)
