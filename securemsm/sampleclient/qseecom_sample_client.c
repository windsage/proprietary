 ...
 ********************************************************************
---------------------------------------------------------------------
 Copyright (c) 2012-2018, 2023 Qualcomm Technologies, Inc.
 All Rights Reserved.
...
#define CLIENT_CMD200_LEGACY_LISTENER 			200
				...
				uint32_t size)
{
    ...
    if (v_addr == MAP_FAILED) {
        LOGE("Error: MMAP failed: heap %d, len %d, errno = %d\n",
                                                 ION_QSECOM_TA_HEAP_ID, len, errno);
        goto map_fail;
    }
...
static struct QSEECom_handle *listenerX;
struct QSEECom_handle *l_QSEEComHandle2 = NULL;
static pthread_t g_process_cmd_thread;
static void *dispatch(void *);

static void *dispatch(void *arg) {
  int ret;
  char buff[10];
  printf("Never Ending Thread \n");
  while (1) {
    printf("WILL BLOCK TILL TZ REQUESTS !!!!!!!!!\n");
    ret = QSEECom_receive_req(listenerX, listenerX->ion_sbuffer, 1 * 1024);
    if (ret == -ERESTARTSYS) {
      printf(" QSEECom_rcv_req got restartsys, resending the request\n");
      continue;
    } else if (ret) {
      printf(" QSEECom_rcv_req failed: ret value: %d\n", ret);
      break;
    }
    sleep(1);
    ret = QSEECom_send_resp(listenerX, buff, sizeof(buff));
	sleep(2);
  }
  pthread_exit(NULL);
  printf("\ncalled pthread exit \n");
...
static int listenerX_start() {
  int ret;

  ret = QSEECom_register_listener((void *)&listenerX, 125, 1 * 1024, 0);

  if (ret == QSEECOM_LISTENER_REGISTER_FAIL) {
    printf("Error: %s Failed to register listenerX listener\n", __func__);
  } else
    printf("LISTENERS ARE READY TO BE TESTED\n");
  return ret;
}

static int listenerX_stop() {
  int ret;
  printf("\n Calling unregister listener\n");
  ret = QSEECom_unregister_listener(listenerX);
  printf("\n Called unregister listener\n");
  if(ret == 0)
     listenerX = NULL;
  if (ret < 0)
    printf("Error: %s Failed to UN register listenerX listener\n", __func__);
  return ret;
}

static int listenerX_init() {
  int ret;
  ...
  ret = listenerX_start();
  if (ret < 0) {
    printf("%s:Error registering the handler\n", __func__);
    goto err;
  }
err:
  return ret;
...
int stop_listener_test_daemon() {

    char pid_str[10];
    char cmd[100] = "kill -9 ";
    int pid;
    FILE *fp = popen("pidof qseecom_sample_client", "r");

    if (!fp) {
        printf("Failed to open fp for getting pid\n");
        return -1;
    }
    fgets(pid_str, sizeof(pid_str), fp);
    strlcat(cmd, pid_str, sizeof(cmd));
    fp = popen(cmd, "r");
    if (!fp) {
        printf("Failed to open fp for killing daemon");
        return -1;
    }
    fclose(fp);
    sleep(1);
    printf("qseecom_sample_client Stopped!\n");
    return 0;
}

int32_t qsc_run_legacy_listener_test(uint32_t cmd_id, int dummy_thread)
{
    ...
    LOGD("Starting qsc_run_legacy_listener_test: cmd %d, thread %d", cmd_id, dummy_thread + 1);
    printf("Starting qsc_run_legacy_listener_test: cmd %d, thread %d\n", cmd_id, dummy_thread + 1);
    lstatus = listenerX_init();
    if (0 != lstatus) {
      printf("Error: registewring listener is failed!\n");
      return -1;
    }

    /* create a new thread to open the listenerX while loop, and return */
    ret = pthread_create(&g_process_cmd_thread, NULL, dispatch, NULL);
    if (0 != ret) {
      printf("Error: Creating a pthread in listenerX_start is failed!\n");
      return -1;
    }
	
	//Sending request from the TA
	ret = qsc_run_crypto_test(CLIENT_CMD200_LEGACY_LISTENER,dummy_thread);
	if (ret != 0) {
      printf("Error: Something went wrong in talking to listener\n");
      LOGE("Error: Something went wrong in talking to listener\n");
      return -1;
    }
		
    printf("\n Calling listener stop\n");
    lstatus = listenerX_stop();

    if (0 != lstatus) {
      printf("Error: stop listener is failed!\n");
      return -1;
    }
	
	ret = stop_listener_test_daemon();
	if (ret != 0) {
      printf("Error: Something went wrong in killing the client\n");
      LOGE("Error: Something went wrong in killing the client\n");
	  return -1;
    }

    pthread_join(g_process_cmd_thread, NULL);
...
static int32_t qsc_rpmb_check_legacy() {
	int ret = 0;
	uint32_t status = 0xFFFFFFFF; /* Here 0xFFFFFFFF is invalid case */
...
#ifndef TARGET_ENABLE_QSEECOM
int32_t qsc_rpmb_check_smci() {
   int ret = 0;
...
#ifndef TARGET_ENABLE_QSEECOM
int32_t qsc_rpmb_provision_key_smci() {
  int ret = 0;
  Object clientEnv = Object_NULL;
  Object rpmbObj = Object_NULL;
...
int qsc_run_latest_test_app(int argc, char *argv[])
{
	...
	case CLIENT_CMD200_LEGACY_LISTENER:
		ret = qsc_run_legacy_listener_test(atoi(argv[3]),dummy_thread);
		break;
/*
