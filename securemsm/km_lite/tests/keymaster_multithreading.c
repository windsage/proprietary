...
static void *keymaster_delete_multi(void *arg) {
    ...
    *err = keymaster_delete_sample(true);
    if (*err != KM_ERROR_OK)
        KM_LOGE("return %d\n", *err);
...
static keymaster_error_t keymaster_prepare_old_key(keymaster_key_upgraded_blob_t *key_blob) {
    ...
    if (err != KM_ERROR_OK) {
        KM_LOGE("fail to get old aes key return %d\n", err);
        return err;
    }
    ...
    if (err != KM_ERROR_KEY_REQUIRES_UPGRADE) {
         ...
         * Make sure the OS version/patch passed during initialization differ from the values
         * and embedded in the old key material.
         */
...
keymaster_error_t keymaster_multithreading(void) {
    ...
    if (pthread_join_check(tids, THREAD_NUMS, arg) != KM_ERROR_OK) {
        KM_LOGE("keymaster aes multithreading fail\n");
        return KM_ERROR_UNKNOWN_ERROR;
    }
    ...
    if (pthread_join_check(tids, THREAD_NUMS, arg) != KM_ERROR_OK) {
        KM_LOGE("keymaster rsa multithreading fail\n");
        return KM_ERROR_UNKNOWN_ERROR;
    }
    ...
    if (pthread_join_check(tids, THREAD_NUMS, arg) != KM_ERROR_OK) {
        KM_LOGE("keymaster ecdsa multithreading fail\n");
        return KM_ERROR_UNKNOWN_ERROR;
    }
    ...
    if (pthread_join_check(tids, THREAD_NUMS, arg) != KM_ERROR_OK) {
        KM_LOGE("keymaster hmac multithreading fail\n");
        return KM_ERROR_UNKNOWN_ERROR;
    }
    ...
    if (pthread_join_check(tids, THREAD_NUMS, arg) != KM_ERROR_OK) {
        KM_LOGE("keymaster import/export key multithreading fail\n");
        return KM_ERROR_UNKNOWN_ERROR;
    }
    ...
    if (pthread_join_check(tids, THREAD_NUMS, arg) != KM_ERROR_OK) {
        KM_LOGE("keymaster delete multithreading fail\n");
        return KM_ERROR_UNKNOWN_ERROR;
    }
    ...
    for (int i = 0; i < THREAD_NUMS; i++) {
        ...
        if (key_blob[i].err != KM_ERROR_OK || keymaster_verify_upgrade_key(&key_blob[i]) != KM_ERROR_OK) {
            KM_LOGE("keymaster verify upgrade key multithreading fail\n");
            return KM_ERROR_UNKNOWN_ERROR;
        }
    }
