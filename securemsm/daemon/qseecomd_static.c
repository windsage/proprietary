...
#define SEC_UI_LISTENER_INDEX 3 /* value dependent on struct 'qseecom_listener_services' */
	...
	if (hierarchy == QSEECOMD_PARENT_PROCESS) {
		switch (index) {
			case RPMB_LISTENER_INDEX:
				...
				break;
			case GPT_LISTENER_INDEX:
				ret = gpt_init_service();
				break;
...
int main() {
	...
	if(ret) {
		fprintf(stderr,"ERROR: GPT_INIT failed, shall not start listener services \n");
		return -1;
	}
	fprintf(stderr,"qseecom listener services GPT_INIT ok \n");
