...
int32_t TouchInput::_stStartSession(void)
{
    int32_t ret = 0;
...
int32_t TouchInput::_stTerminateSession(void)
{
    int32_t ret = 0;
    ssize_t writtenBytes = 0;
...
void TouchInput::_waitForInputThreadFunc(int32_t _timeout)
{
...
ScopedAStatus TouchInput::terminate(TUIResponse* _aidl_return)
{
    uint64_t c = 1;
    int32_t ret = -1;
    ssize_t writtenBytes = 0;
    ...
    if (mThread && mThread->joinable()) {
        LOGV("TouchInput::terminate: stopping wait-thread");
        mThread->join();
        mThread = nullptr;
    }
    LOGV("%s: TouchInput::terminate: wait-thread stopped", __func__);

    ret = _stTerminateSession();
    TUI_CHECK_ERR(ret == 0, ret);
end:
    *_aidl_return = (ret == 0) ? TUIResponse::TUI_SUCCESS : TUIResponse::TUI_FAILURE;
    return ndk::ScopedAStatus::ok();
}

ScopedAStatus TouchInput::getInput(int32_t timeout, TUIResponse* _aidl_return)
{
    int32_t ret = 0;
...
ScopedAStatus TouchInput::init(const std::shared_ptr<ITrustedInputCallback>& cb,
                               const std::string& in_displayType, TUIOutputID* outParam,
                               TUIResponse* _aidl_return)
{
    ...
    outParam->trustedInputID = mTouchControllerId;
    *_aidl_return = (ret == 0) ? TUIResponse::TUI_SUCCESS : TUIResponse::TUI_FAILURE;
    return ndk::ScopedAStatus::ok();
}
