...
class InputCBHelper : public TrustedInputCallBackImplBase {
 public:
    ...
    InputCBHelper(int32_t timeoutMs)
        : mInputTimeoutMs(timeoutMs), refcount(1) {
    }
    ...
    /*
     * @brief Get touch-data. This method blocks until data is available or gets timedout or abort() is called.
     ...
     */
    int32_t getInput_b(tsFingerData *fingers, int32_t inputTimeout /* in milliseconds*/);
