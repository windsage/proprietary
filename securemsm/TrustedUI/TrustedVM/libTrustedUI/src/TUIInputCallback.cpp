...
void InputCBHelper::retain() {
  auto old_value = refcount++;
  assert(old_value > 0);
}

bool InputCBHelper::release() {
  auto old_value = refcount--;
  if (refcount <= 1) {
    TUILOGE("Refcount: release %d abort ongoing session",uint32_t(refcount));
    abort();
  }
...
int32_t InputCBHelper::getInput_b(tsFingerData *fingers, int32_t inputTimeout) {
