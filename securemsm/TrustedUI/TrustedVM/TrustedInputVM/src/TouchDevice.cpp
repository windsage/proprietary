...
{
    bool bFound = false;
    if (str.find(mDeviceBusPath) != std::string::npos) {
        bFound = true;
    }
    return bFound;
}
...
int32_t TouchDevice::readTouchData(vector<int8_t> &buf, size_t &bufSize)
{
    ...
    int bytes_read = 0;
    int32_t newPrimaryEvent = 0;
    int32_t otherFingerEvent = 0;
    ...
    do {
       ...
       Need to have the whole packet, otherwise, don't decode*/
        ...
        if (ev->type == 3 && ev->code == 0x35) {
            ...
            gPrevX = mTSFinger.finger[curr_slot_id].x;
        ...
        if (ev->type == 0 && ev->code == 0 && ev->value == 0) {
            if (newPrimaryEvent == 1) {
                ...
                if (mTSFinger.finger[curr_slot_id].event != TOUCH_EVENT_NONE) {
                    if (mTSFinger.finger[curr_slot_id].x == -1) {
                        mTSFinger.finger[curr_slot_id].x = gPrevX;
                    }
                    if (mTSFinger.finger[curr_slot_id].y == -1) {
                        mTSFinger.finger[curr_slot_id].y = gPrevY;
                    }
                }
                ...
                break;
            } else if (otherFingerEvent == 1) {
                ret = ITrustedInput::ERROR_INVALID_FINGER_NUM;
                break;
            }
        }
    } while (1);
...
int32_t TouchDevice::openTouchDeviceFd(const uint32_t screenX,
                                       const uint32_t screenY,
                                       string deviceBusPath)
{
    int32_t ret = ITrustedInput::SUCCESS;
    string deviceNode = "";
    string touchDataLoc = "";
    char deviceNodeName[64] = "";
    mDeviceBusPath = deviceBusPath;
