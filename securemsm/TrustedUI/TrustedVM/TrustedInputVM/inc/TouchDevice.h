...
class TouchDevice
{
   ...
   public:
    TouchDevice() {
        memset(&mTSFinger, 0, sizeof(mTSFinger));
        mTSFinger.finger[curr_slot_id].x = -1;
        mTSFinger.finger[curr_slot_id].y = -1;
    }
    ...
    * In : screenX : screen X value from the Coreservice.
    *      screenY : screen Y value from the Coreservice.
    *      deviceBusPath: touch bus path from trusted_touch_device_path node
    *
    * Return : ITrustedInput::ERROR_READ_DEVICENODE,
    *          ITrustedInput::ERROR_GET_DEVICENODE,
    ...
    *
    */
    int32_t openTouchDeviceFd(const uint32_t screenX, const uint32_t screenY, string deviceBusPath);
