...
static int32_t getSecureIndicatorObj(Object *pSI2Obj)
{
    ...
    ret = IOpener_open(gTVMEnv, CSecureIndicator_UID, pSI2Obj);
...
int32_t loadSecureIndicator(void)
{
    ...
    ret = getSecureIndicatorObj(&SI2Obj);
    if (ret != Object_OK || Object_isNull(SI2Obj)) {
