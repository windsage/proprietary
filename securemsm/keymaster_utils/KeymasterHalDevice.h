...
class KeymasterHalDevice {
  public:
    KeymasterHalDevice(keymaster_security_level_t security_level)
        : cryptoDevice(new KeymasterCryptoDevice), utils(new KeymasterUtils(security_level)) {
        level = security_level;
    }
    virtual ~KeymasterHalDevice() {}

    uint32_t get_keymaster_version();
    uint32_t get_keymaster_api_version();
    ...
    keymaster_error_t destroy_att_keybox(void);
    ...
    keymaster_error_t compute_shared_hmac_with_seed_len(std::vector<hmac_sharing_with_seed_len_params_t> params,
                                          std::vector<uint8_t>& sharingCheck);
    ...
    keymaster_error_t keymint_begin_operation(keymaster_purpose_t purpose,
                                      const keymaster_key_blob_t* key,
                                      const keymaster_key_param_set_t* in_params,
                                      keymaster_key_param_set_t* out_params,
                                      keymaster_operation_handle_t* operation_handle);

    keymaster_error_t keymint_update_operation(keymaster_operation_handle_t operation_handle,
                                       const keymaster_key_param_set_t* in_params,
                                       const keymaster_blob_t* input, size_t* input_consumed,
    ...
    keymaster_error_t keymint_finish_operation(keymaster_operation_handle_t operation_handle,
                                       const keymaster_key_param_set_t* in_params,
                                       const keymaster_blob_t* input,
                                       const keymaster_blob_t* signature,
                                       keymaster_key_param_set_t* out_params,
    ...
    keymaster_error_t keymint_update_aad_operation(keymaster_operation_handle_t operation_handle,
                                        const keymaster_key_param_set_t* in_params);

    keymaster_error_t keymint_generate_key(const keymaster_key_param_set_t* params,
                                        keymaster_key_blob_t* attest_key_blob,
                                        keymaster_key_param_set_t* attest_key_params,
                                        keymaster_blob_t *attest_key_isssuer_subject_name,
                                        keymaster_key_blob_t* gen_key_blob,
                                        keymaster_key_characteristics_t* gen_key_characteristics,
                                        keymaster_cert_chain_t *gen_key_cert_chain);

     keymaster_error_t keymint_import_key(const keymaster_key_param_set_t* params,
                                        keymaster_key_format_t key_format,
                                        const keymaster_blob_t* key_data,
                                        /*AttestKey: keyBlob, attestKeyParams, issuerSubjectName*/
                                        keymaster_key_blob_t* attest_key_blob,
                                        keymaster_key_param_set_t* attest_key_params,
                                        keymaster_blob_t *attest_key_issuer_subject_name,
                                        /*KeyCreationResult: keyBlob, keyCharacteristics, certificateChain*/
                                        keymaster_key_blob_t* gen_key_blob,
                                        keymaster_key_characteristics_t* gen_key_characteristics,
                                        keymaster_cert_chain_t *gen_key_cert_chain);

    keymaster_error_t keymint_import_wrapped_key(const keymaster_key_param_set_t* params,
                                        const uint64_t passwordSid,
                                        const uint64_t biometricSid,
                                        const std::vector<uint8_t> wrappedKeyData,
                                        const std::vector<uint8_t> wrappingData,
                                        const std::vector<uint8_t> maskingKey,
                                        /*KeyCreationResult: keyBlob, keyCharacteristics, certificateChain*/
    ...
    keymaster_error_t keymint_convert_storage_key_to_ephemeral(const keymaster_key_blob_t* key_to_export,
                                        keymaster_blob_t* export_data);

    keymaster_error_t keymint_generate_timestamp(uint64_t challenge,
                                        verification_token_t* verification_token);

    keymaster_error_t keymint_generate_csr(const GenerateCsrRequest& request,
                                        const std::vector<uint8_t>& ephemeralMacKey,
                                        KeymasterBlob keys_to_sign_mac,
                                        std::vector<uint8_t>& deviceInfo,
                                        std::vector<uint8_t>& coseEncrypted);

    keymaster_error_t keymint_generate_csr_v2(const std::vector<uint8_t>& challenge,
                                              const std::vector<uint8_t>& pubKeysToSign,
                                              std::vector<uint8_t>& csr_blob);

    keymaster_error_t keymint_set_version(void);

    keymaster_error_t keymint_get_root_of_trust(keymaster_blob_t challenge, keymaster_blob_t* root_of_trust);
