...
int InstallKeybox::ValidateProvisionedKeys() {
    ...
    int result = provisioningClient->SetProvisioningKeyboxSuccess();
    if (result) {
        cerr << "SetProvisioningKeyboxSuccess error" << result << endl;
        return result;
    }
...
int InstallKeybox::ParseFile(bool provision_only_device_id) {
    ...
    GUARD_EXIT(provisioningClient->SetProvisioningDeviceIdSuccess());
...
int main(int argc, char** argv) {
    ...
    if (argc == 7 || argc == 8) {
        ...
        PROV_EXIT(installer_sb.ParseFile(provision_only_device_id));
        if (!provision_only_device_id)
