...
void* initTXSTATUSRSPOpParms(A_UINT8 *pParmsCommon, PARM_OFFSET_TBL *pParmsOffset, PARM_DICT *pParmDict)
{
    ...
    memset(pTXSTATUSRSPParms->latestMeasPwrOut, 0, sizeof(pTXSTATUSRSPParms->latestMeasPwrOut));
    ...
    fillParmOffsetTbl((A_UINT32)PARM_LATESTMEASPWROUT, (A_UINT32)(((A_UINT32)&(pTXSTATUSRSPParms->latestMeasPwrOut)) - (A_UINT32)pTXSTATUSRSPParms), pParmsOffset);
...
//#ifdef _DEBUG
    ...
    for (i = 0; i < 8 ; i++) // can be modified to print up to 16 entries
    {
        A_PRINTF("TXSTATUSRSPOp: latestMeasPwrOut %u\n", pTXSTATUSRSPParms->latestMeasPwrOut[i]);
    }
