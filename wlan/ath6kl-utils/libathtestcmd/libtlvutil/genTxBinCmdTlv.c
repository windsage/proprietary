...
int addParameterToCommandWithRawdata(uint8_t *rawData, uint32_t len, int isHeader)
{
	...
	if (isHeader) {
		...
		//int ret = FALSE;
		if (rawData == NULL ||
		    len < (sizeof(_TESTFLOW_CMD_STREAM_HEADER) + sizeof(_ONE_CMD_HEADER))) {
			printf("%s:Error! rawData NULL or len:%d incorrect.\n",
			 __func__, len);
			return FALSE;
                }
	...
	while (1) {
		...
		if (RawDataPos && (parmLen < RawDataPos)) {
			printf("Error: invalid parmlen:%d", parmLen);
			return FALSE;
		}
