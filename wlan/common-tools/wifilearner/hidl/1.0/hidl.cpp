...
int wifilearner_hidl_process(struct wifilearner_ctx *wlc)
{
	...
	if (service_wifistats->registerAsService(WIFI_STATS_SERVICE_NAME) !=
	    android::OK) {
	...
	if (manager != nullptr) {
		ret = manager->registerClientCallback(
				WIFI_STATS_FQ_SERVICE_NAME,
				WIFI_STATS_SERVICE_NAME, service_wifistats,
				new WifiStatsClientCallback);
	}
	if(!ret) {
