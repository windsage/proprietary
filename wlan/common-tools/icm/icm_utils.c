...
/*
 * Function     : convert_to_mhz_chan_width
 * Description  : Convert ICM channel width to channel width in MHz
 * Input params : channel width
...
uint32_t convert_to_mhz_chan_width(ICM_CH_BW_T ch_width)
{
    switch (ch_width) {
       case ICM_CH_BW_320:
           return CHWIDTH_320_MHZ;
       case ICM_CH_BW_160:
           return CHWIDTH_160_MHZ;
       case ICM_CH_BW_80:
           return CHWIDTH_80_MHZ;
       case ICM_CH_BW_40PLUS:
       case ICM_CH_BW_40MINUS:
       case ICM_CH_BW_40:
           return CHWIDTH_40_MHZ;
       default:
           return CHWIDTH_20_MHZ;
    }
}
...
int icm_convert_mhz2channel(u_int32_t freq)
{
    ...
    if (freq > BAND_6GHZ_FREQ_BASE) {
        if (freq == BAND_6GHZ_FREQ_ORPHAN_CHAN_2)
            return (freq - BAND_6GHZ_FREQ_BASE) / 5;
...
u_int32_t icm_convert_ieee2mhz(int chan, ICM_BAND_T band)
{
    ...
    if (band == ICM_BAND_6G) {
        if (chan == BAND_6GHZ_ORPHAN_CHAN_NUM)
            return BAND_6GHZ_FREQ_BASE + (chan * 5);
...
int icm_get_iface_addr(ICM_DEV_INFO_T* pdev, const char* ifname, u_int8_t *ifaddr)
{
...
int icm_phy_spec_to_str(ICM_PHY_SPEC_T physpec, char *str, int strbufflen)
{
    ...
    switch(physpec)
    {
        ...
        case ICM_PHY_SPEC_11BEA:
            os_strlcpy(str, "11BEA", strbufflen);
            status = SUCCESS;
            break;

        case ICM_PHY_SPEC_11BEG:
            os_strlcpy(str, "11BEG", strbufflen);
            status = SUCCESS;
            break;
                         ...
                         char *str, int strbufflen)
{
    ...
    if (str == NULL || strbufflen < ICM_MAX_CH_POWER_MODE_SIZE) {
        return status;
    }
    ...
    switch(power_mode)
    {
        case ICM_STANDARD_POWER_MODE:
            os_strlcpy(str, "SP", strbufflen);
            status = SUCCESS;
            break;
        case ICM_LOW_POWER_MODE:
            os_strlcpy(str, "LPI", strbufflen);
            status = SUCCESS;
            break;
        case ICM_VERY_LOW_POWER_MODE:
            os_strlcpy(str, "VLP", strbufflen);
            status = SUCCESS;
            break;
        default:
            os_strlcpy(str, "none", strbufflen);
        ...
        int phymodelen)
{
    ...
    switch(physpec)
    {
        ...
        case ICM_PHY_SPEC_11BEA:
            cont = os_strlcpy(phymode, "11AEHT", phymodelen);
            break;

        case ICM_PHY_SPEC_11BEG:
            cont = os_strlcpy(phymode, "11GEHT", phymodelen);
            break;
    ...
    switch (width)
    {
        ...
        case ICM_CH_BW_320:
            if (physpec != ICM_PHY_SPEC_11BEA) {
                err("%s: Invalid PHY spec enumeration %d with width 320 MHz",
                        __func__,
                        physpec);
                return NULL;
            }

            os_strlcpy(phymode + cont, "320", phymodelen - cont);
            break;
