...
/*
 * Function     : icm_mark_secondary_40MHz_80MHz_channels
 * Description  : Based EHTOP IEs,mark the secondary 40 and secondary 80 channels
 *                Used only for 320 MHz operation
 * Input params : pointer to icm, pointer to scan state, control channel, segment center freq,
 *                extension channel offset
 * Return       : void
 *
 */
void icm_mark_secondary_40MHz_80MHz_channels(ICM_INFO_T* picm,
        struct iwscan_state *state,
        int ctrl_channel,
        int segment1_cf,
        int segment2_cf,
        int ext_chan_offset)
{
    ...
    if (ext_chan_offset == SEC_CHANNEL_OFFSET_ABOVE) {
        ...
        if (ICM_IEEE80211_IS_CHAN_11BEA_EHT40PLUS(ch)) {
            extension_channel = ctrl_channel + SEC_CHANNEL_OFFSET_ABOVE_IDX;
        } else {
            extension_channel = ctrl_channel + SEC_CHANNEL_OFFSET_BELOW_IDX;
        }
    ...
    if (ctrl_channel < segment1_cf) {
        start_channel = segment1_cf - CHANNEL_BOUNDARY_OFFSET_160MHZ;

        /* Loop through the channel list and find the channels
         * between start_channel and primary 160 MHz cfreq.
         */

        for (i = 0; i < pchlist->count; i++) {
            if ((pchlist->ch[i].channel >= start_channel) &&
                    (pchlist->ch[i].channel <= segment1_cf)) {

                if ((pchlist->ch[i].channel != ctrl_channel) &&
                        (pchlist->ch[i].channel != extension_channel)) {
                    pchlist->ch[i].used_as_320_secondary_40++;
                    pchlist->ch[i].used_as_160_80p80_secondary_40--;
                }
            }
        }

    } else if (ctrl_channel > segment1_cf) {
        end_channel = segment1_cf + CHANNEL_BOUNDARY_OFFSET_160MHZ;

        /* Loop through the channel list and find the channels
         * between start_channel and segment2_cf.
         */
        for (i = 0; i < pchlist->count; i++) {
            if ((pchlist->ch[i].channel >= segment1_cf) &&
                    (pchlist->ch[i].channel <= end_channel)) {
                if ((pchlist->ch[i].channel != ctrl_channel) &&
                        (pchlist->ch[i].channel != extension_channel)) {
                    pchlist->ch[i].used_as_320_secondary_40++;
    ...
    if (ctrl_channel < segment2_cf) {
        start_channel = segment2_cf - CHANNEL_BOUNDARY_OFFSET_320MHZ;
        ...
        for (i = 0; i < pchlist->count; i++) {
            if ((pchlist->ch[i].channel >= start_channel) &&
                    (pchlist->ch[i].channel <= segment1_cf)) {
                pchlist->ch[i].used_as_320_secondary_80++;
                pchlist->ch[i].used_as_160_secondary_80--;
            }
        }

    } else if (ctrl_channel > segment2_cf) {
        end_channel = segment2_cf + CHANNEL_BOUNDARY_OFFSET_320MHZ;

        /* Loop through the channel list and find the channels
         * between start_channel and 320 MHz cfreq.
         */
        for (i = 0; i < pchlist->count; i++) {
            if ((pchlist->ch[i].channel >= segment1_cf) &&
                    (pchlist->ch[i].channel <= end_channel)) {
...
/*
 * Function     : icm_analyze_eht_channels
 * Description  : Based on EHTOP IEs, analyze the usage pattern of
 *                EHT Channels.
 * Input params : pointer to icm, pointer to scan state
 * Return       : void
 *
 */
void icm_analyze_eht_channels(ICM_INFO_T* picm, struct iwscan_state *state)
{
    ICM_DEV_INFO_T* pdev = get_pdev();
    ICM_CHANNEL_LIST_T *pchlist = NULL;
    int i = 0;
    int chn_idx = 0;
    int chan_info_is_valid = 0;
    int ctrl_channel = 0;
    ICM_HTINFO_T *htinfo = NULL;
    ICM_HEOP_T *heop = NULL;
    ICM_HEOP_6GHZOPINFO_T *heop_6ghzopinfo = NULL;
    ICM_EHTOP_T *ehtop = NULL;
    ...
    if ((picm->slist.elem[state->ap_num].band == ICM_BAND_6G) &&
        (heop->is_valid && heop_6ghzopinfo->is_valid)) {
        chan_info_is_valid = 1;
        chn_idx = icm_get_channel_index(pchlist, heop_6ghzopinfo->primary_channel);
        ctrl_channel = heop_6ghzopinfo->primary_channel;
    } else if (htinfo->is_valid) {
        chan_info_is_valid = 1;
        chn_idx = icm_get_channel_index(pchlist, htinfo->control_channel);
        ctrl_channel = htinfo->control_channel;
    }

    ehtop = &(picm->slist.elem[state->ap_num].ehtop);

    if (chan_info_is_valid && ehtop->is_valid) {
        switch (ehtop->channel_width) {
            case EHT_CHANNEL_WIDTH_40_MHZ:
                {
                    if (chn_idx != -1) {
                        pchlist->ch[chn_idx].num_wnw_40++;
                        pchlist->ch[chn_idx].num_wnw_20--;
                    ...
                    ICM_DPRINTF(pdev, ICM_PRCTRL_FLAG_NONE, ICM_DEBUG_LEVEL_DEFAULT,
                        ICM_MODULE_ID_SCAN, "EHT 20/40 (No Sec 40)\n");
                }
                break;
            case EHT_CHANNEL_WIDTH_80_MHZ:
                {
                    if (chn_idx != -1) {
                        pchlist->ch[chn_idx].num_wnw_80++;
                        pchlist->ch[chn_idx].num_wnw_20--;
                ...
                break;
            case EHT_CHANNEL_WIDTH_160_MHZ:
                {
                    int segment_cf = ehtop->channel_cf_seg2;
                ...
                break;
            case EHT_CHANNEL_WIDTH_320_MHZ:
                {
                    int segment_one_cf = ehtop->channel_cf_seg1;
                    int segment_two_cf = ehtop->channel_cf_seg2;

                    if (chn_idx != -1) {
                        pchlist->ch[chn_idx].num_wnw_320++;
                        pchlist->ch[chn_idx].num_wnw_160--;
                        pchlist->ch[chn_idx].num_wpf_320 += picm->wpf;
                        pchlist->ch[chn_idx].num_wpf_160 -= picm->wpf;
                    }

                    /* Mark all 40MHz and 80MHz Secondary Channels */
                    icm_mark_secondary_40MHz_80MHz_channels(picm, state, ctrl_channel,
                            segment_one_cf, segment_two_cf, 0);

                    for (i = 0; i < pchlist->count; i++)
                    {
                        if (((ctrl_channel < segment_two_cf) &&
                             (pchlist->ch[i].channel >= (segment_two_cf + 2)) &&
                             (pchlist->ch[i].channel <= (segment_two_cf + 30))) ||
                            ((ctrl_channel > segment_two_cf) &&
                             (pchlist->ch[i].channel <= (segment_two_cf - 2)) &&
                             (pchlist->ch[i].channel >= (segment_two_cf - 30)))) {
                            pchlist->ch[i].used_as_320_secondary_160++;
                        }
                    }
                    ICM_DPRINTF(pdev, ICM_PRCTRL_FLAG_NONE,
                            ICM_DEBUG_LEVEL_DEFAULT, ICM_MODULE_ID_SCAN,
...
/*
 * Function     : icm_parse_and_update_ehtop
 * Description  : Parse and store values of interest in EHT Operation for the
 *                given BSS.
 *                The caller is expected to have verified that the buffer being
 *                passed contains a valid EHT Operation IE.
 * Input params : pointer to icm, pointer to scan state, IE buffer, IE buffer
 *                length
 * Return       : void
 */
void icm_parse_and_update_ehtop(ICM_INFO_T* picm, struct iwscan_state *state,
        u8 *iebuf, int buflen)
{
    struct ieee80211_ie_ehtoperation *ehtop_ie = NULL;
    ICM_EHTOP_T *ehtop = NULL;

    bool is_eht_channel_analysis_required = false;

    ICM_ASSERT(NULL != picm);
    ICM_ASSERT(NULL != state);
    ICM_ASSERT(NULL != iebuf);

    if (buflen < sizeof(struct ieee80211_ie_ehtoperation)) {
        return;
    }

    ehtop_ie = (struct ieee80211_ie_ehtoperation *)iebuf;

    ehtop = &(picm->slist.elem[state->ap_num].ehtop);

    if (EHTOPER_GET_EHTOPINFO_PRESENT(ehtop_ie->ehtop_param)) {
        ehtop->is_valid = 1;

        ehtop->channel_width = EHTOPER_GET_EHT_CHANNEL_WIDTH(ehtop_ie->eht_control);
        ehtop->channel_cf_seg1 = ehtop_ie->eht_chan_center_freq_seg0;
        ehtop->channel_cf_seg2 = ehtop_ie->eht_chan_center_freq_seg1;

        is_eht_channel_analysis_required = true;
    }

    if (true == is_eht_channel_analysis_required) {
        icm_analyze_eht_channels(picm, state);
    }
}
        ...
        unsigned char *iebuf, int buflen)
{
    ...
    switch(iebuf[2])
    {
        ...
        case WLAN_EXTN_EID_EHTOP:
            icm_parse_and_update_ehtop(picm, state, iebuf, buflen);
            break;
#endif
        default:
            break;
    ...
    for (i = 0; i < pchlist->count; i++) {
        if (pchlist->ch[i].channel == channel) {
            ...
            if ((ICM_IEEE80211_IS_CHAN_11AC_VHT80(c) ||
                    ...
                    !pchlist->ch[i].ic_ch_freq_seg1) {
                pchlist->ch[i].ic_ch_freq_seg1 = c->ic_vhtop_ch_num_seg1;
            }

            if ((ICM_IEEE80211_IS_CHAN_11AC_VHT80_80(c) ||
                    ICM_IEEE80211_IS_CHAN_11AXA_HE80_80(c)) &&
                    !pchlist->ch[i].ic_ch_freq_seg2) {
                pchlist->ch[i].ic_ch_freq_seg2 = c->ic_vhtop_ch_num_seg2;
            }
        ...
        const struct ieee80211_ath_channel *c)
{
    ...
    for (i = 0; i < pchlist->count; i++) {
        ...
        if (pchlist->ch[i].channel == channel) {
            ...
            found = TRUE;
            pchlist->ch[i].ic_ch_freq_seg1_160 = c->ic_vhtop_ch_num_seg1;
