...
icm_scan_and_select_channel(ICM_INFO_T* picm, bool setchannel)
{
    ...
    ret = icm_select_home_channel_perband_per_pwrmode(picm, band, picm->channel_width,
                &selresults);
...
static const uint16_t valid_puncture_pattern_80[]  = {0x1, 0x2, 0x4, 0x8, 0x0};
static const uint16_t valid_puncture_pattern_160[] = {0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x03, 0x0C, 0x30, 0xC0, 0x0};
static const uint16_t valid_puncture_pattern_320[] = {0x0003, 0x000C, 0x0030, 0x00C0, 0x0300, 0x0C00, 0x3000, 0xC000, 0x000F, 0x00F0,
                                                      0x0F00, 0xF000, 0xF003, 0xF00C, 0xF030, 0xF0C0, 0xF300, 0xFC00, 0x003F, 0x00CF,
                                                      0x030F, 0x0C0F, 0x300F, 0xC00F, 0x0};
/*
 * Function     : icm_find_puncture_bitmap
 * Description  : Helper function to find the puncture bitmap for
 *                puncturing due to CW interference
 * Input params : -pointer to channel info structure,
 *                -width
 * Return       : 1 on success, 0 on failure
 */
int icm_validate_puncture_bitmap(ICM_CHANNEL_T *ch, ICM_CH_BW_T ch_bw)
{
    uint16_t const *pp_ptr = NULL;
    int i;
    int len;
    uint16_t mask = ICM_PUNCTURE_NONE;

    ICM_ASSERT(ch != NULL);

    if (!ch->puncture_bitmap)
        return 1;

    if (!ICM_IEEE80211_IS_CHAN_11BEA(ch)) {
        ch->puncture_bitmap = ICM_PUNCTURE_INVALID;
        return 1;
    }

    if (ch_bw == ICM_CH_BW_20 || ch_bw == ICM_CH_BW_40) {
       ch->puncture_bitmap = ICM_PUNCTURE_NONE;
        return 1;
    }

    switch (ch_bw) {
        case ICM_CH_BW_320:
            pp_ptr = valid_puncture_pattern_320;
            len = QDF_ARRAY_SIZE(valid_puncture_pattern_320);
            mask = ICM_PUNCTURE_320MHZ_MASK;
            break;
        case ICM_CH_BW_160:
            pp_ptr = valid_puncture_pattern_160;
            len = QDF_ARRAY_SIZE(valid_puncture_pattern_160);
            mask = ICM_PUNCTURE_160MHZ_MASK;
            break;
        case ICM_CH_BW_80:
            pp_ptr = valid_puncture_pattern_80;
            len = QDF_ARRAY_SIZE(valid_puncture_pattern_80);
            mask = ICM_PUNCTURE_80MHZ_MASK;
            break;
        default:
            icm_printf("Invalid channel width %d\n", ch_bw);
            return 0;
    }

    /* Intersect input puncture pattern with legitimate patterns.
     * Due to uniqueness of valid puncture patterns intersection
     * will give nearest best possible puncture pattern.
     */
    for (i = 0; i < len; i++) {
        if (pp_ptr[i] == ((pp_ptr[i] | ch->puncture_bitmap) & mask)) {
            ch->puncture_bitmap = pp_ptr[i];
            return 1;
        }
    }
    return 0;
...
/*
 * Function: icm_find_non_punctured_bw
 *
 * Description: Given a puncture pattern, find the
 * largest non-punctured bandwidth including
 * the primary channel.
 *
 * Input params: -pointer to channel info structure,
 *               -width
 *               - segment ID indicating the 320 MHz channel
 *                  bonding to use.
 *
 * Return: non-punctured bandwidth
 */
int icm_find_non_punctured_bw(ICM_CHANNEL_T *ch, ICM_CH_BW_T ch_bw, u_int8_t seg_id)
{
    int non_punc_width = ICM_CH_BW_20;
    u_int16_t puncture_pattern;

    ICM_ASSERT(ch != NULL);

    /* If the channel is not punctured, return the original channel width */
    if (!ch->puncture_bitmap) {
        return convert_to_mhz_chan_width(ch_bw);
    }

    puncture_pattern = ch->puncture_bitmap;

    switch (ch_bw) {
        case ICM_CH_BW_320:
            puncture_pattern &= ICM_PUNCTURE_320MHZ_MASK;
            if (!puncture_pattern) {
                break;
            }
            non_punc_width = CHWIDTH_160_MHZ;
            if (ch->ic_ch_freq_seg1_160 > ch->ic_ch_freq_seg1_320[seg_id]) {
                puncture_pattern >>= (CHWIDTH_160_MHZ/CHWIDTH_20_MHZ);
            }
        /* fallthrough */
        case ICM_CH_BW_160:
            puncture_pattern &= ICM_PUNCTURE_160MHZ_MASK;
            if (!puncture_pattern) {
                break;
            }
            non_punc_width = CHWIDTH_80_MHZ;
            if (ch->ic_ch_freq_seg1 > ch->ic_ch_freq_seg1_160) {
                puncture_pattern >>= (CHWIDTH_80_MHZ/CHWIDTH_20_MHZ);
            }
        /* fallthrough */
        case ICM_CH_BW_80:
            puncture_pattern &= ICM_PUNCTURE_80MHZ_MASK;
            if (!puncture_pattern) {
                break;
            }
            non_punc_width = CHWIDTH_40_MHZ;

            if (ch->freq > ch->ic_ch_freq_seg1) {
                puncture_pattern >>= (CHWIDTH_40_MHZ/CHWIDTH_20_MHZ);
            }

            puncture_pattern &= (ICM_PUNCTURE_80MHZ_MASK >>
                                 (CHWIDTH_40_MHZ/CHWIDTH_20_MHZ));
            if (!puncture_pattern)
                break;

           /* fallthrough */
            default:
                non_punc_width = CHWIDTH_20_MHZ;
                break;
    }
    return non_punc_width;
}

/*
 * Function: icm_find_punctured_bw
 *
 * Description: Find bandwidth of a given puncture bitmap
 * for both non-ofdma and ofdma modes.
 *
 * Input params: -pointer to channel info structure
 *               -width
 *               -nofdma_bw: non ofdma bandwidth to be derived
 *               -ofdma_bw: ofdma bandwidth derived to be derived
 *
 * Return: void
 */
void icm_find_punctured_bw(ICM_CHANNEL_T *chan, int bw,
                           int *nofdma_bw, int *ofdma_bw)
{
    uint32_t nbits = 0;
    uint16_t puncture_bitmap_nofdma = 0;
    uint16_t puncture_bitmap_ofdma = 0;
    int i;

    nbits = bw/CHWIDTH_20_MHZ;

    *nofdma_bw = bw;
    *ofdma_bw = bw;

    if ((bw < CHWIDTH_20_MHZ) || (bw > CHWIDTH_320_MHZ)) {
        icm_printf("Invalid bw %d", bw);
        return;
    }
    /* Currently puncture bitmap is legitimate pattern so
    * non ofdma pattern and ofdma patterns will be same */
    puncture_bitmap_nofdma = chan->puncture_bitmap;
    puncture_bitmap_ofdma = chan->puncture_bitmap;

    for (i = 0; i < nbits; i++) {
        if (puncture_bitmap_nofdma & (BIT(i))) {
            *nofdma_bw -= CHWIDTH_20_MHZ;
        }
        if (puncture_bitmap_ofdma & (BIT(i))) {
            *ofdma_bw -= CHWIDTH_20_MHZ;
        }
    }
}

/*
 * Function: icm_calculate_puncture_usability
 *
 * Description: Calculate the channel usability, given a puncture
 *              bitmap for the channel.
 *
 * Input params: -pointer to icm info structure
 *               -pointer to channel info structure,
 *               -segment ID indicating the 320 MHz channel
 *               -width
 *               -usability: channel usability to be derived
 *
 * Return: void
 */
void icm_calculate_puncture_usability(ICM_INFO_T* picm, ICM_CHANNEL_T *ch, u_int8_t seg_id,
                                      ICM_CH_BW_T ch_bw, u_int16_t *usability)
{
    u_int32_t legacy_usab = 0;
    u_int32_t nofdma_usab = 0;
    u_int32_t ofdma_usab = 0;
    int legacy_bw = CHWIDTH_20_MHZ;
    int nofdma_bw;
    int ofdma_bw;
    int bw;

    if (!ICM_IEEE80211_IS_CHAN_11BEA(ch) ||
        ch_bw == ICM_CH_BW_20 || ch_bw == ICM_CH_BW_40) {
        *usability = 0;
        return;
    }

    if (!icm_validate_puncture_bitmap(ch, ch_bw)) {
        *usability = 0;
        return;
    }

    if ((ch->puncture_bitmap == ICM_PUNCTURE_NONE) ||
        (ch->puncture_bitmap == ICM_PUNCTURE_INVALID))
        return;

    bw = convert_to_mhz_chan_width(ch_bw);
    legacy_bw = icm_find_non_punctured_bw(ch, ch_bw, seg_id);

    icm_find_punctured_bw(ch, bw, &nofdma_bw, &ofdma_bw);

    legacy_usab = *usability * legacy_bw * picm->puncture_legacy_weightage;
    nofdma_usab = *usability * nofdma_bw * picm->puncture_nofdma_weightage;
    ofdma_usab = *usability * ofdma_bw * picm->puncture_ofdma_weightage;

    *usability = (legacy_usab + nofdma_usab + ofdma_usab)/(100 * bw);
    icm_printf("Puncture bitmap : %u scaling down usability to %u\n", ch->puncture_bitmap, *usability);
}
 ...
 *                -band
 *                -width
 *                -segment ID (valid for 80+80 and 320 MHz, ignored otherwise) - 0 for
 *                primary 80 MHz, 1 for secondary 80 MHz.
 *                For 320 MHz, 0 indicates 320+ channel bondings and 1 indicates
 *                320- channel bondings.
        ...
        ICM_AFC_POWER_MODE_T power_mode)
{
    ...
    u_int16_t prepnl_msrd_usability_320_ppdu = 0;
    ...
    double num_ap, num_ap_pri40, num_ap_sec40, num_ap_pri80, num_ap_sec80,
              num_ap_80, num_ap_160, num_ap_320;
    double num_ap_pri40_aci, num_ap_sec40_aci, num_ap_pri80_aci,
              num_ap_sec80_aci, num_ap_sec160_aci;
    double num_ap_tot_40ppdu_compete, num_ap_tot_80ppdu_compete,
              num_ap_tot_160ppdu_compete, num_ap_tot_320ppdu_compete;
    u_int16_t num_ap_srp_supported = 0;
    u_int16_t num_ap_client_srp_allowed = 0;
    u_int16_t num_ap_client_obsspd_allowed = 0;
    ...
    ICM_ASSERT(!((ch_bw > ICM_CH_BW_20) &&
                 ...
                 (picm->phy_spec != ICM_PHY_SPEC_11BEA)));
    ...
    for (chan_idx = 0; chan_idx < chanlist->count; chan_idx++) {
        ...
        ICM_DPRINTF(pdev,
                ...
                "320 MHz OBSS APs with primary on this channel                :%d\n"
                ...
                "320 MHz OBSS APs with secondary 40 MHz on this channel       :%d\n"
                "320 MHz OBSS APs with secondary 80 MHz on this channel       :%d\n"
                "320 MHz OBSS APs with secondary 160 MHz on this channel      :%d\n"
                "Measured free time indication for this channel alone         :%u\n"
                "Primary 20 MHz channel performance grade                     :%hhu\n"
                "Average Tx power for max throughput                          :%d dBm\n"
                "Average Tx power for max range                               :%d dBm\n"
                "Off-Channel CAC status                                       :%hhu\n\n",
                chan_num,
                ch->num_wnw,
                ch->num_wnw_20,
                ch->num_wnw_40,
                ...
                ch->used_as_320_secondary_40,
                ch->used_as_320_secondary_80,
                ch->used_as_320_secondary_160,
                ...
                ICM_GET_CHANNEL_OCAC_STATUS(picm, band, chan_num));
#endif /* ANDROID */
        ...
        /*
         * Pri20 exclusion may be requested by the driver based on the 6GHz power
         * mode (SP/LPI/VLP) because a channel is incapable of supporting the power
         * mode.
         */
        if (is_rejection_allowed(ICM_SELDBG_REJCODE_PRI_EXCLD)
            && ((band == ICM_BAND_6G && picm->afc_capable &&
                (power_mode < ICM_MAX_POWER_MODE) &&
                (ICM_GET_CHANNEL_PWR_MODE_EXCLUDE(picm, band,
                      chan_num, power_mode) == TRUE))) == TRUE) {
            ICM_SET_CHANNEL_USABLITY(picm, band, chan_num, 0);
            ICM_SELDBG_SETFIELD(decdbginfo[chan_idx].usability, 0);
            ICM_SELDBG_SETFIELD(prichandbginfo->is_excluded, true);
            ICM_SELDBG_SETFIELD(decdbginfo[chan_idx].rejreason,
        ...
        /* Check if the DFS channel has completed its Off-Channel CAC */
        if (is_rejection_allowed(ICM_SELDBG_REJCODE_PRI_OCAC) &&
            ICM_IEEE80211_IS_CHAN_DFS(ch) &&
            (ICM_GET_CHANNEL_OCAC_STATUS(picm, band, chan_num) ==
                       OCAC_STATUS_PENDING)) {
            ICM_SELDBG_SETFIELD(prichandbginfo->is_ocac_pending, true);
            ICM_SELDBG_SETFIELD(decdbginfo[chan_idx].rejreason,
                                ICM_SELDBG_REJCODE_PRI_OCAC);
            ICM_SET_CHANNEL_USABLITY(picm, band, chan_num, 0);
            ICM_SELDBG_SETFIELD(decdbginfo[chan_idx].usability, 0);
            continue;
        } else {
            /* If rejection is not allowed or if OCAC status is clear or
             * if status is invalid, then don't reject the channel
             */
            ICM_SELDBG_SETFIELD(prichandbginfo->is_ocac_pending, false);
        }
        ...
        if (is_rejection_allowed(ICM_SELDBG_REJCODE_PRI_INCAP) &&
                ...
                (((picm->phy_spec == ICM_PHY_SPEC_11NA) &&
                  !(ICM_IEEE80211_IS_CHAN_11N_CTL_U_CAPABLE(ch))) ||
                 ((picm->phy_spec == ICM_PHY_SPEC_11AC) &&
                  !(ICM_IEEE80211_IS_CHAN_11AC_VHT40PLUS(ch))) ||
                 ((picm->phy_spec == ICM_PHY_SPEC_11AXA) &&
                  !(ICM_IEEE80211_IS_CHAN_11AXA_HE40PLUS(ch))) ||
                 ((picm->phy_spec == ICM_PHY_SPEC_11BEA) &&
                  !(ICM_IEEE80211_IS_CHAN_11BEA_EHT40PLUS(ch))))) {
        ...
        if (is_rejection_allowed(ICM_SELDBG_REJCODE_PRI_INCAP) &&
            ...
            (((picm->phy_spec == ICM_PHY_SPEC_11NA) &&
              !(ICM_IEEE80211_IS_CHAN_11N_CTL_L_CAPABLE(ch))) ||
             ((picm->phy_spec == ICM_PHY_SPEC_11AC) &&
              !(ICM_IEEE80211_IS_CHAN_11AC_VHT40MINUS(ch))) ||
             ((picm->phy_spec == ICM_PHY_SPEC_11AXA) &&
              !(ICM_IEEE80211_IS_CHAN_11AXA_HE40MINUS(ch))) ||
             ((picm->phy_spec == ICM_PHY_SPEC_11BEA) &&
              !(ICM_IEEE80211_IS_CHAN_11BEA_EHT40MINUS(ch))))) {
            ICM_SELDBG_SETFIELD(prichandbginfo->is_incapable, true);
            ICM_SELDBG_SETFIELD(decdbginfo[chan_idx].rejreason,
                    ICM_SELDBG_REJCODE_PRI_INCAP);
        ...
        if (is_rejection_allowed(ICM_SELDBG_REJCODE_PRI_INCAP) &&
            (((ch_bw == ICM_CH_BW_80) &&
              !(ICM_IEEE80211_IS_CHAN_11AC_VHT80(ch)) &&
              !(ICM_IEEE80211_IS_CHAN_11AXA_HE80(ch)) &&
              !(ICM_IEEE80211_IS_CHAN_11BEA_EHT80(ch))) ||
             ((ch_bw == ICM_CH_BW_80_PLUS_80) && (seg_id == 0) &&
              !(ICM_IEEE80211_IS_CHAN_11AC_VHT80_80(ch)) &&
              !(ICM_IEEE80211_IS_CHAN_11AXA_HE80_80(ch))) ||
             ((ch_bw == ICM_CH_BW_160) &&
              !(ICM_IEEE80211_IS_CHAN_11AC_VHT160(ch)) &&
              !(ICM_IEEE80211_IS_CHAN_11AXA_HE160(ch)) &&
              !(ICM_IEEE80211_IS_CHAN_11BEA_EHT160(ch))))) {
            ICM_SELDBG_SETFIELD(prichandbginfo->is_incapable, true);
            ICM_SELDBG_SETFIELD(decdbginfo[chan_idx].rejreason,
                    ICM_SELDBG_REJCODE_PRI_INCAP);
        ...
        /* Check if 320 MHz has been requested and if the current
           channel is capable of the same.
         */
        if (is_rejection_allowed(ICM_SELDBG_REJCODE_PRI_INCAP) &&
            ((ch_bw == ICM_CH_BW_320) &&
              (!(ICM_IEEE80211_IS_CHAN_11BEA_EHT320(ch)) ||
              (ch->ic_ch_freq_seg1_320[seg_id] == 0)))) {
            ICM_SELDBG_SETFIELD(prichandbginfo->is_incapable, true);
            ICM_SELDBG_SETFIELD(decdbginfo[chan_idx].rejreason,
                    ICM_SELDBG_REJCODE_PRI_INCAP);
        ...
        if (ch_bw == ICM_CH_BW_80) {
            ...
            if (chan_num == (ch->ic_ch_freq_seg1 - 6)) {
            ...
            } else if (chan_num == (ch->ic_ch_freq_seg1 - 2)) {
            ...
            } else if (chan_num == (ch->ic_ch_freq_seg1 + 6)) {
            ...
            } else if (chan_num == (ch->ic_ch_freq_seg1_160 + 14)) {
            ...
            adj_check_min_adj = cw_check_min_adj - 1;
            adj_check_max_adj = cw_check_max_adj + 1;
        } else if (ch_bw == ICM_CH_BW_320) {
            int cw_check_min_adj_firstchan = 0;
            int cw_check_max_adj_firstchan = 16;

            if (band == ICM_BAND_5G) {
                cw_check_min_adj_firstchan = 0;
                cw_check_max_adj_firstchan = 12;
            }

            if (chan_num == (ch->ic_ch_freq_seg1_320[seg_id] - 30)) {
                /* This is the first channel in the 320 MHz block, being
                   at a displacement of -30 channels * 5 MHz = -150 MHz from
                   the center */
                cw_check_min_adj = cw_check_min_adj_firstchan;
                cw_check_max_adj = cw_check_max_adj_firstchan;
            } else if (chan_num == (ch->ic_ch_freq_seg1_320[seg_id] - 26)) {
                /* This is the second channel in the 320 MHz block, being
                   at a displacement of -26 channels * 5 MHz = -130 MHz from
                   the center */
                cw_check_min_adj = cw_check_min_adj_firstchan - 1;
                cw_check_max_adj = cw_check_max_adj_firstchan - 1;
            } else if (chan_num == (ch->ic_ch_freq_seg1_320[seg_id] - 22)) {
                /* This is the third channel in the 320 MHz block, being
                   at a displacement of -22 channels * 5 MHz = -110 MHz from
                   the center */
                cw_check_min_adj = cw_check_min_adj_firstchan - 2;
                cw_check_max_adj = cw_check_max_adj_firstchan - 2;
            } else if (chan_num == (ch->ic_ch_freq_seg1_320[seg_id] - 18)) {
                /* This is the fourth channel in the 320 MHz block, being
                   at a displacement of -18 channels * 5 MHz = -90 MHz from
                   the center */
                cw_check_min_adj = cw_check_min_adj_firstchan - 3;
                cw_check_max_adj = cw_check_max_adj_firstchan - 3;
            } else if (chan_num == (ch->ic_ch_freq_seg1_320[seg_id] - 14)) {
                /* This is the fourth channel in the 320 MHz block, being
                   at a displacement of -14 channels * 5 MHz = -70 MHz from
                   the center */
                cw_check_min_adj = cw_check_min_adj_firstchan - 4;
                cw_check_max_adj = cw_check_max_adj_firstchan - 4;
            } else if (chan_num == (ch->ic_ch_freq_seg1_320[seg_id] - 10)) {
                /* This is the fourth channel in the 320 MHz block, being
                   at a displacement of -10 channels * 5 MHz = -50 MHz from
                   the center */
                cw_check_min_adj = cw_check_min_adj_firstchan - 5;
                cw_check_max_adj = cw_check_max_adj_firstchan - 5;
            } else if (chan_num == (ch->ic_ch_freq_seg1_320[seg_id] - 6)) {
                /* This is the fourth channel in the 320 MHz block, being
                   at a displacement of -6 channels * 5 MHz = -30 MHz from
                   the center */
                cw_check_min_adj = cw_check_min_adj_firstchan - 6;
                cw_check_max_adj = cw_check_max_adj_firstchan - 6;
            } else if (chan_num == (ch->ic_ch_freq_seg1_320[seg_id] - 2)) {
                /* This is the fourth channel in the 320 MHz block, being
                   at a displacement of -2 channels * 5 MHz = -10 MHz from
                   the center */
                cw_check_min_adj = cw_check_min_adj_firstchan - 7;
                ...
                /* This is the fifth channel in the 320 MHz block, being
                   at a displacement of +2 channels * 5 MHz = +10 MHz from
                   the center */
                cw_check_min_adj = cw_check_min_adj_firstchan - 8;
                cw_check_max_adj = cw_check_max_adj_firstchan - 8;
            } else if (chan_num == (ch->ic_ch_freq_seg1_320[seg_id] + 6)) {
                /* This is the sixth channel in the 320 MHz block, being
                   at a displacement of +6 channels * 5 MHz = +30 MHz from
                   the center */
                cw_check_min_adj = cw_check_min_adj_firstchan - 9;
                cw_check_max_adj = cw_check_max_adj_firstchan - 9;
            } else if (chan_num == (ch->ic_ch_freq_seg1_320[seg_id] + 10)) {
                /* This is the seventh channel in the 320 MHz block, being
                   at a displacement of +10 channels * 5 MHz = +50 MHz from
                   the center */
                cw_check_min_adj = cw_check_min_adj_firstchan - 10;
                cw_check_max_adj = cw_check_max_adj_firstchan - 10;
            } else if (chan_num == (ch->ic_ch_freq_seg1_320[seg_id] + 14)) {
                /* This is the eighth channel in the 320 MHz block, being
                   at a displacement of +14 channels * 5 MHz = +70 MHz from
                   the center */
                cw_check_min_adj = cw_check_min_adj_firstchan - 11;
                cw_check_max_adj = cw_check_max_adj_firstchan - 11;
            } else if (chan_num == (ch->ic_ch_freq_seg1_320[seg_id] + 18)) {
                /* This is the eighth channel in the 320 MHz block, being
                   at a displacement of +18 channels * 5 MHz = +90 MHz from
                   the center */
                cw_check_min_adj = cw_check_min_adj_firstchan - 12;
                cw_check_max_adj = cw_check_max_adj_firstchan - 12;
            } else if (chan_num == (ch->ic_ch_freq_seg1_320[seg_id] + 22)) {
                /* This is the eighth channel in the 320 MHz block, being
                   at a displacement of +22 channels * 5 MHz = +110 MHz from
                   the center */
                cw_check_min_adj = cw_check_min_adj_firstchan - 13;
                cw_check_max_adj = cw_check_max_adj_firstchan - 13;
            } else if (chan_num == (ch->ic_ch_freq_seg1_320[seg_id] + 26)) {
                /* This is the eighth channel in the 320 MHz block, being
                   at a displacement of +26 channels * 5 MHz = +130 MHz from
                   the center */
                cw_check_min_adj = cw_check_min_adj_firstchan - 14;
                cw_check_max_adj = cw_check_max_adj_firstchan - 14;
            } else if (chan_num == (ch->ic_ch_freq_seg1_320[seg_id] + 30)) {
                /* This is the eighth channel in the 320 MHz block, being
                   at a displacement of +30 channels * 5 MHz = +150 MHz from
                   the center */
                cw_check_min_adj = cw_check_min_adj_firstchan - 15;
                cw_check_max_adj = cw_check_max_adj_firstchan - 15;
            } else {
                /* We shouldn't get here! */
                ICM_ASSERT(0);
            }
        ...
        for (bondspan_chan_adj = cw_check_min_adj; bondspan_chan_adj < cw_check_max_adj; bondspan_chan_adj++) {
            ...
            if (bondspan_chan_num != chan_num) {
                ...
                /* Exclude channels not supporting current 6GHz power mode */
                if (is_rejection_allowed(ICM_SELDBG_REJCODE_BOND_EXCLD) &&
                    ((band == ICM_BAND_6G && picm->afc_capable &&
                    (power_mode < ICM_MAX_POWER_MODE) &&
                    (ICM_GET_CHANNEL_PWR_MODE_EXCLUDE(picm, band,
                     bondspan_chan_num, power_mode) == TRUE))) == TRUE) {
                    if (relchandbginfo != NULL) {
                        ICM_SELDBG_SETFIELD(relchandbginfo->is_excluded, true);
                    }
                    /* Exclude all the cases where AFC puncturing is not allowed
                       and mark the usability as zero for these cases:
                       - Any power mode other than SP
                       - Channel does not support 11be
                       - Channel width is 20 MHz or 40 MHz
                     */
                    if (power_mode != ICM_STANDARD_POWER_MODE ||
                        !ICM_IEEE80211_IS_CHAN_11BEA(ch) ||
                        !ICM_IS_PUNCTURING_SUPPORTED_FOR_CHAN_WIDTH(ch_bw)) {
                        ICM_SELDBG_SETFIELD(decdbginfo[chan_idx].rejreason,
                        ICM_SELDBG_REJCODE_BOND_EXCLD);
                        usability = 0;
                        break;
                    }
                    /* Mark the extension channel as punctured if SP power. */
                    ch->puncture_bitmap |= BIT(bondspan_chan_adj - adj_check_min_adj - 1);
                } else if (relchandbginfo != NULL) {
                    ICM_SELDBG_SETFIELD(relchandbginfo->is_excluded, false);
                }
            }
            ...
            if (bondspan_chan_num != chan_num) {
                ...
                } else if ((ch_bw == ICM_CH_BW_80 || ch_bw == ICM_CH_BW_160 || ch_bw == ICM_CH_BW_320 ||
                            ((ch_bw == ICM_CH_BW_80_PLUS_80) && (seg_id == 0))) &&
            ...
            if (pconf->nf_enab) {
                if (nf > nf_threshold &&
                    ...
                    if (bondspan_chan_num == chan_num) {
                        ...
                        /* Mark the extension channel as punctured */
                        ch->puncture_bitmap |= BIT(bondspan_chan_adj - adj_check_min_adj - 1);
                    }
            ...
            if (bondspan_chan_num != chan_num) {
                ...
                if (is_rejection_allowed(ICM_SELDBG_REJCODE_BOND_OCAC) &&
                    ICM_IEEE80211_IS_CHAN_DFS(&chanlist->ch[bondspan_chan_idx]) &&
                    (ICM_GET_CHANNEL_OCAC_STATUS(picm, band, bondspan_chan_num)
                                  == OCAC_STATUS_PENDING)) {

                    if (relchandbginfo != NULL) {
                        ICM_SELDBG_SETFIELD(relchandbginfo->is_ocac_pending,
                                            true);
                    }
                    ICM_SELDBG_SETFIELD(decdbginfo[chan_idx].rejreason,
                                        ICM_SELDBG_REJCODE_BOND_OCAC);
                     ...
                     * if status is invalid, then don't reject the channel
                     */
                    ICM_SELDBG_SETFIELD(relchandbginfo->is_ocac_pending, false);
                }
            }
        }
        if (ICM_IEEE80211_IS_CHAN_11BEA(ch) && ch->puncture_bitmap)
            icm_calculate_puncture_usability(picm, ch, seg_id, ch_bw, &usability);
        ...
        if (ch_bw < ICM_CH_BW_80) {
            ...
            for (bondspan_chan_adj = adj_check_min_adj; bondspan_chan_adj < adj_check_max_adj; bondspan_chan_adj++) {
                ...
                if (((bondspan_chan_adj == adj_check_min_adj) ||
                    ...
                    ICM_SELDBG_MARKFIELD_NA(relchandbginfo->is_ocac_pending);
                ...
                if (bondspan_chan_adj == adj_check_min_adj)
                {
                    ...
                    prepnl_msrd_usability_160_ppdu =
                        MIN(prepnl_msrd_usability_160_ppdu,
                                ICM_GET_CHANNEL_FREE_TIME(picm, band,
                                            bondspan_chan_num));
                } else if (((chan_num < ch->ic_ch_freq_seg1_160) &&
                            (bondspan_chan_num <
                             ch->ic_ch_freq_seg1_160)) ||
                        ((chan_num > ch->ic_ch_freq_seg1_160) &&
                         (bondspan_chan_num >
                          ch->ic_ch_freq_seg1_160)))  {
            ...
            /* The usability is computed based on
               ...
               c) potential 160 MHz PPDU Tx requirement
               AND
               d) Potential 320 MHz PPDU Tx requirement

               |--Independent--|---------------|--Independent--|--Independent--|--Independent--|---------------|--Independent--|--Independent--|
               20/40 MHz Tx                    20/40 MHz Tx    20/40 MHz Tx     20/40 MHz Tx                    20/40 MHz Tx    20/40 MHz Tx

               |-----Independent 80 MHz Tx-----|-----Independent 80 MHz Tx-----|-----Independent 80 MHz Tx-----|-----Independent 80 MHz Tx-----|

               |-----------------------160 MHz span Tx-------------------------|-----------------------160 MHz span Tx-------------------------|

               |--------------------------------------------------------320 MHz span Tx--------------------------------------------------------|

               The 20/40 MHz PPDU Tx requirement is affected by
               i)  Competing 320 MHz PPDU Tx.
               ii)  Competing 160 MHz PPDU Tx.
               iii) Competing 80 MHz PPDU Tx in the same 80 MHz segment as ours,
               or 80+80 MHz PPDU Tx with our 80 MHz segment as the secondary
               80 MHz.
               iv)Competing 20/40 MHz PPDU Tx in our own 40 MHz block. We treat
               20 and 40 MHz under the same granularity.
               v) Transmissions in immediate adjacent channel outside 320 MHz
               block due to ACI, if we are at the edge. (XXX: To be
               quantified further).
               ...
               or 80+80 MHz PPDU Tx with our 80 MHz segment as the secondary
               80 MHz.
               iv)Competing 20/40 MHz PPDU Tx in the same 80 MHz segment as
               ours.
               v) Transmissions in adjacent channel outside 320 MHz block due
               to ACI.

               The 160 MHz PPDU Tx requirement is affected by
               i)  Competing 320 MHz PPDU Tx.
               ii)  Competing 160 MHz PPDU Tx.
               iii) Competing 80/80+80 MHz PPDU Tx in either segment.
               iv)Competing 20/40 MHz PPDU Tx in our own primary 40 MHz block,
               or 20/40 MHz PPDU Tx anywhere in the secondary 80 MHz segment.
               v) Transmissions in both adjacent channels outside 320 MHz
               block due to ACI.

               The 320 MHz PPDU Tx requirement is affected by
               i)  Competing 320 MHz PPDU Tx.
               ii)  Competing 160 MHz PPDU Tx.
               iii) Competing 80/80+80 MHz PPDU Tx in either segment.
               iv)Competing 20/40 MHz PPDU Tx in our own primary 40 MHz block,
               ...
               We compute separate usabilities for each requirement,
               and then combine the usabilities in a configurable ratio.
               While computing these usabilities, we take into consideration
               how each is affected to decide which APs to count.

               Thus, the 20/40 MHz usability is computed by dividing the existing
               usability value by the sum of the following:
               i)  Competing 320 MHz APs.
               ii)  Competing 160 MHz APs.
               iii) Competing 80 MHz APs in the same 80 MHz segment as ours,
               or 80+80 MHz APs with our 80 MHz segment as the secondary
               80 MHz.
               iv)Competing 20/40 MHz APs in our own 40 MHz block.
               v) APs in immediate adjacent channel outside 320 MHz
               block due to ACI, if we are at the edge.
               ...
               80 MHz.
               iv)Competing 20/40 MHz APs in our own 40 MHz block.
               v) APs in adjacent channel outside 320 MHz block due
               to ACI.
            ...
            existing usability value by the sum of the following:
               i)  Competing 320 MHz APs.
               ii)  Competing 160 MHz APs.
               iii) Competing 80/80+80 MHz APs in either segment.
               iv)Competing 20/40 MHz APs in our own 40 MHz block.
                or 20/40 MHz APs anywhere in the secondary 80 MHz segment.
               v) APs in adjacent channel outside 320 MHz block due
               to ACI.

               The 320 MHz usability is computed separately by dividing the
            existing usability value by the sum of the following:
               i)  Competing 320 MHz APs.
               ii)  Competing 160 MHz APs.
               iii) Competing 80/80+80 MHz APs in either segment.
               ...
               required in the future.
            */
            uint8_t usage_factor_320_bw320, usage_factor_320_bw160, usage_factor_320_bw80;
            uint8_t usage_factor_320_bw40;

            num_ap_pri40 = num_ap_pri80 = num_ap_sec80 = num_ap_160 = num_ap_320 = 0;
            num_ap_pri40_aci = num_ap_pri80_aci = num_ap_sec80_aci = num_ap_sec160_aci = 0;
            num_ap_tot_40ppdu_compete = num_ap_tot_80ppdu_compete =
                num_ap_tot_160ppdu_compete = num_ap_tot_320ppdu_compete = 0;
            usability_40_ppdu = usability_80_ppdu = usability_160_ppdu = usability_320_ppdu =
                usability;
            skip_channel = 0;
            ...
            prepnl_msrd_usability_40_ppdu =
                prepnl_msrd_usability_80_ppdu =
                prepnl_msrd_usability_160_ppdu =
                prepnl_msrd_usability_320_ppdu =
                ICM_GET_CHANNEL_FREE_TIME(picm, band, chan_num);
            ...
            for (bondspan_chan_adj = adj_check_min_adj;
                ...
                /* Note that the focus of the loop is to determine
                   how suitable the current chan_num is for becoming the
                   primary 20 MHz channel for the 320 MHz block.
                   In each pass of the loop, the effect of each
                   bondspan_chan_num on that suitability is
                   determined. */
                relchandbginfo = NULL;
                ...
                if (bondspan_chan_adj == adj_check_min_adj)
                {
                    ...
                    num_ap_320 +=
                        chanlist->ch[bondspan_chan_idx].num_wpf_320;
                } else if ((((picm->phy_spec == ICM_PHY_SPEC_11AC) &&
                                ICM_IEEE80211_IS_CHAN_11AC_VHT40PLUS(ch)) ||
                            ((picm->phy_spec == ICM_PHY_SPEC_11AXA) &&
                                ICM_IEEE80211_IS_CHAN_11AXA_HE40PLUS(ch)) ||
                    ...
                    prepnl_msrd_usability_320_ppdu =
                        MIN(prepnl_msrd_usability_320_ppdu,
                                ICM_GET_CHANNEL_FREE_TIME(picm, band,
                                            bondspan_chan_num));
                } else if ((((picm->phy_spec == ICM_PHY_SPEC_11AC) &&
                                ICM_IEEE80211_IS_CHAN_11AC_VHT40MINUS(ch)) ||
            ...
            num_ap_tot_40ppdu_compete = (num_ap_pri40 + num_ap_pri40_aci + \
                    num_ap_pri80 + num_ap_160 + num_ap_320);
            num_ap_tot_80ppdu_compete = (num_ap_pri40 + num_ap_pri40_aci + \
                    num_ap_pri80_aci + num_ap_pri80 +   \
                    num_ap_160 + num_ap_320);
            num_ap_tot_160ppdu_compete = (num_ap_pri40 + num_ap_pri40_aci + \
                    num_ap_pri80_aci + num_ap_sec80_aci +\
                    num_ap_pri80 + num_ap_sec80 + \
                    num_ap_160 + num_ap_320);
            num_ap_tot_320ppdu_compete = (num_ap_pri40 + num_ap_pri40_aci + \
                    num_ap_pri80_aci + num_ap_sec80_aci + num_ap_sec160_aci + \
                    num_ap_pri80 + num_ap_sec80 + \
                    num_ap_160 + num_ap_320);
            ...
            usability_80_ppdu /= (num_ap_tot_80ppdu_compete + 1);
            usability_160_ppdu /= (num_ap_tot_160ppdu_compete + 1);
            usability_320_ppdu /= (num_ap_tot_320ppdu_compete + 1);

            if (band == ICM_BAND_5G) {
                usage_factor_320_bw320 = picm->usage_factor_5g_320_bw320;
                usage_factor_320_bw160 = picm->usage_factor_5g_320_bw160;
                usage_factor_320_bw80 = picm->usage_factor_5g_320_bw80;
                usage_factor_320_bw40 = picm->usage_factor_5g_320_bw40;
            } else if (band == ICM_BAND_6G) {
                usage_factor_320_bw320 = picm->usage_factor_6g_320_bw320;
                usage_factor_320_bw160 = picm->usage_factor_6g_320_bw160;
                usage_factor_320_bw80 = picm->usage_factor_6g_320_bw80;
                usage_factor_320_bw40 = picm->usage_factor_6g_320_bw40;
            }

            usability = (u_int16_t)((((u_int32_t)usability_320_ppdu) *       \
                            (usage_factor_320_bw320)/100) + \
                            (((u_int32_t)usability_160_ppdu) *       \
                             (usage_factor_320_bw160)/100) +\
                            (((u_int32_t)usability_80_ppdu) *       \
                             (usage_factor_320_bw80)/100) +\
                            (((u_int32_t)usability_40_ppdu) *       \
                             (usage_factor_320_bw40)/100));
            ...
            prepnl_msrd_usability = (u_int16_t)((((u_int32_t)prepnl_msrd_usability_320_ppdu) * \
                                     (usage_factor_320_bw320)/100) + \
                                     (((u_int32_t)prepnl_msrd_usability_160_ppdu) * \
                                     (usage_factor_320_bw160)/100) +\
                                     (((u_int32_t)prepnl_msrd_usability_80_ppdu) * \
                                     (usage_factor_320_bw80)/100) +\
                                     (((u_int32_t)prepnl_msrd_usability_40_ppdu) * \
            ...
            ICM_SELDBG_SETFIELD(decdbginfo[chan_idx].tot_num_aps_320w_40ppdu,
                    num_ap_tot_40ppdu_compete + 1);
            ICM_SELDBG_SETFIELD(decdbginfo[chan_idx].tot_num_aps_320w_80ppdu,
                    num_ap_tot_80ppdu_compete + 1);
            ICM_SELDBG_SETFIELD(decdbginfo[chan_idx].tot_num_aps_320w_160ppdu,
                    num_ap_tot_160ppdu_compete + 1);
            ICM_SELDBG_SETFIELD(decdbginfo[chan_idx].tot_num_aps_320w_320ppdu,
                    num_ap_tot_320ppdu_compete + 1);
            ICM_SELDBG_SETFIELD(decdbginfo[chan_idx].usage_factor_320_bw40,
                    usage_factor_320_bw40);
            ICM_SELDBG_SETFIELD(decdbginfo[chan_idx].usage_factor_320_bw80,
                    usage_factor_320_bw80);
            ICM_SELDBG_SETFIELD(decdbginfo[chan_idx].usage_factor_320_bw160,
                    usage_factor_320_bw160);
            ICM_SELDBG_SETFIELD(decdbginfo[chan_idx].usage_factor_320_bw320,
                    usage_factor_320_bw320);
            ICM_SELDBG_SETFIELD(decdbginfo[chan_idx].usability_40ppdu,
                    usability_40_ppdu);
            ICM_SELDBG_SETFIELD(decdbginfo[chan_idx].usability_80ppdu,
                    usability_80_ppdu);
            ICM_SELDBG_SETFIELD(decdbginfo[chan_idx].usability_160ppdu,
                    usability_160_ppdu);
            ICM_SELDBG_SETFIELD(decdbginfo[chan_idx].usability_320ppdu,
                    usability_320_ppdu);
        } else if ((ch_bw == ICM_CH_BW_80_PLUS_80) && (seg_id == 0)) {
        ...
        if (band == ICM_BAND_6G) {
            if (!ICM_IEEE80211_IS_CHAN_PSC(ch)) {
                ICM_SELDBG_SETFIELD(\
        ...
        ICM_SET_CHANNEL_USABLITY(picm, band, chan_num, usability);
        ICM_SET_CHANNEL_PUNCTURE_BITMAP(picm, band, chan_num, ch->puncture_bitmap);
        ICM_SELDBG_SETFIELD(decdbginfo[chan_idx].usability, usability);

        /* The maximum regulatory power is set as per the STA and it can be 7dB more
         * for the AP. The maximum value allowed is the maxpower supported by HW.
         */
        /* XXX: Check for 802.11ac related changes here */
        ICM_SET_CHANNEL_MAX_REGPWR(picm, band, chan_num,
                MIN(ch->ic_maxpower, ch->ic_maxregpower));
...
/*
 * Function     : icm_select_home_channel_perband_per_pwrmode
        ...
        ICM_CH_BW_T desired_width, ICM_SELECTION_RESULTS_INFO_T *selresults)
{
    ...
    int best_chan_320plus = -1;
    int best_chan_320minus = -1;
    u_int16_t best_usability_320plus = 0;
    u_int16_t best_usability_320minus = 0;
    ...
    do {
        ...
        switch(cnddt_channel_width)
        {
            ...
            case ICM_CH_BW_40:
                {
                    ...
                    if (band == ICM_BAND_2_4G) {
                        ...
                        best_chan_40plus = icm_select_best_chan_5_or_6ghz(picm,
                                band, ICM_CH_BW_40PLUS, 0, 0, selresults->selected_pwr_mode);
                    }
                    ...
                    if (band == ICM_BAND_2_4G) {
                        ...
                        best_chan_40minus = icm_select_best_chan_5_or_6ghz(picm,
                                band, ICM_CH_BW_40MINUS, 0, 0, selresults->selected_pwr_mode);
                    }
            ...
            case ICM_CH_BW_80:
                {
                    if (band == ICM_BAND_2_4G) {
                        ...
                        best_chan = icm_select_best_chan_5_or_6ghz(picm,
                                band, ICM_CH_BW_80, 0, 0, selresults->selected_pwr_mode);
                    }
            ...
            case ICM_CH_BW_160:
                {
                    if (band == ICM_BAND_2_4G) {
                        ...
                        best_chan = icm_select_best_chan_5_or_6ghz(picm,
                                band, ICM_CH_BW_160, 0, 0, selresults->selected_pwr_mode);
                    }
            ...
            case ICM_CH_BW_320:
                {
                    if (band == ICM_BAND_6G) {
                    /* Try with both 320+ and 320- channel bondings,
                       and choose the best channel from among the two. */
                        ...
                        ICM_DPRINTF(pdev,
                            ...
                            ICM_DEBUG_LEVEL_DEFAULT,
                            ICM_MODULE_ID_SELECTOR,
                            "First, finding best channel for 320+ bonded channels\n");
                    }
                    ...
                    if (band == ICM_BAND_2_4G) {
                        ...
                        ICM_ASSERT(0);
                                        ...
                                        (band == ICM_BAND_6G)) {
                        best_chan_320plus = icm_select_best_chan_5_or_6ghz(picm,
                                band, ICM_CH_BW_320, 0, 0, selresults->selected_pwr_mode);
                    }

                    /* 5 GHz band has only one possible instance of 320 MHz channel width */
                    if (band == ICM_BAND_5G) {
                        if (best_chan_320plus == -1) {
                            ICM_DPRINTF(pdev,
                                ICM_PRCTRL_FLAG_NONE,
                                ICM_DEBUG_LEVEL_DEFAULT,
                                ICM_MODULE_ID_SELECTOR,
                            ...
                            if (pdev->conf.server_mode) {
                                ...
                                /* Try with a lower width */
                                cnddt_channel_width = ICM_CH_BW_160;
                            }
                        } else {
                            best_chan = best_chan_320plus;
                            cnddt_channel_width = ICM_CH_BW_320;
                            is_search_active = false;
                        }
                    }

                    if (band == ICM_BAND_6G) {
                        if ((best_chan_320plus < 0) || (best_chan_320plus >= MAX_NUM_CHANNEL)) {
                            best_chan_320plus = 0;
                            best_usability_320plus = 0;
                        } else {
                           best_usability_320plus =
                                ICM_GET_CHANNEL_USABLITY(picm, band,
                                            best_chan_320plus);
                        }
                        ...
                        ICM_DPRINTF(pdev,
                            ...
                            ICM_DEBUG_LEVEL_DEFAULT,
                            ICM_MODULE_ID_SELECTOR,
                            "Next, finding best channel for 320- bonded channels\n");

                        if ((band == ICM_BAND_2_4G) || (band == ICM_BAND_5G)) {
                            /* We should not land here! */
                            ICM_ASSERT(0);
                        } else if (band == ICM_BAND_6G) {
                            best_chan_320minus = icm_select_best_chan_5_or_6ghz(picm,
                                     band, ICM_CH_BW_320, 1, 0, selresults->selected_pwr_mode);
                        }

                        if ((best_chan_320minus < 0) || (best_chan_320minus >= MAX_NUM_CHANNEL)) {
                            best_chan_320minus = 0;
                            best_usability_320minus = 0;
                        } else {
                            best_usability_320minus = ICM_GET_CHANNEL_USABLITY(picm, band,
                                                      best_chan_320minus);
                        }
                        chan_cmp_result =
                                icm_compare_channels(picm,
                        ...
                        switch (chan_cmp_result)
                        {
                            ...
                            case ICM_CHAN_CMP_RESULT_CHAN2_BETTER:
                                best_chan = best_chan_320minus;
                                cnddt_channel_width = ICM_CH_BW_320;
                                /* There is no need to reset usability computations,
                                   as this was the last width used in call
                                   to best channel selection logic */
                                is_search_active = false;
                                break;
                            ...
                            case ICM_CHAN_CMP_RESULT_BOTH_EQUAL:
                                ...
                                ICM_DPRINTF(pdev,
                                      ...
                                      ICM_DEBUG_LEVEL_DEFAULT,
                                      ICM_MODULE_ID_SELECTOR,
                                      "Best channels from both 320 MHz "
                                      "channel bondings are equally good.\n");
                                best_chan = best_chan_320plus;
                                cnddt_channel_width = ICM_CH_BW_320;
                                is_search_active = false;
                                break;
                            ...
                            case ICM_CHAN_CMP_RESULT_FAILURE:
                                fprintf(stderr,
                                    "Error when trying to compare 320MHz "
                                    "channels\n");
                                best_chan = -1;
                                cnddt_channel_width = ICM_CH_BW_320;
                                is_search_active = false;
    ...
    if (best_chan != -1) {
        ...
        selresults->selected_punc_bitmap =
            ICM_GET_CHANNEL_PUNCTURE_BITMAP(picm, band, best_chan);

        ICM_DPRINTF(pdev,
                    ICM_PRCTRL_FLAG_NONE,
                    ICM_DEBUG_LEVEL_DEFAULT,
                    ICM_MODULE_ID_SELECTOR,
                    "Puncture Bitmap for selected channel %x\n",
                    selresults->selected_punc_bitmap);
...
/*
 * Function     : icm_is_6g_power_type_valid
 * Description  : Helper function to check if there is any valid channel for
 *                given 6g power type
 * Input params : - pointer to channel list
 *                - given power level to check valid channels for
 * Return       : TRUE on success, FALSE on failure
 */
int icm_is_6g_power_type_valid(ICM_CHANNEL_LIST_T *chanlist,
                               ICM_AFC_POWER_MODE_T power_level)
{
    u_int16_t chan_idx;
    ICM_CHANNEL_T *ch;
    ...
    for (chan_idx = 0; chan_idx < chanlist->count; chan_idx++) {
        ...
        if (ch->supp_power_modes & (1 << power_level))
            return true;
    }
    return false;
}
...
/*
 * Function     : icm_set_afc_chan_power_info
 * Description  : Helper function to set the AFC power information for
 *                given 6g power type
 * Input params : - pointer to icm info structure
 *                - band
 *                - pointer to ICM_SELECTION_RESULTS_INFO_T to fill the
 *                  current power mode information.
 *                - given power level to check valid channels for
 * Return       : 0 on success, -1 on failure
 */
int icm_set_afc_chan_power_info(ICM_INFO_T *picm, ICM_BAND_T band,
                                ICM_SELECTION_RESULTS_INFO_T *selresults_perband,
                                ICM_AFC_POWER_MODE_T power_level)
{
    ICM_CHANNEL_LIST_T *chanlist = NULL;
    ICM_CHANNEL_T *ch;
    ...
    ICM_ASSERT(picm != NULL);
    ICM_ASSERT(selresults_perband != NULL);

    chanlist = ICM_GET_BAND_CHANNEL_LIST_PTR(picm, band);

    selresults_perband->selected_pwr_mode = power_level;

    if (power_level == ICM_MAX_POWER_MODE)
        return 0;

    for (chan_idx = 0; chan_idx < chanlist->count; chan_idx++) {
        ch = &chanlist->ch[chan_idx];
        ch->ic_maxregpower = ch->afc_tx_power[power_level];

        ICM_CLEAR_CHANNEL_PWR_MODE_EXCLUDE(picm, band, ch->channel, power_level);
        if (!(ch->supp_power_modes & (1 << power_level)))
            ICM_SET_CHANNEL_PWR_MODE_EXCLUDE(picm, band, ch->channel, power_level);
    }
    return 0;
}

/*
 * Function     : icm_select_home_channel_power_modes
 * Description  : Select the best home channel and power mode for
 *                given band.
 * Input params : - pointer to icm info structure
 *                - band
 *                - desired width
 ...
 * Return       : 0 on success, -1 on failure
 */
int icm_select_home_channel_power_modes(ICM_INFO_T *picm, ICM_BAND_T band,
        ICM_CH_BW_T desired_width, ICM_SELECTION_RESULTS_INFO_T *selresults_perband)
{
    int best_chan = -1;
    char power_mode_str[ICM_MAX_CH_POWER_MODE_SIZE];
    ICM_AFC_POWER_MODE_T cnddt_power_level;
    bool is_search_active = true;
    ICM_CHANNEL_LIST_T *chanlist = NULL;
    int ret = 0;
    ...
    chanlist = ICM_GET_BAND_CHANNEL_LIST_PTR(picm, band);
    /*
     * The first candidate power level in case of 6GHz AFC capable run is
     * Standard power. If there are no valid channels present for SP, the
     * candidate power level will start from LPI.
     */

    cnddt_power_level = ICM_MAX_POWER_MODE;
    if (band == ICM_BAND_6G && picm->afc_capable) {
        cnddt_power_level = ICM_STANDARD_POWER_MODE;

        if (!icm_is_6g_power_type_valid(chanlist, ICM_STANDARD_POWER_MODE)) {
            ICM_DPRINTF(pdev,
                        ICM_PRCTRL_FLAG_NONE,
                        ICM_DEBUG_LEVEL_DEFAULT,
                        ICM_MODULE_ID_SELECTOR,
                        "No SP channels found, moving to LPI\n");
            cnddt_power_level = ICM_LOW_POWER_MODE;
        }
    }

    memset(selresults_perband, 0, sizeof(*selresults_perband));

    /* Start a loop to find an appropriate candidate power level, degrading
       from SP to LPI to VLP */
    do {
        if (band == ICM_BAND_6G && picm->afc_capable) {
            icm_pwr_level_to_str(cnddt_power_level,
                             power_mode_str,
                             sizeof(power_mode_str));
            ICM_DPRINTF(pdev,
                ICM_PRCTRL_FLAG_NONE,
                ICM_DEBUG_LEVEL_DEFAULT,
                ICM_MODULE_ID_SELECTOR,
                "Attempting to find best channel for %s power mode\n",
                power_mode_str);
        }

        ret = icm_set_afc_chan_power_info(picm, band, selresults_perband,
                                          cnddt_power_level);
        if (ret == -1) {
            ICM_DPRINTF(pdev,
                        ICM_PRCTRL_FLAG_NONE,
                        ICM_DEBUG_LEVEL_DEFAULT,
                        ICM_MODULE_ID_SELECTOR,
                        "Unable to fill AFC Channel informationI\n");
        }
        best_chan = icm_select_home_channel_perband_per_pwrmode(picm,
                                                band, desired_width,
                                                selresults_perband);
        if (best_chan == -1 && cnddt_power_level < ICM_MAX_POWER_MODE) {
            ICM_DPRINTF(pdev,
                    ICM_PRCTRL_FLAG_NONE,
                    ICM_DEBUG_LEVEL_DEFAULT,
                    ICM_MODULE_ID_SELECTOR,
                    "Failed to find best channel for %s power mode\n",
                    power_mode_str);
        }
        switch(cnddt_power_level)
        {
            case ICM_STANDARD_POWER_MODE:
                if (best_chan == -1)
                    /* Downgrade power mode */
                    cnddt_power_level = ICM_LOW_POWER_MODE;
                else
                    is_search_active = false;
                break;
            case ICM_LOW_POWER_MODE:
                if (best_chan == -1)
                    /* Downgrade power mode */
                    cnddt_power_level = ICM_VERY_LOW_POWER_MODE;
                else
                    is_search_active = false;
                break;
            case ICM_VERY_LOW_POWER_MODE:
                is_search_active = false;
                break;
            case ICM_MAX_POWER_MODE:
                is_search_active = false;
                break;
            default:
                ICM_DPRINTF(pdev,
    ...
    if (best_chan == -1) {
        ICM_DPRINTF(pdev,
                    ...
                    ICM_DEBUG_LEVEL_MAJOR,
                    ICM_MODULE_ID_SELECTOR,
                    "Band %s: Failed all attempts to find best power mode\n",
                    icm_band_to_string(band));
        return -1;
    } else {
        if (band == ICM_BAND_6G && picm->afc_capable) {
            ICM_DPRINTF(pdev,
                    ...
                    ICM_DEBUG_LEVEL_DEFAULT,
                    ICM_MODULE_ID_SELECTOR,
                    "Band %s: Selected power mode is %s\n",
                    icm_band_to_string(band), power_mode_str);
        }
        return 0;
    }
}
        ...
        u_int16_t *scaled_usability)
{
    ...
    *scaled_usability = 0;
    ...
    switch(selected_width)
    {
        ...
        case ICM_CH_BW_320:
            *scaled_usability = (u_int16_t)(((u_int32_t)original_usability *
                                    sinfo->multiband_scaling_factor_bw320) /
                                        sinfo->multiband_scaling_factor_max);
            ret = 0;
            break;
        ...
        ICM_CH_BW_T desired_width, ICM_SELECTION_RESULTS_INFO_T *selresults)
{
    ...
    for (curr_band = 0; curr_band < ICM_BAND_MAX; curr_band++) {
        ...
        ret = icm_select_home_channel_power_modes(picm,
        ...
        ICM_CH_BW_T width)
{
    ...
    for(i = 0; i < arraysize; i++) {
        ...
        if (band == ICM_BAND_2_4G) {
            ...
            ICM_SELDBG_MARKFIELD_NA(decdbginfo[i].prichaninfo.is_ocac_pending);
            ...
            ICM_SELDBG_MARKFIELD_NA(decdbginfo[i].usage_factor_320_bw40);
            ICM_SELDBG_MARKFIELD_NA(decdbginfo[i].usage_factor_320_bw80);
            ICM_SELDBG_MARKFIELD_NA(decdbginfo[i].usage_factor_320_bw160);
            ICM_SELDBG_MARKFIELD_NA(decdbginfo[i].usage_factor_320_bw320);
            ...
            if (band == ICM_BAND_6G) {
                ICM_SELDBG_MARKFIELD_NA(\
                    decdbginfo[i].prichaninfo.is_ocac_pending);
            }
        }
        ...
        for(j = 0; j < ARRAY_LEN(decdbginfo[i].relchaninfo); j++) {
            ...
            if (band == ICM_BAND_2_4G) {
                ...
                ICM_SELDBG_MARKFIELD_NA(
                        decdbginfo[i].relchaninfo[j].is_ocac_pending);
            } else if ((band == ICM_BAND_5G) || (band == ICM_BAND_6G)) {
                ICM_SELDBG_MARKFIELD_NA(
        ...
        int arraysize)
{
    ...
    for (chan_idx = 0; chan_idx < arraysize; chan_idx++) {
        ...
        /* Off-channel CAC pending? */
        ICM_SELDBG_PRINTENTRY_STRCONV(seldbgfile->file,
                prichandbginfo->is_ocac_pending,
                icm_bool_str);
...
void icm_seldbg_printlegend(ICM_DEV_INFO_T* pdev)
{
    ...
    ICM_DPRINTF(pdev,
            ...
            "OCAC?         : Is off-channel CAC completed on channel?\n"
        ...
        int arraysize)
{
    ...
    ICM_DPRINTF(pdev,
            ...
            "Pri  |Reltd|Reltn|Incap|Excld|OBSS |Sec20|Sec40|NF   |NF   |Reg  |Wthr |OCAC |CW   |MWO  |FHSS |Pri20|lized|Msrd |Usab |Usab |Code      |Chan |\n"
...
XXX: Change this if no longer applicable in the future. */
                ...
                rel_chan_idx++)
        {
            ...
            ICM_SELDBG_PRINTENTRY_STRCONV_CONS(pdev,
                    ICM_MODULE_ID_SELECTOR,
                    relchandbginfo->is_ocac_pending,
                    icm_bool_shortstr);
...

{
    ...
    for (chan_idx = 0; chan_idx < chanlist->count; chan_idx++) {
        ...
        if ((((picm->phy_spec == ICM_PHY_SPEC_11AC) &&
             ...
             ((picm->phy_spec == ICM_PHY_SPEC_11AXA) &&
                    ICM_IEEE80211_IS_CHAN_11AXA_HE80_80(ch))) &&
             (ch->ic_ch_freq_seg1 == (ch->channel + 6)) &&
             ((ch->ic_ch_freq_seg1 <
               (ch_pri20_80_80->ic_ch_freq_seg1 - 16)) ||
              (ch->ic_ch_freq_seg1 >
               (ch_pri20_80_80->ic_ch_freq_seg1 + 16))) &&
             ((!emiwar80p80_enab) ||
              ICM_IS_EMIWAR80P80_COMB_ALLOWED(ch_pri20_80_80->ic_ch_freq_seg1,
                  ch->ic_ch_freq_seg1)))
        {
    ...
    for (st80_chan_idx = 0; st80_chan_idx < st80_chan_count; st80_chan_idx++) {
        ...
        for (bondspan_chan_adj = bond_range_start_idx;
             ...
             * 7dB more for the AP. The maximum value allowed is maximum power supported
             * by the HW.
             */
            /* XXX: Check for 802.11ac related changes here */

            if (bondspan_chan_num == chan_num) {
                seglowest_maxregpower = MIN(ch->ic_maxpower, ch->ic_maxregpower);
                ...
                tmp_maxregpower =
                    MIN(ch->ic_maxpower, chanlist->ch[bondspan_chan_idx].ic_maxregpower);
        ...
        ICM_PHY_SPEC_T physpec)
{
    ...
    ICM_DPRINTF(pdev,
            ...
            "Pri  |Reltd|Reltn|Excld|OBSS |Reg  |Wthr |OCAC |CW   |lized|Msrd |Usab |Code      |Chan |\n"
            "Chan |Chan |     |?    |     |Pwr  |?    |?    |?    |?    |Usab |     |          |?    |\n");
    ICM_SELDBG_DRAW_CFREQ2_CDUMP_LINE(pdev);
