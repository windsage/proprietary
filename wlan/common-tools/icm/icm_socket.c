...
int icm_init_netlink_socket(ICM_DEV_INFO_T* pdev)
{
    ...
    spectral_samp_msg = malloc(NLMSG_SPACE(MAX_SPECTRAL_PAYLOAD));
    ...
    pdev->nlh->nlmsg_len = NLMSG_SPACE(MAX_SPECTRAL_PAYLOAD);
...
int icm_init_spectral_nlsock_operation(ICM_DEV_INFO_T *pdev)
{
    ...
    req_temp = NLMSG_SPACE(MAX_SPECTRAL_PAYLOAD) * ICM_SPECTRALSAMP_MAX_BUFF;
