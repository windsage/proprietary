...
int icm_wal_mblcfg_set_width_and_channel(ICM_INFO_T *picm, char *dev_ifname)
{
    ...
    int status = FAILURE;
    char *read_ptr = NULL;
    char *mldlinkname_cpy = strdup(dev_ifname);
...
send:
    ...
    if (ICM_IEEE80211_IS_CHAN_2GHZ(&(picm->chlist.ch[ch_index])) &&
        ...
        if (strncmp(dev_ifname, "ath", 3) == 0) {
            ...
            if (mld_name) {
                snprintf(cmd, sizeof(cmd), "%s %s %s %d %s %s", "cfg80211tool",
                    mld_name, "link_id", link_id, "mode", modestr);
        ...
        /* Set channel */
        if (strncmp(dev_ifname, "ath", 3) == 0) {
            snprintf(cmd, sizeof(cmd), "%s %s %s %1d %s", "cfg80211tool",
                dev_ifname, "channel", channel, "1");

        } else if(strncmp(dev_ifname, "mld", 3) == 0) {
                snprintf(cmd, sizeof(cmd), "%s %s %s %d %s %1d %s", "cfg80211tool",
                    mld_name, "link_id", link_id, "channel", channel, "1");
        }
