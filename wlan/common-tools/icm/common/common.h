...
#define WLAN_EXTN_EID_EHTOP                106
#define WLAN_EXTN_EID_EHTCAP               108
#endif
...
/* Get whether EHT Operation Info is present */
#define EHTOPER_GET_EHTOPINFO_PRESENT(ehtop_param) \
        iefield_val_extract(&ehtop_param, \
            EHTOP_INFO_PRESENT_IDX, EHTOP_INFO_PRESENT_BITS)

/* Get EHT Channel width from the EHTOP control subfield */
#define EHTOPER_GET_EHT_CHANNEL_WIDTH(ehtop_control) \
        iefield_val_extract(&ehtop_control, \
            EHTOP_INFO_CHAN_WIDTH_IDX, EHTOP_INFO_CHAN_WIDTH_BITS)

/*
 * 802.11be EHT operation element
 */
struct ieee80211_ie_ehtoperation {
    u_int8_t elem_id;
    u_int8_t elem_len;
    u_int8_t elem_id_ext;
    u_int8_t ehtop_param;
    uint8_t basic_mcs_nss_set[EHTOPER_MCS_NSS_SIZE];
    uint8_t eht_control;
    uint8_t eht_chan_center_freq_seg0;
    uint8_t eht_chan_center_freq_seg1;
