...
static int icm_vap_name_to_ifindex(const char *vapname, int *ifidx)
{
#ifdef SINGLE_WIPHY_ICM
    char *read_ptr = NULL;
    char *vapname_cpy = strdup(vapname);
    const char *mld_name = NULL;
#endif /* SINGLE_WIPHY_ICM */
    int ret = 1;

    if (strncmp(vapname, "ath", 3) == 0) {
        *ifidx = if_nametoindex(vapname);
#ifdef SINGLE_WIPHY_ICM
    } else if(strncmp(vapname, "mld", 3) == 0) {
        mld_name = strtok_r(vapname_cpy, "-", &read_ptr);
        if(mld_name) {
            *ifidx = if_nametoindex(mld_name);
        } else {
            icm_printf("Error forming name for MLD VAP\n");
            ret = -1;
        }
#endif /* SINGLE_WIPHY_ICM */
    } else {
         icm_printf("Incorrect interface name %s \n",vapname);
         ret = -1;
    }
    if (!ifidx) {
...
int get_ifindex(char *ifname)
{
    ...
    if (ifname == NULL)
        return ret;
    ret = icm_vap_name_to_ifindex(ifname, &ifidx);
    return ifidx;
}
...
static int icm_ifindex_to_vap(int ifidx, char *vapname, int link_id)
{
    ...
    if (strncmp(ifname, "ath", 3) == 0) {
        strlcpy(vapname, ifname, IFNAMSIZ);
#ifdef SINGLE_WIPHY_ICM
    } else if (strncmp(ifname, "mld", 3) == 0) {
        // Convert the link id integer to a string
        snprintf(link_id_str, sizeof(link_id_str), "%d", link_id);
        ptr = ifname;
        if ((strlcpy(vapname, ptr, IFNAMSIZ) >= IFNAMSIZ)
            || (strlcat(vapname, "-" , IFNAMSIZ) >= IFNAMSIZ)
            || (strlcat(vapname,"link", IFNAMSIZ) >= IFNAMSIZ)
            || (strlcat(vapname,link_id_str, IFNAMSIZ) >= IFNAMSIZ)) {
            icm_printf("Error forming name for MLD VAP\n");
            return FALSE;
        }
#endif /* SINGLE_WIPHY_ICM */
    } else {
        icm_printf("VAP name is incorrect \n");
    }
    icm_printf("%s Vap ", vapname);
    return TRUE;
}

static void qca_nl80211_scan_done_event(ICM_INFO_T *picm, u8 *data, size_t len, int ifidx)
{
    struct nlattr *tb[QCA_WLAN_VENDOR_ATTR_SCAN_MAX + 1];
#ifdef SINGLE_WIPHY_ICM
    ICM_DEV_INFO_T *pdev = get_pdev();
    char vap_parent[IF_NAMESIZE] = {0};
#endif /* SINGLE_WIPHY_ICM */
    ...
    icm_ifindex_to_vap(ifidx, vap_ifname, link_id);

    if (strncmp(vap_ifname, "ath", 3) == 0) {
        picm->dev_ifname_global_event = vap_ifname;
        picm->link_id = link_id;
#ifdef SINGLE_WIPHY_ICM
    } else if (strncmp(vap_ifname, "mld", 3) == 0) {
        if (!icm_is_mld_link_radiochild(vap_ifname, picm->radio_ifname)) {
            snprintf(vap_parent, sizeof(vap_parent),"%s%d", "wifi", link_id);
            picm = &pdev->icm[icm_get_picm_index(pdev, vap_parent)];
            if (picm == NULL) {
        ...
        icm_printf("Invalid vap name \n");
        return;
    }

    icm_printf("qca_nl80211_scan_done_event started for %s which is part of %s\n",vap_ifname, picm->radio_ifname);
...
static int qca_nl80211_start_acs_event(ICM_INFO_T *picm,
        u8 *data, size_t len, int ifidx)
{
    ...
    if (tb[QCA_WLAN_VENDOR_ATTR_EXTERNAL_ACS_EVENT_LINK_ID])
        link_id = nla_get_u8(tb[QCA_WLAN_VENDOR_ATTR_EXTERNAL_ACS_EVENT_LINK_ID]);

    /* Find vap interface name from interface index */
    icm_ifindex_to_vap(ifidx, vap_ifname, link_id);
    ...
    icm_printf("qca_nl80211_start_acs_event started for %s which is part of %s\n",vap_ifname, picm->radio_ifname);
        ...
        u8 *data, size_t len, int ifidx)
{
    switch (subcmd) {
        case QCA_NL80211_VENDOR_SUBCMD_EXTERNAL_ACS:
            qca_nl80211_start_acs_event(picm, data, len, ifidx);
            break;
        case QCA_NL80211_VENDOR_SUBCMD_SCAN_DONE:
            qca_nl80211_scan_done_event(picm, data, len, ifidx);
            break;
        default:
...
static void nl80211_vendor_event(ICM_INFO_T *picm, struct nlattr **tb, int ifidx)
{
    ...
    switch (vendor_id) {
        case OUI_QCA:
            nl80211_vendor_event_qca(picm, subcmd, data, len, ifidx);
            break;
        default:
        ...
        struct nlattr **tb, int ifidx)
{
    ...
    icm_printf("nl80211: Drv Event %d (%s) received\n",
            cmd, nl80211_command_to_string(cmd), picm->dev_ifname);

    switch(cmd) {
        case NL80211_CMD_VENDOR:
            nl80211_vendor_event(picm, tb, ifidx);
            break;
        default:
...
static int icm_is_intf_radiochild(ICM_INFO_T *picm, int ifidx)
{
    ...
    if (strncmp(ifname, "ath", 3) == 0) {
        return icm_is_vap_radiochild(ifname, picm->radio_ifname);
#ifdef SINGLE_WIPHY_ICM
    } else if (strncmp(ifname, "mld", 3) == 0) {
        /* For Single wiphy, check that the MLD interface is a part of the
         * dev interface list created during Radio init.
         */
        for (i = 0; i < picm->numdevs; i++) {
            if (!strncmp(picm->dev_ifnames_list[i], ifname, 4)) {
                icm_printf("Interface %s is part of %s\n", ifname, picm->radio_ifname);
                return TRUE;
            }
        }
#endif /* SINGLE_WIPHY_ICM */
    } else
        icm_printf("Error Invalid interface name\n");
    return FALSE;
}
...
static int process_global_event(struct nl_msg *msg, void *arg)
{
    ...
    for (i = 0; i < pdev->conf.num_radios; i++) {
        ...
        /* Initialize the radio interface if the radio init is complete but the iface address
           is still not populated.
         */
        if (picm->init_complete && (memcmp(picm->radio_addr, zero_addr, sizeof(picm->radio_addr)) == 0)) {
            if (icm_init_radio_iface(picm, picm->radio_ifname) != SUCCESS) {
        ...
        if ((ifidx  == -1 && !wdev_id_set) ||
            ...
            do_process_drv_event(picm, gnlh->cmd, tb, ifidx);
            return NL_SKIP;
        }
    }
...
int nl80211_handle_ifindx_change(void *ctx, struct nl80211_global *global) {
    ...
    if (nl80211_get_macaddr(picm, picm->radio_ifname, picm->radio_addr)) {
...
int driver_nl80211_vendor_scan(void *data, struct nl80211_scan_config *params)
{
    ...
    if (attr == NULL)
        goto fail;
#ifdef SINGLE_WIPHY_ICM
    if ((picm->link_id) != INVALID_LINK_ID) {
        if (nla_put_u8(msg, QCA_WLAN_VENDOR_ATTR_SCAN_LINK_ID,
                    picm->link_id))
            goto fail;
    }
#endif /* SINGLE_WIPHY_ICM */
...
static int nl80211_get_noise_for_scan_results(struct nl80211_data *drv,
        struct nl80211_noise_info *info, int ifidx)
{
    struct nl_msg *msg;

    memset(info, 0, sizeof(*info));
    msg = nl80211_ifindex_msg(drv->global, ifidx, NLM_F_DUMP, NL80211_CMD_GET_SURVEY);
...
struct scan_results * driver_nl80211_get_scan_results(void *ctx)
{
    ...
    ifidx = if_nametoindex(picm->radio_ifname);
    if (!ifidx) {
        icm_printf("nl80211: if_nametoindex() failed\n");
        return NULL;
    }

    if (!(msg = nl80211_ifindex_msg(drv->global, ifidx, NLM_F_DUMP,
    ...
    if (ret == 0) {
        ...
        if (nl80211_get_noise_for_scan_results(drv, &res->info, ifidx) == 0) {
...
int driver_nl80211_set_channel(void *ctx, struct nl80211_channel_config *chan_config)
{
    ...
    ret = icm_vap_name_to_ifindex(chan_config->ifname, &ifidx);
    ...
    if (nla_put_u8(msg, QCA_WLAN_VENDOR_ATTR_EXTERNAL_ACS_LINK_ID, picm->link_id))
        goto fail;
...
int nl80211_get_macaddr(void *ctx, const char *ifname, u8* addr)
{
    ...
    if (!ifidx) {
        icm_printf("Failed to get interface index for %s\n",ifname);
        return -1;
    }
    mode = nl80211_get_ifmode(picm, ifidx, addr);
