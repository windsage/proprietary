 ...
 */
#define ICM_NL80211_EVENT_DESIRED_MSG_SIZE   (14336)
...
/*
 * We provision for a max of 14 currently available channels from 2.4 GHz, 41
 * channels (currently available and potential) from 5 GHz, approx. 60 potential
 * channels from 6 GHz
 */
#define MAX_NL80211_NOISE_FREQS (115)
...
/* enum nl80211_ocac_status - Specifies the supported codes for a given
 * channel's Off-Channel Channel Availability Check (OCAC) status.
 */
enum nl80211_ocac_status {
    OCAC_STATUS_CLEAR,
    OCAC_STATUS_PENDING,
    OCAC_STATUS_MAX,
};
...
/*
 * struct nl80211_ocac_linst - Specifies the individual list entry instances in
 * the Off-Channel Channel Availability Check (OCAC) list. It provides a status
 * for a given channel described by its primary frequency. The status provides
 * information about the state of a channel with respect to the completion of
 * its Channel Availability Check.
 * @primary_freq: Primary channel center frequency (u32) in MHz
 * @ocac_status: OCAC status (u8) with emphasis on throughput
 */
...
/*
 * struct nl80211_channel_status - Specifies channel status information
 * @ocac_linst: Off-Channel Channel Availability Check (OCAC) channel list,
 * a pointer to an array of nl80211_ocac_linst instances (OCAC array).
 * @num_ocac_linst: Number of nl80211_ocac_linst instances, i.e. size of OCAC
 * array
 */
struct nl80211_channel_status {
    struct nl80211_ocac_linst *ocacl;
    u32 num_ocaclinst;
};
...
struct nl80211_chan_args {
    /* Primary 20 MHz channel frequency in MHz. Required in all cases (but can
     * be 0 to signal channel selection failure). */
    u32 primary_freq;

    /* Secondary 20 MHz channel frequency in MHz. Required for 40 MHz. */
    u32 secondary_freq;

    /* nl80211 channel width. Required in all cases. */
    u8 channel_width;

    /* Segment 0 center frequency in MHz */
    u32 seg0_center_freq;

    /* Segment 1 center frequency in MHz. Required for 80+80 MHz. */
    u32 seg1_center_freq;
...
struct nl80211_user_chanlist
{
    ...
    /* Frequencies */
    u16 freq[NL80211_USER_CHANLIST_MAX_ENTRIES];
};
