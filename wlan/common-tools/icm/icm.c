...
static int icm_build_mld_vap_listing(sys_ifnames *vap_listing,
        char *mld_name, char *link_path);
#endif /* SINGLE_WIPHY_ICM */
static int icm_build_vap_listing(sys_ifnames *vap_listing, bool *isath);
...
#ifdef SINGLE_WIPHY_ICM
/*
 * Function     : icm_is_mld_ifname_valid
...
int icm_is_mld_ifname_valid(const char *ifname)
{
    ...
    /* We don't make any assumptions on max no. of mld VAP interfaces,
       at this step. */
...
/*
 * Function     : icm_is_link_ifname_valid
 * Description  : find if the argument string stands for a valid
 *                link id name
 * Input params : string
 * Return       : 1: valid, 0: invalid
...
#ifdef SINGLE_WIPHY_ICM
/*
 * Function     : icm_is_mld_link_radiochild
 * Description  : Determine if a MLD Link with a given name is the child of a radio
 *                with a given name. To determine this, we check the parent name
 *                in the sysfs entry for the MLD link.
 * Input params : MLD link name, radio name
 *                It is the caller's responsibility to ensure that the names are
 *                correct.
 * Return       : TRUE if the MLD Link is the child of the radio, FALSE if the MLD Link is
...
bool icm_is_mld_link_radiochild(const char *mldlinkname, const char *radioname)
{
    ...
    char *linkid = NULL;
    char *read_ptr = NULL;
    char *mldlinkname_cpy = strdup(mldlinkname);
    ...
    mld_name = strtok_r(mldlinkname_cpy, "-", &read_ptr);
    linkid = strtok_r(NULL, "-", &read_ptr);

    if ((strlcpy(path, PATH_SYSNET, bufsize) >= bufsize)
            || (strlcat(path, mld_name, bufsize) >= bufsize)
            || (strlcat(path, "/", bufsize) >= bufsize)
            || (strlcat(path, linkid, bufsize) >= bufsize)
...
#ifdef SINGLE_WIPHY_ICM
/*
 * Function     : icm_build_mld_vap_listing
...
{
    DIR *vap_dir;
    struct dirent *vap_dentry;
    char temp_name[IFNAMSIZ], mld_link_name[IFNAMSIZ];
    int ret;
    int status = FAILURE;

    vap_dir = opendir(link_path);
    if (NULL == vap_dir) {
        perror("icm : link_path");
        goto bad;
    }

    while ((vap_dentry = readdir(vap_dir)) != NULL) {
        if (!((vap_dentry->d_type & DT_DIR) || (vap_dentry->d_type & DT_LNK)))
            continue;

        if (0 > snprintf(temp_name, IFNAMSIZ, "%s", vap_dentry->d_name))
            continue;

        if (icm_is_link_ifname_valid(temp_name)) {
            if ((strlcpy(mld_link_name, mld_name, IFNAMSIZ) >= IFNAMSIZ)
                    || (strlcat(mld_link_name, "-" , IFNAMSIZ) >= IFNAMSIZ)
                    || (strlcat(mld_link_name, temp_name, IFNAMSIZ) >= IFNAMSIZ)) {
                ICM_DPRINTF(pdev, ICM_PRCTRL_FLAG_NONE, ICM_DEBUG_LEVEL_MAJOR,
                    ICM_MODULE_ID_MAIN, "Error forming name for MLD VAP\n");
                goto bad;
            }

            ret = icm_sys_ifnames_add(vap_listing, mld_link_name);
...
static int icm_build_vap_listing(sys_ifnames *vap_listing, bool *isath)
{
    ...
    int status = FAILURE;
#ifdef SINGLE_WIPHY_ICM
    char link_path[LINK_PATH_SZ] = {0};
    size_t bufsize = sizeof(link_path);
#endif
    ...
    while ((dentry = readdir(dir)) != NULL)
    {
        ...
        if (isath && icm_is_dev_ifname_valid(temp_name)) {
...
#ifdef SINGLE_WIPHY_ICM
        } else if (icm_is_mld_ifname_valid(temp_name)){
            *isath = FALSE;
            if ((strlcpy(link_path, PATH_SYSCLASS_NET, bufsize) >= bufsize)
                    || (strlcat(link_path, "/" , bufsize) >= bufsize)
                    || (strlcat(link_path, temp_name, bufsize) >= bufsize)) {
                ICM_DPRINTF(pdev, ICM_PRCTRL_FLAG_NONE, ICM_DEBUG_LEVEL_MAJOR,
                    ICM_MODULE_ID_MAIN, "Error forming path name for MLD VAP\n");
                goto bad;
            }

            ret = icm_build_mld_vap_listing(vap_listing, temp_name, link_path);
            if (!ret) {
                err("Could not create mld vap listing");
                goto bad;
            }
        ...
        bool *ispresent)
{
    ...
    *ispresent = FALSE;
    ...
    if (icm_build_vap_listing(&vap_listing, &isath) != SUCCESS) {
...
int icm_init_radio_iface(ICM_INFO_T* picm, const char *radio_ifname)
{
    ...
    if (icm_sys_ifnames_init(&vap_listing, EXPECTED_MAX_VAPS) < 0) {
        err("Could not initialize MLD VAP listing");
        return FAILURE;
    }
...
#ifdef SINGLE_WIPHY_ICM
        if ((isath
             && icm_is_vap_radiochild(vap_listing.ifnames[i], radio_ifname))
             || (!isath
             && icm_is_mld_link_radiochild(vap_listing.ifnames[i], radio_ifname))) {
#else
        if (isath
             && icm_is_vap_radiochild(vap_listing.ifnames[i], radio_ifname)) {
#endif /* SINGLE_WIPHY_ICM */
