...
#define CFR_DUMP_STREAMFS_FILE "/sys/kernel/debug/ieee80211/%s/%s/cfr_capture0"
#else
...
{
	printf("Usage:cfr_test_app -i <interfacename> [-c <chip_set>]\n");
	printf("Example:cfr_test_app -i phyX\n");
	printf("Example:cfr_test_app -i phyX -c ath11k\n");
	printf("Example:cfr_test_app -i phyX -c ath12k_hwX\n");
}
...
int open_streamfs_file(char *iface, char *chipset)
{
	int fd = -1;
	char filename[128];

#ifdef WIN_AP_HOST_OPEN
	snprintf(filename, sizeof(filename), CFR_DUMP_STREAMFS_FILE, iface, chipset);
#else
...
/* Signal Handler to override CTRL+C */
...
int main(int argc, char *argv[])
{
	...
	streamfs_fd = open_streamfs_file(iface, chipset);
	...
	printf("Starting CFR capture daemon, Press CTRL+C to exit \n");
