...
namespace GSCAN_TEST
{
    ...
    void GScanTestSuite::gscanSendSetPasspointListRequest()
    {
        ...
        memset(networks, 0, sizeof(wifi_passpoint_network) * MAX_EPNO_NETWORKS);
        ...
        if (f_read) {
            ...
            if (num_networks > MAX_EPNO_NETWORKS) {
                fprintf(stderr, "gscanSendSetPasspointListRequest: number of "
                    "networks:%d should not be greater than max allowed "
                    "networks\n", num_networks);
                fclose(f_read);
                free(networks);
                return;
