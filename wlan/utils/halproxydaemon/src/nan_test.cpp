...
#define NAN_MAX_OEM_DATA_LEN 1024
int gExitflag = 0;
...
namespace NAN_TEST
{
    ...
    const char *NanTestSuite::NAN_SET_OEM_CMD = "set_oem_cmd";
        ...
        :wifiHandle_(handle)
    {
        ...
        VendorNanCallbackHandler vendorcallbackHandler =
        {
            .NotifyVendorResponse = NanTestSuite::nanNotifyVendorResponse,
            .VendorEventIndication = NanTestSuite::nanVendorEventIndication
        };
#endif
...
#if (NAN_CERT_VERSION >= 6)
        nan_register_vendor_handler(handle, vendorcallbackHandler);
#endif
        gExitflag = 0;
    }
...
#if (NAN_CERT_VERSION >= 6)
        if(strcasecmp(argv[2], NAN_SET_OEM_CMD) == 0)
            return nanSendOemCommandRequest(argc, argv);
#endif
...
#else
                   fprintf(stderr, "%s: cipher capabilities not supported in this version of HAL\n"
                           "current HAL Version is %d.%d, required version is 4.0.0 or above\n",
                           __FUNCTION__, NAN_MAJOR_VERSION, NAN_MINOR_VERSION);
#endif
                   break;
                case 1022:
...
#else
                   fprintf(stderr, "%s: gtk protection not supported in this version of HAL\n"
                           "current HAL Version is %d.%d, required version is 4.0.0 or above\n",
                           __FUNCTION__, NAN_MAJOR_VERSION, NAN_MINOR_VERSION);
#endif
                   break;
...
#else
                   fprintf(stderr, "%s: skda not supported in this version of HAL\n"
                           "current HAL Version is %d.%d, required version is 4.0.0 or above\n",
                           __FUNCTION__, NAN_MAJOR_VERSION, NAN_MINOR_VERSION);
#endif
                   break;
...
#else
                   fprintf(stderr, "%s: csia capabilities not supported in this version of HAL\n"
                           "current HAL Version is %d.%d, required version is 4.0.0 or above\n",
                           __FUNCTION__, NAN_MAJOR_VERSION, NAN_MINOR_VERSION);
#endif
                   break;
...
#if (NAN_CERT_VERSION >= 6)
    void NanTestSuite::nanSendOemCommandRequest(int argc, char **argv)
    {
        u8 data;
        FILE *fp;
        NanVendorCmdData req;
        transaction_id id = 0;
        u8 nan_oem_data[NAN_MAX_OEM_DATA_LEN];
        int long_index = 0, opt = 0, size = 0, idx = 0;
        ...
        memset(&req, 0, sizeof(NanVendorCmdData));
        memset(nan_oem_data, 0, sizeof(nan_oem_data));
                ...
                long_options, &long_index )) != -1)
        {
            switch (opt)
            {
                ...
                case 'f':
                    fp = fopen(optarg, "r");
                    if (fp == NULL) {
                        fprintf(stderr, "Can't read the file %s", optarg);
                        return;
                    }
                    while (fscanf(fp, "%hhx", &data)) {
                        nan_oem_data[idx++] = data;
                        if (idx >= NAN_MAX_OEM_DATA_LEN) {
                            fprintf(stderr, "OEM command data exceeds max length");
                            break;
                        }
                    }
                    fclose(fp);

                    req.cmd = nan_oem_data;
                    req.cmd_len = idx;
                    break;
                case 'm':
                    size = (int)strlen(optarg) >> 1;
                    if(size > NAN_MAX_OEM_DATA_LEN) {

                        fprintf(stderr, "%s:Failed:OEM cmd len exceeded = %d \n",
                                __FUNCTION__, size);
                        return;
                    }
                    nanParseHexString(optarg, nan_oem_data, (int*)&size);
                    req.cmd = nan_oem_data;
                    req.cmd_len = size;
                    break;
                case 'h':
                default:
                    nanPrintCmdUsage(argv, NAN_CMD, NAN_SET_OEM_CMD, long_options,
                                    sizeof(long_options)/sizeof(struct option));
                    return;
            }
        }
        ALOGI("%s: entered \n", __FUNCTION__);
	if (req.cmd)
        	hexdump(req.cmd, req.cmd_len);

        nan_set_command(id, wifiHandle_, &req);
    }

    /* NotifyVendorResponse invoked to notify the status of the oem Request */
    void NanTestSuite::nanNotifyVendorResponse(transaction_id id,
                                               NanVendorResponseMsg *response)
    {
        ALOGI("%s: req_id=%d\n", __FUNCTION__, id);
        hexdump(response->rsp_data, response->rsp_len);

        fprintf(stderr, "%s: req_id %d\n", __FUNCTION__, id);
        nanhexdump(response->rsp_data, response->rsp_len);
    ...
    /* Events Callback */
    void NanTestSuite::nanVendorEventIndication(NanVendorEventInd* event)
    {
        ALOGI("%s: entered \n", __FUNCTION__);
        hexdump(event->event_data, event->event_len);

        fprintf(stderr, "%s: entered \n", __FUNCTION__);
        nanhexdump(event->event_data, event->event_len);
