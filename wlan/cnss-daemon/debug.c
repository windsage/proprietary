/*
 * Copyright (c) 2014, 2017, 2021-2023 Qualcomm Technologies, Inc.
 * All Rights Reserved.
...
static char *log_level_to_str(enum log_level level)
{
	switch (level) {
	case MSG_ERROR:
		return "error";
...
#ifdef CONFIG_DEBUG_LOGCAT
static int logcat_priority(enum log_level level)
{
	switch (level) {
	case MSG_INVAL:
		return -1;
...
#if defined(CONFIG_DEBUG) && !defined(INCLUDE_BREAKPAD)
...
static void _wsvc_vprintf(enum log_level level, const char *fmt, va_list argp)
{
	FILE *file = stderr;
	time_t now;
	struct tm tm;
	...
	fprintf(file, "%s %2d %02d:%02d:%02d %5s: %s\n",
		...
		tm.tm_min, tm.tm_sec, log_level_to_str(level), buf);

	return;
}

void wsvc_printf(enum log_level level, const char *fmt, ...)
{
	va_list argp;
...
#ifdef IPQ
void _wsvc_printf_kmsg(const char *fmt, ...)
{
	va_list argp;
...
void _wsvc_hexdump(enum log_level level,
		   const char *title, const void *p, int len)
{
...
void _wsvc_printf_mac_addr(enum log_level level, const char *ftm,
	const unsigned char *addr)
{
	wsvc_printf(level, "%s: " MACSTR,
		    ftm,
		    MAC2STR(addr));
}
