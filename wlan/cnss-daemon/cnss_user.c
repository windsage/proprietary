...
#ifdef UJAIL_SUPPORT
	if (lchmod(un_address.sun_path,  S_IRWXU | S_IRWXG | S_IRWXO) < 0) {
		wsvc_printf_err("Fail to change server socket permission for path: %s Error: %s\n",
                                un_address.sun_path, strerror(errno));
	}
#endif
out:
...
#ifdef IPQ
static void cnss_user_send_cnsscli_param(
				 cnss_plat_ipc_qmi_config_param_type_v01 type,
				 uint32_t instance_id, uint64_t value)
{
	cnss_plat_ipc_qmi_send_config_param_req(type, instance_id, value);
}
#else
static void cnss_user_send_cnsscli_param(
				 cnss_plat_ipc_qmi_config_param_type_v01 type,
				...
				struct cnss_cli_config_param_data *data)
{
	...
	case WLFW_QMI_TIMEOUT:
		...
		break;
	case QDSS_SUPPORT:
		cnss_user_send_cnsscli_param
				(CNSS_PLAT_IPC_PARAM_TYPE_QDSS_SUPPORT_V01,
				 instance_id, data->value);
		break;
	case QDSS_TRACE_CONFIG_AND_START:
		cnss_user_send_cnsscli_param
				(CNSS_PLAT_IPC_PARAM_TYPE_QDSS_START_V01,
				 instance_id, data->value);
		break;
	case QDSS_TRACE_STOP:
		cnss_user_send_cnsscli_param
					(CNSS_PLAT_IPC_PARAM_TYPE_QDSS_STOP_V01,
					 instance_id, data->value);
		break;
	case DAEMON_SUPPORT:
		cnss_user_send_cnsscli_param
				(CNSS_PLAT_IPC_PARAM_TYPE_DAEMON_SUPPORT_V01,
				 instance_id, data->value);
		break;
	case COLD_BOOT_SUPPORT:
		cnss_user_send_cnsscli_param
				(CNSS_PLAT_IPC_PARAM_TYPE_COLD_BOOT_SUPPORT_V01,
				 instance_id, data->value);
		break;
	case HDS_SUPPORT:
		cnss_user_send_cnsscli_param
				(CNSS_PLAT_IPC_PARAM_TYPE_HDS_SUPPORT_V01,
				 instance_id, data->value);
		break;
	case REGDB_SUPPORT:
		cnss_user_send_cnsscli_param
				(CNSS_PLAT_IPC_PARAM_TYPE_REGDB_SUPPORT_V01,
				 instance_id, data->value);
		break;
	default:
...
#ifdef IPQ
/* For IPQ Platforms, qdss_trace_stop is notified to platform driver over
 * NL and platform driver sends the QMI message to FW
 */
static
void handle_qdss_trace_stop(uint32_t instance_id,
			    struct cnss_cli_config_param_data *data)
{
	handle_config_param(QDSS_TRACE_STOP, instance_id, data);
}
#else
static
void handle_qdss_trace_stop(uint32_t instance_id,
