...
static int cld80211_nl_recvmsgs()
{
	...
	sock = cld80211_get_nl_socket_ctx(nl_loop.cldctx);
	if (!sock) {
		wsvc_printf_err("cld sock is NULL\n");
		return -1;
	}
	ret = nl_recvmsgs(sock, nl_loop.cb);
	if(ret < 0)
...
static void initialize_nl_fd (void)
{
	...
	nl_loop.nl_fd = nl_socket_get_fd(sock);
...
static void nl_loop_handle_alarm(int sig)
{
	...
	wsvc_printf_err("terminate: %d"
		    ...
		    nl_loop.terminate);
	wsvc_printf_kmsg("%s: terminate: %d Could not terminate loop in 2 seconds! sig num %d",
			 __func__, nl_loop.terminate, sig);
...
int nl_loop_terminate(void)
{
	...
	wsvc_printf_kmsg("%s called", __func__);
...
int nl_loop_run(void)
{
	...
	while (!nl_loop.terminate) {
		...
		result = poll(pfd, ARRAY_SIZE(pfd), -1);
		if (result < 0) {
...
int nl_loop_run(void)
{
	...
	pfd[0].fd = cnss_user_get_fd();
	pfd[0].events = POLLIN;

	pfd[1].fd = cnss_genl_get_fd();
	pfd[1].events = POLLIN;

	pfd[2].fd = cnss_plat_ipc_qmi_get_fd();
	pfd[2].events = POLLIN;
	...
	while (!nl_loop.terminate) {
		...
		if (pfd[0].revents & (POLLIN | POLLHUP | POLLERR)) {
			wsvc_printf_dbg("pfd[0].fd: %d has data to process\n",
					pfd[0].fd);
			handle_cnss_user_event(pfd[0].fd);
		}
