				      ...
				      struct wlfw_client_data *gdata)
{
	...
	if (CNSS_BDF_BIN == bdf_type) {
		...
		} else if (gdata->chip_info.chip_id == UMC_CHIP_ID) {
	...
	if (bdf_type == CNSS_BDF_BIN && gdata->foundry_name) {
				    ...
				    struct wlfw_client_data *gdata)
{
	...
	if (remaining == 0 && (resp.end_valid && resp.end)) {
		...
		snprintf(filename, sizeof(filename),
			 CAL_FILE"%02d%s.bin", cal_id,
			 get_file_suffix_by_instance_id(gdata->instance_id));
		...
		if (!file_name) {
			...
			if (file_suffix) {
				...
				snprintf(full_path_filename,
					 ...
					 "/data/vendor/wifi/%s%s%s",
					 file_prefix,
					 get_file_suffix_by_instance_id(
							gdata->instance_id),
					 file_suffix);
			} else {
				snprintf(full_path_filename,
...
void wlfw_set_qmi_timeout(uint32_t instance_id, uint32_t msec)
{
	...
	gdata->wlfw_qmi_timeout_ms = msec;
}
...
static int wlfw_get_firmware_path(uint32_t instance_id, char *fw_path,
				  uint32_t fw_path_len)
{
	...
	/* For Integrated targets, path is obtained from board_name */
	if (instance_id == HAWKEYE_ID) {
		board_name_file = fopen("/tmp/sysinfo/board_name", "r");
		if (!board_name_file) {
			wsvc_printf_err("%s Unable to get board_name",
					__func__);
			return -EIO;
		}

		fscanf(board_name_file, "%" MAXLEN(MAX_BOARD_NAME) "s",
		       board_name);
		fclose(board_name_file);

		if ((strstr(board_name, "hk")) ||
		    (strstr(board_name, "oak")) ||
		    (strstr(board_name, "ac")))
			snprintf(fw_path, fw_path_len, "IPQ8074");
		else if (strstr(board_name, "cp"))
			snprintf(fw_path, fw_path_len, "IPQ6018");
		else if (strstr(board_name, "mp"))
			snprintf(fw_path, fw_path_len, "IPQ5018");
		else if (strstr(board_name, "al"))
			snprintf(fw_path, fw_path_len, "IPQ9574");
	} else {
		/* For Discrete targets, path is from wlfw_device_name_table */
		snprintf(device_name, sizeof(device_name),
			 "%s", get_device_name_by_instance_id(instance_id));
		/* Device name is of the format qcnXXXX_pciX.
		 * Cut the string before "_".
		 */
		temp_fw_path = strtok_r(device_name, "_", &saveptr);
		if (!temp_fw_path)
			return -ENODEV;

		snprintf(fw_path, fw_path_len, "%s", temp_fw_path);
	}
					     ...
					     uint32_t full_file_path_len)
{
	...
	char fw_path[MAX_DEVICE_NAME_LEN] = {'\0'};
	...
	if (wlfw_get_firmware_path(gdata->instance_id, fw_path,
				   sizeof(fw_path))) {
		wsvc_printf_err("%s: Failed to get FW path for device %d\n",
	...
	snprintf(temp_full_path, sizeof(temp_full_path),
		 "/lib/firmware/%s/%s",
		 fw_path, QDSS_TRACE_CONFIG_FILE);
			    ...
			    void *err_cb_data)
{
	...
	wsvc_printf_kmsg("%s: Server disconnect, QMI ERR %d",
			 __func__, error);
	...
	wsvc_printf_kmsg("%s: Attempting server reconnect", __func__);
...
static int wlfw_ahb_init(struct wlfw_client_data *gdata)
{
	int rc;
...
static int wlfw_pci_init(struct wlfw_client_data *gdata)
{
	int rc;
	uint64_t fw_status = 0;
	struct timespec ts;
...
void *wlfw_service_request(void *arg)
{
	...
	case WAIKIKI_3_ID:
	case WAIKIKI_4_ID:
		rc = wlfw_pci_init(gdata);
		break;
	case HAWKEYE_ID:
	case SPRUCE_1_ID:
	case SPRUCE_2_ID:
		rc = wlfw_ahb_init(gdata);
		break;
	case ADRASTEA_ID:
...
release_client:
	wsvc_printf_err("%s: Failed to init client with instance_id %d\n",
			__func__, gdata->instance_id);
...
int wlfw_start(enum wlfw_svc_flag flag, uint8_t index)
{
	...
	wsvc_printf_kmsg("%s: Starting", __func__);
	...
	if (!gdata->wlfw_qmi_timeout_ms)
		wlfw_set_qmi_timeout(gdata->instance_id, WLFW_TIMEOUT_MS);
...
int wlfw_stop(enum wlfw_svc_flag flag, uint8_t index)
{
	...
	wsvc_printf_kmsg("%s: Stopping: %d", __func__, flag);
