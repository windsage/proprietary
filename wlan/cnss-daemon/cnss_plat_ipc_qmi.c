...
static int cnss_plat_ipc_qmi_send_reg_client_req(void)
{
	cnss_plat_ipc_qmi_reg_client_req_msg_v01 req;
	cnss_plat_ipc_qmi_reg_client_resp_msg_v01 resp;
	int ret;
	...
	req.client_id = CNSS_PLAT_IPC_DAEMON_QMI_CLIENT_V01;

	ret = qmi_client_send_msg_sync(plat_ipc_qmi.svc_clnt_handler,
				      CNSS_PLAT_IPC_QMI_REG_CLIENT_REQ_V01,
	...
	wsvc_printf_info("CNSS daemon client registered");
out:
	return ret;
}
...
{
	cnss_plat_ipc_qmi_send_config_param_req_msg_v01 req;
	cnss_plat_ipc_qmi_send_config_param_resp_msg_v01 resp;
	int ret = QMI_NO_ERR;
	...
	ret = qmi_client_send_msg_sync(
				 plat_ipc_qmi.svc_clnt_handler,
				 CNSS_PLAT_IPC_QMI_SEND_CONFIG_PARAM_REQ_V01,
				 &req, sizeof(req), &resp, sizeof(resp),
				 CNSS_PLAT_IPC_QMI_TIMEOUT_MS);
	...
	wsvc_printf_info("Config param sent successfully");
out:
	return ret;
}
...
static int cnss_plat_ipc_qmi_send_init_setup_req(void)
{
	...
	wsvc_printf_info("%s: Driver Status: 0x%" PRIx64, __func__,
			 resp.drv_status);
out:
	return ret;
}
...
void cnss_plat_ipc_qmi_msg_process(void)
{
	...
	wsvc_printf_info("%s: RX %s %d", __func__, event, n);
...
int cnss_plat_ipc_qmi_init(void)
{
	...
	ret = cnss_plat_ipc_qmi_send_reg_client_req();
