...
#ifndef IPQ
		"   -b, --max_file_size specify max size of each fw data file, unit[MB]\n"
		"   -c, --max_file_count specify max count of fw data files\n"
#endif
...
static void wlan_service_sighandler(int signum)
{
	...
	wsvc_printf_kmsg("Caught Signal: %d", signum);
...
void configure_default_debug_level(void)
{
	wsvc_kmsg_logging = MSG_INFO_HIGH;
}
#else
void configure_default_debug_level(void)
{
...
#ifdef INCLUDE_BREAKPAD
/*
 * Callback function for breakpad.
 */
void breakpad_exception_cb(void)
{
	/* Add any cleanup code here after a crash */
	wsvc_printf_err("%s: Breakpad callback invoked for cnss-daemon\n",
			__func__);
	return;
}
#endif

void cnssdaemon_exit()
{
	wsvc_printf_kmsg("!!EXITING FROM cnss-daemon!!\n");
}
...
int main(int argc, char *argv[])
{
	...
	unsigned int max_file_count = DEFAULT_FILE_COUNT;
	unsigned long max_file_size = DEFAULT_FILE_SIZE;
	static struct option options[] =
	{
		{"help", no_argument, NULL, 'h'},
		{"nodaemon", no_argument, NULL, 'n'},
		{"max_file_size", required_argument, NULL, 'b'},
		{"max_file_count", required_argument, NULL, 'c'},
	...
	value = atexit(cnssdaemon_exit);

#ifdef INCLUDE_BREAKPAD
        breakpad_ExceptionHandler(&breakpad_exception_cb);
#endif

	progname = argv[0];

	while (1) {
		c = getopt_long(argc, argv, "hdnf:sli:b:c:", options, NULL);
...
#if defined(ICNSS_QMI) || defined(IPQ)
		case 'i':
			interface_name = optarg;

			if (!strncmp(interface_name, "none", 4)) {
				skip_qmi_service = 1;
				break;
			}

			if (get_instance_id_by_device_name(interface_name,
							   &instance_id)) {
			...
			if (num_interfaces >= MAX_NUM_RADIOS) {
				fprintf(stderr,
					"Too many interfaces!! Max number of radios supported is %d\n",
					MAX_NUM_RADIOS);
				usage();
				break;
			}

			g_instance_id_array[num_interfaces++] = instance_id;
			break;
#endif
#ifndef IPQ
		case 'b':
			if (!optarg) {
				fprintf(stderr,
					"invalid -b option, expecting a number\n");
				usage();
				break;
			}

			max_file_size = (unsigned long)strtol(optarg, &endp, 0);
			max_file_size *= 1024 * 1024;
			if (endp == optarg || max_file_size > MAX_FILE_SIZE) {
				fprintf(stderr,
					"invalid setting of max fw data size, max value allowed is %dMB\n",
					MAX_FILE_SIZE / 1024 / 1024);
				usage();
				break;
			}
		...
		case 'c':
			if (!optarg) {
				fprintf(stderr,
					"invalid -c option, expecting a number\n");
				usage();
				break;
			}

			max_file_count = (unsigned int)strtol(optarg, &endp, 0);
			if (endp == optarg || max_file_count > MAX_FILE_COUNT) {
				fprintf(stderr,
					"invalid setting of max fw data count, max value allowed is %d\n",
					MAX_FILE_COUNT);
				usage();
				break;
			}
	...
	if (value != 0)
               wsvc_printf_kmsg("%s: Failed to register exit function\n",
				__func__);
	...
	if (cnss_fw_data_init(max_file_count, max_file_size))
	...
	if (cnss_plat_ipc_qmi_init() < 0)
		wsvc_printf_err("Failed to setup CNSS Platform QMI IPC");
