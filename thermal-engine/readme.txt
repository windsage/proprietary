...
weight_denominator <Optional, If it is defined, the virtual sensor uses this value to divide
                    the weighted sum. Otherwise the sum of weights for different sensors
                    mentioned in 'weights' field is used to divide the weighted sum>
y_intercept        <Optional, If it is defined, the virtual sensor will add this value for
                    final calculation>
math               <0 is default, to use the weights. "math" is optional for default. 1 for
                    minimum of sensors, 2 for max of sensors, 3 for the sum of
		    sensors, 4 for lock and cache a sensor value when the trip
		    triggers>
...
8) weights define an array of weighted value that corresponding to the array of sensors.
   weights format is int type.
9) weight_denominator is defined for weight average virtual sensor
10) y_intercept is defined for virtual sensor offset data.
11) If math type = 2 (max) or 1 (min), Virtual temperature = max or min of(sensors)
    If math type = 0 (weight) and define weight_denominator, virtual-sensor-0 calculated result = 
    (40 * tsens_tz_sensor1 + 60 * tsens_tz_sensor5)/10 + 1234
    If math type = 0 (weight) but not define weight_denominator, virtual-sensor-0 calculated result
    = ((40 * tsens_tz_sensor1 + 60 * tsens_tz_sensor5)/(40 + 60)) + 1234
12) set_point of virtual sensor has to be less than the set_point for pid so that pid
...
13) If trip sensor does not support changing from interrupt mode to polling mode,
