/*===========================================================================
Copyright (c) 2013-2015,2019,2021 Qualcomm Technologies, Inc.
All Rights Reserved.
...
static void handle_trip_sens_thresh_sig(struct v_sensor_info *vs_info, bool init)
{
	...
	/* Clear the temp caching if trip is cleared */
	if (!vs_info->rs.trip_reached)
		vs_info->rs.cached_temp = (vs_info->setting->tracks_low)
						? INT_MAX : INT_MIN;
	pthread_mutex_unlock(&vs_info->wait_mutex);
...
int vs_get_temperature(struct sensors_mgr_sensor_info *sensor)
{
	...
	/* Check if Temperature is already cached */
	if (vs_info->setting->math_type == LOCK_AND_CACHE &&
			vs_info->rs.cached_temp != INT_MAX &&
			vs_info->rs.cached_temp != INT_MIN) {
		temp = vs_info->rs.cached_temp;
		goto log_unlock_exit;
	}
	...
	for (i = 0; i < vs_info->setting->list_cnt; i++) {
		...
		if (vs_info->setting->math_type == MATH_WEIGHT) {
			...
			dbgmsg("%s: read[%d] %s %d mC, weight[%d] %d denominator %d\n",
				__func__, i,
				vs_data->sensors_arr[i].sensor_clnt->sensor_mgr->name,
				current_reading, i, vs_info->setting->weight_list[i],
				vs_info->setting->denominator);
		...
		} else if (vs_info->setting->math_type == LOCK_AND_CACHE) {
			vs_info->rs.cached_temp = current_reading;
			temp = vs_info->rs.cached_temp;
			break;
		}
	}
	...
	if (vs_info->setting->math_type == MATH_MAX ||
		...
		temp = math_result;
	else if (vs_info->setting->denominator)
		temp = temp / vs_info->setting->denominator;
	else if (weight_sum)
		temp = temp / (weight_sum);

	if (vs_info->setting->math_type == MATH_WEIGHT &&
		vs_info->setting->y_intercept) {
		dbgmsg("%s: Virtual sensor: %s y_intercept %d mC", __func__,
		vs_info->vs.name, vs_info->setting->y_intercept);
		temp = temp + vs_info->setting->y_intercept;
	}
...
void vs_interrupt_wait(struct sensors_mgr_sensor_info *sensor)
{
	...
	/* Let us clear it first to avoid any race condition later */
...
int vs_setup(struct sensors_mgr_sensor_info *sensor)
{
	...
	vs_info->rs.cached_temp = (vs_info->setting->tracks_low) ? INT_MAX
					: INT_MIN;
