...
static struct bw_client bw_client_list[] = {
	{
		.name = "camera_bw",
	},
	{
...
int get_bw_client_sensors_list(struct sensor_info **sensors)
{
	...
	count = (ARRAY_SIZE(bw_client_list) + MAX_CAMERA_BW_CLIENT - 1) * BW_SENSOR_MAX;
	if (count <= 0)
		return -1;
	...
	for (i = 0, j = 0; i < count && j < ARRAY_SIZE(bw_client_list); j++) {
		for(k = 0; k < (bw_client_list[j].total_clients + 1); k++) {
			char* bw_client_name = calloc(CLIENT_NAME_MAX, sizeof(char));
			if (bw_client_name == NULL) {
				msg("%s: calloc failed for sensor bw client %s\n",
						__func__, bw_client_list[j].name);
				goto err_exit;
			}

			if(!bw_client_list[j].total_clients)
				snprintf(bw_client_name,
						CLIENT_NAME_MAX,
						"%s", bw_client_list[j].name);
			else
				snprintf(bw_client_name,
						CLIENT_NAME_MAX,
						bw_client_list[j].name, k);
...
#else
			thermal_server_register_client_req_handler(bw_client_name,
				bw_client_hal_callback, bw_clnt);
#endif
			i += BW_SENSOR_MAX;
		}
