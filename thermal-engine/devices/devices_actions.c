...
#ifdef ANDROID
		ret =  android_reboot(ANDROID_RB_THERMOFF, 0, (char *)"thermal-engine");
#else
		sync();
...
int npu_device_init(void)
{
	...
	if (max_state == INT_MIN || max_state == INT_MAX ||
...
static int reset_gpu_cooling_device(void)
{
	...
	cdev_id = get_cdevn(GPU_COOLING_DEV_NAME1);
	if (cdev_id < 0) {
		msg("Could not find %s cooling device\n", GPU_COOLING_DEV_NAME1);
		return cdev_id;
	}
...
#if defined(ENABLE_MODEM_MITIGATION) && !defined(ENABLE_QMI_TMD_V2)
