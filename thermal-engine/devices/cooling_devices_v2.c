...
static char **cdev_used;
static int cdev_idx, cdev_size;
...
int cooling_device_get_cur_request(struct devices_manager_dev *dev_mgr, int *lvl)
{
	...
	if (curr_lvl == INT_MIN || curr_lvl == INT_MAX ||
...
int tmd_add_cooling_device(struct tmd_cdev_info_t *cdev)
{
	...
	dev = (struct devices_manager_dev *)calloc(1, sizeof(*dev));
	if (dev == NULL) {
		msg("%s: calloc failed.\n", __func__);
		ret_val = -(ENOMEM);
		goto handle_error;
...
static int is_cdev_used(char *buf)
{
	int idx = 0;

	for(; idx < cdev_idx; idx++) {
		if (strncmp(cdev_used[idx], buf,
...
static int alloc_cdev(int ct)
{
	int idx = cdev_idx;
	cdev_used = realloc(cdev_used, ct * sizeof(char *));
	if (!cdev_used)
		return -1;
	cdev_size = ct;
	while (idx < cdev_size)
	{
		cdev_used[idx] = malloc(DEVICES_MAX_NAME_LEN);
		if (!cdev_used[idx])
			return -1;
		idx++;
	}
...
static void fetch_used_cooling_devices(void)
{
	DIR *tdir = NULL;
	...
	if (ret < 0) {
		msg("%s: Unable to change path to %s, err:%d\n",
			__func__, THERMAL_SYSFS, errno);
		return;
	}
	...
	while ((tdirent = readdir(tdir))) {
		...
		/* Skip if it is cooling device */
		if (!strncmp(tdirent->d_name, CDEV_DIR_NAME,
					strlen(CDEV_DIR_NAME)))
			continue;

		if (snprintf(name, MAX_PATH, THERMAL_CDEV_TYPE,
					tdirent->d_name, idx) < 0)
			continue;

		ret = read_line_from_file(name, buf, sizeof(buf));
		while (ret >= 0) {
			if (cdev_idx >= cdev_size && alloc_cdev(2 * cdev_size))
				return;
			/* remove the \n at the end */
			buf[strlen(buf) - 1] = '\0';
			if (strlcpy(cdev_used[cdev_idx], buf, DEVICES_MAX_NAME_LEN)
...
static void parse_cooling_devices(void)
{
	...
	while ((tdirent = readdir(tdir))) {
		...
		/* Skip if it is not a cooling device */
		if (strncmp(tdirent->d_name, CDEV_DIR_NAME,
					strlen(CDEV_DIR_NAME)))
			continue;
		...
		if (dev_mgr || is_cdev_used(buf)) {
			closedir(cdevdir);
			continue;
		}

		cdev = (struct tmd_cdev_info_t *)calloc(1, sizeof(*cdev));
		if (cdev == NULL) {
			msg("%s: calloc failed.\n", __func__);
			ret = -(ENOMEM);
			closedir(cdevdir);
...
int cooling_devices_init(void)
{
	...
	fetch_used_cooling_devices();
	parse_cooling_devices();
	while (idx < cdev_size)
		free(cdev_used[idx++]);
	free(cdev_used);
	cdev_used = NULL;
	cdev_size = cdev_idx = 0;
	return 0;
}
