...
static struct table v_table[] = {
	...
	{.field_name = "trip_always_monitor", .offset = offsetof(struct setting_info,
		...
		.callback_func = &parse_int_opt},
	{.field_name = "weight_denominator", .offset = offsetof(struct setting_info,
		data.v.denominator),
		.callback_func = &parse_int_opt},
	{.field_name = "y_intercept", .offset = offsetof(struct setting_info,
		data.v.y_intercept),
		.callback_func = &parse_int_opt}
};
...
void add_settings(struct thermal_setting_t *setting,
					struct setting_info *cfg, int arr_size)
{
	int i;

	if ((setting == NULL) || (cfg == NULL) || (arr_size <= 0)) {
		msg("%s: Invalid args.\n", __func__);
		return;
	}
...
static void print_vs_setting(struct setting_info *setting, int flag)
{
	...
	if (vs->denominator)
		PRINT_SETTING(flag, "weight_denominator %d", vs->denominator);

	if (vs->y_intercept)
		PRINT_SETTING(flag, "y_intercept %d", vs->y_intercept);

	snprintf(info_buf, sizeof(info_buf), "trip_sensor");
	...
	if (vs->trip_cnt > 1) {
		snprintf(info_buf, sizeof(info_buf), "trip_sensor_logic");
