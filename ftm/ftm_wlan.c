...
# Copyright (c) 2011, 2013-2014, 2021, 2023-2024 by Qualcomm Technologies, Inc.
# All Rights Reserved.
...
static void *ftm_wlan_common_op(ftm_wlan_req_pkt_type *wlan_ftm_pkt, int pkt_len)
{
   ...
   bool resp = false;
   int curr_iface_id;
   volatile uint8_t bufferSize  = sizeof(ifname);

   snprintf(ifname, bufferSize, "%s%d", g_ifname,
   ...
   cmd_tmp = wlan_ftm_pkt->cmd.common_ops.data[0];
   cmd = &cmd_tmp;
