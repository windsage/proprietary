...
int ftm_hard_reset_nfcc( uint8_t chip_version )
{
    ...
    switch( chip_version ) {
        ...
        case CHIP_ID_SN220:
            LOG_INFORMATION( "%s: SN1xx or SN2xx NFC HW, not toggling VEN\n", __FUNCTION__ );
            ret = 0;
            break;
        default:
...
PARAMETERS
  char msg_type - Message type , C-> Command, R-> Response/Notification
...
===========================================================================*/
void PrintBytes( char msg_type, uint8_t *buf, uint16_t len)
{
#ifdef NFC_FTM_DEBUG
    int idx;

    LOG_INFORMATION( "%s: Length: %d bytes, TYPE: %c \n", __func__, len, msg_type );    // print the number of bytes
...
int ftm_nfc_send( uint8_t* buf )
{
    ...
    do
    {
        ...
        if(in_fw_dwl_mode)
        {
            nciSendMessageLength = DL_GET_PAYLOAD_LEN_LITTLE_ENDIAN(pFirmwarePacketsToSend->payloadLen) +
        ...
        LOG_MESSAGE("%s: packet len %d bytes\n", __func__, nciSendMessageLength);
        PrintBytes('C', buf, ((nciSendMessageLength>32)?32:nciSendMessageLength));
        do
        {
            ...
            {
                LOG_MESSAGE("%s: nfc write passed\n", __func__);
                break;                              // done
            }
        ...
        if(!retries)
        {
          LOG_ERROR("%s: nfc write failed after max retries\n", __func__);
        }
    } while( 0 );
...
int ProcessCommand( uint8_t *nci_data )
{
    ...
    do
    {
        ...
        LOG_MESSAGE( "%s: Waiting for response \n", __func__ );
        ...
        ret = sem_timedwait( &sRspReady,                    // start waiting
                             &time_sec );
        LOG_MESSAGE("%s: semaphore wait returned %d\n", __func__, ret);
...
void ProcessReturnedMessage( PNCI_MESSAGE pReadNCIMessage )
{
    ...
    case NCIMT_RSP:                                 // reply?
        LOG_MESSAGE("%s:%d posting about NCI resp\n", __func__, __LINE__);
    ...
    case NCIMT_DATA:                                // data?
        ...
        LOG_MESSAGE("%s: posting about data pkt\n", __func__);
        sem_post( &sRspReady );
        break;
...
void *nfc_read_thread( void *arg )
{
    ...
    for( ; ; )                                                                          // keep reading
    {
        ...
        LOG_MESSAGE("%s: ftm_nfc_read for hdr returned %d\n", __func__, readLength);
        PrintBytes( 'R', nciReplyMessage, readNCIUpToLength );
        ...
        if( readLength == readNCIUpToLength )                                           // read the message up to NCI Len?
        {
            ...
            LOG_ERROR("%s: ftm_nfc_read for payload returned %d\n", __func__, readLength);

            if( readLength == pNCIMessage->len )                                        // successful?
            {
                PrintBytes( 'R', nciReplyMessage, pNCIMessage->len + readNCIUpToLength );
...
int ftm_nfc_nq_vs_nxp( void )
{
    ...
    do
    {
        ...
        case 0x51:
            whatNQChip = NQ_330;
            LOG_INFORMATION( "Connected to NQ330 or NQ440 \n" );
            break;
        ...
        case CHIP_ID_SN220:
            whatNQChip = SN_100;
            LOG_INFORMATION( "Connected to SN1xx or SN2xx\n" );
            break;
