...
#define PINTEST_SWR_DEVPATH "/dev/btfmswr"
...
boolean ftm_bt_hci_hal_read_event (uint8 * event_buf_ptr)
{
  ...
  boolean is_slimbus_transport = isSlimbusTransport();
  ...
  */
  ...
  if ( event_buf_ptr[0] == BT_HCI_EVT_PKT &&
       ...
       is_slim_bus_test == 1)
  {
    if (is_slimbus_transport) {
      printf("\nInitializing slimbus for pin-connectivity\n");
      fd_pintest = open(PINTEST_DEVPATH, O_RDONLY | O_NONBLOCK);
    } else {
      printf("\nInitializing soundwire for pin-connectivity\n");
      fd_pintest = open(PINTEST_SWR_DEVPATH, O_RDONLY | O_NONBLOCK);
    }
    ...
    if (ret_val < 0)
    {
      ...
      printf("\nFailed to initialise %s %d\n",
               is_slimbus_transport ? "slimbus": "soundwire" ,ret_val);
      status = FALSE;
    } else
    {
      event_buf_ptr[PIN_CON_EVT_STATUS_BIT] = 0;
      printf("\n %s initialized succesfully\n",
                is_slimbus_transport ? "slimbus": "soundwire");
    }
    if (verbose == 1)
    {
...
/*===========================================================================
FUNCTION   isSlimbusTransport

DESCRIPTION
Returns true if slimbus transport is being used

DEPENDENCIES
  NIL

RETURN VALUE
  true = if slimbus is used, else false

SIDE EFFECTS
  None

===========================================================================*/
boolean isSlimbusTransport()
{
    int ret = 0;
    char bt_soc_type[PROPERTY_VALUE_MAX];
#ifndef USE_LIBBT_VENDOR
    ...
    if (ret != 0)
    {
      if (!strncasecmp(bt_soc_type, "hastings", sizeof("hastings")) ||
          !strncasecmp(bt_soc_type, "hamilton", sizeof("hamilton")) ||
          !strncasecmp(bt_soc_type, "cherokee", sizeof("cherokee")) ||
          !strncasecmp(bt_soc_type, "moselle", sizeof("moselle")) )
      {
        return TRUE;
      }
    }
