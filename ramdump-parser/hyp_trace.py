...
class HypDump(object):
    def __init__(self, ramdump):
        ...
        self.vmtype = ramdump.svm
        if hasattr(ramdump.board, 'vmid'):
            self.vmid = ramdump.board.vmid
        else:
            self.vmid = 45
            if self.vmtype == "oemvm":
                self.vmid = 49
        self.ttbr1 = None
        self.per_partition_phys_list = {}
    ...
    def get_phys_offset(self,thread_list_next,partition_base):
        ...
        if partition_base not in self.per_partition_phys_list:
            ...
            self.per_partition_phys_list[partition_base] = phys_offset_list
        else:
            phys_offset_list = self.per_partition_phys_list[partition_base]
    ...
    def get_trace_phy(self):
        ...
        while(partition_list != next):
            ...
            while(thread_list_head != thread_list_next):
                ...
                val = self.get_val_thread(kind_data, size=32)
                if val == THREAD_KIND_VCPU or val == THREAD_KIND_VCPU_NEW:
                    ...
                    if addrspace_res:
                        ...
                        vmid_res = self.get_val_thread(vmid_data, size=16)
