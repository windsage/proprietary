...
int main(int argc, char *argv[])
{
	...
	for (i = 0; i < num_providers; i++) {
		...
		if (pd_rc != PD_NOTIFIER_SUCCESS) {
			ALOGE("Failed to register for notifications: %d",pd_rc);
			goto error;
		} else {
    ...
    if (notifier){
        ALOGE("Restart PD\n");
        pd_rc = pd_notifier_restart_pd(notifier);
        if (pd_rc != PD_NOTIFIER_SUCCESS) {
            ALOGE("Failed to restart PD: %d", pd_rc);
        } else {
            ALOGI("Successfully restarted PD");
        }
