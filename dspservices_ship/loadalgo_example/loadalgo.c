...
extern int run_gaussian_algo_qtvm(char* loadalgot_TA_path, char* module_path, unsigned dynamic_heap_size, unsigned hlos_physpool_size);
                                  ...
                                  buf_type_t type, int64_t* state)
{
    ...
    shared_buf_rsp = (int64_t *)((int64_t)handle->ion_sbuffer + (int64_t)req_buf_len);
...
static int initialize_securepd(loadalgo_handle_t* ld_handle) {
    ...
    nErr = secure_mem_alloc(&heapHandle, dynamic_heap_size, ALIGNMENT, SECURE_HEAP);  //Allocate heap memory
    if (nErr) {
    ...
    nErr = secure_mem_alloc(&poolHandle, hlos_physpool_size, ALIGNMENT, SECURE_HEAP);  //Allocate secure memory
    if (nErr) {
...
static void ld_proxy_close()
{
    ...
    // Free all secure buffer if not already freed after proxy close
    if (heapHandle.fd && heapHandle.fd != -1) {
        secure_mem_free(&heapHandle);
    }
    if (poolHandle.fd && poolHandle.fd != -1) {
        secure_mem_free(&poolHandle);
    }
    if (secureHandle.fd && secureHandle.fd != -1) {
...
static void example_usage(void)
{
 printf("\n"
   ...
   " \t\t -l <path to algo elf>  : load algo elf in secure pd (legacy)\n"
   " \t\t -r <path to algo elf>  : load and run algo elf in secure pd (legacy)\n"
   " \t\t -e <path to algo elf>  : load and run algo elf in secure pd and exit\n"
   " \t\t -s                     : run static gaussian in secure pd (legacy)\n"
   " \t\t -t                     : load algo through QTVM\n"
   " \t\t -h or --help           : print this help message\n\n"
   " \t\t Examples : \n"
   " \t\t adb shell /vendor/bin/loadalgo -r \"/vendor/bin/example_image.so\" (legacy)\n"
   " \t\t adb shell /vendor/bin/loadalgo -d A00000 -p 800000 -s (legacy)\n"
   " \t\t adb shell /vendor/bin/loadalgo -t /data/local/tmp/Loadalgo_TA -e /data/local/tmp/example_image.so\n"
   " \t\t--------------------------------------------------------------\n\n\n",
   DEFAULT_DYNAMIC_HEAP_SIZE, DEFAULT_HLOS_PHYSPOOL_SIZE);
}

/**
  * @brief: Parse file to obtain file size.
  * @param stream: Associated stream with the opened file.
  * @param memsz : Size of the file in bytes.
  * @retval : 0 in case of success.
  */
int get_file_size(FILE* stream, size_t* memsz)
{
    int nErr = 0;
    int fd = -1;
    struct stat st;
...
/**
  ...
  * @param only_load: 1 if only load signed and encrypted algo elf.
  * @retval: 0 in case of success.
  */
static int start_load_algo_elf(char* path, int only_load)
{
    int nErr = 0, ret = 0;
    ...
    nErr = get_file_size(stream, &memsz);
    if (nErr) {
    ...
    nErr = secure_mem_alloc(&nonsecureHandle, memsz, ALIGNMENT, NON_SECURE_HEAP); //Allocate non secure memory
    if (nErr) {
    ...
    nErr = secure_mem_alloc(&secureHandle, memsz, ALIGNMENT, SECURE_HEAP);  //Allocate secure memory
    if (nErr) {
...
bail:
    ...
    printf("%s: freed non-secure buffers\n", __func__);
    if (only_load) {
        ret = send_sec_dsp_cmd_to_ta(DEREGISTER_BUFFER, heapHandle.fd, heapHandle.size, 0, 0, HEAP, &state); //TODO: Do we need to deregister here??
        if (ret || state) {
            ALOGE("%s: Failed to deregister heap buffer fd:%d  with ret:%d state:0x%llx\n", __func__, heapHandle.fd, ret, state);
        } else if (heapHandle.fd) {
            secure_mem_free(&heapHandle);
        }
        printf("%s: ERROR:0x%x deregistered HEAP buffer from TA, response:0x%lx\n", __func__, ret, state);

        ret = send_sec_dsp_cmd_to_ta(DEREGISTER_BUFFER, poolHandle.fd, poolHandle.size, 0, 0, POOL, &state);
        if (ret || state) {
            ALOGE("%s: Failed to deregister pool buffer fd:%d  with ret:%d state:0x%llx\n", __func__, poolHandle.fd, ret, state);
        } else if (poolHandle.fd) {
            secure_mem_free(&poolHandle);
        }
        printf("%s: ERROR:0x%x deregistered POOL buffer from TA, response:0x%lx\n", __func__,ret, state);

        //Deregister secure buffer
        ret = send_sec_dsp_cmd_to_ta(DEREGISTER_BUFFER, secureHandle.fd, secureHandle.size, 0, 0, ALGO, &state);
        if (ret || state) {
            ALOGE("%s: Failed to deregister secure buffer fd:%d  with ret:%d state:0x%llx\n", __func__, secureHandle.fd, ret, state);
        } else if (secureHandle.fd) {
            secure_mem_free(&secureHandle);
        }
        printf("%s: ERROR:0x%x deregistered ALGO buffer from TA, response:0x%lx\n", __func__, ret, state);
    }
...
int run_gaussian_algo(char* path)
{
    ...
    printf("%s: running through TZ (legacy)\n", __func__);
    if (NULL != path) {
        printf("%s: User entered path %s to algo elf\n", __func__, path);
        nErr = start_load_algo_elf(path, 0);
        if (nErr != 0) {
    ...
    nErr = secure_mem_alloc(&srcMemHandle, srcSize, ALIGNMENT, SECURE_HEAP);
    if (nErr != 0) {
    ...
    nErr = secure_mem_alloc(&dstMemHandle, dstSize, ALIGNMENT, SECURE_HEAP);
    if (nErr) {
...
bail:
    ...
    ret = send_sec_dsp_cmd_to_ta(DEREGISTER_BUFFER, srcMemHandle.fd, srcMemHandle.size, 0, 0, DATA, &state);
    if (ret || state) {
        ALOGE("%s: Failed to deregister secure buffer fd:%d  with ret:%d state:0x%llx\n", __func__, srcMemHandle.fd, ret, state);
    } else if (srcMemHandle.fd) {
       secure_mem_free(&srcMemHandle);
    }
    printf("%s: ERROR:0x%x deregistered SRC buffer from TA, response:0x%lx\n", __func__, ret, state);

    ret = send_sec_dsp_cmd_to_ta(DEREGISTER_BUFFER, dstMemHandle.fd, dstMemHandle.size, 0, 0, DATA, &state);
    if (ret || state) {
        ALOGE("%s: Failed to deregister secure buffer fd:%d  with ret:%d state:0x%llx\n", __func__, dstMemHandle.fd, ret, state);
    } else if (dstMemHandle.fd) {
       secure_mem_free(&dstMemHandle);
    }
    printf("%s: ERROR:0x%x deregistered DST buffer from TA, response:0x%lx\n", __func__, ret, state);
...
int main(int argc, char *argv[])
{
    ...
    nErr = secure_mem_init();
    if (nErr) {
        printf("ERROR %d: %s: setting up of memory allocators failed\n", nErr, __func__);
        goto bail;
    }

    while ((command = getopt_long(argc, argv, "d:p:l:r:e:h:t:s", example_options, NULL)) != -1) {
        switch (command) {
            case 'd':
            ...
            case 'l':
                while (1) {
                   ...
                   retVal = start_load_algo_elf(optarg, 1);
            ...
            case 'e':
                ...
                if (qtvm)
                   retVal = run_gaussian_algo_qtvm(loadalgot_TA_path, optarg, dynamic_heap_size, hlos_physpool_size);
                else
                   retVal = run_gaussian_algo(optarg);
                printf("%s: run_gaussian_algo returned 0x%x (%d)\n", __func__, retVal, retVal);
                restart = retry_sequence_after_ssr(retVal);
           ...
           case 't':
                qtvm = true;
                strlcpy(&loadalgot_TA_path, optarg, sizeof(loadalgot_TA_path));
                printf("%s: loadalgo_TA abs path %s\n", __func__, loadalgot_TA_path);
                break;
           case 'h':
                example_usage();
                break;
