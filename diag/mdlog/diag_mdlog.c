...
#if !defined(FEATURE_LE_DIAG) && !defined(FEATURE_OPENWRT_DIAG)
#include <log/log.h>
...
/* if only le feature is defined use UID/GID as 53
*  else if openwrt feature is defined use 2901 irrespective
*  of le feature.
*/
#if defined(FEATURE_LE_DIAG) && !defined(FEATURE_OPENWRT_DIAG)
...
#elif defined (FEATURE_OPENWRT_DIAG)
#define LE_UID_DIAG 2901
#define LE_GID_DIAG 2901
#define LE_GID_SDCARD 1015
#define LE_GID_INET_RAW	3004
#endif
...
static void parse_args(int argc, char **argv)
{
	...
	int file_num = 0;
	char mask_file_name[FILE_NAME_LEN];
	...
	while ((command = getopt_long(argc, argv, "f:m:l:o:p:s:w:n:t:x:y:g:q:j:cdkebrahiu", longopts, NULL))
		...
		switch (command) {
			...
			case 'e':
				...
				break;
			case 'f':
				strlcpy(mask_file_name, optarg, FILE_NAME_LEN);
				mask_file_entered = 1;
				break;
			case 'g':
			...
			case 'y':
				...
				if(etr_buffer_size == 0) {
					...
					if (etr_buffer_size < ETR1_BUF_SIZE_MIN) {
						ALOGE("diag: Invalid etr1 buffer size provided\n");
						exit(0);
					} else {
	...
	if (mask_file_entered) {
		if (disable_hdlc)
			strlcpy(mask_file2_proc[MSM], mask_file_name, FILE_NAME_LEN);
		else
			strlcpy(mask_file_proc[MSM], mask_file_name, FILE_NAME_LEN);
	}
}
...
fall_back_con_all:
	...
	for (i = 0; i < NUM_PROC; i++) {
		if (device_mask & (1 << i))
			if (params.peripheral_mask & DIAG_CON_MPSS)
