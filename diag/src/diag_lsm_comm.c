	...
	if (diag_use_dev_node) {
		...
		case DIAG_IOCTL_GET_DELAYED_RSP_ID:
		{
			...
			num_bytes_read = read(fd, (void*)read_buffer_comm,
						READ_BUF_SIZE);
			delayed_rsp_id = (read_buffer_comm[5] << 8) | read_buffer_comm[4];
		...
		case DIAG_IOCTL_HDLC_TOGGLE:
			data_type = DIAG_IOCTL_HDLC_TOGGLE;
			ptr = malloc(len + sizeof(data_type));
			if (!ptr)
				return -ENOMEM;
			memcpy(ptr, &data_type, sizeof(data_type));
			memcpy(ptr + sizeof(data_type), buf, len);
			ret = write(fd, ptr, sizeof(data_type) + len);
			free(ptr);
			if (ret < 0)
				return ret;
			break;
		case DIAG_IOCTL_MDM_HDLC_TOGGLE:
			data_type = DIAG_IOCTL_MDM_HDLC_TOGGLE;
			ptr = malloc(len + sizeof(data_type));
			if (!ptr)
				return -ENOMEM;
			memcpy(ptr, &data_type, sizeof(data_type));
		...
		case DIAG_IOCTL_REMOTE_DEV:
			...
			return 1; /* return 1 to match kernel diag implementation */
		...
		{
			 struct diag_dci_reg_pkt {
				int data_type;
				int pid;
				struct diag_dci_reg_tbl_t params;
			};
			struct diag_dci_reg_pkt *ptr;

			len = sizeof(struct diag_dci_reg_tbl_t);
			ptr = malloc(sizeof(*ptr));
			if (!ptr)
				return -ENOMEM;
			ptr->data_type = DIAG_IOCTL_DCI_REG;
			ptr->pid = getpid();
			memcpy(&ptr->params, buf, len);
			ret = write(fd, ptr, sizeof(*ptr));
			if (ret < 0) {
				free(ptr);
				return ret;
			}
		...
		case DIAG_IOCTL_SET_OVERRIDE_PID:
			data_type = DIAG_IOCTL_SET_OVERRIDE_PID;
			ptr = malloc(len + sizeof(data_type));
			if (!ptr)
				return -ENOMEM;
			memcpy(ptr, &data_type, sizeof(data_type));
		...
		case DIAG_IOCTL_UPDATE_QDSS_ETR1_SUP:
			data_type = DIAG_IOCTL_UPDATE_QDSS_ETR1_SUP;
			ptr = malloc(len + sizeof(data_type));
			if (!ptr)
				return -ENOMEM;
			memcpy(ptr, &data_type, sizeof(data_type));
			...
			free(ptr);
			break;
		case DIAG_IOCTL_GET_TRANSPORT_MODE:
		{
			struct transport_mode{
				int data_type;
				int mode;
			}__packed;
			struct transport_mode *rsp;

			data_type = DIAG_IOCTL_GET_TRANSPORT_MODE;
			...
			memset(read_buffer_comm, 0, READ_BUF_SIZE);
			ret = read(fd, (void*)read_buffer_comm, READ_BUF_SIZE);
			if (ret < 0)
				break;
			rsp = (struct transport_mode *)read_buffer_comm;
			if (rsp->data_type != DIAG_IOCTL_GET_TRANSPORT_MODE) {
				ret = -1;
				break;
			}
			*(int *)buf = rsp->mode;
		...
		case DIAG_IOCTL_DCI_DRAIN_IMMEDIATE:
			data_type = DIAG_IOCTL_DCI_DRAIN_IMMEDIATE;
			ptr = malloc(len + sizeof(data_type));
			if (!ptr)
				return -ENOMEM;
			memcpy(ptr, &data_type, sizeof(data_type));
		...
		case DIAG_IOCTL_VOTE_DCI_BUFFERING_MODE:
			data_type = DIAG_IOCTL_VOTE_DCI_BUFFERING_MODE;
			ptr = malloc(len + sizeof(data_type));
			if (!ptr)
				return -ENOMEM;
			memcpy(ptr, &data_type, sizeof(data_type));
