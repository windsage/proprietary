...
JNIEXPORT void JNICALL Java_com_qualcomm_qti_LibsocHelper_native_1getSocInfo
    ...
    if (NULL != mDlHandler.getSocInfo) {
        soc_info_v0_1_t soc = {MSM_CPU_UNKNOWN, MSM_CPU_UNKNOWN};
        ...
        if (NULL != clazz) {
            jfieldID msm_cpu_id = env->GetStaticFieldID(clazz, "msmCpu", "I");
            jfieldID cpu_variant_id = env->GetStaticFieldID(clazz, "cpuVariant", "I");
            if ((NULL != msm_cpu_id) && (NULL != cpu_variant_id)) {
                env->SetStaticIntField(clazz, msm_cpu_id, soc.msm_cpu);
                env->SetStaticIntField(clazz, cpu_variant_id, soc.cpu_variant);
            }
