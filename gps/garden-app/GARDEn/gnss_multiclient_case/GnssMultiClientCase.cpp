...
GARDEN_RESULT GnssMultiClientCase::menuTest()
{
    ...
    while(!exit_loop) {
        PRINTLN("\n\n"
                ...
                "11: AIDL API Validation - IGnss \n"
                "12: AIDL API Validation - ILocAidlGnss \n"
#endif
...
//            result = GnssMultiClientHidlIGnss::get().menuTest();
        ...
        case 11:
            ...
            result = GnssMultiClientAidlIGnss::get().menuTest();
            break;
        case 12:
            ABinderProcess_startThreadPool();
            result = GnssMultiClientAidlILocAidlGnss::get().menuTest();
            break;
#endif
        case 90:
...
GARDEN_RESULT GnssMultiClientCase::createLocationAPIClient()
{
    ...
    PRINTLN("Added location API client at index: %u",
...
{
    PRINTLN("Location API Clients count: %u", mLocClientList.size());
        ...
        uint64_t enabledMask, uint64_t disabledMask)
{
    ...
    if (NULL != gnssInterface) {
        ...
        PRINTLN("Submitting Gnss Constellation config to Gnss Interface "
                "( enabled 0x%" PRIx64 " disabled 0x%" PRIx64 " )",
...
GARDEN_RESULT GnssMultiClientCase::gnssIfaceAPIGetSvTypeConfig()
{
    ...
    if (NULL != gnssInterface) {
        ...
        GnssSvTypeConfigCallback callback = [](const GnssSvTypeConfig& config) {
            ...
            PRINTLN("Enabled Constellation Mask: 0x%" PRIx64, config.enabledSvTypesMask);
            PRINTLN("Disabled Constellation Mask: 0x%" PRIx64, config.blacklistedSvTypesMask);
