...
static CommandLineOptionsType sOptions = {
    ...
    {0}, // Path to IGnssApiCase command input file
    {0}, // IGnssApiCase command string to be executed
...
void readConfigFile(char* path) {
    ...
    loc_param_s_type cfg_parameter_table[] =
    {
      ...
      {"SESSION_NUM",   &sOptions.sessionCount,                             NULL, 'n'},
    ...
    gardenPrint("SESSION_NUM   : %d", sOptions.sessionCount);
...
static void printHelp(char **arg) {
     ...
     gardenPrint("    -l:  Number of sessions to loop through. Takes an argument. "
                 "An argument of 0 means no sessions. Defaults:%d", sOptions.sessionCount);
     ...
     gardenPrint("    -J:  Load IGnssApiCase commands from a file. "
                 "e.g: -J /vendor/etc/ignssapicmds.txt");
     gardenPrint("    -j:  Load IGnssApiCase commands from a string. "
                 "e.g: -j \"gnssStart;sleep -s 60;gnssStop;\"");
...
static GARDEN_RESULT parse_args(int argc, char *argv[]) {
    ...
    while ((opt = getopt(argc, argv,
            "r:l:t:u:hR:s:d:m:i:a:x:P:N:g:D:w:c:nye:o:z:A:B:T::Z:F:G:f:C:MKS:U:D:E:j:J:")) != -1) {
        switch (opt) {
        case 'r':
            ...
            break;
        case 'l':
            sOptions.sessionCount = atoi(optarg);
            gardenPrint("Number of Sessions to loop through:%d", sOptions.sessionCount);
            break;
        case 't':
        ...
        case 'C':
            ...
            break;
        case 'j':
            // reset the defaults to prevent auto session start
            sOptions.sessionCount = 0;
            sOptions.fixThresholdInSecs = 0;
            // Now parse cmd string
            strlcpy(sOptions.iGnssApiCmdStr, optarg, 1024);
            gardenPrint("IGnssApi command string: %s", sOptions.iGnssApiCmdStr);
            break;
        case 'J':
            // reset the defaults to prevent auto session start
            sOptions.sessionCount = 0;
            sOptions.fixThresholdInSecs = 0;
            // Now parse cmd file
            strlcpy(sOptions.iGnssApiCmdFile, optarg, 256);
            gardenPrint("IGnssApi command file: %s", sOptions.iGnssApiCmdFile);
            break;
        case 'U':
        ...
        case 'S': {
                ...
                if (setRequiredPermToRunAsLocClient()) {
                    ...
                    clientApiCase->autoStartCases(
                            sOptions.interval, sOptions.sessionCount,
...
static GARDEN_RESULT autoTest() {
    ...
    for (int i = 0; i < sOptions.sessionCount; i++) {
        GARDEN_ADD(GnssCase, "SessionLoop",
            {
                ...
                if(sOptions.rilInterface) {
                    api->updateNetworkAvailability(0x0123456789ABCDEF, 88, 0,
                                                   "MY_APN_FOR_RIL_TEST");
    ...
    if (strlen(sOptions.iGnssApiCmdStr) > 0) {

        GARDEN_ADD(IGnssApiCase, "Load IGnssAPI commands from string",
            {
                IGnssApiCase* iGnssApiCase = (IGnssApiCase*)icase;
                iGnssApiCase->loadFromCmdStr(sOptions.iGnssApiCmdStr);
            });
    } else if (strlen(sOptions.iGnssApiCmdFile) > 0) {

        GARDEN_ADD(IGnssApiCase, "Load IGnssAPI commands from file",
            {
                IGnssApiCase* iGnssApiCase = (IGnssApiCase*)icase;
                iGnssApiCase->loadFromCmdFile(sOptions.iGnssApiCmdFile);
            });
    }
