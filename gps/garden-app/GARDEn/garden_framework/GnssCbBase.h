...
public:
    ...
    virtual void gnssLocationCb(const Location& location);
    virtual void gnssStatusCb(uint32_t status);
    virtual void gnssSvStatusCb(const GnssSvNotification& svNotify);
    virtual void gnssNmeaCb(uint64_t timestamp, const char* nmea, int length);
    virtual void gnssMeasurementsCb(
            const GnssMeasurementsNotification& gnssMeasurementsNotification);
    virtual void gnssDataCb(const GnssDataNotification& dataNotify);
    virtual void gnssSetCapabilitiesCb(uint32_t capabilities);
    virtual void gnssRequestUtcTimeCb();
    virtual void gnssNiNotifyCb(uint32_t id, const GnssNiNotification& notification);
    ...
    virtual void apObsLocDataUpdateCallback(
            std::vector<izat_remote_api::WiFiDBUploader::APObsData> apObsLocDataList,
    ...
    // Emergency status receiver
    virtual void onEsStatusChanged(bool isEmergencyMode) {}
