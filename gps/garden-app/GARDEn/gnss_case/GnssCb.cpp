...
void GnssCb::gnssLocationCb(const Location& location) {
...
void GnssCb::gnssSvStatusCb(const GnssSvNotification& svNotify) {
...
void GnssCb::gnssMeasurementsCb(const GnssMeasurementsNotification& gnssMeasurementsNotification) {
    if (mGnssCase->mMeasurements) {
        ...
        gardenPrint("isNHz: %d", gnssMeasurementsNotification.isNhz);
    }
}
...
void GnssCb::gnssDataCb(const GnssDataNotification& dataNotify) {
