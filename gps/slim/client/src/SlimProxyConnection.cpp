...
{
  SLIM_LOGD("Service change: svc=%d %08" PRIx32" fullScale:%d",
      eService, (uint32_t)qServiceProviderMask, qSensorInfo.fullScaleSetting);
  ...
  zMsg.m_zPayload.m_zServiceChange.m_qSensorInfo.fullScaleSetting = qSensorInfo.fullScaleSetting;
  memscpy(&zMsg.m_zPayload.m_zServiceChange.m_qSensorInfo.vendorName[0],
    sizeof(zMsg.m_zPayload.m_zServiceChange.m_qSensorInfo.vendorName),
    &qSensorInfo.vendorName[0], sizeof(qSensorInfo.vendorName));
  zMsg.m_zPayload.m_zServiceChange.m_qSensorInfo.samplingRate = qSensorInfo.samplingRate;
...
)
{
      ...
      eService <= eSLIM_SERVICE_LAST)
  {
    ...
    slim_Memscpy(&zServiceState.m_qSensorInfo, sizeof(zServiceState.m_qSensorInfo),
            &qsensorInfo, sizeof(qsensorInfo));
    ...
    slim_Memscpy(&zData.qSensorInfo, sizeof(zData.qSensorInfo), &qsensorInfo, sizeof(qsensorInfo));
