...
)
{
  ...
  if (qOldMask != qNewMask)
  {
    ...
    slimSensorInfoStructT    q_sensorInfo;
    memscpy(&q_sensorInfo, sizeof(q_sensorInfo),
        &zData.qSensorInfo, sizeof(q_sensorInfo));
    m_pzOwner->handleServiceProviderChange(
        zData.service, qRemoved, qAdded, qNewMask, q_sensorInfo);
  }
}
/**
 * @brief Internal method for service request result.
