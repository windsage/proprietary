  ...
  for (;;)
  {
    ...
    case eIPC_CONNECTION_SERVICE_CHANGE:
    {
      ...
      SLIM_LOGD("Service change: svc=%d %08" PRIx32" fullScale:%d",
        ...
        zMsg.m_zPayload.m_zServiceChange.m_qSensorInfo.fullScaleSetting);
      memscpy(&zMsg, sizeof(zMsg), pzMsg->p_Data, pzMsg->q_Size);
      handleConnectionServiceChange(
          zMsg.m_pHandle,
          zMsg.m_zPayload.m_zServiceChange.m_eService,
          zMsg.m_zPayload.m_zServiceChange.m_qServiceProviderMask,
          zMsg.m_zPayload.m_zServiceChange.m_qSensorInfo);
      break;
    }
...
)
{
  ...
  {
      SLIM_LOGD("SensorService: %d fullScale:%x", e_Service,
                 pz_Status->qSensorInfo.fullScaleSetting);
...
(
  ...
  /**< [in] Full Scale setting */
)
{
  Connection *pzConnection = findConnection(pHandle);
  if (0 != pzConnection)
  {
    pzConnection->handleConnectionServiceChange(eService, qProviderMask, qsensorInfo);
  }
}
/**
