/*
Copyright (c) 2011-2012, 2014-2015, 2017, 2020, 2022, 2024 Qualcomm Technologies, Inc.
All Rights Reserved.
...
)
{
    ...
    m_tLastSampleDelay = (int64_t)(readCoreTimeUs() / 1000) - (int64_t)tTimestampMs;
...
) const
{
    if (m_uRunning && m_zSensorData.samples_len > 0)
    {
        ...
        if ((m_tLastSampleDelay >= 0) ||
                ((m_tLastSampleDelay < 0) &&
                ((m_zSensorData.timeBase + tBatchingIntervalMs) > (uint64_t)(-m_tLastSampleDelay))))
        {
            tTimeMsOut = (uint64_t) ((int64_t)m_zSensorData.timeBase +
                            (int64_t)tBatchingIntervalMs + m_tLastSampleDelay);
            return true;
        }
        else
        {
            SLIM_LOGE("tTimeMsOut Expiration time can not be negative number\n");
            return false;
        }
    }
    return false;
