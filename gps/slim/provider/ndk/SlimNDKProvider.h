/*
Copyright (c) 2011-2012, 2014-2015, 2017, 2022 Qualcomm Technologies, Inc.
All Rights Reserved.
...
private:
  ...
  enum
  {
    ...
    /** Message to inform timer expiry */
  ...
  enum
  {
    ...
    eIPC_TIMER_ID_RELINK = 2,
    /**< Timer to retry link to death recipient if previous attempt has failed. */
  };
...
private:
  ...
  static const uint32_t  s_qDeathRecipientReLinkTimeoutMs;
  /** @brief max number of retry attempts for link to DeathRecipient */
  static const uint32_t  s_qMaxReLinkAttempts;
  ...
  /*! @brief Reference for sensor manager for death recipient registration */
  sp<ISensorManager>  m_zSensorMgr;

  /*! @brief Event queue */

  /*! @brief Timer for retrying service bind attempts */
  slim_TimerT         *m_pzBinderTimer;
  /*! @brief Timer for retrying death recipient attempts */
  slim_TimerT         *m_pzDeathRecipientTimer;
  ...
  */
  static uint16_t calcMsTimeDiff
  (
    uint64_t tTs1Ms,
    uint64_t tTs2Ms
  );
  ...
  */
  ...
  /**
  @brief Link to death recipient framework from Android Sensor Manager

  Method connects to ISensorManager dead recipient interface by registering
  a callback. If the service is removed or disconnected, notification is
  received in the registered callback.
