...
int SensorHalProvider::readTempASM330(void)
{
  ...
  if (EOF == mTempFilePtr.asmTempFile.tScaleFile->peek() ||
      EOF == mTempFilePtr.asmTempFile.tOffsetFile->peek() ||
      EOF == mTempFilePtr.asmTempFile.tRawDataFile->peek())
  {
...
int SensorHalProvider::readTempBMI(void)
{
  ...
  /* Read Temp data */
  if (EOF == mTempFilePtr.bmiTempFile.tTempFile->peek())
  {
    SLIM_LOGD("Temperature data not available in file");
    return 0;
  }
...
int SensorHalProvider::readTempSMI(void)
{
  ...
  if (EOF == mTempFilePtr.smiTempFile.tempFile->peek())
  {
    SLIM_LOGD("Temperature data not available in file");
    return 0;
  }
 ...
 * @return void.
 */
int SensorHalProvider::readTempSMI230(void)
{
  ...
  {
    SLIM_LOGD("Temperature string not available in file");
    return 0;
  }
  ...
  if (split.size() < 2 ){
    SLIM_LOGE("temperature value unavailable");
    return 0;
  }
  tempRead = std::stoi(split[1]);
  temperature =  tempRead / 1000.0; //Convert to Degree celsius
  SLIM_LOGD("Read Raw:%d, Temperature: %f\n", tempRead, temperature);
  ...
  while (accelNormalMode && gyroNormalMode && -1 != readReturn) {
    ...
    switch(m_zLibInfo.sensorType)
    {
      ...
      case SENSOR_TYPE_SMI:
        ...
        break;
      case SENSOR_TYPE_SMI_230:
        readReturn = readTempSMI230();
        break;
      default:
        readReturn = -1;
    ...
    case SENSOR_TYPE_SMI_230:
      {
        ...
        if (SENSOR_TYPE_SMI == m_zLibInfo.sensorType )
          memscpy(smiSearchKey, sizeof(smiSearchKey), SMI_TEMP_SEARCH, sizeof(SMI_TEMP_SEARCH));
        else
          memscpy(smiSearchKey, sizeof(smiSearchKey),
                    SMI230_TEMP_SEARCH, sizeof(SMI230_TEMP_SEARCH));
        findPath(DYN_INPUT_TYPE, smiPathTemp, smiSearchKey, sizeof(smiPathTemp));
 ...
 * @return void.
 */
void scalingSMI230BufferData(slimServiceEnumT aeSensorType, slimSensorSampleStructT *azEventBuffer)
{
  int cnt;
  float scaleFactor = 1;
  /* Get the scale factor based on sensor type */
...
void SensorHalProvider::bufferDataScaling( slimServiceEnumT aeServiceType, slimSensorSampleStructT *zEventBuffer )
{
  switch( m_zLibInfo.sensorType )
  {
    ...
    case SENSOR_TYPE_SMI:
      ...
      break;
    case SENSOR_TYPE_SMI_230:
      scalingSMI230BufferData(aeServiceType, zEventBuffer);
      break;
    case SENSOR_TYPE_ASM:
...
void SensorHalProvider::bufferDataprocessTask()
{
  ...
  switch ( m_zLibInfo.sensorType )
  {
    ...
    case SENSOR_TYPE_SMI:
      ...
      break;
    case SENSOR_TYPE_SMI_230:
      findPath(DYN_INPUT_TYPE, acc_boot_sample, SMI230_TEMP_SEARCH, sizeof(acc_boot_sample));
      findPath(DYN_INPUT_TYPE, gyr_boot_sample, SMI230_GYR_SEARCH, sizeof(gyr_boot_sample));
      break;
    case SENSOR_TYPE_ASM:
  ...
  // if enable request already comes from the client, lets proceed to read buffer data.
  pthread_mutex_lock (&mHalBuffMutex);
  if (mBufferTxStatus && mAccServiceStatus && mGyroServiceStatus) {
    SLIM_LOGD("already got enable request, Triggering buffer data read %d %d %d \n",
            mBufferTxStatus, mAccServiceStatus, mGyroServiceStatus);
  } else {
    SLIM_LOGD("Waiting for service enable request from clients");
...
void SensorHalProvider::runEventLoop()
{
   ...
   while (1)
   {
      ...
      for (int i=0 ; i<n ; i++) {
        ...
        if (m_qEnabledServices == 0) {
          /* if none of the service is enable, exit processing */
          SLIM_LOGE("Sensor samples are not processed as services are disabled.");
          break;
        }
        ...
        switch(data.type) {
            case SENSOR_TYPE_ACCELEROMETER:
               /* If buffer data is in progress, keep these samples in secondary buffer */
                if ((mBufferTxStatus == KERNEL_BUFFER_DATA_IN_PROGRESS ||
                       ...
                       0 != SLIM_MASK_IS_SET(m_qEnabledServices, eSLIM_SERVICE_SENSOR_ACCEL))
                {
         ...
         case SENSOR_TYPE_GYROSCOPE_UNCALIBRATED:
            ...
            if ((mBufferTxStatus == KERNEL_BUFFER_DATA_IN_PROGRESS ||
                     mBufferTxStatus == SLIM_BUFFER_DATA_IN_PROGRESS) &&
                     0 != SLIM_MASK_IS_SET(m_qEnabledServices, eSLIM_SERVICE_SENSOR_GYRO))
            {
              SLIM_LOGD("sensor=%s, time=%lld,current time - %lld, value=<%f,%f,%f>\n",
                     getSensorName(data.type),
                     (data.timestamp / 1000000),
...
long long CalculateLatency(const float *aRates, int aSR, int aSC, int aType, int aLength)
{
  ...
  double constants[5] = {ASM_BATCH_TIME, BMI_BATCH_TIME, IAM_BATCH_TIME, SMI_BATCH_TIME, \
                         SMI230_BATCH_TIME};
...
)
{
  ...
  const float rateMapping[5][2][12] = RATE_MAPPING;
  long long latency = 0;
  ...
  if (m_zLibInfo.sensorType >= SENSOR_TYPE_ASM && m_zLibInfo.sensorType <= SENSOR_TYPE_SMI_230 &&
  ...
  for (int i=0 ; i<count ; i++) {
    if ((NULL != list) && (list[i].type == sensor))
    {
      SLIM_LOGD("Action for '%s' (%d) is %s\n",
        ...
        /* flush data in buffer for disable event */
        if (0 == uEnable) {
          SLIM_MASK_CLEAR(m_qEnabledServices, eService);
          /* driver buffer flush */
          device->flush(device, list[i].handle);
          /* slim buffer flush */
          flushBuffer(eService);
        }
...
void SensorHalProvider::flushBuffer(slimServiceEnumT eService)
{
