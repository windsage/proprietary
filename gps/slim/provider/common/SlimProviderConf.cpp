...
)
{
  ...
  switch (e_Provider)
  {
  case SLIM_PROVIDER_SENSOR_PROTOBUF:
    /* Initialize SensorProtobuf only when it is configured */
#ifdef FEATURE_LOCTECH_SLIM_DSPS
    if (SLIM_MASK_IS_SET(pz_ConfData->q_EnabledMask, SLIM_PROVIDER_SENSOR_PROTOBUF))
    {
      SLIM_LOGD("Creating SensorProtobuf provider");
      pz_Provider = slim::SensorProtobufProvider::getServiceProvider();
    }
#endif
    break;
...
#endif
    break;
  case SLIM_PROVIDER_SENSOR_HAL_DAEMON:
#ifdef FEATURE_LOCTECH_SLIM_SENSOR_HAL_DAEMON
    SLIM_LOGD("Initializing Sensor HAL Daemon provider");
    if (SLIM_MASK_IS_SET(pz_ConfData->q_EnabledMask, SLIM_PROVIDER_SENSOR_HAL_DAEMON))
    {
      SLIM_LOGD("Creating Sensor HAL Daemon provider");
      pz_Provider = slim::SensorHalDaemonProvider::getProvider();
    }
#endif
    break;
...
)
{
    ...
    if (SLIM_PROVIDER_NONE == e_Provider &&
        SLIM_MASK_IS_SET(q_AvailableProviders, SLIM_PROVIDER_SENSOR_HAL_DAEMON) &&
        SLIM_MASK_IS_SET(q_AllowedProviders, eSLIM_SERVICE_PROVIDER_NATIVE))
    {
      e_Provider = SLIM_PROVIDER_SENSOR_HAL_DAEMON;
    }
    ...
    if (SLIM_PROVIDER_NONE == e_Provider &&
        ...
        SLIM_MASK_IS_SET(q_AvailableProviders, SLIM_PROVIDER_SENSOR_PROTOBUF) &&
        SLIM_MASK_IS_SET(q_AllowedProviders, eSLIM_SERVICE_PROVIDER_SSC))
    {
      e_Provider = SLIM_PROVIDER_SENSOR_PROTOBUF;
    }
