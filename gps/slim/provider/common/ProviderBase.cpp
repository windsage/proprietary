/*
Copyright (c) 2014-2015, 2021-2022 Qualcomm Technologies, Inc.
All Rights Reserved.
...
(
  slimServiceMaskFullScaleT maskAndRange
)
{
  slim_CoreNotifyConfigurationChange(m_eProvider, maskAndRange);
}
...
)
{
  ...
  case eMSG_ID_CONFIGURATION_CHANGE:
    if (sizeof(slimAvailableServiceMaskT) == qMsgPayloadSize ||
        (sizeof(slimServiceMaskFullScaleT) == qMsgPayloadSize))
    {
      slimServiceMaskFullScaleT maskAndRange;
      if (sizeof(slimAvailableServiceMaskT) == qMsgPayloadSize)
      {
        slim_Memscpy(&maskAndRange.availableServicesMask, sizeof(slimAvailableServiceMaskT),
          pMsgPayload, sizeof(slimAvailableServiceMaskT));
        memset(&maskAndRange.m_qsensorInfo, 0,
          sizeof(maskAndRange.m_qsensorInfo));
      }
      else
      {
        slim_Memscpy(&maskAndRange, sizeof(slimServiceMaskFullScaleT),
          pMsgPayload,  sizeof(slimServiceMaskFullScaleT));
      }
      dispatchConfigurationChange(maskAndRange);
    }
    break;
  default:
    break;
...
@param[in] qChangedServiceMask Modified services.
@param[in] qFullScaleSetting Full Scale setting for each modified service.
...
(
  slimAvailableServiceMaskT qChangedServiceMask,
    slimSensorInfoStructT qsensorInfo[32]
)
{
  slimServiceMaskFullScaleT maskAndRange;
  SLIM_LOGE("Change Mask:%x qFullScaleSetting[0]:%d",
    qChangedServiceMask, qsensorInfo[0].fullScaleSetting);

  slim_Memscpy(&maskAndRange.availableServicesMask, sizeof(slimAvailableServiceMaskT),
          &qChangedServiceMask, sizeof(slimAvailableServiceMaskT));
  slim_Memscpy(&maskAndRange.m_qsensorInfo[0], sizeof(maskAndRange.m_qsensorInfo),
          &qsensorInfo[0], (sizeof(slimSensorInfoStructT) * 32));
