...
#define SLIM_VNW_TIMESTAMP_SRC_CAN_CONTROLLER    (0)
#define SLIM_VNW_TIMESTAMP_SRC_SYS_BOOT_TIME     (1)
...
public:
  ...
  */
  ...
  /**
  @brief Get user input about VNW buffered data

  Function returns a value specified in configuration file or 1
  if value is not set or cannot be parsed.

  @return value if VNW buffered data is required or not
  */
  uint8_t GetVnwEnableBufferedDataValue() const;

  /**
  @brief Get CAN timestamp src confiration

  Function returns a value specified in configuration file or default value.

  @return time timestamp source for VNW CAN data
  */
  uint32_t GetVnwDataTimeSrcValue() const;
...
private:
  ...
  */
  ...
  uint8_t               mVnwEnableBufferedData;
  uint32_t              mVnwDataTimeSource;
