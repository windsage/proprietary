...
void VehicleNetworkProvider::threadMain(void *pData)
{
  ...
  pzVNWProvider->m_pzVehicleNetworkCfg = new VehicleNetworkConfiguration();
  if (nullptr == pzVNWProvider->m_pzVehicleNetworkCfg) {
    SLIM_LOGE("VehicleNetworkConfiguration failed !.. Exiting thread \n");
    slim_TaskReadyAck();
    slim_ThreadDetach();
    return;
  }
  pzVNWProvider->m_pzVehicleNetworkCfg->Load();
  /* Service support is enabled */
  if (SLIM_VNW_TIMESTAMP_SRC_SYS_BOOT_TIME ==
          pzVNWProvider->m_pzVehicleNetworkCfg->GetVnwDataTimeSrcValue()) {
      pzVNWProvider->m_pzCanWrapper = CanWrapper::getInstance(
              DRIVER_MODE_RAW_FRAMES, TIMESTAMP_SRC_SYS_BOOT_TIME);
  } else {
      pzVNWProvider->m_pzCanWrapper = CanWrapper::getInstance(
              DRIVER_MODE_RAW_FRAMES, TIMESTAMP_SRC_CAN_CONTROLLER);
  }
...
void VehicleNetworkProvider::runEventLoop()
{
  ...
  uint8_t enableBufferedData;
  enableBufferedData = m_pzVehicleNetworkCfg->GetVnwEnableBufferedDataValue();

  if (0 == enableBufferedData)
  {
      SLIM_LOGD("VNW Buffered Data Disabled");
      mearIdAccel = 1;
      mearIdGyro = 1;
      mearIdOdo = 1;
      mearIdSpeed = 1;
...
void VehicleNetworkProvider::flushPrevData(slimServiceEnumT eService)
{
  switch (eService) {
    case eSLIM_SERVICE_VEHICLE_ACCEL:
      if (m_pzAccelProc) {
        m_pzAccelProc->flushPrevData();
      }
      break;
    case eSLIM_SERVICE_VEHICLE_GYRO:
      if (m_pzGyroProc) {
        m_pzGyroProc->flushPrevData();
      }
      break;
    case eSLIM_SERVICE_VEHICLE_ODOMETRY:
      if (m_pzOdometryProc) {
        m_pzOdometryProc->flushPrevData();
      }
      break;
    case eSLIM_SERVICE_VEHICLE_SPEED:
      if (m_pzSpeedProc) {
        m_pzSpeedProc->flushPrevData();
      }
      break;
    case eSLIM_SERVICE_VEHICLE_DWS:
      if (m_pzDwsProc) {
        m_pzDwsProc->flushPrevData();
      }
      break;
    case eSLIM_SERVICE_VEHICLE_GEAR:
      if (m_pzGearProc) {
        m_pzGearProc->flushPrevData();
      }
      break;
    case eSLIM_SERVICE_VEHICLE_STEERING_WHEEL:
      if (m_pzSteeringWheelProc) {
        m_pzSteeringWheelProc->flushPrevData();
      }
      break;
  }
}
