...
volatile boolean SensorPlayBackProvider::timerElapsed = FALSE;
...
void SensorPlayBackProvider::processGPS2APSSClkLog(unsigned char *pRaw)
{
  ...
  if (NULL == pRaw || 0 == s_uSession_Started )
  {
    SLIM_LOGE("pRaw:%x session:%d validGpsRx- %d", pRaw, s_uSession_Started, s_bValidGpsTimeRxd);
    return;
  }
  ...
  epDiagGnssImplMeasurementReport *pLog;

  log_hdr_type *pHdr = (log_hdr_type *) pRaw;
  double diagTS = CDMATimeToUTCMs(pHdr->ts_hi, pHdr->ts_lo);
  SLIM_LOGE("Version:%d", ((epDiagLogGenericHeader *)pRaw)->u_Version);

  pLog = (epDiagGnssImplMeasurementReport *)pRaw;

  apTimeStampUncertaintyMs =
    pLog->msrReport.msrInfo.lastPpsLocaltimeStamp.apTimeStampUncertaintyMs;

  SLIM_LOGD("0x1C5e received, Float tunc- %f", apTimeStampUncertaintyMs);
  SLIM_LOGD("SysWeekMsecValid:%u SysWeek:%d valid:%d val:%f",
     pLog->msrReport.msrInfo.gpsSystemTime.validityMask.isSystemWeekMsecValid,
      pLog->msrReport.msrInfo.gpsSystemTime.validityMask.isSystemWeekValid,
      s_bValidGpsTimeRxd,
      apTimeStampUncertaintyMs);

  // once tunc is less than 1.1ms, the validity becomes true throughtout run unless
  ...
  if ( 1 != pLog->msrReport.msrInfo.gpsSystemTime.validityMask.isSystemWeekMsecValid ||
     ...
     ( !s_bValidGpsTimeRxd && ((float)1.1 < apTimeStampUncertaintyMs) ) )
  {
    SLIM_LOGD("Received invalid validityMask %f ", apTimeStampUncertaintyMs);
  ...
  else
  {
    ...
    SLIM_LOGD("nGPSMs = %d Update every time nSysRtcTime = %d deltaMs:%d TimerOffset:= %d",
    nGPSMs, nSysRtcTime, deltaMs, qGPS2APSSTimerOffset);
    pthread_mutex_lock(&s_zGPS2APSSTimerOffsetMutex);
...
void SensorPlayBackProvider::process_dci_log_stream(unsigned char *ptr, int len)
{
  ...
  SLIM_LOGD("process_dci_log_stream :pHdr->code = %x ", pHdr->code);
  if (0x1C5E == pHdr->code)
  {
    ...
    SLIM_LOGE("process_dci_log_stream: 0x1C5E");
...
slimErrorEnumT SensorPlayBackProvider::Diag_DCI_Init(int *pmClient_id)
{
  ...
  {
    SLIM_LOGD("SUCCESS initialize DIAG.");
  }
  ...
  SLIM_LOGD("diag_register_dci_client Success Going for diag_register_dci_stream_proc:%d",
            *pmClient_id);
...
bool SensorPlayBackProvider::readDiscardHeading(FILE *fPTR)
{
  bool bStatus =  true;
  char ReadLine[255];
  int result = -1;
  ...
  if ( NULL != fPTR )
  {
    /* Read and discard file until heading as below is found in file
     *    "% Column #1   : GpsMsec"
     */
    ...
    SLIM_LOGD("Further Discard 7 more lines\n");
...
void SensorPlayBackProvider::runFileReadLoop()
{
  ...
  SLIM_LOGE("Waiting for m_zPlayBackStartCond");
  pthread_mutex_lock (&m_zPlayBackStartMutex2);
  SLIM_LOGE("Waiting for m_zPlayBackStartCond Done");
  ...
  while (1)
  {
    ...
    SLIM_LOGD("_fptr - %p, %s, %s", fPTR, mqSensorPlayBackFile, strerror(errno));
    if ( NULL != fPTR )
    {
      /* Read and discard Header from Sensor data file */
      readDiscardHeading(fPTR);
      ...
      while (1)
      {
        ...
        SLIM_LOGD("Inside the While (1) PTR:%x Buffer_Size:%d vectorSize:%d \n",
                 fPTR, Buffer_Size, m_zSensorDataVector.size());
        while (!feof(fPTR) /* && (m_zSensorDataVector.size() < Buffer_Size) */)
        {
          if (NULL == fscanf(fPTR, "%d %d %d %lf %lf %lf", &GpsMsec, &RTCMs,
              &SensorType, &SensorXaxis, &SensorYaxis, &SensorZaxis))
          {
              SLIM_LOGD("File Read Failed NULL \n");
              break;
          }
          SLIM_LOGD("File Read Success: %d testCount:%d GpsMsec:%d m_qKeyGpsMsec:%d\n",
                    Count, testCount, GpsMsec, m_qKeyGpsMsec);
          ...
          if (0 == Count % 10)
              SLIM_LOGD("GpsMsec: %d %d %d %lf %lf %lf Count:%d", GpsMsec, RTCMs, SensorType,
              SensorXaxis, SensorYaxis, SensorZaxis, Count);
  ...
  SLIM_LOGE(" SensorPlayback File Read exiting");
}
...
void SensorPlayBackProvider::timer_thread(union sigval arg)
{
  ...
  timerElapsed = TRUE;
  SLIM_LOGD("Timer = %d ptr = %p", timerElapsed, &timerElapsed);
...
void SensorPlayBackProvider::SendSensorData(slimSensorDataStructT& SensorData, uint8 sensorType)
{
  ...
  /*SLIM_LOGD("tCurrentTimestamp = %" PRIu64 " ms. sensor=%d, time=%lld, value=<%f,%f,%f> Len:%d\n",
     tCurrentTimestamp,
     sensorType,
     tCurrentTimestamp,
...
//SLIM_LOGD("APSS t_Time = %d", t_Time);
  ...
  SLIM_LOGD("Entered ..   %d", m_zSensorDataVector.back( ).GpsMsec);
  ...
  {
    m_bSearchFile = TRUE;
    SLIM_LOGE("m_bSearchFile Or Re-Read \n CurrentGpsMsec = %" PRIu64 " gpsMsec = %"
        PRIu64 "", CurrentGpsMsec, m_zSensorDataVector.back( ).GpsMsec);
    ...
    for ( ; ; ++it)
    {
      if ((*it).GpsMsec > CurrentGpsMsec || it > last)
      {
            ...
            m_zAccSensorData.samples_len < SLIM_SENSOR_MAX_SAMPLE_SETS)
        {
          ...
          SLIM_LOGD("Final m_zAccSensorData.timeBase = %d Leng = %d\n",
                    m_zAccSensorData.timeBase, m_zAccSensorData.samples_len);
            ...
            m_zGyroSensorData.samples_len < SLIM_SENSOR_MAX_SAMPLE_SETS)
        {
          ...
          SLIM_LOGD("Final m_zGyroSensorData.timeBase = %d Len = %d\n",
                     m_zGyroSensorData.timeBase, m_zGyroSensorData.samples_len);
...
//  SLIM_LOGD("Before Erase m_zSensorDataVector Length = %d", m_zSensorDataVector.size());
    ...
    apss_kerneltime_qtEnd = slim_TimeTickGetMilliseconds();
    SLIM_LOGD("Time Taken For RetrieveSensorData = %d vectorSize= %d",
        (apss_kerneltime_qtEnd - apss_kerneltime_qt), m_zSensorDataVector.size());
