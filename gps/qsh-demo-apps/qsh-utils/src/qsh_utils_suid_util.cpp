    ...
    _cb(cb)
{
  ...
  _event_cb=[this](const uint8_t* msg, int msgLength, uint64_t time_stamp)
                { this->handle_ssc_event(msg, msgLength, time_stamp); };
  ssc_conn_config conn_config;
  _conn = ssc_interface::create(SSC_QMI, conn_config);
  if (nullptr != _conn) {
...
void suid_lookup::request_suid(std::string datatype, bool default_only)
{
    ...
    LOC_LOGd("requesting suid for %s, ts = %fs", datatype.c_str(),
    ...
    if (pStdReq) {
        pStdReq->set_payload(pb_suid_req_encoded);
    } else {
    ...
    if (pStdSuid) {
        pStdSuid->set_suid_high(_suid.high);
        pStdSuid->set_suid_low(_suid.low);
    } else {
...
void suid_lookup::handle_ssc_event(const uint8_t *data, size_t size, uint64_t time_stamp)
{
    ...
    for (int i = 0; i < pb_event_msg.events_size(); i++) {
        ...
        LOC_LOGd("suid_event for %s, num_suids=%d, ts=%fs", datatype.c_str(),
