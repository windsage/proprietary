...
Copyright (c) 2012-2013, 2015-2023 Qualcomm Technologies, Inc.
All Rights Reserved.
...
public:
  ...
  enum eRequestType
  {
    ...
    /** Send SNOOP RTT REGISTER request*/
    LOWI_SNOOP_RTT_REGISTER_REQUEST
  };
...
///////////////////////////
// SNOOP RTT Register Request
///////////////////////////

/**
 * Snoop RTT Register Request
 */
class LOWISnoopRTTRegisterRequest: public LOWIRequest
{
private:
  /**
   ...
   * SNOOP_RTT_REG_REQ_VALIDITY_PERIOD_SEC_MAX
   */
...
public:
  /** Request type
  */
  enum eSnoopRTTRequestType {
    SNOOP_RTT_REGISTER_CLIENT,
    SNOOP_RTT_UNREGISTER_CLIENT,
    UNKNOWN_SNOOP_RTT_REQUEST_TYPE
  };

  eSnoopRTTRequestType snoop_rtt_req_type;

    /**
   * Constructor
   * A client registers this request to receive the snoop RTT
   * scan results as and when they are available.
   * LOWI provides the ranging results to the client for as long
   * as the request is valid or for a max period
   * (SNOOP_RTT_REG_REQ_VALIDITY_PERIOD_SEC_MAX), after which it
   * drops the request assuming that the client is no longer
   * interested in the scan results. The client will be indicated
   * when this happens. If the client is interested
   * in listening for the snoop RTT results, it should keep
   ...
   * ranging results, it has two ways:
   *    -> it can send Unregister Client request. Response will
   *       be recevied almost immediately, because this request
   *       won't be added to queue in lowi-server and will be
   *       processed immediately.
   *    -> it can refresh the request with Register request with
   *       0 timeout value. Response may get delayed as the request
   *       will be added to the queue in lowi-server and processed
   *       when this request's turn comes.
   *
   * @param uint32 Request id generated by the client
   *        This will be echoed back in the corresponding response.
   ...
   *               SNOOP_RTT_REG_REQ_VALIDITY_PERIOD_SEC_MAX
   */
  LOWISnoopRTTRegisterRequest (uint32 requestId,
      uint32 request_timer_sec, eSnoopRTTRequestType req_type);
  ...
  /** Destructor*/
  virtual ~LOWISnoopRTTRegisterRequest ();
...
public:
  ...
  /**
   * Set scan request issued due to emergency call
   */
  inline void setEmergencyScanReq(bool val)
  {
    is_emergency_req = val;
  }

  /**
   * Get whether request is emergency request or not
   */
  inline bool isEmergencyScanReq()
  {
    return is_emergency_req;
  }
...
private:
  ...
  /** bool flag to indicate this scan request is due to emergency */
  bool is_emergency_req;
};
