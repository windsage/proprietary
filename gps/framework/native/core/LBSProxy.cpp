...
bool LBSProxy::checkIfLocationExtendedClientExists()
{
    ...
    if((access(COM_QUALCOMM_LOCATION_APK_FILE_PATH, F_OK) != -1) ||
        ...
        LOC_LOGd("File Exists");
        return true;
    } else {
        LOC_LOGd("File does not Exist");
        return false;
    }
}
...
void LBSProxy::getIzatProvider() {
    if (NULL == mIzatProvider) {
        void * libHandle = nullptr;
        getIzatProviderInst* getter = (getIzatProviderInst*)
            dlGetSymFromLib(libHandle, IzatProviderLib, "getIzatProvider");
        if (nullptr != getter) {
            mIzatProvider = (IzatProviderBase*)(*getter)();
        }
        if (NULL == mIzatProvider) {
            LOC_LOGe("IzatProvider load failed");
        } else {
            LOC_LOGd("IzatProvider load success");
        }
    }
}

void LBSProxy::populateAltitudeAndBroadCast(Location location, trackingCallback broadcaster) const {
    if (mIzatProvider) {
        mIzatProvider->getLocationWithZ(location, broadcaster);
    } else {
