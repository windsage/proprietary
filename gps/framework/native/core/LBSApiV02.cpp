/*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*
  Copyright (c) 2013-2014, 2016-2018, 2021-2023 Qualcomm Technologies, Inc.
  All Rights Reserved.
...
void LBSApiV02::injectFdclData(const FdclData& fdclData)
{
    ...
    for (uint32_t i = 0; i < fdclData.bsLocationDataList.size() &&
        ...
        LOC_LOGd("cell with loc info: %u %u %u %u %u, loc info: %f %f %f",
                 qmiData.cellId.airInterfaceType, qmiData.cellId.id1,
                 qmiData.cellId.id2, qmiData.cellId.id3,
                 qmiData.cellId.id4, qmiData.latitude,
                 qmiData.longitude, qmiData.horCoverageRadius);
    }
    ...
    for (uint32_t i = 0, index = fdclData.bsLocationDataList.size() + i;
        ...
        LOC_LOGd("cell without loc info: %u %u %u %u %u, type info: %d",
                 qmiData.cellId.airInterfaceType, qmiData.cellId.id1,
                 qmiData.cellId.id2, qmiData.cellId.id3, qmiData.cellId.id4,
                 injectFdclDataReq.cellRecordTypeList[index]);
    }
