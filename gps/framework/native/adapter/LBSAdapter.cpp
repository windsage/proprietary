...
bool LBSAdapter::getZppFixSync(LocGpsLocation &gpsLocation, LocPosTechMask &tech_mask) {
    return mLocApi->getBestAvailableZppFixSync(gpsLocation, tech_mask);
}
    ...
    struct sendMapDataMsg : public LocMsg {
        ...
        inline virtual void proc() const {
            ...
            if (mLBSAdapter->isFeatureSupported(LOC_SUPPORTED_FEATURE_MEASUREMENTS_CORRECTION)) {
                LOC_LOGd("mDataSize = %zu", mMaString.length());
...
void LBSAdapter::injectPositionCommand(const Location& location, bool onDemandCpi)
{
    struct InjectPositionMsg : public LocMsg {
        LocApiBase* mLocApi;
        Location mLocation;
        bool mOnDemandCpi;
        inline InjectPositionMsg(LocApiBase* locApi, const Location& location, bool onDemandCpi) :
            LocMsg(), mLocApi(locApi), mLocation(location),
            mOnDemandCpi(onDemandCpi){}
        inline virtual void proc() const {
            if (nullptr == mLocApi) {
                return;
            }
            LOC_LOGv("injectPositionCommand sent");
            mLocApi->injectPosition(mLocation, mOnDemandCpi);
        }
    };
    sendMsg(new InjectPositionMsg(mLocApi, location, onDemandCpi));
}
