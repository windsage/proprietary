...
void IzatPassiveLocationListener :: onLocationChanged (const IzatLocation *location, int locCnt,
        const IzatLocationStatus status)
{
    ...
    IzatLocation* bestLoc = NULL;
    // report best location. when RTT and RSSI both available, report RTT fix
    if (locCnt == 2) {
        if (location[0].mHasNetworkPositionSource &&
                (location[0].mNetworkPositionSource == IZAT_NETWORK_POSITION_FROM_WIFI_RTT_FTM ||
                location[0].mNetworkPositionSource == IZAT_NETWORK_POSITION_FROM_WIFI_RTT_SERVER)) {
            bestLoc = const_cast<izat_manager::IzatLocation*>(location);
        } else if (location[1].mHasNetworkPositionSource &&
                (location[1].mNetworkPositionSource == IZAT_NETWORK_POSITION_FROM_WIFI_RTT_FTM ||
                location[1].mNetworkPositionSource == IZAT_NETWORK_POSITION_FROM_WIFI_RTT_SERVER)) {
            bestLoc = const_cast<izat_manager::IzatLocation*>(location) + 1;
        }
    } else if (locCnt == 1) {
        bestLoc = const_cast<izat_manager::IzatLocation*>(location);
    }
    ...
    do {
        ...
        if (!bestLoc || !bestLoc->isValid()) {
            result = 3;
            break;
        }
        ulpLocationFromIzatLocation(bestLoc, &ulpLoc, &locExtended);

        BREAK_IF_ZERO(4, mIPCMessagingProxy);
        // send out the bestLoc update
...
)
{
    ...
    if ((NULL != izatLoc) && (NULL != ulpLoc)) {
        ...
        if (izatLoc->mHasPositionTechMask) {
            ulpLoc->tech_mask = izatLoc->mPositionTechMask;
        }
    }
    ...
    if ((NULL != izatLoc) && (NULL != locExtended)) {
        ...
        if (izatLoc->mHasSystemTick) {
            locExtended->flags |= GPS_LOCATION_EXTENDED_HAS_SYSTEM_TICK;
            locExtended->systemTick = izatLoc->mSystemTick;
        }
    }
}
