...
GENERAL DESCRIPTION
  ...
  Copyright (c) 2015, 2018, 2023 Qualcomm Technologies, Inc.
  All Rights Reserved.
...
void LocationProvider::broadcastLocation(const LocationReport *locReport, int locCnt) {
    int result = 0;
    ENTRY_LOG();
    ...
    do {
        ...
        for (; it != mLocRespObjs.end(); it++)
        {
            ...
            respObj->reportLocation(locReport, locCnt, this);
        }
    } while(0);
