...
IPCHandler :: RegLocMsg :: RegLocMsg
 (IPCHandler * parent, const string & name, IOsObserver * pOBIface)
...
void IPCHandler:: HandleIPCMsg :: proc () const
{
    ...
    ENTRY_LOG();
#ifdef USE_GLIB
    BackhaulContext ctxt;
    const char * clientNameStr;
    const char * prefApnStr;
#endif
    ...
    do {
        ...
        if (0 == mIncard->getString ("REQ", &req)) {
            ...
            if (0 == request.compare ("SUBSCRIBE")) {
...
#ifdef USE_GLIB
            } else if (0 == request.compare ("REQUEST-DATA-CONNECTION") ||
                       0 == request.compare ("RELEASE-DATA-CONNECTION")) {
                mIncard->getString("BACKHAUL-CLIENT", &clientNameStr);
                mIncard->getUInt16("BACKHAUL-SUB", ctxt.prefSub);
                mIncard->getString("BACKHAUL-APN", &prefApnStr);
                mIncard->getUInt16("BACKHAUL-IPTYPE", ctxt.prefIpType);
                ctxt.clientName = clientNameStr;
                ctxt.prefApn = prefApnStr;
                ...
                if (0 == request.compare("REQUEST-DATA-CONNECTION")) {
                    this->mParent->mOBIface->connectBackhaul(ctxt);
                } else {
                    this->mParent->mOBIface->disconnectBackhaul(ctxt);
                }
#endif
            } else {
                result = 5;
...
void IPCHandler :: HandleIPCMsg :: getSetOfDataItems
(
...
) const
{
    ...
    do {
        ...
        for (int i = 0; i <numDataItems; i++) {
            out.insert (static_cast <DataItemId> (dataItems [i]));
        }
    } while (0);
...
void IPCHandler :: reg(const string & name, IOsObserver * iOBface)
{
     int result = -1;
     ENTRY_LOG ();
     do {
