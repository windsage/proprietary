...
#ifdef QTI_BT_UART_SYSFS_SUPPORTED
    ALOGI("%s: opening %s", __func__, BT_HS_UART_OPERATION);
    if ((sysfs_fd_ = open(BT_HS_UART_OPERATION, O_RDWR)) == -1) {
      ALOGE("%s: unable to open %s: %s(%d)", __func__, BT_HS_UART_OPERATION,
            strerror(errno), errno);
      return false;
    }
#endif //QTI_BT_UART_SYSFS_SUPPORTED
...
#ifdef QTI_BT_UART_SYSFS_SUPPORTED
  ALOGV("%s: sysfs_fd_ = %d", __func__, sysfs_fd_);
  if (!sysfs_fd_ || sysfs_fd_ == -1) {
    return;
  }

  if ((ret = close(sysfs_fd_)) < 0 ) {
    ALOGE("%s: Close returned Error: %d\n", __func__, ret);
  }
...
int HciUartTransport::ClockOperation(int cmd)
{
  ...
  struct timeval tv;
#ifdef QTI_BT_UART_SYSFS_SUPPORTED
  char buf[BT_HS_UART_OP_MAX_BUF];
  snprintf(buf, BT_HS_UART_OP_MAX_BUF, "%x", cmd);
#endif //QTI_BT_UART_SYSFS_SUPPORTED
  switch (cmd) {
...
#ifdef QTI_BT_UART_SYSFS_SUPPORTED
        ret = write(sysfs_fd_, &buf, strlen(buf));
#else
...
#ifdef QTI_BT_UART_SYSFS_SUPPORTED
      char buf[BT_HS_UART_OP_MAX_BUF];
      snprintf(buf, BT_HS_UART_OP_MAX_BUF, "%x", MSM_GENI_SERIAL_TIOCFAULT);
      err = write(sysfs_fd_, buf,  strlen(buf));      break;
#else
      ioctl(ctrl_fd_, MSM_GENI_SERIAL_TIOCFAULT, NULL);
#endif //QTI_BT_UART_SYSFS_SUPPORTED
    default:
      break;
  }
