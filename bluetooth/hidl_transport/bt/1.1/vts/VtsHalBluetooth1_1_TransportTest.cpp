...
class BluetoothTransportTest : public ::testing::VtsHalHidlTargetTestBase {
 public:
  virtual void SetUp() override {
    ...
    if ((bluetooth1_1 = V1_1::IBluetoothHci::getService()) != nullptr) {
      // Trying to get the BT 1.1 service for VTS
      ALOGI("%s VTS using Bluetooth-1.1 service, getService() bluetooth is %s",
           __func__, bluetooth1_1->isRemote() ? "remote" : "local");
      bluetooth = bluetooth1_1;
    } else if ((bluetooth1_0 = V1_0::IBluetoothHci::getService()) != nullptr)      {
      // Trying to get the BT 1.0 service for VTS
      ALOGI("%s VTS using Bluetooth-1.0 service, getService() bluetooth is %s",
            __func__, bluetooth1_0->isRemote() ? "remote" : "local");
      bluetooth = bluetooth1_0;
    } else {
      // Both BT 1.0 and 1,1 services are not avilable for VTS
      ALOGI("%s Failed to use both IBluetoothHci 1.0 and 1.1 for VTS", __func__);
      return;
    }
    ...
    if (bluetooth1_1 != nullptr) {
      ASSERT_TRUE(bluetooth1_1->initialize_1_1(bluetooth_cb).isOk());
    } else {
      ASSERT_TRUE(bluetooth1_0->initialize(bluetooth_cb).isOk());
    }
  ...
  class BluetoothHciCallbacks
      : public ::testing::VtsHalHidlTargetCallbackBase<BluetoothTransportTest>,
        public V1_1::IBluetoothHciCallbacks {
    BluetoothTransportTest& parent_;
  ...
  android::sp<V1_0::IBluetoothHci> bluetooth, bluetooth1_0;
  android::sp<V1_1::IBluetoothHci> bluetooth1_1;
...
void BluetoothTransportTest::sendAndCheckISO(int num_packets, size_t size,
  ...
  if (bluetooth1_1 != nullptr)
  {
    ...
    bluetooth1_1->sendIsoData(iso_vector);
    }
  } else {
