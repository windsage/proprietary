...
Return<void> A2dpOffloadAudioProvider::startSession_2_1(
  ...
  AudioConfiguration audio_config_ = audioConfig;
  vendor::qti::bluetooth_audio::ModifyRxConfigForLc3(session_type_,
                                                  audio_config_.codecConfig);
  if (!vendor::qti::bluetooth_audio::IsOffloadCodecConfigurationValid(
                 session_type_, audio_config_.codecConfig)) {
    _hidl_cb(BluetoothAudioStatus::UNSUPPORTED_CODEC_CONFIGURATION,
             DataMQ::Descriptor());
    return Void();
  }

  return BluetoothAudioProvider::startSession_2_1(hostIf, audio_config_, _hidl_cb);
}
