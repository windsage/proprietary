...
bool BluetoothAudioSession::UpdateSinkLatency(uint16_t sink_latency) {
  std::lock_guard<std::recursive_mutex> guard(mutex_);
  if (!IsSessionReady()) {
    LOG(DEBUG) << __func__ << " - SessionType=" << toString(session_type_)
               << " has NO session";
  ...
  if (client_2_0_) {
    auto hal_retval = stack_iface_2_0_->updateAptxMode(sink_latency);
    if (!hal_retval.isOk()) {
      LOG(WARNING) << __func__ << " - IBluetoothAudioPort_2_0 SessionType="
                   << toString(session_type_) << " failed";
      return false;
    ...
    auto hal_retval = stack_iface_->updateAptxMode(sink_latency);
    if (!hal_retval.isOk()) {
      LOG(WARNING) << __func__ << " - IBluetoothAudioPort SessionType="
                   << toString(session_type_) << " failed";
      return false;
