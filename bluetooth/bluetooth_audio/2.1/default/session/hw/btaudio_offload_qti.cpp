...
void aac_decoder_parser(AudioConfiguration *codec_cfg, uint32_t *sample_freq)
{
...
static void* a2dp_decoder_parser(AudioConfiguration*codec_cfg,
                               ...
                               uint32_t *sample_freq)
{
    ...
    if (codecConfig.codecType == CodecType::SBC) {
        ...
        *codec_type = AUDIO_FORMAT_AAC;
        aac_decoder_parser(codec_cfg, sample_freq);
        return ((void *)(&aac_dec));
    }
...
void * audio_get_decoder_config_qti(audio_format_t *codec_type)
{
    ALOGI("%s: state = %s", __func__, dump_a2dp_hal_state(audio_stream.state));
    if (audio_stream.session_ready) {
      if (audio_stream.hal_2_1)
        return (a2dp_decoder_parser(&audio_stream.codec_cfg, codec_type, NULL));
    }
    ALOGW("%s: return", __func__);
    return NULL;
}
...
bool audio_update_sink_latency_qti(uint16_t sink_latency)
{
    struct timespec* data_position = { };
    uint64_t total_bytes_readed;
    uint64_t remoteDelay =0;
    pthread_mutex_lock(&audio_stream.lock);
    if (audio_stream.session_ready) {
        BluetoothAudioSessionControl::UpdateSinkLatency(audio_stream.sessionType,
                                                        sink_latency);
        ALOGI("%s: Sink Latency = %d", __func__, sink_latency);
    }
    pthread_mutex_unlock(&audio_stream.lock);
    return true;
}
