...
static bool IsOffloadLC3ConfigurationValid(
  ...
  LOG(WARNING) << __func__
               << ": rxConfigSet=" << static_cast<uint32_t>(LC3_data.rxConfigSet);
  if (!IsSingleBit(static_cast<uint32_t>(LC3_data.txConfig.sampleRate), 0x03ff) ||
    ...
    return false;
  } else if (((LC3_data.rxConfigSet == 0x1) || (LC3_data.rxConfigSet == 0x3)) &&
    ...
    if (((LC3_data.rxConfigSet == 0x1) || (LC3_data.rxConfigSet == 0x3)) &&
      ...
      return true;
    } else if (LC3_data.rxConfigSet == 0x2) {
      return true;
    } else {
      return false;
    }
...
bool ModifyRxConfigForLc3(const SessionType& session_type,
                          CodecConfiguration& codec_config) {
  CodecConfiguration::CodecSpecific& codec_specific = codec_config.config;
  if (session_type != SessionType::A2DP_HARDWARE_OFFLOAD_DATAPATH) {
    LOG(ERROR) << __func__
  ...
  LOG(ERROR) << __func__
             << ": Stack passed Current rxConfigSet: "
             << static_cast<uint32_t>(codec_specific.lc3Config.rxConfigSet);

  if (codec_config.codecType == CodecType::LC3 &&
      codec_specific.lc3Config.rxConfigSet == 0) {
    if ((IsSingleBit(static_cast<uint32_t>(
         codec_specific.lc3Config.rxConfig.sampleRate), 0x03ff) &&
         IsSingleBit(static_cast<uint32_t>(
         codec_specific.lc3Config.rxConfig.channelMode), 0x1f) &&
         IsSingleBit(static_cast<uint32_t>(
         codec_specific.lc3Config.rxConfig.bitsPerSample), 0x07))) {
       codec_specific.lc3Config.rxConfigSet = 3;
    } else {
       codec_specific.lc3Config.rxConfigSet = 2;
    }
    LOG(ERROR) << __func__
               << ": Modified rxConfigSet: "
               << static_cast<uint32_t>(codec_specific.lc3Config.rxConfigSet);
  }
  return true;
}
