...
#ifndef RIL_FOR_MDM_LE
#ifndef FEATURE_TARGET_ARCH_32
#define LIB_DEFAULT_PATH "/vendor/lib64"
#else
#define LIB_DEFAULT_PATH "/vendor/lib"
#endif
#else
#define LIB_DEFAULT_PATH "/usr/lib"
#endif
std::shared_ptr<PluginRegistry> loadPluginRegistry() {
    std::shared_ptr<PluginRegistry> registry = std::make_shared<PluginRegistry>();
    auto msg = std::make_shared<DatabaseExecStatementMessage>(
            "select m.module, m.mandatory, m.internal, m.prio "
            "FROM modules m "
            "INNER JOIN config_modules cm ON m.rowid = cm.module_id "
            "INNER JOIN configs c ON c.rowid = cm.config_id "
            "WHERE c.rowid in ( "
            "    SELECT value FROM qcril_properties_table "
            "    WHERE property = 'qcril-current-config' "
            " ) "
            "ORDER BY m.prio ASC; ",
            [registry] (const QtiRilDbRow &row) -> bool {
                assert(row.getColumnCount() == 4);
                std::string moduleName = row[0].get<std::string>();
                auto mandatoryParam = row[1];
                bool mandatory = row[1].get<int64_t>() != 0;
                bool internal = row[2].get<int64_t>() != 0;
                size_t prio = row[3].get<int64_t>();

                if (!internal) {
                    registry->add(moduleName, std::make_shared<LoadableModule>(moduleName, LIB_DEFAULT_PATH), mandatory, prio);
                }
                return true;
            }
    );
    if (msg) {
        msg->setCallback(
                [registry] (std::shared_ptr<Message> msg,
                    Message::Callback::Status status,
                    std::shared_ptr<void>) {
                    if (status != Message::Callback::Status::SUCCESS) {
                        QCRIL_LOG_ERROR("Unable to obtain plugin list");
                    }
                }
        );
        Dispatcher::getInstance().dispatchSync(msg);
...
extern "C" void RIL_loadPlugins() {
    QCRIL_LOG_INFO("Loading PluginRegistry");
    std::shared_ptr<PluginRegistry> reg = loadPluginRegistry();

    if (!reg) {
        QCRIL_LOG_WARN("Null PluginRegistry. Falling back to default");

        reg = std::make_shared<PluginRegistry>();
        if (!reg) {
            QCRIL_LOG_ERROR("Unable to allocate memory for PluginRegistry!!! No plugins will be loaded");
            return;
        }
