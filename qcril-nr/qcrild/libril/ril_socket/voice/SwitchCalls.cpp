                                             ...
                                             Marshal& p)
{
  ...
  do {
    ...
    RIL_SwitchWaitingOrHoldingAndActive switchReqParam{};
    Marshal::Result ret = p.read(switchReqParam);
    if (ret != Marshal::Result::SUCCESS) {
      QCRIL_LOG_ERROR("Failed to read parameter");
      resultCode = RIL_E_NO_MEMORY;
      break;
    }
    if (switchReqParam.hasCallType) {
      msg->setCallType(convertCallType(switchReqParam.callType));
    }
