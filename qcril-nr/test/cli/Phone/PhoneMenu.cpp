...
void PhoneMenu::init() {
  ...
  std::vector<std::shared_ptr<ConsoleAppCommand>> commandsListPhoneSubMenu = {
    ...
    CREATE_COMMAND(PhoneMenu::setNetworkSelectionManual_2,
                   "Manually Select Network_2"),
    ...
    CREATE_COMMAND(PhoneMenu::getImei, "getImei"),
  };
...
void PhoneMenu::setNetworkSelectionAutomatic(std::vector<std::string> userInput) {
  ...
  Status s = rilSession.setNetworkSelectionAutomatic(
      static_cast<RIL_AccessMode>(accessMode),
      [](RIL_Errno e)
      {
        std::cout << std::endl
...
void PhoneMenu::setNetworkSelectionManual_2(std::vector<std::string> userInput)
{
  RIL_SetNetworkSelectionManualParam networkSelectionMode = {};
  std::cin.clear();

  std::cout << "Enter Network Selection Mode Info" << std::endl;
  int ran;
  ...
  std::cout << "Enter Ran: " << std::endl;
  std::cin >> ran;
  networkSelectionMode.ran = static_cast<RIL_RadioAccessNetworks>(ran);
  ...
  std::cout << "Enter AccessMode: " << std::endl;
  std::cin >> accessMode;
  networkSelectionMode.accessMode = static_cast<RIL_AccessMode>(accessMode);

  if (networkSelectionMode.accessMode == RIL_ACCESS_MODE_PLMN) {
    int isCagInfoValid = 0;
    std::cout << "isCagInfo applicable: (0 or 1)" << std::endl;
    std::cin >> isCagInfoValid;
    if (isCagInfoValid) {
      uint32_t cagId;
      std::cout << "Enter cagId: " << std::endl;
      std::cin >> cagId;
      networkSelectionMode.cagId = cagId;

      networkSelectionMode.cagIdValid = 1;
    }
  } else {
    std::cout << "Enter SnpnNid Length: (max length is 6)" << std::endl;
    std::cin >> networkSelectionMode.snpnNidLength;
    if (networkSelectionMode.snpnNidLength > MAX_SNPN_NID_LEN) {
      return;
    }
    std::cout << "Enter SnpnNid value: " << std::endl;
    for (int i = 0; i < networkSelectionMode.snpnNidLength; i++) {
      int snpnNid;
      std::cout << "Enter snpnNid[" << i << "]: ";
      std::cin >> snpnNid;
      networkSelectionMode.snpnNid[i] = snpnNid;
    }
  }

  std::string operatorNumeric;
  std::cout << "Enter Operator Numeric value: (mcc + mnc)" << std::endl;
  std::cin >> operatorNumeric;
  networkSelectionMode.operatorNumeric = const_cast<char*>(operatorNumeric.c_str());
  ...
  if (s != Status::SUCCESS) {
    std::cout << std::endl << "Failed to issue request to select network." << std::endl;
  }
}
...
void PhoneMenu::getImei(std::vector<std::string> userInput)
{
  std::cin.clear();
  Status s = rilSession.getImei([](RIL_Errno e, RIL_ImeiInfo imeiInfo) {
    std::cout << "Got response for Get Imei: " << imeiInfo << "\n"
              << "RIL Errno: " << e << std::endl;
  });
  if (s != Status::SUCCESS) {
    std::cout << "Failed to issue request to Get Imei" << std::endl;
  }
}

std::ostream& operator<<(std::ostream& os, RIL_ImeiType type)
{
  os << "RIL_ImeiType::";
  switch (type) {
    case IMEI_TYPE_PRIMARY:
      os << "IMEI_TYPE_PRIMARY";
      break;
    case IMEI_TYPE_SECONDARY:
      os << "IMEI_TYPE_SECONDARY";
      break;
    case IMEI_TYPE_INVALID:
    default:
      os << "IMEI_TYPE_INVALID";
      break;
  }
...
std::ostream& operator<<(std::ostream& os, RIL_ImeiInfo imeiInfo)
{
  os << "RIL_ImeiInfo: {"
     << "\nimeiType: [" << imeiInfo.imeiType
     << "]\nimei: [" << imeiInfo.imei
     << "]\nimeiLen: [" << std::to_string(imeiInfo.imeiLen) << "]\n}" << std::endl;
