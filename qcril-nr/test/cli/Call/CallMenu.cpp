...
void CallMenu::init() {
  std::vector<std::shared_ptr<ConsoleAppCommand>> commandsListCallSubMenu = {
    ...
    CREATE_COMMAND(CallMenu::udub, "Udub"),
...
std::ostream& operator <<(std::ostream& os, const RIL_SuppSvcNotification &arg)
{
  ...
  os << "call id: " << arg.connId << std::endl;
  return os;
}
...
void CallMenu::setCallWaiting(std::vector<std::string> userInput) {
  ...
  std::cin >> enabled;
  for (auto& arg : userInput) std::cout << __func__ << ":" << arg << std::endl;
  Status status = rilSession.setCallWaiting(enabled, serviceClass, [](RIL_Errno err) -> void {
...
void CallMenu::udub(std::vector<std::string> userInput) {
  for (auto& arg : userInput) std::cout << __func__ << ":" << arg << std::endl;

  Status s = rilSession.udub([](RIL_Errno err) -> void {
    std::cout << "Got response for udub request." << err << std::endl;
  });
  std::cout << ((s == Status::SUCCESS) ? "Udub request sent succesfully" : "Failed to send Udub request")
            << std::endl;
}
...
void printCall(const RIL_Call* call) {
  std::cout << "Call Index: " << call->index << std::endl
    ...
    if (call->redirNum) {
      std::cout << "Redirected Number: " << call->redirNum << std::endl;
    }
}
