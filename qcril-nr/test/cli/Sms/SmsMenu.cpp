...
void SmsMenu::sendSms(std::vector<std::string> userInput) {
  std::string payload;
  std::cout << "Enter PDU bytes encoded as ascii hex (100 - Exit): ";

  std::cin >> payload;

  if (payload == "100") {
    std::cout << "Exit sendSms" << std::endl;
    return;
  }
  ...
  std::cout << "Enter expectMore to keep SMS relay protocol link open" << std::endl;
  std::cout << "Enter 0 (preferred for normal SMS), !0 (to verify protocol link) : ";
  int expectMore = 0;
  std::cin >> expectMore;
