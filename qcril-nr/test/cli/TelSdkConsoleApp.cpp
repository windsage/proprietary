...
//            "6", "Data", {}, std::bind(&TelSdkConsoleApp::dataMenu, this, std::placeholders::_1)));
    ...
    std::vector<std::shared_ptr<ConsoleAppCommand>> mainMenuCommands = {
       ...
       CREATE_COMMAND(TelSdkConsoleApp::rilCallMenu, "RILCall"),
       ...
       CREATE_COMMAND(TelSdkConsoleApp::moduleMenu, "Modules"),

       #ifdef QCRIL_BUILD_AFL_TESTCASE_GEN
       CREATE_COMMAND(TelSdkConsoleApp::saveTestcasesMenu, "Save Testcases for AFL"),
       #endif // QCRIL_BUILD_AFL_TESTCASE_GEN
    };
...
void TelSdkConsoleApp::moduleMenu(std::vector<std::string> userInput) {
  ModuleMenu moduleMenu("Modules Menu", "modules> ", rilSession);
  moduleMenu.init();
  moduleMenu.mainLoop();
  TelSdkConsoleApp::displayMenu();
}
...
void TelSdkConsoleApp::rilCallMenu(std::vector<std::string> userInput) {
  RILCallMenu rilCallMenu("RIL CALL Menu", "RILCALL> ", rilSession);
  rilCallMenu.init();
  rilCallMenu.mainLoop();
  TelSdkConsoleApp::displayMenu();
}
...
#ifdef QCRIL_BUILD_AFL_TESTCASE_GEN
void TelSdkConsoleApp::saveTestcasesMenu(std::vector<std::string> userInput) {
    SaveTestcasesMenu saveTestcasesMenu("Save Testcases", "Save Testcases>", rilSession);
    saveTestcasesMenu.init();
    saveTestcasesMenu.mainLoop();
    TelSdkConsoleApp::displayMenu();
}
#endif // QCRIL_BUILD_AFL_TESTCASE_GEN
