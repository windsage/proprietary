  ...
  if (rat_valid)
  {
     switch (rat_val)
     {
       ...
       case IMSA_C_IWLAN_V01:
         tmp_accTechStatus[0]->has_networkMode = TRUE;
         tmp_accTechStatus[0]->networkMode = ims_RadioTechType_RADIO_TECH_C_IWLAN;
         break;
     }
  }
}
...
void ril_utf_ims_if::set_ims_request_set_supp_svc_response_resp_pwd_req(bool req) {
  ims_SuppSvcResponse *ptr = ( ims_SuppSvcResponse* ) payload;
  ptr->has_isPasswordRequired = true;
  ptr->isPasswordRequired = req;
}
...
/*========================================================
  ims_MsgId_UNSOL_SRTP_ENCRYPTION_STATUS
========================================================*/
void ril_utf_ims_if::update_default_ims_unsol_srtp_encryption_status(void)
{
  payload = malloc(sizeof(ims_SrtpEncryptionStatus));
  ims_SrtpEncryptionStatus* info = (ims_SrtpEncryptionStatus*)payload;
  payload_len = sizeof(ims_SrtpEncryptionStatus);
  memset(payload, 0x00, payload_len);
  payload_type = ril_utf_ims_indication;
  msg_id = ims_MsgId_UNSOL_SRTP_ENCRYPTION_STATUS;
}

void ril_utf_ims_if::set_ims_unsol_srtp_encryption_call_id(uint32_t call_id){
    ims_SrtpEncryptionStatus* info = (ims_SrtpEncryptionStatus*)payload;
    info->encryptionInfo.call_id = call_id;
}

void ril_utf_ims_if::set_ims_unsol_srtp_encryption_categories(uint32_t categories){
    ims_SrtpEncryptionStatus* info = (ims_SrtpEncryptionStatus*)payload;
    info->encryptionInfo.categories = categories;
}
...
utf_ims_sms_status_report_t* ril_utf_ims_if::update_default_ims_sms_status_report()
{
    payload_type = ril_utf_ims_indication;
    msg_id = ims_MsgId_UNSOL_SMS_STATUS_REPORT;

    payload = malloc(sizeof(utf_ims_sms_status_report_t));
    assert(payload != nullptr);
    payload_len = sizeof(utf_ims_sms_status_report_t);
    memset(payload, 0x00, payload_len);

    return static_cast<utf_ims_sms_status_report_t*>(payload);
}
