...
#  Confidential and Proprietary - Qualcomm Technologies, Inc.
  ...
  @file    ril_utf_androidTel_payload.cpp
...
RIL_LastCallFailCauseInfo* copy_ril_request_last_call_fail_cause_resp(RIL_LastCallFailCauseInfo* causeInfo)
{
  ...
  {
    ret->vendor_cause = (char *)malloc(strlen((const char *)causeInfo->vendor_cause)+1);
...
enum ril_utf_expectation_t verify_ril_get_phone_capability(void *exp_node, int exp_len, void *data,
                                                           int data_len)
{
  enum ril_utf_expectation_t result = met;
  RIL_PhoneCapability* expect = static_cast<RIL_PhoneCapability*>(exp_node);
  RIL_PhoneCapability* recv = static_cast<RIL_PhoneCapability*>(data);

  if (exp_len != data_len)
  {
    RIL_UTF_ESSENTIAL("\n QMI_RIL_UTF: expected payload length = %d, "
        "received payload lenth = %d\n", exp_len, data_len);
    result = not_met;
    return result;
  }
  if (expect->maxActiveData != recv->maxActiveData)
  {
    RIL_UTF_ESSENTIAL("\n QMI_RIL_UTF: expected maxActiveData = %d, "
        "received maxActiveData = %d\n", expect->maxActiveData, recv->maxActiveData);
    result = not_met;
    return result;
  }
  if (expect->maxActiveInternetData != recv->maxActiveInternetData)
  {
    RIL_UTF_ESSENTIAL("\n QMI_RIL_UTF: expected maxActiveInternetData = %d, "
        "received maxActiveInternetData = %d\n", expect->maxActiveInternetData,
        recv->maxActiveInternetData);
    result = not_met;
    return result;
  }
  if (expect->isInternetLingeringSupported != recv->isInternetLingeringSupported)
  {
    RIL_UTF_ESSENTIAL("\n QMI_RIL_UTF: expected isInternetLingeringSupported = %d, "
        "received isInternetLingeringSupported = %d\n", expect->isInternetLingeringSupported,
        recv->isInternetLingeringSupported);
    result = not_met;
    return result;
  }

  for (size_t i = 0; i < MAX_MODEM_COUNT; i++ )
  {
    if (expect->logicalModemList[i].modemId != recv->logicalModemList[i].modemId)
    {
      RIL_UTF_ESSENTIAL("\n QMI_RIL_UTF: expected modemId = %d, "
          "received modemId = %d\n at %d\n", expect->logicalModemList[i].modemId,
          recv->logicalModemList[i].modemId, i);
      result = not_met;
      return result;
    }
  }
...
enum ril_utf_expectation_t verify_ril_cdma_get_broadcast_sms_config(void* exp_node,
                                                                    int exp_len,
                                                                    void* data,
                                                                    int data_len)
{
  RIL_CDMA_BroadcastSmsConfigInfo** expect = (RIL_CDMA_BroadcastSmsConfigInfo**)exp_node;
  RIL_CDMA_BroadcastSmsConfigInfo** recv = (RIL_CDMA_BroadcastSmsConfigInfo**)data;
  ...
  // For each config item verify contents
  size_t num_configs = (exp_len / sizeof(RIL_CDMA_BroadcastSmsConfigInfo*));
  for (size_t i = 0; i < num_configs; ++i) {
    if (expect[i]->language != recv[i]->language) {
      RIL_UTF_ESSENTIAL("\n QMI_RIL_UTF: expected language = %d, received language = %d\n",
                        expect[i]->language,
                        recv[i]->language);
      return not_met;
    } else if (expect[i]->service_category != recv[i]->service_category) {
      RIL_UTF_ESSENTIAL(
          "\n QMI_RIL_UTF: expected service_category = %d, received service_category = %d\n",
          expect[i]->service_category,
          recv[i]->service_category);
      return not_met;
    } else if (expect[i]->selected != recv[i]->selected) {
      RIL_UTF_ESSENTIAL("\n QMI_RIL_UTF: expected selected = %d, received selected = %d\n",
                        expect[i]->selected,
                        recv[i]->selected);
      return not_met;
    }
...
enum ril_utf_expectation_t verify_ril_operator_name(void *exp_node, void *data)
{
  ...
  free_ril_request_operator_resp(expect);
  free_ril_request_operator_resp(recv);
...
bool is_same(const RIL_CagInfo& expect, const RIL_CagInfo& recv)
{
  if ((strcmp(expect.cagName, recv.cagName) != 0)) {
    RIL_UTF_ESSENTIAL(
        "\n QMI_RIL_UTF: mismatch cagName: exp: %s, recv: %s", expect.cagName, recv.cagName);
    return false;
  }
  if (expect.cagId != recv.cagId) {
    RIL_UTF_ESSENTIAL("\n QMI_RIL_UTF: mismatch cagId: exp: %d, recv: %d", expect.cagId, recv.cagId);
    return false;
  }
  if (expect.cagOnlyAccess != recv.cagOnlyAccess) {
    RIL_UTF_ESSENTIAL("\n QMI_RIL_UTF: mismatch cagOnlyAccess: exp: %d, recv: %d",
                      expect.cagOnlyAccess,
                      recv.cagOnlyAccess);
    return false;
  }
  if (expect.cagInAllowedList != recv.cagInAllowedList) {
    RIL_UTF_ESSENTIAL("\n QMI_RIL_UTF: mismatch cagInAllowedList: exp: %d, recv: %d",
                      expect.cagInAllowedList,
                      recv.cagInAllowedList);
    return false;
  }
  return true;
}
...
{
  if ((strcmp(expect.mcc, recv.mcc) != 0)) {
    RIL_UTF_ESSENTIAL("\n QMI_RIL_UTF: mismatch mcc: exp: %s, recv: %s", expect.mcc, recv.mcc);
    return false;
  }
  if ((strcmp(expect.mnc, recv.mnc) != 0)) {
    RIL_UTF_ESSENTIAL("\n QMI_RIL_UTF: mismatch mnc: exp: %s, recv: %s", expect.mnc, recv.mnc);
    return false;
  }
  if (expect.cag_info_valid) {
    if (expect.cag_info_valid != recv.cag_info_valid) {
      RIL_UTF_ESSENTIAL("\n QMI_RIL_UTF: mismatch cag_info_valid: exp: %d, recv: %d",
                        expect.cag_info_valid,
                        recv.cag_info_valid);
      return false;
    }
    return is_same(expect.cag_info, recv.cag_info);
  }
  return true;
}

bool is_same(const RIL_CellInfoNr& expect, const RIL_CellInfoNr& recv)
{
  return is_same(expect.cellIdentityNr, recv.cellIdentityNr);
}

bool is_same(const RIL_CellInfo_v12& expect, const RIL_CellInfo_v12& recv)
{
  if (expect.registered != recv.registered) {
    RIL_UTF_ESSENTIAL("\n QMI_RIL_UTF: mismatch registered: exp: %d, recv: %d",
                      expect.registered,
                      recv.registered);
    return false;
  }
  if (expect.connStatus != recv.connStatus) {
    RIL_UTF_ESSENTIAL("\n QMI_RIL_UTF: mismatch connStatus: exp: %d, recv: %d",
                      expect.connStatus,
                      recv.connStatus);
    return false;
  }
  if (expect.cellInfoType != recv.cellInfoType) {
    RIL_UTF_ESSENTIAL("\n QMI_RIL_UTF: mismatch cellInfoType: exp: %d, recv: %d",
                      expect.cellInfoType,
                      recv.cellInfoType);
    return false;
  }
  // TODO compare other TYPEs also
  if (expect.cellInfoType == RIL_CELL_INFO_TYPE_NR) {
    return is_same(expect.CellInfo.nr, recv.CellInfo.nr);
  }
  return true;
}
...
enum ril_utf_expectation_t verify_ril_unsol_network_scan_result(void* exp_node, void* data)
{
  ...
  do {
    if ((expect->status != recv->status)) {
      RIL_UTF_ESSENTIAL(
          "\n QMI_RIL_UTF: mismatch status: exp: %d, recv: %d", expect->status, recv->status);
      result = not_met;
      break;
    }
    if (expect->network_infos_length && expect->network_infos) {
      if (expect->network_infos_length != recv->network_infos_length) {
        RIL_UTF_ESSENTIAL("\n QMI_RIL_UTF: mismatch network_infos_length: exp: %d, recv: %d",
                          expect->network_infos_length,
                          recv->network_infos_length);
        result = not_met;
        break;
      }
      for (size_t i = 0; i < expect->network_infos_length; i++) {
        if (!is_same(expect->network_infos[i], recv->network_infos[i])) {
          RIL_UTF_ESSENTIAL("\n QMI_RIL_UTF: mismatch network_infos[%d]", i);
          result = not_met;
          break;
        }
      }
...
enum ril_utf_expectation_t verify_ril_is_vonr_enabled
(
 void *exp_node, void *data
)
{
  enum ril_utf_expectation_t result = met;

  ril_request_set_vo_nr_enabled_t* expect = (ril_request_set_vo_nr_enabled_t*) exp_node;
  ril_request_set_vo_nr_enabled_t* recv = (ril_request_set_vo_nr_enabled_t*) data;
  RIL_UTF_DEBUG("\n verify_ril_is_vonr_enabled expect = %d recv = %d", expect->mEnable, recv->mEnable);

  if (expect->mEnable != recv->mEnable)
  {
      result = not_met;
  }
  return result;
...
)
{
   ...
   int i;
   RIL_CardStatus_v6 *ret = (RIL_CardStatus_v6 *)malloc(len);
   ...
   for (i = 0; i < RIL_CARD_MAX_APPS; i++)
   {
      if (cardstatus->applications[i].aid_ptr != NULL)
      {
         ...
         ret->applications[i].aid_ptr = (char *)malloc(size);
      ...
      if (cardstatus->applications[i].app_label_ptr != NULL)
      {
         ...
         ret->applications[i].app_label_ptr = (char *)malloc(size);
