...
int ril_utf_test_t::handle_inject_step( struct ril_utf_node_base_t* &test_step )
{
  ...
  case ril_utf_dsi_internal:
      err_val = send_ril_utf_dsi_internal_event( inject_ptr );
      break;
...
enum ril_utf_expectation_t ril_utf_test_t::verify_ril_get_imei_resp(void* exp,
                                                                    int exp_len,
                                                                    void* recv,
                                                                    int rcv_len)
{
  // Check mismatching payload lengths
  if (exp_len != rcv_len) {
    RIL_UTF_DEBUG("\n QMI_RIL_UTL: expect payload len %d got payload len %d\n", exp_len, rcv_len);
    return not_met;
  }

  // Check mismatching null pointers
  if ((exp == nullptr || recv == nullptr) && exp != recv) {
    RIL_UTF_DEBUG("\n QMI_RIL_UTL: expect %snull payload, recieved %snull payload\n",
                  exp != nullptr ? "non-" : "",
                  recv != nullptr ? "non-" : "");
    return not_met;
  }

  // Verify actual payload contents
  RIL_ImeiInfo *exp_payload = (RIL_ImeiInfo*)exp, *rcv_payload = (RIL_ImeiInfo*)recv;

  if (exp_payload->imeiLen != rcv_payload->imeiLen) {
    RIL_UTF_DEBUG("\n QMI_RIL_UTL: Expected imei len %d rcvd imei len %d",
                  exp_payload->imeiLen,
                  rcv_payload->imeiLen);
    return not_met;
  }

  if (strncmp(exp_payload->imei, rcv_payload->imei, exp_payload->imeiLen) != 0) {
    RIL_UTF_DEBUG("\n QMI_RIL_UTL: Expected imei");
    print_msg_in_hex(exp_payload->imei, exp_payload->imeiLen);
    RIL_UTF_DEBUG("\n QMI_RIL_UTL: Recvd imei");
    print_msg_in_hex(rcv_payload->imei, rcv_payload->imeiLen);
    return not_met;
  }

  if (exp_payload->imeiType != rcv_payload->imeiType) {
    RIL_UTF_DEBUG("\n QMI_RIL_UTL: Expected imei len %d rcvd imei len %d",
                  exp_payload->imeiType,
                  rcv_payload->imeiType);
    return not_met;
  }
                                   ...
                                   struct ril_utf_msg_t *data)
{
  ...
  else if ( exp_node->get_msg_id() == ims_MsgId_UNSOL_SMS_STATUS_REPORT)
  {
    validate_unsol_incoming_ims_sms_status_report( exp_node, data);
  }
  ...
  } else if (exp_node->get_msg_id() == ims_MsgId_UNSOL_SRTP_ENCRYPTION_STATUS) {
    validate_ims_unsol_srtp_encryption_status(exp_node, data);
...
)
{
  ...
  if (exp && recv)
  {
    ...
    if(recv->has_isPasswordRequired &&
      ( (!exp->has_isPasswordRequired) || exp->isPasswordRequired != recv->isPasswordRequired) ) {
        RIL_UTF_ESSENTIAL("\n QMI_RIL_UTL: Expected isPasswordRequired: %d Received isPasswordRequired: %d",
                      exp->isPasswordRequired,
                      recv->isPasswordRequired);
        exp_node->set_expect_status(not_met);
        return;
      }
  }
...
void ril_utf_test_t::validate_unsol_incoming_ims_sms_status_report(ril_utf_node_base_t *exp_node,
        struct ril_utf_msg_t *data)
{
    auto exp = static_cast<utf_ims_sms_status_report_t*>(exp_node->get_payload());
    auto recv = static_cast<utf_ims_sms_status_report_t*>(data->payload);
  ...
  for (i = 0; recv[i] != NULL; ++i)
  {
    ...
    if (recv[i]->has_callDetails && expect[i]->has_callDetails)
    {
      do {
        ...
        // TODO: Currently validating these only if it is expecting as per the test case
        // It requires all the voice related test cases to modify to add the has_localAbility
        // or has_peerAbility validation.
        // Local Ability
        if (expect[i]->callDetails.has_localAbility &&
            expect[i]->callDetails.has_localAbility != recv[i]->callDetails.has_localAbility)
        {
          RIL_UTF_ESSENTIAL("\nQMI_RIL_UTF: For call %d: failed has_localAbility check", i);
          validation_status = false;
          break;
        }
        ...
        // Peer Ability
        if (expect[i]->callDetails.has_peerAbility &&
            expect[i]->callDetails.has_peerAbility != recv[i]->callDetails.has_peerAbility)
        {
          RIL_UTF_ESSENTIAL("\nQMI_RIL_UTF: For call %d: failed has_peerAbility check", i);
          validation_status = false;
          break;
        }
       ...
       (exp_node->get_payload() != NULL) )
  {
    ...
    else if (exp_node->get_msg_id() == RIL_UNSOL_RINGBACK_TONE)
    {
        exp_node->set_expect_status(verify_ril_unsol_ringback_tone(exp_node->get_payload(), data->payload));
             ...
             exp_node->get_msg_id() == RIL_QTI_RADIO_UNSOL_NETWORK_SCAN_RESULT)
    {
                                        ...
                                        struct ril_utf_msg_t *data)
{
  ...
  else if ( exp_node->get_msg_id() == RIL_REQUEST_IS_VONR_ENABLED )
  {
    exp_node->set_expect_status(verify_ril_is_vonr_enabled(exp_node->get_payload(), data->payload));
  }
  ...
  } else if (exp_node->get_msg_id() == RIL_REQUEST_GET_IMEI) {
    exp_node->set_expect_status(verify_ril_get_imei_resp(
        exp_node->get_payload(), exp_node->get_payload_len(), data->payload, data->payload_len));
  } else if (exp_node->get_msg_id() == RIL_REQUEST_CDMA_GET_BROADCAST_SMS_CONFIG) {
    exp_node->set_expect_status(verify_ril_cdma_get_broadcast_sms_config(
        exp_node->get_payload(), exp_node->get_payload_len(), data->payload, data->payload_len));
  }
  else if (exp_node->get_msg_id() == RIL_REQUEST_CONFIG_GET_PHONE_CAPABILITY)
  {
    verify_ril_get_phone_capability(exp_node, exp_node->get_payload_len(), data, data->payload_len);
  }
...
bool ril_utf_test_t::compare_pre_alerting_call_info(const ims_PreAlertingCallInfo& expected,
  ...
  if (expected.modemCallId!= received.modemCallId) {
    RIL_UTF_DEBUG("\n QMI_RIL_UTF: modem call id, expected = %d, received = %d",
                  expected.modemCallId, received.modemCallId);
    return false;
  }
  if (expected.isDcCall != received.isDcCall) {
    RIL_UTF_DEBUG("\n QMI_RIL_UTF: isDcCall, expected = %d, received = %d", expected.isDcCall,
                  received.isDcCall);
    return false;
  }
...
void ril_utf_test_t::validate_ims_unsol_auto_call_rejection_ind2(ril_utf_node_base_t* exp_node,
  ...
  if (exp->isDcCall != recv->isDcCall) {
    RIL_UTF_DEBUG("\n QMI_RIL_UTF: isDcCall, expected = %d, received = %d", exp->isDcCall,
                  recv->isDcCall);
    exp_node->set_expect_status(not_met);
    return;
  }
...
void ril_utf_test_t::validate_ims_unsol_srtp_encryption_status(ril_utf_node_base_t* exp_node,
                                                               struct ril_utf_msg_t* data) {
  ims_SrtpEncryptionStatus* exp_payload = (ims_SrtpEncryptionStatus*) exp_node->get_payload();
  ims_SrtpEncryptionStatus* rcv_payload= (ims_SrtpEncryptionStatus*) data->payload;
  if (exp_payload->encryptionInfo.call_id == rcv_payload->encryptionInfo.call_id &&
      exp_payload->encryptionInfo.categories == rcv_payload->encryptionInfo.categories) {
    exp_node->set_expect_status(met);
  } else {
    RIL_UTF_DEBUG("\n QMI_RIL_UTF: srtpEncryptionStatus, expected:(call_id = %d, categories = %d),"
                  " received:(call_id = %d, categories = %d)\n",exp_payload->encryptionInfo.call_id,
                  exp_payload->encryptionInfo.categories, rcv_payload->encryptionInfo.call_id,
                  rcv_payload->encryptionInfo.categories);
  }
}
