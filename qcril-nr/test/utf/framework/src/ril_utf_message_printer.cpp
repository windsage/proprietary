...
const char* lookup_sar_message_name(int msg_id)
{
  int i;
  for (i = 0; i < SAR_MESSAGE_SIZE; ++i)
  {
    if (specific_absorption_rate_service_msg_string_table[i].number == msg_id)
    {
      return specific_absorption_rate_service_msg_string_table[i].name;
    }
  }
  RIL_UTF_ESSENTIAL("\n@@@@@@     ERROR! NO MATCHING MESSAGE!!!\n");
  return NULL;
...
void print_oem_message_name(int service_id, int msg_id, int oem_command_id, enum ril_utf_q_element_t type)
{
...
#endif
  {
    ...
    case QMI_RIL_OEM_HOOK_QMI_TUNNELING_SERVICE_QTUNER:
      RIL_UTF_ESSENTIAL("\n@@@@@@     Service QMI_RIL_OEM_HOOK_QMI_TUNNELING_SERVICE_QTUNER");
...
void print_qmi_message_name(int qmi_srv_id, int msg_id, enum ril_utf_q_element_t type)
{
  ...
  case QMI_RF_SAR:
    RIL_UTF_ESSENTIAL("\n@@@@@@     Service QMI_RF_SAR_SERVICE");
    tmp = lookup_sar_message_name(msg_id);
    RIL_UTF_ESSENTIAL("\n@@@@@@     Message %s%s", tmp, suffix);
    break;
