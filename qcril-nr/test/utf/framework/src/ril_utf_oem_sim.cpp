...
int utf_oem_recv_msg(struct oem_buff_t *msg)
{
  ...
  length = oem_unpack_int_bigendian((char *)tmp_buf);
...
void handle_oem_msg(int size, uint8_t *payload, int msg_type, int msg_token, uint32_t oem_error)
{
  ...
  qmi_idl_service_object_type qmi_idl_tunneling_service_object;
   ...
   RIL_UTF_DEBUG("handle_oem_msg: size: %d msg_type: %d, msg_token: %d, oem_error: %d",size, msg_type, msg_token, oem_error);
  ...
  enqueue_oem_expect(event_id, message_id, service_id, token_id, type, c_struct_ptr, c_struct_len, (RIL_Errno)oem_error);
}
