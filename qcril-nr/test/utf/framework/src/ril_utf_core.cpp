...
int send_ril_utf_dsi_internal_event(struct ril_utf_msg_t *test_step)
{
  int err_val = QCRIL_UNITTTEST_NO_ERR;
  int sig_err = QCRIL_UNITTTEST_NO_ERR;
  /*========================================================
  ...
  RIL_UTF_DEBUG("\n QMI_RIL_UTL: Waking up dsi_cb thread for dsi internal event.");
  // Set predicate to signal wakeup
  ...
  if ( sig_err != QCRIL_UNITTTEST_NO_ERR )
  {
    RIL_UTF_DEBUG("\n QMI_RIL_UTL: Failed to wake up dsi_cb thread.");
  }
...
int send_ril_utf_dsi_indication(struct ril_utf_msg_t *test_step)
{
  ...
  RIL_UTF_DEBUG("\n QMI_RIL_UTL: Waking up dsi_cb thread for dsi indication.");
  // Set predicate to signal wakeup
  dsi_cb_thread_ready = 1;
  pthread_cond_signal(&wakeup_dsi_cb_thread);
...
int send_ril_utf_db_request (struct ril_utf_msg_t *test_step)
{
  ...
  switch (ptr->action)
  {
    case UTF_DB_INSERT_CLIR_PREF:
    {
      ret = qcril_db_insert_iccid_clir_pref(ptr->iccid, ptr->clir_pref, 0);
      RIL_UTF_ESSENTIAL(
                        "\n return value for qcril_db_insert_iccid_clir_pref(%s, %d, %d): %d",
                        ptr->iccid, ptr->clir_pref, 0, ret);
      if (ret == ptr->expected_ret) {
        ret = 0;
      }
      break;
    }
    case UTF_DB_UPDATE_CLIR_PREF:
    {
      ret = qcril_db_update_iccid_clir_pref(ptr->iccid, ptr->clir_pref);
      RIL_UTF_ESSENTIAL(
                        "\n return value for qcril_db_update_iccid_clir_pref(%s, %d): %d",
                        ptr->iccid, ptr->clir_pref, ret);
      if (ret == ptr->expected_ret) {
        ret = 0;
      }
      break;
    }
    case UTF_DB_DELETE_CLIR_PREF:
    {
      ret = qcril_db_delete_iccid_clir_pref(ptr->iccid);
      RIL_UTF_ESSENTIAL(
                        "\n return value for qcril_db_delete_iccid_clir_pref(%s): %d",
                        ptr->iccid, ret);
      if (ret == ptr->expected_ret) {
        ret = 0;
      }
    ...
    case UTF_DB_CLEAR_ICCID_CLIR_PREF_TABLE:
    {
      ret = qcril_db_clear_iccid_clir_pref_table();
      RIL_UTF_ESSENTIAL(
                        "\n return value for qcrild_db_clear_iccid_clir_pref_table(): %d",
                        ret);
      if (ret == ptr->expected_ret) {
        ret = 0;
      }
  ...
  if (test_step) {
    ((DBTestnode*)test_step)->clear_internals();
    if (test_step->payload) {
      free(test_step->payload);
      test_step->payload = NULL;
    }
...
int send_ril_utf_db_response (void* payload, int payload_len)
{
  ...
  bool found;
  uint32_t clir_pref;
  int32_t clir_pref_entry_count;
  char iccid[QCRIL_ICCID_STR_MAX_SIZE];
  size_t iccid_len;
  switch (ptr->action)
  {
    case UTF_DB_IS_MCC_PART_OF_EMERGENCY_NUMBER_TABLE:
    {
      ...
      RIL_UTF_ESSENTIAL(
                        "\n return value for qcril_db_is_mcc_part_of_emergency_numbers_table"
                        "(%d, %s, %s): %d", ptr->source, ptr->mcc, ptr->mnc, ret);
      if (ret == ptr->expected_ret)
      {
        ret = 0;
      }
    ...
    case UTF_DB_QUERY_CLIR_PREF:
    {
      ret = qcril_db_query_iccid_clir_pref(ptr->iccid, &clir_pref, &found);
      RIL_UTF_ESSENTIAL("\n return value for qcril_db_query_iccid_clir_pref"
                        "(%s, %d, %d): %d", ptr->iccid, clir_pref, found, ret);
      if (ret == ptr->expected_ret && found == ptr->iccid_found && clir_pref == ptr->clir_pref) {
        ret = 0;
      }
      else {
        ret = 1;
    ...
    case UTF_DB_QUERY_CLIR_PREF_ENTRY_COUNT:
    {
      ret = qcril_db_query_iccid_clir_pref_entry_count(&clir_pref_entry_count);
      RIL_UTF_ESSENTIAL(
                        "\n return value for qcril_db_query_iccid_clir_pref_entry_count(%d): %d",
                        clir_pref_entry_count, ret);
      if (ret == ptr->expected_ret && clir_pref_entry_count == ptr->clir_pref_entry_count) {
        ret = 0;
      }
      else {
        ret = 1;
    ...
    case UTF_DB_QUERY_OLDEST_CLIR_PREF:
    {
      ret = qcril_db_query_iccid_clir_pref_oldest_entry(iccid);
      iccid_len = strnlen(ptr->iccid, QCRIL_ICCID_STR_MAX_SIZE);
      RIL_UTF_ESSENTIAL(
                        "\n return value for qcril_db_query_iccid_clir_pref_entry_count(%s): %d",
                        iccid, ret);
      if (ret == ptr->expected_ret && (strncmp(ptr->iccid, iccid, iccid_len) == 0)) {
        ret = 0;
      }
      else {
        ret = 1;
