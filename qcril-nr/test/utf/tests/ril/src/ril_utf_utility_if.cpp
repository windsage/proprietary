...
)
{
  ...
  if (cell_barring) {
    QMI_node.set_qmi_nas_sys_info_ind_wcdma_signalling_bar_status();
    nas_gw_sys_info3_type_v01* wcdma_sys_info3_ind = QMI_node.set_qmi_nas_sys_info_ind_wcdma_sys_info3();
    wcdma_sys_info3_ind->cs_bar_status = NAS_CELL_ACCESS_NO_CALLS_V01;
    wcdma_sys_info3_ind->ps_bar_status = NAS_CELL_ACCESS_NO_CALLS_V01;
  }
  if(NULL != mcc)
  {
  ...
  if(is_rat_change)
  {
    ...
    if (cell_barring) {
      nas_gw_sys_info3_type_v01* wcdma_sys_info3_resp = QMI_node.set_qmi_nas_get_sys_info_resp_wcdma_sys_info3();
      wcdma_sys_info3_resp->cs_bar_status = NAS_CELL_ACCESS_NO_CALLS_V01;
      wcdma_sys_info3_resp->ps_bar_status = NAS_CELL_ACCESS_NO_CALLS_V01;
    }
    if(NULL != mcc)
    {
    ...
    if (cell_barring) {
      RIL_node.update_default_ril_unsol_restricted_state_changed(20);
      t.expect(RIL_node);
    }
...
{
  util_voice_radio_tech_wcdma_in_service(t, QMI_node, RIL_node, is_rat_change, rat, srv_status, true_srv_status, domain, roam_status, FALSE, NULL, NULL, 0);
}
...
void util_ril_utf_init_3gpp_sim_app(Testcase &t, QMIRILTestnode &QMI_node, AndroidTelTestnode& RIL_node, uim_app_state_enum_v01 app_state)
{
  ...
  label_req2->application_information.slot = (uim_slot_enum_v01)0x01;
  label_req2->application_information.aid_len = 0x10;
  memcpy(label_req2->application_information.aid, aid_value_1, 16);
  t.expect(QMI_node);

  //Inject: QMI_UIM_GET_LABEL_RESP
  uim_get_label_resp_msg_v01 *response2 = QMI_node.update_default_qmi_uim_get_label_resp();
  response2->resp.result = QMI_RESULT_SUCCESS_V01;
  strlcpy(response2->label, "USIM", 5);
  response2->label_len = 4;
  response2->label_valid = TRUE;
  t.inject(QMI_node);
...
void util_ril_utf_init_3gpp2_sim_app(Testcase &t, QMIRILTestnode &QMI_node, AndroidTelTestnode& RIL_node)
{
  ...
  uim_get_label_resp_msg_v01 *response1 = QMI_node.update_default_qmi_uim_get_label_resp();
  response1->resp.result = QMI_RESULT_SUCCESS_V01;
  strlcpy(response1->label, "CSIM", 5);
  response1->label_len = 4;
  response1->label_valid = TRUE;
  t.inject(QMI_node);
  ...
  uim_get_label_resp_msg_v01 *response2 = QMI_node.update_default_qmi_uim_get_label_resp();
  response2->resp.result = QMI_RESULT_SUCCESS_V01;
  strlcpy(response2->label, "CSIM", 5);
  response2->label_len = 4;
  response2->label_valid = TRUE;
  t.inject(QMI_node);
  ...
  strcpy(status_ptr->applications[0].aid_ptr, "a0000000871002f310ffff89080000ff");
  status_ptr->applications[0].app_label_ptr = (char *)malloc(5);
  strlcpy(status_ptr->applications[0].app_label_ptr, "CSIM", 5);
  t.expect(RIL_node);
  t.set_end();
}
...
void util_voice_registration_state_for_nr5g(Testcase& t,
                                            QMIRILTestnode& QMI_node,
                                            AndroidTelTestnode& RIL_node,
                                            int rat,
                                            int reg_state,
                                            ...
                                            const char* mcc,
                                            const char* mnc,
                                            const char* primary_mcc,
                                            const char* primary_mnc,
                                            ...
                                            bool is_data_rat_change)
{
    ...
    // Inject: QMI_NAS_GET_SYS_INFO_RESP_MSG_V01
    nas_get_sys_info_resp_msg_v01* ptr = QMI_node.update_default_qmi_nas_get_sys_info_resp_msg_v01();
    nas_3gpp2_srv_status_info_type_v01* cdma_srv_status_info_resp =
        QMI_node.set_qmi_nas_get_sys_info_resp_cdma_srv_status_info();
    cdma_srv_status_info_resp->srv_status = NAS_SYS_SRV_STATUS_NO_SRV_V01;
    cdma_srv_status_info_resp->is_pref_data_path = 0;
    nas_3gpp_srv_status_info_type_v01* gsm_srv_status_resp =
        QMI_node.set_qmi_nas_get_sys_info_resp_gsm_srv_status_info();
    gsm_srv_status_resp->srv_status = NAS_SYS_SRV_STATUS_NO_SRV_V01;
    gsm_srv_status_resp->true_srv_status = SYS_SRV_STATUS_NO_SRV_V01;
    gsm_srv_status_resp->is_pref_data_path = 0;
    nas_3gpp_srv_status_info_type_v01* wcdma_srv_status_resp =
        QMI_node.set_qmi_nas_get_sys_info_resp_wcdma_srv_status_info();
    wcdma_srv_status_resp->srv_status = NAS_SYS_SRV_STATUS_NO_SRV_V01;
    wcdma_srv_status_resp->true_srv_status = SYS_SRV_STATUS_NO_SRV_V01;
    wcdma_srv_status_resp->is_pref_data_path = 0;
    nas_3gpp_srv_status_info_type_v01* lte_srv_status_resp =
    ...
    nas_3gpp_srv_status_info_type_v01* nr5g_srv_status_info =
        QMI_node.set_qmi_nas_get_sys_info_resp_nr5g_srv_status_info();
    nr5g_srv_status_info->srv_status = srv_status;
    nr5g_srv_status_info->true_srv_status = true_srv_status;
    nr5g_srv_status_info->is_pref_data_path = 0;

    nas_nr5g_sys_info_type_v01* nr5g_sys_info =
        QMI_node.set_qmi_nas_get_sys_info_resp_nr5g_sys_info();
    nr5g_sys_info->common_sys_info.srv_domain_valid = TRUE;
    nr5g_sys_info->common_sys_info.srv_domain = domain;
    nr5g_sys_info->common_sys_info.srv_capability_valid = TRUE;
    nr5g_sys_info->common_sys_info.srv_capability = SYS_SRV_DOMAIN_PS_ONLY_V01;
    nr5g_sys_info->common_sys_info.roam_status_valid = TRUE;
    nr5g_sys_info->common_sys_info.roam_status = roam_status;
    nr5g_sys_info->threegpp_specific_sys_info.reg_reject_info_valid = reg_reject_info_valid;
    nr5g_sys_info->threegpp_specific_sys_info.reg_reject_info.reject_srv_domain = reject_srv_domain;
    nr5g_sys_info->threegpp_specific_sys_info.reg_reject_info.rej_cause = rej_cause;
    nr5g_sys_info->threegpp_specific_sys_info.network_id_valid = net_id_valid;

    if (nullptr != mcc && nullptr != mnc) {
        util_ril_utf_fill_qmi_mcc_mnc(nr5g_sys_info->threegpp_specific_sys_info.network_id.mcc, mcc);
        util_ril_utf_fill_qmi_mcc_mnc(nr5g_sys_info->threegpp_specific_sys_info.network_id.mnc, mnc);
    }

    ptr->voice_support_on_nr5g_valid = TRUE;
    ptr->voice_support_on_nr5g = voice_support_on_nr5g;
    ptr->nr5g_voice_domain_valid = nr5g_voice_domain_valid;
    ptr->nr5g_voice_domain = nr5g_voice_domain;

    if (nullptr != primary_mcc && nullptr != primary_mnc) {
        ptr->primary_plmn_info_valid = TRUE;
        ptr->primary_plmn_info.radio_if = NAS_RADIO_IF_LTE_V01;
        util_ril_utf_fill_qmi_mcc_mnc(ptr->primary_plmn_info.plmn_id.mcc, primary_mcc);
        util_ril_utf_fill_qmi_mcc_mnc(ptr->primary_plmn_info.plmn_id.mnc, primary_mnc);
    }

    if (cag_id_name) {
        ptr->cag_id_name_valid = TRUE;
        ptr->cag_id_name = *cag_id_name;
    }
    if (snpn_id_name) {
        ptr->snpn_id_name_valid = TRUE;
        ptr->snpn_id_name = *snpn_id_name;
    }
    t.inject(QMI_node);
    ...
    // Expect: RIL_REQUEST_REGISTRATION_STATE_RESP
    RIL_VoiceRegistrationStateResponse* reg_state_resp =
        RIL_node.update_default_ril_request_registration_state_resp();
    reg_state_resp->regState = (RIL_RegState)reg_state;
    reg_state_resp->rat = (RIL_RadioTechnology)rat;
    ...
    if (reg_state == RIL_VAL_REG_REGISTERED_HOME_NET ||
        ...
        reg_state == RIL_VAL_REG_NOT_REGISTERED_SEARCHING_LTD_SRV ||
        srv_status == NAS_SYS_SRV_STATUS_SRV_V01) {
        reg_state_resp->cellIdentity.cellInfoType = RIL_CELL_INFO_TYPE_NR;
        if (nullptr != mcc && nullptr != mnc) {
            strlcpy(reg_state_resp->cellIdentity.cellIdentityNr.mcc, mcc, MAX_RIL_MCC_MNC_SIZE);
            strlcpy(reg_state_resp->cellIdentity.cellIdentityNr.mnc, mnc, MAX_RIL_MCC_MNC_SIZE);
        }
#if 0
        if (nullptr != primary_mcc && nullptr != primary_mnc) {
            strlcpy(
                reg_state_resp->cellIdentity.cellIdentityNr.mcc, primary_mcc, MAX_RIL_MCC_MNC_SIZE);
            strlcpy(
                reg_state_resp->cellIdentity.cellIdentityNr.mnc, primary_mnc, MAX_RIL_MCC_MNC_SIZE);
        }
#endif
    }
