...
/* SMS_3GPP2_marshal_sms_broadcast_config_info_array_invalid_size
 * Description: Create an invalid parcel with a large
 *              size that doesn't reflect the size of the
 *              RIL_CDMA_BroadcastSmsConfigInfo array.
 * Expectation: read should return Marshal::Result::NOT_ENOUGH_DATA.
 */
TEST(Marshalling, SMS_3GPP2_marshal_sms_broadcast_config_info_array_invalid_size) {
    Marshal marshal;
    char arr[] = "\x10\x00\x00\x00"         // size
                 "\x00\x00\x00\x02"         // service_category
                 "\x00\x00\x00\x05"         // language
                 "\x01"                     // selected
                 "\x00\x00\x00\x01"         // service_category
    ...
    size_t rdNum = 0;
    RIL_CDMA_BroadcastSmsConfigInfo* pRdConfigInfo = nullptr;
    ASSERT_EQ(marshal.readAndAlloc<RIL_CDMA_BroadcastSmsConfigInfo>(
            pRdConfigInfo, rdNum), Marshal::Result::NOT_ENOUGH_DATA);
}

/* SMS_3GPP2_marshal_sms_broadcast_config_info_array_release
 * Description: Test for RIL_CDMA_BroadcastSmsConfigInfo release function
*/
TEST(Marshalling, SMS_3GPP2_marshal_sms_broadcast_config_info_array_release) {
    Marshal marshal;
    RIL_CDMA_BroadcastSmsConfigInfo configInfo[] = {
        { 2, 5, 1 }, { 1, 6, 0}, {3, 4, 1} };
    size_t num = sizeof(configInfo) / sizeof(RIL_CDMA_BroadcastSmsConfigInfo);
    ...
    RIL_CDMA_BroadcastSmsConfigInfo* pRdConfigInfo = nullptr;
    ASSERT_EQ(marshal.readAndAlloc<RIL_CDMA_BroadcastSmsConfigInfo>(
            pRdConfigInfo, rdNum), Marshal::Result::SUCCESS);
    ASSERT_EQ(num, rdNum);
    ASSERT_EQ(memcmp(configInfo, pRdConfigInfo, sizeof(configInfo)), 0);
    ASSERT_EQ(marshal.release(pRdConfigInfo, rdNum), Marshal::Result::SUCCESS);
}
