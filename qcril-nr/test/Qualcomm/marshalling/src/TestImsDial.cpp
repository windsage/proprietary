...
/* Ims_Dial_marshall_large_extras_length
...
TEST(Marshalling, Ims_Dial_marshall_large_extras_length) {
    Marshal marshal;
    char arr[] =
        "\x00\x00\x00\x0a"          // address length
        "1234567890"                // address
        ...
        "\x00\x00\x00\x01"          // callDetails.callType
        "\x00\x00\x00\x03"          // callDetails.callDomain
        "\x10\x00\x00"              // callDetails.extrasLength
        "\x00\x00\x00\x11"          // callDetails.extras[0] len
        "DisplayText=hello"         // callDetails.extras[0]
        "\x00\x00\x00\x16"          // callDetails.extras[1] len
        "RetryCallFailReason=21"    // callDetails.extras[1]
        ...
        "\x00\x00\x00\x02"          // callDetails.localAbilityLength
        "\x00\x00\x00\x01"          // callDetails.localAbility[0].callType
        "\x00\x00\x00\x13"          // callDetails.localAbility[0]
                                    //            .accTechStatus.networkMode
        "\x00\x00\x00\x02"          // callDetails.localAbility[0]
                                    //            .accTechStatus.status
        "\x00\x00\x00\x00"          // callDetails.localAbility[0]
                                    //            .accTechStatus.restrictCause
        "\x00\x00\x00\x01"          // callDetails.localAbility[0]
                                    //            .accTechStatus.registration.state
        "\x00\x00\x00\x00"          // callDetails.localAbility[0]
                                    //            .accTechStatus.registration.errorCode
        "\xff\xff\xff\xff"          // callDetails.localAbility[0]
                                    //            .accTechStatus.registration.errorMessage = nullptr
        "\x00\x00\x00\x13"          // callDetails.localAbility[0]
                                    //            .accTechStatus.registration.radioTech
        "\x00\x00\x00\x09"          // callDetails.localAbility[0]
                                    //            .accTechStatus.registration.pAssociatedUris len
        "//testuri"                 // callDetails.localAbility[0]
                                    //            .accTechStatus.registration.pAssociatedUris
        "\x00\x00\x00\x01"          // callDetails.localAbility[0]
                                    //            .accTechStatus.rttMode
        "\x00\x00\x00\x03"          // callDetails.localAbility[1].callType
        "\x00\x00\x00\x14"          // callDetails.localAbility[1]
                                    //            .accTechStatus.networkMode
        "\x00\x00\x00\x02"          // callDetails.localAbility[1]
                                    //            .accTechStatus.status
        "\x00\x00\x00\x00"          // callDetails.localAbility[1]
                                    //            .accTechStatus.restrictCause
        "\x00\x00\x00\x02"          // callDetails.localAbility[1]
                                    //            .accTechStatus.registration.state
        "\x00\x00\x01\xf4"          // callDetails.localAbility[1]
                                    //            .accTechStatus.registration.errorCode
        "\x00\x00\x00\x0c"          // callDetails.localAbility[1]
                                    //            .accTechStatus.registration.errorMessage len
        "Server error"              // callDetails.localAbility[1]
                                    //            .accTechStatus.registration.errorMessage
        "\x00\x00\x00\x14"          // callDetails.localAbility[1]
                                    //            .accTechStatus.registration.radioTech
        "\x00\x00\x00\x09"          // callDetails.localAbility[1]
                                    //            .accTechStatus.registration.pAssociatedUris len
        "//testuri"                 // callDetails.localAbility[1]
                                    //            .accTechStatus.registration.pAssociatedUris
        "\x00\x00\x00\x01"          // callDetails.localAbility[1]
                                    //            .accTechStatus.rttMode
        "\x00\x00\x00\x00"          // callDetails.peerAbilityLength
        "\x00\x00\x00\x05"          // callDetails.callSubstate
        "\x00\x00\x00\x03"          // callDetails.mediaId
    ...
    std::string invalid_input(arr, sizeof(arr));
    marshal.setData(invalid_input);
    RIL_IMS_Dial rdValue{};
    ASSERT_EQ(marshal.read(rdValue), Marshal::Result::NOT_ENOUGH_DATA);
    ASSERT_NE(marshal.release(rdValue), Marshal::Result::FAILURE);
}
