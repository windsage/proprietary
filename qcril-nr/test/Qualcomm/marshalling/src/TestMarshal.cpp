...
TEST(Marshalling, Marshal_read_write_char_star) {
    ...
    ASSERT_EQ(marshal.write(emptystr), Marshal::Result::SUCCESS);
    cur_size += sizeof(int32_t);
    ASSERT_EQ(marshal.dataPosition(), cur_size);

    ASSERT_EQ(marshal.write(nullstr), Marshal::Result::SUCCESS);
    cur_size += sizeof(int32_t);
    ASSERT_EQ(marshal.dataPosition(), cur_size);
    ...
    ASSERT_EQ(marshal.read(out), Marshal::Result::SUCCESS);
    ASSERT_EQ(strcmp(out, emptystr), 0);
    if (out) delete []out;
