...
TEST(Marshalling, NAS_voice_reg_state) {
  ...
  char arr[] =
      ...
      "\x00\x00\x00\x03"
      "460"               // cellIdentity.cellIdentityLte.reg_plmn_mcc
      "\x00\x00\x00\x02"
      "00"                // cellIdentity.cellIdentityLte.reg_plmn_mnc
      "\x00\x00\x00\x03"
      "460"               // cellIdentity.cellIdentityLte.primary_plmn_mcc
      "\x00\x00\x00\x02"
      "00"                // cellIdentity.cellIdentityLte.primary_plmn_mnc
      "\x00\x00\x00\x29"  // cellIdentity.cellIdentityLte.band
      ;
...
TEST(Marshalling, NAS_data_reg_state) {
  ...
  RIL_DataRegistrationStateResponse request = {
    ...
    .nrVopsInfo =
        {
            .isVopsSupported = 0,
            .isEmcSupported = NR5G_EMC_NOT_SUPPORTED,
            .isEmfSupported = NR5G_EMF_NOT_SUPPORTED,
        },
  };
  char arr[] =
      "\x00\x01\x00\x0e"  // regState and rat
      ...
      "\x01"  // nrVopsInfo
      "\x00"  // nrVopsInfo.isVopsSupported
      "\x00\x00\x00\x00" // nrVopsInfo.isEmcSupported
      "\x00\x00\x00\x00" // nrVopsInfo.isEmfSupported
      ;
  std::string expected(arr, sizeof(arr));
  ASSERT_NE(marshal.write(request), Marshal::Result::FAILURE);
  marshal.write(static_cast<char>(0));
