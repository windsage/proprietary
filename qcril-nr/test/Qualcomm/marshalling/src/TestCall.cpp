...
TEST(Marshalling, Call_marshall_write_001) {
  ...
  std::string s =
      ...
      "3536373839"  // redirNum ("56789")
      "00000000"    // redirNumPresentation
      ;
  ...
  ASSERT_STREQ(request.redirNum, request_r.redirNum);
  ASSERT_EQ(request.redirNumPresentation, request_r.redirNumPresentation);
}
...
TEST(Marshalling, Call_marshall_write_002) {
  ...
  std::string s =
      ...
      "00000002"    // RIL_AUDIO_QUAL_AMR_WB (2)
      "ffffffff"    // redirNum length
      "00000001"    // redirNumPresentation
      ;
