    ...
    do
    {
      ...
      if (cellinfobuf)
      {
        ...
        if (li->nr5g_serving_cell_info_valid)
        {
            ...
            /* report -1 dB for db values between 0(0 db) and -10(-1 db)
               some customers has set the lower threshold[0] as 0 dB carrier_config */
            curr_cel->CellInfo.nr.signalStrengthNr.ssSinr =
              (int32_t)floor(((float)li->nr5g_serving_cell_info.snr)/10);
            curr_cel ++;
        }
