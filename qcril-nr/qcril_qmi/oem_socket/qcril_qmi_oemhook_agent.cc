...
void sendOemhookAidlIndication(qcril_instance_id_e_type instId, uint8_t* buf, size_t bufLen);
...
)
{
    ...
    if (send_buffer)
    {
        ...
        // Send indication to oemhook AIDL service
        sendOemhookAidlIndication(oem_socket_instance_id, send_buffer, final_len);
#endif
        delete []send_buffer;
    }
    else
...
#ifndef RIL_FOR_MDM_LE
      // Send response to oemhook AIDL service
      sendOemhookAidlResponse(oem_socket_instance_id, token, error, send_buffer, msg_len);
#endif
      delete []send_buffer;
    }
    else
