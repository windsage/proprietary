...
class RequestManager {
    ...
    public:
        ...
        Status hasValidIndicationHandler(int32_t indicationId);
        #ifdef QCRIL_BUILD_AFL_TESTCASE_GEN
        void setSaveTestcases(bool value);
        bool getSaveTestcases();
        Status setTestcaseOutDir(std::string path);
        std::string getTestcaseOutDir();
        #endif
    ...
    private:
        ...
        #ifdef QCRIL_BUILD_AFL_TESTCASE_GEN
        void saveAflTestcase(uint64_t token, int32_t requestId, std::unique_ptr<Marshal> parcel);
    ...
    private:
        ...
        std::mutex socketWriteMutex; // to prevent concurrent writes to the RIL socket

        #ifdef QCRIL_BUILD_AFL_TESTCASE_GEN
        std::string testcaseOutDir;
        std::mutex testcaseOutDirMutex;
        std::atomic_bool saveTestcases = false;
        std::map<int, std::string> requestApiMap;
        #endif // QCRIL_BUILD_AFL_TESTCASE_GEN
};