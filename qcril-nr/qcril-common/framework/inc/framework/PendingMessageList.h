...
public:
  ...
  std::shared_ptr<Message> find(message_id_ref messageId);
  ...
  template <typename T>
  std::shared_ptr<T> extract(uint16_t token) {
    std::shared_ptr<Message> msg = extract(token);
    return std::static_pointer_cast<T>(msg);
  }

  std::shared_ptr<Message> extract(message_id_ref messageId);

  template <typename T>
  std::shared_ptr<T> extract() {
    std::shared_ptr<Message> msg = extract(T::get_class_message_id());
    return std::static_pointer_cast<T>(msg);
  }
