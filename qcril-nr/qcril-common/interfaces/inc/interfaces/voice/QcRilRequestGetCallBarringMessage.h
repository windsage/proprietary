...
class QcRilRequestGetCallBarringMessage
 ...
 public:
  ...
  bool hasExpectMore() {
    return mExpectMore ? true : false;
  }
  bool getExpectMore() {
    return *mExpectMore;
  }
  void setExpectMore(bool val) {
    mExpectMore = val;
  }
  inline string dumpExpectMore() {
    return " mExpectMore = " + (mExpectMore ? std::to_string(*mExpectMore) : "<invalid>");
  }

  virtual string dump() {
    return QcRilRequestMessage::dump() + "{" + dumpFacilityType() + dumpServiceClass() +
           dumpExpectMore() + "}";
  }
};
