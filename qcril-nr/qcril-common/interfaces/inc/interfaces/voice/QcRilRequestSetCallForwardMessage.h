...
class QcRilRequestSetCallForwardMessage : public QcRilRequestMessage,
 ...
 public:
  ...
  inline string dumpNumber() { return " mNumber = " + (mNumber ? ((*mNumber).length() > QCRIL_SHOW_PHONE_NUMBER_MAX ? (QCRIL_HIDE_PHONE_NUMBER_STR + (*mNumber).substr((*mNumber).length() - QCRIL_SHOW_PHONE_NUMBER_MAX)).c_str() : *mNumber) : "<invalid>"); }
