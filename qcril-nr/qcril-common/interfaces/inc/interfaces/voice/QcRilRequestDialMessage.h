...
class QcRilRequestDialMessage : public QcRilRequestMessage,
                                public add_message_id<QcRilRequestDialMessage> {
 private:
 ...
 public:
  static constexpr const char* MESSAGE_NAME = "QcRilRequestDialMessage";
  QcRilRequestDialMessage() = delete;

  ~QcRilRequestDialMessage() {
  }

  inline QcRilRequestDialMessage(std::shared_ptr<MessageContext> context)
  ...
  virtual std::string dump() {
    ...
    os += "{";
    os += ".mAddress=" +
          (mAddress ? ((*mAddress).length() > QCRIL_SHOW_PHONE_NUMBER_MAX ? (QCRIL_HIDE_PHONE_NUMBER_STR + (*mAddress).substr((*mAddress).length() - QCRIL_SHOW_PHONE_NUMBER_MAX)).c_str() : *mAddress) : "<invalid>");
