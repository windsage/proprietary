...
inline string RilUnsolNwRegistrationRejectMessage::dump()
{
  ...
  ss << " : add'lCauseCode_valid = " << static_cast<int>(mRegRejInfo.additionalCauseCode_valid);
  ss << " : add'lCauseCode = " << mRegRejInfo.additionalCauseCode;
  ss << " : domain = " << mRegRejInfo.domain;
  ss << " : plmn = " << mRegRejInfo.choosenPlmn;
  return ss.str();
}
