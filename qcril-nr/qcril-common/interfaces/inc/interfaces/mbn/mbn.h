...
class ModemsConfigResp : public qcril::interfaces::BasePayload {
public:
  ModemsConfigResp(uint8_t num = 1): numOfModems(num) {}
  uint8_t numOfModems;
};

enum class PdcRefreshEvent {
  UNKNOWN,
  REFRESH_START,
  REFRESH_COMPLETE,
  CLIENT_REFRESH
...
inline std::string toString(const PdcRefreshEvent& o) {
  if (o == PdcRefreshEvent::REFRESH_START) {
    return "REFRESH_START";
  }
  if (o == PdcRefreshEvent::REFRESH_COMPLETE) {
    return "REFRESH_COMPLETE";
  }
  if (o == PdcRefreshEvent::CLIENT_REFRESH) {
