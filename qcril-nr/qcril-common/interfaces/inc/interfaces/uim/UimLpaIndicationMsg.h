...
typedef enum
{
  QCRIL_UIM_LPA_NO_USER_CONFIRMATION_REQD     = 0,
  QCRIL_UIM_LPA_SIMPLE_USER_CONFIRMATION_REQD = 1,
  QCRIL_UIM_LPA_STRONG_USER_CONFIRMATION_REQD = 2
}qcril_uim_lpa_usr_consent_type;
...
typedef struct
{
  ...
  char                                   * profile_name;
  qcril_uim_lpa_usr_consent_type           user_consent_type;
...
} qcril_uim_lpa_user_consent_ind_type;
                       ...
                       UnSolicitedMessage(get_class_message_id())
    {
      ...
      switch (ind_id)
      {
        ...
        case UIM_LPA_USER_CONSENT_IND_ID:
          if(data_ptr != nullptr)
          {
            qcril_uim_lpa_user_consent_ind_type *user_consent_type =
                        new qcril_uim_lpa_user_consent_ind_type;
            if(user_consent_type != nullptr)
            {
              memcpy(user_consent_type,data_ptr,sizeof(qcril_uim_lpa_user_consent_ind_type));
            }
            mDataPtr = user_consent_type;
          }
          break;
        default:
          break;
