...
#include <telephony/ril_module_list.h>
 ...
 *                    Updated data structures: RIL_DataProfileInfo, RIL_InitialAttachApn_v15
...
#define MAX_ROUTE_SELECTORS 10
#define MAX_SLICING_CONFIG 20
#define MAX_URSP_RULES 10
#define MAX_SLICE_INFO 10
#define MAX_DNN_LISTS 10
...
typedef enum {
    ...
    /** Audio Manager related error*/
    RIL_E_AUDIO_SERVER_DIED = 72,
    ...
    /** Internal RIL error - used when RIL fails
        request without sending it to modem */
    RIL_E_INTERNAL_NO_MEMORY = 1015,
    RIL_E_INTERNAL_INVALID_STATE = 1016,
    RIL_E_INTERNAL_INVALID_ARGUMENTS = 1017,
    RIL_E_INTERNAL_GENERIC_FAILURE = 1018,
} RIL_Errno;
...
typedef struct {
    Ril_BarringServiceType barringService;
    Ril_BarringType barringType;
    Ril_BarringTypeSpecificInfo typeSpecificInfo;
} RIL_BarringInfo;

typedef struct {
    RIL_BarringInfo barringInfo[10];
    uint8_t cellIdentityValid;
 ...
 */
typedef enum {
  /** RIL_REPORT_FILTER_DISABLE */
  RIL_REPORT_FILTER_DISABLE,
  /** RIL_REPORT_FILTER_ENABLE */
  RIL_REPORT_FILTER_ENABLE
}RIL_ReportFilter;
 ...
 */
typedef enum {
  /** RIL_LINK_CAP_CRITERIA_RESULT_SUCCESS */
  RIL_LINK_CAP_CRITERIA_RESULT_SUCCESS = RIL_E_SUCCESS,
  /** RIL_LINK_CAP_CRITERIA_RESULT_RADIO_NOT_AVAILABLE */
  RIL_LINK_CAP_CRITERIA_RESULT_RADIO_NOT_AVAILABLE = RIL_E_RADIO_NOT_AVAILABLE,
  /** RIL_LINK_CAP_CRITERIA_RESULT_REQUEST_NOT_SUPPORTED */
  RIL_LINK_CAP_CRITERIA_RESULT_REQUEST_NOT_SUPPORTED = RIL_E_REQUEST_NOT_SUPPORTED,
  /** RIL_LINK_CAP_CRITERIA_RESULT_INTERNAL_ERR */
  RIL_LINK_CAP_CRITERIA_RESULT_INTERNAL_ERR = RIL_E_INTERNAL_ERR,
  /** RIL_LINK_CAP_CRITERIA_RESULT_INVALID_ARGUMENTS */
  RIL_LINK_CAP_CRITERIA_RESULT_INVALID_ARGUMENTS = RIL_E_INVALID_ARGUMENTS,
}RIL_LinkCapCriteriaResult;
...
typedef struct {
    ...
    int mtu;
    /** maximum transmission unit (MTU) size in bytes for V4 */
    int mtuV4;
    /** maximum transmission unit (MTU) size in bytes for V6 */
    int mtuV6;
     ...
     * must overwrite it. */
    int persistent;
    /** If true, trafficDescriptor is valid */
...
typedef struct {
    /**
     * Modem Id for each slot
     *
     * For slot 0 -> Modem Id 0
     * For slot 1 -> Modem Id 1
     */
    uint8_t modemId;
} RIL_ModemInfo;

/* Modem capability info */
typedef struct {
    /**
    ...
    /**
     * Maximum number of subscriptions listed in this configuration that can be simultaneously
     * active.
     *
     */
    ...
    /**
     * bool value to determine whether internet lingering supported or not.
     */
    uint8_t isInternetLingeringSupported;

    /**
     * Number of Modem supported.
     *
     * For slot 0 -> Modem Id 0
     * For slot 1 -> Modem Id 1
     */
    RIL_ModemInfo logicalModemList[MAX_MODEM_COUNT];
} RIL_PhoneCapability;
...
/**
 * Manual Network scan parameter list
 */
typedef struct {
    char* operatorNumeric;             /* 5 or 6 digit numeric code (MCC + MNC) */
    RIL_RadioAccessNetworks ran;       /* The type of Netwrok to scan */
    RIL_AccessMode accessMode;         /* The type of access mode */
    uint8_t cagIdValid;                /* Indicates the CAG Id is valid */
    uint32_t cagId;                    /* CAG ID of CAG Cell  */
    uint32_t snpnNidLength;            /* Length of the SNPN id */
    uint8_t snpnNid[MAX_SNPN_NID_LEN]; /* SNPN identifier */
} RIL_SetNetworkSelectionManualParam;
 ...
 */
typedef enum {
    RIL_UNKNOWN_USAGE_SETTING = 0, // Uknown usage setting mode
    RIL_VOICE_CENTRIC = 1, // Voice-centric
    RIL_DATA_CENTRIC = 2, // Data-centric
 ...
 */
typedef enum {
    /** RADIO_DATA_REASON_NORMAL */
    RADIO_DATA_REASON_NORMAL      = 0x01,
    /** RADIO_DATA_REASON_SHUTDOWN */
    RADIO_DATA_REASON_SHUTDOWN    = 0x02,
    /** RADIO_DATA_REASON_HANDOVER */
    RADIO_DATA_REASON_HANDOVER    = 0x03,
} RIL_RadioDataRequestReasons;
 ...
 * Initiate voice call
 *
 * This request is deprecated; use ril_call.h request
 ...
 * RIL_REQUEST_QUERY_CALL_FORWARD_STATUS
 * This request is deprecated; use ril_call.h request
 * RIL_CALL_REQUEST_QUERY_CALL_FORWARD_STATUS instead.
 *
 ...
 * This request is deprecated; use ril_call.h request
 * RIL_CALL_REQUEST_SET_CALL_FORWARD instead.
 *
 ...
 * This request is deprecated; use ril_call.h request
 * RIL_CALL_REQUEST_QUERY_CALL_WAITING instead.
 *
 ...
 * This request is deprecated; use ril_call.h request
 * RIL_CALL_REQUEST_SET_CALL_WAITING instead.
 *
 ...
 *
 * "data" is NULL
 * "response" is a RIL_ImeiInfo containing the IMEI information
 *
 * Valid errors:
 *  SUCCESS
 *  RADIO_NOT_AVAILABLE (radio resetting)
 ...
 *
 * Call barring functionality is deprecated in this request; use ril_call.h request
 ...
 *
 * "data" is a const RIL_SetNetworkSelectionManualParam
 * "response" is NULL
 *
 * This request must not respond until the new operator is selected
 * and registered
...
/**
 * RIL_REQUEST_DEVICE_IDENTITY - DEPRECIATED
 * Use RIL_REQUEST_GET_IMEI to gather the IMEI instead
 * Still supported for backwards compatability.
 *
 ...
 *  NO_RESOURCES
 */
#define RIL_REQUEST_SET_NR_DISABLE_MODE 160
 ...
 *  REQUEST_NOT_SUPPORTED
 */
#define RIL_REQUEST_QUERY_NR_DISABLE_MODE 161
...
#define RIL_SEND_LPA_HTTP_REQUEST 221

/**
 * RIL_REQUEST_SET_DATA_THROTTLING
 *
 */
#define RIL_REQUEST_SET_DATA_THROTTLING 222

/**
 * RIL_REQUEST_SETUP_DATA_CALL_URSP
 *
 */
#define RIL_REQUEST_SETUP_DATA_CALL_URSP 223

/**
 * RIL_REQUEST_GET_SLICING_CONFIG
 * Get Slicing Config from Modem
 * Data is nullptr
 * "Response" is of type rilSlicingConfig
*/

#define RIL_REQUEST_GET_SLICING_CONFIG 224

/**
 * RIL_REQUEST_GET_USAGE_SETTING
 *
 * Get the UE usage setting for
 * data/voice centric usage
 *
 * "data" is NULL
 *
 * "response" is RIL_UsageSettingMode
 *
 *  Valid errors:
 *  SUCCESS
 *  RADIO_NOT_AVAILABLE
 ...
 *  INVALID STATE
 */
#define RIL_REQUEST_GET_USAGE_SETTING 225

/**
 * RIL_REQUEST_SET_USAGE_SETTING
 *
 * Set the UE usage setting for
 * data/voice centric usage.
 *
 * "data" is RIL_UsageSettingMode
 *
 * "response" is NULL
 *
 *  Valid errors:
 ...
 *  INVALID_ARGUMENTS
 */
#define RIL_REQUEST_SET_USAGE_SETTING 226

/**
 * RIL_REQUEST_REGISTER_INDICATIONS
 *
 * Used to register for the required UNSOL indications
 *
 * "data" is RIL unsol_indication Id
 *
 * "response" is NULL
 *
 *  Valid errors:
 ...
 *  INVALID_ARGUMENTS
 */
#define RIL_REQUEST_REGISTER_INDICATIONS 227

/**
 * RIL_REQUEST_GET_MODULE_LIST
 *
 * Return the list of available loadable modules.
 *
 * "data" is const char * containing a config to query, or NULL to query
 *            all available modules.
 *
 * "response" is RIL_ModuleList *;
 */
#define RIL_REQUEST_GET_MODULE_LIST 228
#define RIL_REQUEST_GET_CONFIG_LIST 229
#define RIL_REQUEST_MOD_GET_CURRENT_CONFIG 230
#define RIL_REQUEST_MOD_SET_CURRENT_CONFIG 231
#define RIL_REQUEST_MOD_SET_CUSTOM_CFG_MODULES 232

/*
 * RIL_REQUEST_SET_MSIM_PREFERENCE
 *
 * Set the MSIM Preference for
 * the device.
 *
 * "data" is Msim_Preference
 *
 * "response" is NULL
 *
 *  Valid errors:
 ...
 *  INVALID_ARGUMENTS
 */
#define RIL_REQUEST_SET_MSIM_PREFERENCE 233

/*
 * RIL_REQUEST_GET_UICC_APPLICATIONS_ENABLEMENT
 *
 * Get whether UICC applications are enabled
 *
 *
 * "data" is NULL
 *
 * "response" is RilGetUiccAppStatusResult_t
 *
 *  Valid errors:
 *  SUCCESS
 *  GENERIC_FAILURE
 *  INTERNAL_ERR
 */
#define RIL_REQUEST_GET_UICC_APPLICATIONS_ENABLEMENT 234

/**
 * RIL_REQUEST_ENABLE_ENDC
 *
 * Set the enable status for endc config
 *
 * "data" is bool
 *
 * "response" is NULL
 ...
 *  INTERNAL_ERR
 */
#define RIL_REQUEST_ENABLE_ENDC 235

/**
 * RIL_REQUEST_QUERY_ENDC_STATUS
 *
 * Query the enable status for endc config
 *
 * "data" is NULL
 *
 * "response" is endc_status
 *
 *  Valid errors:
 *  SUCCESS
 *  RADIO_NOT_AVAILABLE
 ...
 *  INTERNAL_ERR
 */
#define RIL_REQUEST_QUERY_ENDC_STATUS 236
/*
 * RIL_REQUEST_GET_ENHANCED_RADIO_CAPABILITY
 *
 * get the QTI radio capability
 *
 * "data" is NULL
 *
 * "response" is EnhancedRadioAccessFamily
 *
 *  Valid errors:
 *  SUCCESS
 *  RADIO_NOT_AVAILABLE
 ...
 *  INVALID_ARGUMENTS
 */
#define RIL_REQUEST_GET_ENHANCED_RADIO_CAPABILITY 237
 ...
 *
 */
#define RIL_REQUEST_ENABLE_VONR 238
 ...
 *
 */
#define RIL_REQUEST_IS_VONR_ENABLED 239
 ...
 * "data" is NULL
 *
 * "response" is RIL_PhoneCapability type
 *
 * Valid errors:
 * SUCCESS
 * RADIO_NOT_AVAILABLE
 ...
 *
 */
#define RIL_REQUEST_CONFIG_GET_PHONE_CAPABILITY 240

/*
 * RIL_REQUEST_GET_CELL_BARRING_INFO
 *
 * get the current cell barring info
 *
 * "data" is NULL
 *
 * "response" is RilCellBarringInfo_t
 *
 *  Valid errors:
 *  SUCCESS
 *  RADIO_NOT_AVAILABLE
 ...
 *  INVALID_ARGUMENTS
 */
#define RIL_REQUEST_GET_CELL_BARRING_INFO 241

/*
 * RIL_REQUEST_IS_FEATURE_SUPPORTED
 *
 * Query modem/RIL if a feature is supported
 *
 * "data" is particular feature integer
 *
 * "response" is boolean
 * true for feature supported
 * false for feature not supported
 *
 * Valid errors:
 * SUCCESS
 * INVALID_ARGUMENTS
 * RADIO_NOT_AVAILABLE
 */
#define RIL_REQUEST_IS_FEATURE_SUPPORTED 242
 ...
 * This indication is deprecated; use ril_voice.h indication
 ...
 * RIL_UNSOL_ON_SS
 *
 * This indication is deprecated; use ril_call.h indication
...
*/
#define RIL_UNSOL_LPA_ADD_PROF_PROG_IND 1058

/**
* LPA_UNSOL_HTTP_TXN_IND
*/
#define RIL_UNSOL_LPA_HTTP_TXN_IND 1059

/**
* RIL_UNSOL_SLICING_CONFIG_CHANGED_IND
*/
#define RIL_UNSOL_SLICING_CONFIG_CHANGED_IND 1060
/**
* RIL_UNSOL_UNTHROTTLE_APN_TD
*/
#define RIL_UNSOL_UNTHROTTLE_APN_TD 1061
...
/* Maximum length for a given IMEI */
#define IMEI_MAX_LEN 32
#define IMEI_MAX_BUFFER_LEN (IMEI_MAX_LEN + 1)

/*
 * @public
 * RIL_ImeiType
 * Defines whether the IMEI is of the primary or secondary SIM slot
 */
typedef enum ImeiType {
  IMEI_TYPE_INVALID,
  IMEI_TYPE_PRIMARY,
  IMEI_TYPE_SECONDARY,
} RIL_ImeiType;
 ...
 * @public
 * RIL_ImeiInfo
 * Contains information returned from modem about IMEI
 * Including the type (primary or secondary SIM slot),
 * IMEI string, and the length of the IMEI string.
 */
typedef struct {
  RIL_ImeiType imeiType;           // Type of IMEI

  char imei[IMEI_MAX_BUFFER_LEN];  // IMEI string
  uint8_t imeiLen;                 // Length of the IMEI string
} RIL_ImeiInfo;
 ...
 *                          2) dataProfileInfo, RIL_DataProfileInfo
...
typedef struct {
    RIL_RadioAccessNetworks accessNetwork;
    RIL_DataProfileInfo profileInfo;
...
typedef struct {
  uint8_t precedence;
  rilPdpProtocolType ipType;
  uint8_t sscMode;
  size_t sliceInfoSize;
  RIL_SliceInfo sliceInfos[MAX_SLICE_INFO];
  size_t dnnSize;
  rilDnn dnnList[MAX_DNN_LISTS];
} rilRouteSelectorDescriptor;

typedef struct {
  uint8_t precedence;
  size_t tdSize;
  RIL_TrafficDescriptor tdList[MAX_TRAFFIC_DESCRIPTORS];
  size_t routeSelectorSize;
  rilRouteSelectorDescriptor routeSelectors[MAX_ROUTE_SELECTORS];
} rilUrspRule;

typedef struct {
  size_t UsrpRuleSize;
  rilUrspRule ursprules[MAX_URSP_RULES];
  size_t sliceInfoSize;
  RIL_SliceInfo sliceInfos[MAX_SLICE_INFO];
} rilSlicingConfig;
