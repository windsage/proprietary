...
void QtiSharedMutex::lock() {
  ...
  if (!lock_acquired) {
    ...
    if (QtiMutexDefaults::getInstance().getDeadlockDebugLevel()
            != DeadlockDebugLevel::NONE && holder) {
        auto& os = QtiMutexDefaults::getInstance().getOutputStream();
        holder->dump(os);
        QtiMutexDefaults::getInstance().releaseOutputStream(os);
    }
    deadlock_action();
  } else {
    lock_count++;
...
void QtiSharedMutex::lock_shared() {
  ...
  if (!lock_acquired) {
    ...
    if (QtiMutexDefaults::getInstance().getDeadlockDebugLevel()
            != DeadlockDebugLevel::NONE && holder) {
        holder->retrieveSharedLockBacktrace();
    }
  }
...
void QtiSharedMutex::unlock() {
  ...
  //atomic post decrement and return the value held previously
  if (lock_count.fetch_sub(1, std::memory_order_relaxed) == 0) {
    abort(); // calling unlock when not holding the lock
  }
...
void QtiSharedMutex::unlock_shared() {
  ...
  if (QtiMutexDefaults::getInstance().getDeadlockDebugLevel()
        != DeadlockDebugLevel::NONE && holder) {
    holder->releaseSharedLockBacktrace();
  }
