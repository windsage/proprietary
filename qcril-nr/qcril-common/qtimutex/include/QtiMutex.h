...
#ifndef __QTIMUTEX_H_
#define __QTIMUTEX_H_
...
namespace qtimutex {
  ...
  struct QtiMutexDefaults {
    public:
      static constexpr int BACKTRACE_LEN = 10;
      static const constexpr char* DebugFileNamePrex = "/data/vendor/radio/deadlocklog";
      ...
      bool getDebugFlag();
      void setDeadlockDebugLevel(DeadlockDebugLevel level);
      DeadlockDebugLevel getDeadlockDebugLevel();
    ...
    private:
      ...
      bool debug_flag;
      DeadlockDebugLevel deadlock_debug_level;
  };
  ...
  struct QtiRecursiveMutex
  {
    public:
      QtiRecursiveMutex(){
        holder = std::make_unique<QtiRecursiveMutexHolder>(this);
        lock_count = 0;
      }
      void lock();
      bool try_lock();
    ...
    private:
      ...
      std::unique_ptr<QtiRecursiveMutexHolder> holder;
  ...
  struct QtiSharedMutex
  {
    public:
      QtiSharedMutex() {
        holder = std::make_unique<QtiSharedMutexHolder>(this);
        lock_count = 0;
      }
    ...
    private:
      ...
      std::unique_ptr<QtiSharedMutexHolder> holder;
      std::atomic<int> lock_count;
  };
