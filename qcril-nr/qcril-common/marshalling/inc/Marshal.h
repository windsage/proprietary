...
#define RUN_AND_CHECK_RELEASE(action,arg,...)              \
  (({                                      \
    Marshal::Result __ret = action(arg,##__VA_ARGS__);          \
    if (__ret != Marshal::Result::SUCCESS) { \
      Marshal::release(arg,##__VA_ARGS__);                          \
      return __ret;                          \
    }                                      \
  }))
...
class Marshal {
    ...
    public:
        ...
        template <typename T>
        Result write (const std::shared_ptr<T> &ptr) {

            WRITE_AND_CHECK((bool)ptr);
            if (ptr) {
                WRITE_AND_CHECK(*ptr);
            }
            return Result::SUCCESS;
        ...
        template <typename T>
        Result read (std::shared_ptr<T> &ptr) const {
            bool valid;
            READ_AND_CHECK(valid);
            if (valid) {
                T t;
        ...
        template <typename T>
        Result write (const std::optional<T> &opt) {
            WRITE_AND_CHECK((bool)opt);
            if (opt) {
                WRITE_AND_CHECK(*opt);
            }
            return Result::SUCCESS;
        ...
        template <typename T>
        Result read (std::optional<T> &out) const {
            bool opt;
            READ_AND_CHECK(opt);
            if (opt) {
                T t;
    ...
    public:
        ...
        Result readNoSwap(uint64_t& output) const {
            memcpy(&output,(const void*)(mData.c_str() + current_pos), sizeof(output));
            current_pos += sizeof(output);
            return Result::SUCCESS;
        }

        uint64_t fromNetworkRead(uint64_t value) const{
            return FROM_NW64(value);
        }

        Result rawRead(uint64_t& output) const{
            uint64_t tmp;
            Result r = readNoSwap(tmp);
            if (r == Result::SUCCESS) {
                tmp = fromNetworkRead(tmp);
                output = tmp;
            }
            return r;
        }

        Result read(uint64_t& output) const{
            if (RIL_likely(isEnoughToRead(sizeof(uint64_t))))
                return rawRead(output);
            return Result::NOT_ENOUGH_DATA;
        }
         ...
         */
        template <typename T>
        Result release(T *&arg, size_t sz) const;
...
inline Marshal::Result Marshal::read<char>(char *&out) const{
    ...
    if (inlen == -1) {
        ...
        length = inlen;
        // Check length before allocation
        if (isEnoughToRead(length)) {
            val = new(std::nothrow) char[length + 1]{};
            if (!val) return Result::FAILURE;
            RUN_AND_CHECK_RELEASE(read, val, length);
            out = val;
        }
        else {
            res = Result::NOT_ENOUGH_DATA;
        }
    }
    return res;
}
...
template <typename T>
inline Marshal::Result Marshal::release(T *&arg, size_t sz) const {
    if (arg) {
        for (size_t i = 0; i < sz; i++) {
            release(arg[i]);
        }
...
inline Marshal::Result Marshal::readAndAlloc(char *(&arg), std::size_t &sz) const {
    ...
    /* We already returned if size is negative, so it is guaranteed to be
     * positive when it is casted from int32_t to size_t here.
     */
...
inline Marshal::Result Marshal::readAndAlloc(T *&arg, size_t &outlen) const {
    ...
    /* Sanity check to prevent large allocations.
     * If there aren't at least len bytes available,
     * there definitely won't be sizeof(T) * len
     * bytes available. This way, we check size
     * regardless of type.
     */
    if (!isEnoughToRead((size_t)len))
        return Result::NOT_ENOUGH_DATA;
