...
template <>
Marshal::Result Marshal::release<RIL_CallForwardData>(RIL_CallForwardData& arg)
{
  for (size_t i = 0; i < arg.numValidIndexes && i < NUM_SERVICE_CLASSES; i++) {
...
Marshal::Result Marshal::write<RIL_StkCcSsInfo>(const RIL_StkCcSsInfo& arg)
{
  ...
  if ((arg.requestType == SS_INTERROGATION) &&
      (arg.serviceType == SS_CFU || arg.serviceType == SS_CF_BUSY ||
       arg.serviceType == SS_CF_NO_REPLY || arg.serviceType == SS_CF_NOT_REACHABLE ||
       arg.serviceType == SS_CF_ALL || arg.serviceType == SS_CF_ALL_CONDITIONAL)) {
    // CF
