...
template <>
Marshal::Result Marshal::release<RIL_CDMA_SignalInfoRecord>(RIL_CDMA_SignalInfoRecord& arg) {
  release(arg.isPresent);
  release(arg.signalType);
  release(arg.alertPitch);
  release(arg.signal);
  return Result::SUCCESS;
}
