...
template <>
Marshal::Result Marshal::release<RIL_KeepaliveRequest>(RIL_KeepaliveRequest &arg)
{
    release(arg.type);
    release(arg.sourceAddress);
    release(arg.sourcePort);
    release(arg.destinationAddress);
    release(arg.destinationPort);
    release(arg.maxKeepaliveIntervalMillis);
    release(arg.cid);
    return Result::SUCCESS;
}