...
template <>
Marshal::Result Marshal::release<RIL_IMS_CallFwdTimerInfo>(RIL_IMS_CallFwdTimerInfo& arg) {
  release(arg.year);
  release(arg.month);
  release(arg.day);
  release(arg.hour);
...
Marshal::Result Marshal::release<RIL_IMS_CallForwardInfo>(RIL_IMS_CallForwardInfo& arg) {
  ...
  if (arg.hasCallFwdTimerStart) {
    release(arg.callFwdTimerStart);
  }
  release(arg.hasCallFwdTimerStart);
  if (arg.hasCallFwdTimerEnd) {
    release(arg.callFwdTimerEnd);
  }
  release(arg.hasCallFwdTimerEnd);
  return Result::SUCCESS;
}
