...
static void qcril_qmi_imsa_store_service_status_cl(
    ...
    uint8_t dc_service_status_valid,
    imsa_service_status_enum_v01 dc_service_status,
    uint8_t dc_service_rat_valid,
...
void qcril_qmi_imsa_cleanup()
{
   ...
   qcril_qmi_imsa_store_service_status_cl(
       ...
       TRUE, IMSA_NO_SERVICE_V01,  // DATA CHANNEL SERVICE_STATUS
       FALSE, IMSA_WWAN_V01);      // DATA CHANNEL SERVICE_RAT
   ...
   qcril_qmi_imsa_info.ims_srv_status.dc_service_status_valid = FALSE;
   ...
   qcril_qmi_imsa_info.ims_srv_status.dc_service_rat_valid = FALSE;
...
void qcril_qmi_imsa_geo_loc_status_ind_hdlr(void *ind_data_ptr)
{
   ...
   if (NULL != ind_msg_ptr)
   {
       QCRIL_LOG_INFO("lat_v: %c lat: %g long_v: %c long: %g",
                      ...
                      PII(ind_msg_ptr->latitude),
                      ind_msg_ptr->longitude_valid ? 'Y' : 'N',
                      PII(ind_msg_ptr->longitude));
...
)
{
    ...
    if (dc_service_status_valid)
    {
      qcril_qmi_imsa_info.ims_srv_status.dc_service_status_valid = dc_service_status_valid;
      qcril_qmi_imsa_info.ims_srv_status.dc_service_status = dc_service_status;
    }

    if (dc_service_rat_valid)
    {
      qcril_qmi_imsa_info.ims_srv_status.dc_service_rat_valid = dc_service_rat_valid;
      qcril_qmi_imsa_info.ims_srv_status.dc_service_rat = dc_service_rat;
    }
    // IMSA REFACTOR TEST
    qcril_qmi_imsa_broadcast_service_status(qcril_qmi_imsa_info);
...
void qcril_qmi_imsa_service_status_ind_hdlr(void *ind_data_ptr)
{
    ...
    if (NULL != service_ind_msg_ptr)
    {
        ...
        QCRIL_LOG_ESSENTIAL("DC: service_status(%s): %d, rat(%s): %d",
            service_ind_msg_ptr->dc_service_status_valid ? "valid": "not valid",
            service_ind_msg_ptr->dc_service_status,
            service_ind_msg_ptr->dc_service_rat_valid ? "valid": "not valid",
            service_ind_msg_ptr->dc_service_rat);
        ...
        qcril_qmi_imsa_store_service_status_cl(
            ...
            service_ind_msg_ptr->callcomposer_service_rat,
            service_ind_msg_ptr->dc_service_status_valid,
            service_ind_msg_ptr->dc_service_status,
            service_ind_msg_ptr->dc_service_rat_valid,
            service_ind_msg_ptr->dc_service_rat);
...
)
{
  ...
  do {
    ...
    if (resp_msg_ptr->resp.result == QMI_RESULT_SUCCESS_V01)
    {
      ...
      QCRIL_LOG_ESSENTIAL("DC: service_status(%s): %d, rat(%s): %d",
          resp_msg_ptr->dc_service_status_valid ? "valid": "not valid",
          resp_msg_ptr->dc_service_status,
          resp_msg_ptr->dc_service_rat_valid ? "valid": "not valid",
          resp_msg_ptr->dc_service_rat);
      ...
      qcril_qmi_imsa_store_service_status_cl(
          ...
          resp_msg_ptr->callcomposer_service_rat,
          resp_msg_ptr->dc_service_status_valid,
          resp_msg_ptr->dc_service_status,
          resp_msg_ptr->dc_service_rat_valid,
          resp_msg_ptr->dc_service_rat);
...
)
{
  ...
  if (msg != nullptr) {
    do {
      ...
      QCRIL_LOG_INFO(" Query Line MSISDN : %s ", PII(msisdn.c_str()));
