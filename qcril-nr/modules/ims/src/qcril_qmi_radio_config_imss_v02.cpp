...
)
{
  ...
  do
  {
    ...
    // If IMS_SETTINGS_OMADM_SERVICES_ENABLE_BIT_MASK_V01 is set then modem will check for below
    // masks and take action of enable/disable so to disable below masks no need to remove
    ...
    //
    // Other OMADM masks are independent and no need to set
    // IMS_SETTINGS_OMADM_SERVICES_ENABLE_BIT_MASK_V01 to enable/disable.
    switch(config_ptr->config_item)
    {
      ...
      case QCRIL_QMI_RADIO_CONFIG_IMS_DATA_CHANNEL_PREFERENCE:
        qmi_req.data_channel_config_enabled_valid = TRUE;
        qmi_req.data_channel_config_enabled = *((bool *) config_ptr->config_item_value);
        QCRIL_LOG_INFO("Set config ims data channel_enabled to: %d",
                        qmi_req.data_channel_config_enabled);
        break;

      case QCRIL_QMI_RADIO_CONFIG_VOLTE_PROVISIONING_RESTRICT_HOME:
        qmi_req.service_mask_by_network_enabled_valid = TRUE;
        service_mask = (*((bool*)config_ptr->config_item_value)) ?
            (service_mask | IMS_SETTINGS_OMADM_SERVICES_VOLTE_RESTRICT_BIT_MASK_V01) :
            (service_mask & ~(IMS_SETTINGS_OMADM_SERVICES_VOLTE_RESTRICT_BIT_MASK_V01));
        qmi_req.service_mask_by_network_enabled = service_mask;
        QCRIL_LOG_INFO("Set config CLIENT PROVISIONING enable_volte_restrict_home to: %" PRIu64,
                        qmi_req.service_mask_by_network_enabled);
        break;

      case QCRIL_QMI_RADIO_CONFIG_VOLTE_PROVISIONING_RESTRICT_ROAMING:
        qmi_req.service_mask_by_network_enabled_valid = TRUE;
        service_mask = (*((bool*)config_ptr->config_item_value)) ?
            (service_mask | IMS_SETTINGS_OMADM_SERVICES_VOLTE_RESTRICT_ROAMING_BIT_MASK_V01) :
            (service_mask & ~(IMS_SETTINGS_OMADM_SERVICES_VOLTE_RESTRICT_ROAMING_BIT_MASK_V01));
        qmi_req.service_mask_by_network_enabled = service_mask;
        QCRIL_LOG_INFO("Set config CLIENT PROVISIONING enable_volte_restrict_roaming to: %" PRIu64,
                        qmi_req.service_mask_by_network_enabled);
        break;
  ...
  do
  {
    ...
    if(qmi_resp->resp.result == QMI_RESULT_SUCCESS_V01)
    {
      ...
      switch(config_ptr->config_item)
      {
        ...
        case QCRIL_QMI_RADIO_CONFIG_IMS_DATA_CHANNEL_PREFERENCE:
          if (qmi_resp->data_channel_config_enabled_valid)
          {
            int_value = qmi_resp->data_channel_config_enabled;
            config_params.config_item_value_len = sizeof(int_value);
            config_params.config_item_value = (void*) &int_value;
            QCRIL_LOG_INFO("get ims data channel_config_enabledvalue: %d",
                            int_value);
          }
          else
          {
        ...
        case QCRIL_QMI_RADIO_CONFIG_VOLTE_PROVISIONING_RESTRICT_HOME:
          ...
          {
            bool_value = (qmi_resp->service_mask_by_network_enabled &
                IMS_SETTINGS_OMADM_SERVICES_VOLTE_RESTRICT_BIT_MASK_V01) ? TRUE : FALSE;
            config_params.config_item_value_len = sizeof(bool_value);
            config_params.config_item_value = (void*)&bool_value;
            QCRIL_LOG_INFO("get CLIENT PROVISIONING enable_volte_restrict_home value: %d",
                           bool_value);
          }
          else
          {
        ...
        case QCRIL_QMI_RADIO_CONFIG_VOLTE_PROVISIONING_RESTRICT_ROAMING:
          if (qmi_resp->service_mask_by_network_enabled_valid) {
            bool_value = (qmi_resp->service_mask_by_network_enabled &
                IMS_SETTINGS_OMADM_SERVICES_VOLTE_RESTRICT_ROAMING_BIT_MASK_V01) ? TRUE : FALSE;
            config_params.config_item_value_len = sizeof(bool_value);
            config_params.config_item_value = (void*)&bool_value;
            QCRIL_LOG_INFO("get CLIENT PROVISIONING enable_volte_restrict_roaming value: %d",
                           bool_value);
          }
          else
          {
