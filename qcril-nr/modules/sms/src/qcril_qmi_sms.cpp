...
static qcril::interfaces::RegState ims_reg_state = qcril::interfaces::RegState::UNKNOWN;
static RIL_RadioTechnology ims_radio_tech = RADIO_TECH_UNKNOWN;
...
)
{
  ...
  /* Check if the length exceeds the maximum
     Received smsc length should be less than or equal to (WMS_SMSC_ADDRESS_LENGTH_MAX_V01 + 1).
     Added + 1 for smsc length byte.
     WMS_SMSC_ADDRESS_LENGTH_MAX_V01 = TOA + SMSC received smsc = "SmscLen + TOA + SMSC */
  if ( (smsc != NULL) && ((strlen( smsc )/2 ) > (WMS_SMSC_ADDRESS_LENGTH_MAX_V01 + 1) ) )
  {
...
/*===========================================================================
  ...
  FUNCTION:  qcril_sms_update_ims_registration_state

===========================================================================*/
void qcril_sms_update_ims_registration_state
(
...
)
{
  ...
  if(msg != nullptr) {
    ...
    SMS_CACHE_LOCK;
    if (reg->hasState()) {
      ims_reg_state = reg->getState();
      QCRIL_LOG_ERROR("ims reg state - %d", ims_reg_state);
    }

    if (reg->hasRadioTechnology()) {
      ims_radio_tech = reg->getRadioTechnology();
      QCRIL_LOG_ERROR("ims radio tech - %d", ims_radio_tech);
    }
    SMS_CACHE_UNLOCK;
  }
  QCRIL_LOG_FUNC_RETURN();
  ...
  */
  ...
  if (is_processing_ok)
  {
    ...
    if (qcril_sms_check_if_gw_msg_is_status_report(&event_report_ind->transfer_route_mt_message, message_ref))
    {
      ...
      /* If indication comes earlier than response, no chance to set mo_sms_on_ims_map to be true before here*/
      if (is_on_ims || i_ptr->sms_ack_info.gw_send_ack_on_ims)
      {
        auto msg = std::make_shared<RilUnsolNewImsSmsStatusReportMessage>(
...
)
{
  ...
  if (event_report_ind->transfer_route_mt_message_valid)
  {
    ...
    else if (event_report_ind->transfer_route_mt_message.format == WMS_MESSAGE_FORMAT_GW_BC_V01)
    {
      ...
      QCRIL_LOG_INFO("CB SMS Message RAT %d", (int) rat);
      ...
      /* In case of GSM, notify telephony without processing. */
      if(rat != MultipartGsmCmasCellbroadcast::RadioTech::GSM) {
        ...
        auto msg = std::make_shared<RilUnsolNewBroadcastSmsMessage>(std::move(cmas_qmi_buffer));
        Dispatcher::getInstance().dispatchSync(msg);
      }
    }
  }
