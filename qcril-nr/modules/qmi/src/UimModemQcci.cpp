...
std::unordered_map<uint16_t, uint8_t> Prov_Session_type;
...
)
{
  ...
  if(qmi_response->resp.result == QMI_RESULT_SUCCESS_V01)
  {
    ...
    if (qmi_response->mep_information_valid) {
      for (uint8_t index = 0;
           index < qmi_response->mep_information.card_mep_info_len &&
           index < QMI_UIM_MAX_SLOT_COUNT;
           index++) {
        rsp_data->rsp_data.get_card_status_rsp.card_status.card[index]
            .card_mep_info_len =
            qmi_response->mep_information.card_mep_info_len;
        rsp_data->rsp_data.get_card_status_rsp.card_status.card[index]
            .card_mep_info.is_mep =
            qmi_response->mep_information.card_mep_info[index].is_mep;
        rsp_data->rsp_data.get_card_status_rsp.card_status.card[index]
            .card_mep_info.port_id =
            qmi_response->mep_information.card_mep_info[index].port_id;
        rsp_data->rsp_data.get_card_status_rsp.card_status.card[index]
            .card_mep_info.negotiated_mep_mode =
            (qmi_uim_mep_mode)qmi_response->mep_information.card_mep_info[index]
                .negotiated_mep_mode;
      }
    }

    if (qmi_response->unlock_info_valid) {
      QCRIL_LOG_DEBUG("Unlock Status Received from Modem");
      for (int i = 0; i < qmi_response->unlock_info.card_info_len; i++) {
        for (int j = 0; j < qmi_response->unlock_info.card_info[i].app_info_len;
             j++) {
          rsp_data->rsp_data.get_card_status_rsp.card_status.card[i]
              .application[j]
              .unlock_status_type =
              (qmi_uim_unlock_status_type)qmi_response->unlock_info.card_info[i]
                  .app_info[j]
                  .unlock_status;
        }
      }
...
)
{
  ...
  {
    rsp_data->qmi_err_code = QMI_NO_ERR;
    rsp_data->rsp_data.change_provision_resp.session_type = session_type;
  }
  else
  {
    QCRIL_LOG_ERROR( "response error: 0x%x", qmi_response->resp.error);
...
/*===========================================================================
  FUNCTION  qmi_uim_get_sim_type_response
===========================================================================*/
/*!
@brief
  translates response data coming from QCCI into response data
...
*/
/*=========================================================================*/
static void qmi_uim_get_sim_type_response
(
  const uim_get_sim_type_resp_msg_v01   * qmi_response,
  qmi_uim_rsp_data_type                    * rsp_data
)
{
  if(qmi_response == NULL || rsp_data == NULL)
  ...
  rsp_data->rsp_id = QMI_UIM_SRVC_GET_SIM_TYPE_RSP_MSG;
  if(qmi_response->resp.result == QMI_RESULT_SUCCESS_V01)
  {
    rsp_data->qmi_err_code = QMI_NO_ERR;
    if(qmi_response->sim_type_valid)
    {
        rsp_data->rsp_data.sim_type_resp.sim_type_len = qmi_response->sim_type_len;
        for(int i = 0;i<rsp_data->rsp_data.sim_type_resp.sim_type_len;i++)
        {
          rsp_data->rsp_data.sim_type_resp.sim_type[i] = (qmi_uim_sim_type)qmi_response->sim_type[i];
        }
        if(qmi_response->supported_sim_types_valid)
        {
            rsp_data->rsp_data.sim_type_resp.supported_sim_type_len =
                                             qmi_response->supported_sim_types_len;
            for(int i = 0;i<rsp_data->rsp_data.sim_type_resp.supported_sim_type_len;i++)
            {
                rsp_data->rsp_data.sim_type_resp.supported_sim_type[i] =
                                             (qmi_uim_supported_sim_type) qmi_response->supported_sim_types[i];
            }
        }
    }
  }
...
} /* qmi_uim_get_sim_type_response */

/*===========================================================================
  FUNCTION  qmi_uim_set_sim_type_response
===========================================================================*/
/*!
@brief
  translates response data coming from QCCI into response data
...
*/
/*=========================================================================*/
static void qmi_uim_set_sim_type_response
(
  const uim_set_sim_type_resp_msg_v01   * qmi_response,
  qmi_uim_rsp_data_type                    * rsp_data
)
{
  if(qmi_response == NULL || rsp_data == NULL)
  ...
  rsp_data->rsp_id = QMI_UIM_SRVC_SET_SIM_TYPE_RSP_MSG;
...
)
{
  ...
  if(qmi_response->resp.result == QMI_RESULT_SUCCESS_V01)
  {
    ...
    if (qmi_response->eSIM_Port_valid) {
      rsp_data->rsp_data.profile_info_rsp.eSIM_Port = qmi_response->eSIM_Port;
    }
  }
  else
  {
...
)
{
  ...
  if (qmi_response->port_information_valid &&
      qmi_response->port_information_len > 0) {
    rsp_data->rsp_data.slots_status_rsp.port_information_len =
        qmi_response->port_information_len;
    for (uint8_t i = 0;
         i < qmi_response->port_information_len && i < QMI_UIM_MAX_SLOT_COUNT;
         i++) {
      rsp_data->rsp_data.slots_status_rsp.port_information[i].is_mep =
          qmi_response->port_information[i].is_mep;
      QCRIL_LOG_ERROR("is_mep value is %d",
                      qmi_response->port_information[i].is_mep);

      rsp_data->rsp_data.slots_status_rsp.port_information[i]
          .negotiated_mep_mode =
          (qmi_uim_mep_mode)qmi_response->port_information[i]
              .negotiated_mep_mode;
      QCRIL_LOG_ERROR("mep_mode  is %d",
                      rsp_data->rsp_data.slots_status_rsp.port_information[i]
                          .negotiated_mep_mode);

      rsp_data->rsp_data.slots_status_rsp.port_information[i]
          .max_jointly_supported_lsi_for_enabled_profiles =
          qmi_response->port_information[i]
              .max_jointly_supported_lsi_for_enabled_profiles;
      QCRIL_LOG_ERROR("max_jointly_supported_lsi_for_enabled_profiles  %d",
                      rsp_data->rsp_data.slots_status_rsp.port_information[i]
                          .max_jointly_supported_lsi_for_enabled_profiles);

      rsp_data->rsp_data.slots_status_rsp.port_information[i]
          .uim_port_status_len =
          qmi_response->port_information[i].uim_port_status_len;

      QCRIL_LOG_ERROR("uim_port_status_len %d",
                      rsp_data->rsp_data.slots_status_rsp.port_information[i]
                          .uim_port_status_len);

      for (uint8_t j = 0;
           j < rsp_data->rsp_data.slots_status_rsp.port_information[i]
                   .uim_port_status_len;
           j++) {
        rsp_data->rsp_data.slots_status_rsp.port_information[i]
            .uim_port_status[j]
            .port_state =
            (qmi_uim_euicc_port_state)qmi_response->port_information[i]
                .uim_port_status[j]
                .port_state;

        QCRIL_LOG_ERROR("port_state is %d",
                        rsp_data->rsp_data.slots_status_rsp.port_information[i]
                            .uim_port_status[j]
                            .port_state);
        ...
        rsp_data->rsp_data.slots_status_rsp.port_information[i]
            .uim_port_status[j]
            .card_protocol =
            (qmi_uim_card_protocol_type)qmi_response->port_information[i]
                .uim_port_status[j]
                .lsi_protocol;
        QCRIL_LOG_ERROR("card_protocol is %d",
                        rsp_data->rsp_data.slots_status_rsp.port_information[i]
                            .uim_port_status[j]
                            .card_protocol);
        ...
        rsp_data->rsp_data.slots_status_rsp.port_information[i]
            .uim_port_status[j]
            .num_app =
            qmi_response->port_information[i].uim_port_status[j].num_app;
        QCRIL_LOG_ERROR("num_app is %d",
                        rsp_data->rsp_data.slots_status_rsp.port_information[i]
                            .uim_port_status[j]
                            .num_app);
        ...
        rsp_data->rsp_data.slots_status_rsp.port_information[i]
            .uim_port_status[j]
            .logical_slot = (qmi_uim_slot_type)qmi_response->port_information[i]
                                .uim_port_status[j]
                                .logical_slot;
        QCRIL_LOG_ERROR("logical_slot is %d",
                        rsp_data->rsp_data.slots_status_rsp.port_information[i]
                            .uim_port_status[j]
                            .logical_slot);
        ...
        rsp_data->rsp_data.slots_status_rsp.port_information[i]
            .uim_port_status[j]
            .iccid_len =
            MIN(qmi_response->port_information[i].uim_port_status[j].iccid_len,
                QMI_UIM_ICCID_LEN_MAX_V01);
        QCRIL_LOG_ERROR("iccid_len is %d",
                        rsp_data->rsp_data.slots_status_rsp.port_information[i]
                            .uim_port_status[j]
                            .iccid_len);

        if (qmi_response->port_information[i].uim_port_status[j].iccid_len) {
          memcpy(
              rsp_data->rsp_data.slots_status_rsp.port_information[i]
                  .uim_port_status[j]
                  .iccid,
              qmi_response->port_information[i].uim_port_status[j].iccid,
              qmi_response->port_information[i].uim_port_status[j].iccid_len);
        }
      }
    }
  }
...
)
{
  if(ind_data_ptr != NULL)
  {
    ...
    if (ind_data_ptr->mep_information_valid) {
      for (uint8_t index = 0;
           index < ind_data_ptr->mep_information.card_mep_info_len; index++) {
        ind_data->status_change_ind.card_status.card[index].card_mep_info_len =
            ind_data_ptr->mep_information.card_mep_info_len;
        QCRIL_LOG_DEBUG("get_card_status_indication card_mep_info_len  %d",
                        ind_data->status_change_ind.card_status.card[index]
                            .card_mep_info_len);

        ind_data->status_change_ind.card_status.card[index]
            .card_mep_info.is_mep =
            ind_data_ptr->mep_information.card_mep_info[index].is_mep;
        QCRIL_LOG_DEBUG("get_card_Status is_mep  %d",
                        ind_data->status_change_ind.card_status.card[index]
                            .card_mep_info.is_mep);

        ind_data->status_change_ind.card_status.card[index]
            .card_mep_info.port_id =
            ind_data_ptr->mep_information.card_mep_info[index].port_id;
        QCRIL_LOG_DEBUG("get_card_Status  port_id  %d",
                        ind_data->status_change_ind.card_status.card[index]
                            .card_mep_info.port_id);

        ind_data->status_change_ind.card_status.card[index]
            .card_mep_info.negotiated_mep_mode =
            (qmi_uim_mep_mode)ind_data_ptr->mep_information.card_mep_info[index]
                .negotiated_mep_mode;
        QCRIL_LOG_DEBUG("get_card_Status negotiated_mep_mode  %d",
                        ind_data->status_change_ind.card_status.card[index]
                            .card_mep_info.negotiated_mep_mode);
      }
    }
    if (ind_data_ptr->unlock_info_valid) {
      QCRIL_LOG_DEBUG("Unlock Status Received from Modem");
      for (int i = 0; i < ind_data_ptr->unlock_info.card_info_len; i++) {
        for (int j = 0; j < ind_data_ptr->unlock_info.card_info[i].app_info_len;
             j++) {
          ind_data->status_change_ind.card_status.card[i]
              .application[j]
              .unlock_status_type =
              (qmi_uim_unlock_status_type)ind_data_ptr->unlock_info.card_info[i]
                  .app_info[j]
                  .unlock_status;
        }
      }
...
)
{
  ...
  QCRIL_LOG_ERROR("%s", "qcril_qmi_uim_slot_status_change_ind_hdlr");
  ...
  if(ind_data_ptr != NULL && ind_data != NULL)
  {
    ...
    /*TLV port_information valid will be set only for MEP cards*/
    if (ind_data_ptr->port_information_valid &&
        ind_data_ptr->port_information_len > 0) {
      ind_data->port_information_len = ind_data_ptr->port_information_len;
      for (uint8_t i = 0;
           i < ind_data_ptr->port_information_len && i < QMI_UIM_MAX_SLOT_COUNT;
           i++) {
        ind_data->port_information[i].is_mep =
            ind_data_ptr->port_information[i].is_mep;
        QCRIL_LOG_ERROR("is_mep value is %d",
                        ind_data->port_information[i].is_mep);

        ind_data->port_information[i].negotiated_mep_mode =
            (qmi_uim_mep_mode)ind_data_ptr->port_information[i]
                .negotiated_mep_mode;
        QCRIL_LOG_ERROR("mep_mode  is %d",
                        ind_data->port_information[i].negotiated_mep_mode);

        ind_data->port_information[i]
            .max_jointly_supported_lsi_for_enabled_profiles =
            ind_data_ptr->port_information[i]
                .max_jointly_supported_lsi_for_enabled_profiles;
        QCRIL_LOG_ERROR("max_jointly_supported_lsi_for_enabled_profiles  %d",
                        ind_data->port_information[i]
                            .max_jointly_supported_lsi_for_enabled_profiles);

        ind_data->port_information[i].uim_port_status_len =
            ind_data_ptr->port_information[i].uim_port_status_len;
        QCRIL_LOG_ERROR("uim_port_status_len %d",
                        ind_data->port_information[i].uim_port_status_len);

        for (uint8_t j = 0;
             j < ind_data->port_information[i].uim_port_status_len; j++) {
          ind_data->port_information[i].uim_port_status[j].port_state =
              (qmi_uim_euicc_port_state)ind_data_ptr->port_information[i]
                  .uim_port_status[j]
                  .port_state;
          QCRIL_LOG_ERROR(
              "port_state is %d",
          ...
          ind_data->port_information[i].uim_port_status[j].card_protocol =
              (qmi_uim_card_protocol_type)ind_data_ptr->port_information[i]
                  .uim_port_status[j]
                  .lsi_protocol;
          QCRIL_LOG_ERROR(
              "card_protocol is %d",
          ...
          ind_data->port_information[i].uim_port_status[j].num_app =
              ind_data_ptr->port_information[i].uim_port_status[j].num_app;
          QCRIL_LOG_ERROR(
              "num_app is %d",
              ind_data->port_information[i].uim_port_status[j].num_app);

          ind_data->port_information[i].uim_port_status[j].logical_slot =
              (qmi_uim_slot_type)ind_data_ptr->port_information[i]
                  .uim_port_status[j]
                  .logical_slot;
          QCRIL_LOG_ERROR(
              "logical_slot is %d",
              ind_data->port_information[i].uim_port_status[j].logical_slot);
          ind_data->port_information[i].uim_port_status[j].iccid_len =
              ind_data_ptr->port_information[i].uim_port_status[j].iccid_len;
          QCRIL_LOG_ERROR(
              "iccid_len is %d",
              ind_data->port_information[i].uim_port_status[j].iccid_len);

          if (ind_data_ptr->port_information[i].uim_port_status[j].iccid_len) {
            memcpy(
                ind_data->port_information[i].uim_port_status[j].iccid,
                ind_data_ptr->port_information[i].uim_port_status[j].iccid,
                ind_data_ptr->port_information[i].uim_port_status[j].iccid_len);
          }
        }
      }
...
static int qcril_qmi_uim_user_consent_ind_hdlr
(
  uim_user_consent_required_ind_msg_v01 *ind_data_ptr,
  qmi_uim_user_consent_ind_type          *qmi_data_ptr
)
{
  if(ind_data_ptr == NULL || qmi_data_ptr == NULL)
  {
      return QMI_SERVICE_ERR;
  }
  qmi_data_ptr->slot = (qmi_uim_slot_type)ind_data_ptr->slot;
  qmi_data_ptr->user_consent = (qmi_uim_user_consent_type)ind_data_ptr->user_consent_indication;
  return 0;
}
...
)
{
  ...
  qmi_data_ptr->lpa_add_profile_ind.user_consent_type_valid =
                                                  (qmi_uim_bool_type)qcci_data_ptr->user_consent_type_valid;
  qmi_data_ptr->lpa_add_profile_ind.user_consent_type =
                                     (qmi_uim_user_consent_type)qcci_data_ptr->user_consent_type;
  return 0;
    ...
    if (QMI_NO_ERR == qmi_err)/* what is going on here */
    {
      switch(msg_id)
      {
        ...
        case QMI_UIM_SLOT_STATUS_CHANGE_IND_V01:
          ...
          break;
        case QMI_UIM_USER_CONSENT_IND_V01:
           ind_id =QMI_UIM_SRVC_USER_CONSENT_IND_MSG;
           if(qcril_qmi_uim_user_consent_ind_hdlr((uim_user_consent_required_ind_msg_v01 *)decoded_payload,
                                                   &ind_data->user_consent_ind)!=0)
           {
               qcril_free(decoded_payload);
               qcril_free(ind_data);
               return;
  ...
  switch (msg_id)
  {
    ...
    case QMI_UIM_GET_EUICC_INFO2_RESP_V01:
      *rsp_id = QMI_UIM_SRVC_GET_EUICC_INFO2_RSP_MSG;
      break;
    case QMI_UIM_GET_SIM_TYPE_RESP_V01:
      *rsp_id = QMI_UIM_SRVC_GET_SIM_TYPE_RSP_MSG;
      break;
    case QMI_UIM_SET_SIM_TYPE_RESP_V01:
      *rsp_id =  QMI_UIM_SRVC_SET_SIM_TYPE_RSP_MSG;
      break;
    default:
      return QMI_INTERNAL_ERR;
  }
  ...
  else
  {
    ...
    switch (msg_id)
    {
      ...
      case QMI_UIM_SET_SERVICE_STATUS_RESP_V01:
        ...
        break;
      case QMI_UIM_CHANGE_PROVISIONING_SESSION_RESP_V01:
        /* Update the session_type based on the sent token*/
       {
           qcril_uim_user_data             user_data_1    = {};
           user_data_1.user_data = ((qmi_uim_cb_struct_type*)resp_cb_data)->user_data;
           auto sessionType = Prov_Session_type.find(user_data_1.token);
           if(sessionType != Prov_Session_type.end())
           {
               qcril_uim_qmi_conv_change_provisioning_session_resp((
                                         uim_change_provisioning_session_resp_msg_v01*)qmi_response,
                                         &rsp_data,sessionType->second);
           }
           QCRIL_LOG_ERROR("Token is %d and session_type is %d",sessionType->first,sessionType->second);
           Prov_Session_type.erase(user_data_1.token);
       }
        break;
      case QMI_UIM_GET_LABEL_RESP_V01:
      ...
      case QMI_UIM_GET_EUICC_INFO2_RESP_V01:
        ...
        break;
      case QMI_UIM_GET_SIM_TYPE_RESP_V01:
        qmi_uim_get_sim_type_response((uim_get_sim_type_resp_msg_v01 *)qmi_response,&rsp_data);
        break;
      case QMI_UIM_SET_SIM_TYPE_RESP_V01:
        qmi_uim_set_sim_type_response((uim_set_sim_type_resp_msg_v01 *)qmi_response,&rsp_data);
        break;
      default:
...
)
{
  ...
  user_data_1.user_data = user_data;
  ...
  QCRIL_LOG_ERROR("Token value for change_provisioning is %d and sessio_type is %d",
                          user_data_1.token,params->session_type);

  Prov_Session_type.insert({user_data_1.token,params->session_type});
  ...
  else
  {
    ...
    if(!rc)
    {
      qcril_uim_qmi_conv_change_provisioning_session_resp(qmi_response,
                                                          rsp_data,params->session_type);
    }
    qcril_free(qmi_response);
  }
  qcril_free(qmi_request);
...
)
{
  ...
  /* Set the optional is_es10_command for only ES10 command for ap based lpa*/

  if (params->isEs10) {
    qmi_request->is_es10_command_valid = TRUE;
    qmi_request->is_es10_command = 1;
  }
...
)
{
  ...
  if (params->isEs10) {
    qmi_request->is_es10_valid = 1;
    qmi_request->is_es10 = 1;
  }
  if (user_cb)
  {
    cb_data = (qmi_uim_cb_struct_type*)qcril_malloc(sizeof(qmi_uim_cb_struct_type));
...
)
{
  ...
  /* Setting the Port information tlv to valid to send port information during switch slot ext*/
  qmi_request->logical_slot_port_valid = 1;
                   ...
                   slot_id < QMI_UIM_EXTENDED_CARDS_MAX_V01; slot_id++)
  {
    ...
    qmi_request->logical_slot_port[slot_id] =
        (uim_physical_slot_enum_v01)params->port_id[slot_id];
  }

  qmi_request->logical_slot_len = MIN(params->num_slots, QMI_UIM_EXTENDED_CARDS_MAX_V01);
  qmi_request->logical_slot_port_len =
      MIN(params->num_slots, QMI_UIM_EXTENDED_CARDS_MAX_V01);
...
)
{
  ...
  user_consent_req.user_consent_type_valid = 1;
  user_consent_req.user_consent_type =
                    (uim_user_consent_required_enum_type_v01) params_ptr->user_consent_type;
...
/*===========================================================================
  FUNCTION  qcril_qmi_uim_get_sim_type
===========================================================================*/
/*!
@brief
  Uim get_sim_type
...
*/
/*=========================================================================*/
EXTERN int qcril_qmi_uim_get_sim_type
(
  qmi_client_type                                         client_handle,
  qmi_uim_user_async_cb_type                              user_cb,
  void                                                    * user_data,
...
)
{
  qmi_txn_handle                                txn_handle;
  int                                           rc               = 0;
  uim_get_sim_type_req_msg_v01                  get_sim_type_req;
  uim_get_sim_type_resp_msg_v01               * get_sim_type_resp_ptr     = NULL;
  qmi_uim_cb_struct_type                      * cb_data          = NULL;


  /* Allocate response pointer since it is an async command */
  QCRIL_LOG_INFO("qcril_uim_get_sim_type_uimmodemqcci");

  get_sim_type_resp_ptr = (uim_get_sim_type_resp_msg_v01 *)
                     qcril_malloc(sizeof(uim_get_sim_type_resp_msg_v01));
  if (get_sim_type_resp_ptr == NULL)
  {
    QCRIL_LOG_ERROR("error allocating memory for get_sim_type_resp_ptr");
    return QMI_SERVICE_ERR;
  }

  if (user_cb)
  {
    QCRIL_LOG_INFO("qcril_uim_get_sim_type_sending async");
    cb_data = (qmi_uim_cb_struct_type*)qcril_malloc(sizeof(qmi_uim_cb_struct_type));
    if(cb_data == NULL)
    {
      QCRIL_LOG_ERROR("failed to allocate cb_data");
      qcril_free(get_sim_type_resp_ptr);
      return QMI_SERVICE_ERR;
    }
    QMI_UIM_MAKE_CB_STRUCT(cb_data, user_cb, user_data);

    rc = qmi_client_send_msg_async( client_handle,
                                    QMI_UIM_GET_SIM_TYPE_REQ_V01,
                                    &get_sim_type_req,
                                    sizeof(uim_get_sim_type_req_msg_v01),
                                    (void *) get_sim_type_resp_ptr,
                                    sizeof(uim_get_sim_type_resp_msg_v01),
                                    qmi_uim_client_async_cb,
                                    cb_data,
                                    &txn_handle);
    if( rc != QMI_NO_ERR )
  ...
  {
    QCRIL_LOG_INFO("qcril_uim_get_sim_type_sending sync");
    rc = qmi_client_send_msg_sync( client_handle,
                                   QMI_UIM_GET_SIM_TYPE_REQ_V01,
                                   &get_sim_type_req,
                                   sizeof(uim_get_sim_type_req_msg_v01),
    ...
    if(!rc)
    {
      qmi_uim_get_sim_type_response(get_sim_type_resp_ptr, rsp_data);
    }
    qcril_free(get_sim_type_resp_ptr);
  }

  return rc;
} /* qcril_qmi_uim_get_sim_type */

/*===========================================================================
  FUNCTION  qcril_qmi_uim_set_sim_type
===========================================================================*/
/*!
@brief
  Uim get_sim_type
...
*/
/*=========================================================================*/
EXTERN int qcril_qmi_uim_set_sim_type
(
  qmi_client_type                                         client_handle,
  const qmi_uim_set_sim_type_req                                *params_ptr,
  qmi_uim_user_async_cb_type                              user_cb,
  void                                                    * user_data,
  qmi_uim_rsp_data_type                                   * rsp_data
)
{
  qmi_txn_handle                                txn_handle;
  int                                           rc = 0,i=0;
  uim_set_sim_type_req_msg_v01                  set_sim_type_req;
  uim_set_sim_type_resp_msg_v01               * set_sim_type_resp_ptr     = NULL;
  qmi_uim_cb_struct_type                      * cb_data          = NULL;

  if(params_ptr == NULL)
  {
    QCRIL_LOG_ERROR("Invalid arguments for set_sim_type_rsp");
    return QMI_SERVICE_ERR; 
  }
  /* Allocate response pointer since it is an async command */
  set_sim_type_resp_ptr = (uim_set_sim_type_resp_msg_v01 *)
                     qcril_malloc(sizeof(uim_set_sim_type_resp_msg_v01));
  if (set_sim_type_resp_ptr == NULL)
  {
    QCRIL_LOG_ERROR("error allocating memory for get_sim_type_resp_ptr");
    return QMI_SERVICE_ERR;
  }

  set_sim_type_req.sim_type_len = params_ptr->sim_type_len;

  QCRIL_LOG_ERROR("Set_sim_type length is %d" ,params_ptr->sim_type_len);

  for (i = 0;i < params_ptr->sim_type_len;i++)
  {
      set_sim_type_req.sim_type[i] = (uim_sim_type_enum_v01) params_ptr->sim_type[i];
      QCRIL_LOG_ERROR("Set_sim_type value is %d" ,params_ptr->sim_type[i]);
  }
  if (user_cb)
  {
    cb_data = (qmi_uim_cb_struct_type*)qcril_malloc(sizeof(qmi_uim_cb_struct_type));
    ...
    {
      QCRIL_LOG_ERROR("failed to allocate cb_data");
      qcril_free(set_sim_type_resp_ptr);
      return QMI_SERVICE_ERR;
    }
    QMI_UIM_MAKE_CB_STRUCT(cb_data, user_cb, user_data);

    rc = qmi_client_send_msg_async( client_handle,
                                    QMI_UIM_SET_SIM_TYPE_REQ_V01,
                                    &set_sim_type_req,
                                    sizeof(uim_set_sim_type_req_msg_v01),
                                    (void *) set_sim_type_resp_ptr,
                                    sizeof(uim_set_sim_type_resp_msg_v01),
                                    qmi_uim_client_async_cb,
                                    cb_data,
                                    &txn_handle);
    if( rc != QMI_NO_ERR )
  ...
  else
  {
    ...
    if(!rc)
    {
      qmi_uim_set_sim_type_response(set_sim_type_resp_ptr, rsp_data);
    }
    qcril_free(set_sim_type_resp_ptr);
  }
