...
)
{
  ...
  auto module = ModemEndPointFactory<UimRemoteModemEndPoint>::getInstance().buildEndPoint()->mModule;
  auto id = (static_cast<UimRemoteModemEndPointModule*>(module))->getInstanceId();
  if (id != qcril_uim_remote_convert_slot_type_to_instance_id(ind_msg_ptr->slot))
  {
    QCRIL_LOG_INFO("Indication is not for this instance: %d", id);
    return FALSE;
  }
...
{
  auto shared_msg = std::static_pointer_cast<QmiUimRemoteSetupRequest>(msg);
  mId = shared_msg->getInstanceId();
