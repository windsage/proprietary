...
class UimRmtQmiUimRemoteRespMsg : public SolicitedMessage<void>,
 ...
 public:
  static constexpr const char* MESSAGE_NAME = "UimRmtQmiUimRemoteRespMsg";
  UimRmtQmiUimRemoteRespMsg() = delete;
  ~UimRmtQmiUimRemoteRespMsg()
  {
  }

  UimRmtQmiUimRemoteRespMsg(int32_t token,
                            qmi_uim_remote_req_type req_type,
      ...
      : SolicitedMessage<void>(get_class_message_id())
  {
    ...
    Log::getInstance().d("[UimRmtQmiUimRemoteRespMsg]: response token" + std::to_string(token) +
                         "req_type: %d" + std::to_string(req_type));
  }
