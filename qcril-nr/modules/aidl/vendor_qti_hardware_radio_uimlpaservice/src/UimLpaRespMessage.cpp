...
===========================================================================*/
static uint8_t qcril_uim_lpa_iccid_to_byte(uint8_t* iccid_byte_ptr,
                                           ...
                                           uint8_t len)
{
  ...
  if (iccid_ptr == NULL || iccid_byte_ptr == NULL || len == 0 || len > 10 || iccid_byte_len == 0 ||
      len > 2 * 10) {
    return 0;
  }
...
UimLpaRespMessage::UimLpaRespMessage(uimLpaResponseType resp_id, void* data_ptr, int token)
    ...
    case UIM_LPA_USER_RESP_ID:
      if (data_ptr != nullptr) {
        ...
        qcril_uim_lpa_user_resp_type* usr_resp_ptr =
            static_cast<qcril_uim_lpa_user_resp_type*>(data_ptr);
        ...
        if (out_ptr != nullptr) {
          ...
          out_ptr->event = static_cast<qcril_uim_lpa_user_req_event_type>(usr_resp_ptr->event);

          out_ptr->result = static_cast<qcril_uim_lpa_result_type>(usr_resp_ptr->result);
          ...
          if (usr_resp_ptr->euicc_info2 != nullptr && usr_resp_ptr->euicc_info2_len != 0) {
            ...
            if (out_ptr->euicc_info2 != nullptr) {
              memcpy(out_ptr->euicc_info2, usr_resp_ptr->euicc_info2,
                     usr_resp_ptr->euicc_info2_len);
          ...
          if (usr_resp_ptr->no_of_profiles != 0 && usr_resp_ptr->profiles != nullptr) {
            ...
            if (out_ptr->profiles != nullptr) {
              ...
              for (i = 0; i < usr_resp_ptr->no_of_profiles; i++) {
                memcpy(&out_ptr->profiles[i],
                       &usr_resp_ptr->profiles[i],
                ...
                if (usr_resp_ptr->profiles[i].iccid != nullptr &&
                  ...
                  if (out_ptr->profiles[i].iccid != nullptr) {
                    out_ptr->profiles[i].iccid_len =
                        qcril_uim_lpa_iccid_to_byte(out_ptr->profiles[i].iccid,
                ...
                if (usr_resp_ptr->profiles[i].icon != nullptr &&
                  ...
                  if (out_ptr->profiles[i].icon != nullptr) {
                    memcpy(out_ptr->profiles[i].icon,
                           usr_resp_ptr->profiles[i].icon,
                ...
                if (usr_resp_ptr->profiles[i].profileName != nullptr) {
                  ...
                  if (out_ptr->profiles[i].profileName != nullptr) {
                    strlcpy(out_ptr->profiles[i].profileName,
                            usr_resp_ptr->profiles[i].profileName,
                            len + 1);
                  }
                }
                ...
                if (usr_resp_ptr->profiles[i].nickName != nullptr) {
                  ...
                  if (out_ptr->profiles[i].nickName != nullptr) {
                    strlcpy(
                        out_ptr->profiles[i].nickName, usr_resp_ptr->profiles[i].nickName, len + 1);
                  }
                }
...
UimLpaRespMessage::~UimLpaRespMessage()
{
  switch (mRespId) {
    case UIM_LPA_USER_RESP_ID:
      if (mDataPtr != nullptr) {
        qcril_uim_lpa_user_resp_type* out_ptr =
            static_cast<qcril_uim_lpa_user_resp_type*>(mDataPtr);
...
{
  return mName + "{ " + "resp_id = " + std::to_string(mRespId) + " }";
