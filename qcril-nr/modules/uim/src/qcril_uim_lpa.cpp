...
)
{
  ...
  user_consent_req.user_consent_type = (uint8_t)user_consent_type;
...
)
{
  ...
  if(ind_param_ptr->ind_data.lpa_add_profile_ind.profile_name != nullptr)
  {
   ...
   if(ind_param_ptr->ind_data.lpa_add_profile_ind.user_consent_type_valid)
   {
       switch (ind_param_ptr->ind_data.lpa_add_profile_ind.user_consent_type)
       {
          case QMI_UIM_NO_CONFIRMATION_REQD:
             add_profile_ind.user_consent_type = QCRIL_UIM_LPA_NO_USER_CONFIRMATION_REQD;
          break;
          case QMI_UIM_SIMPLE_CONFIRMATION_REQD:
            add_profile_ind.user_consent_type  = QCRIL_UIM_LPA_SIMPLE_USER_CONFIRMATION_REQD;
          break;
          case QMI_UIM_STRONG_CONFIRMATION_REQD:
            add_profile_ind.user_consent_type  = QCRIL_UIM_LPA_STRONG_USER_CONFIRMATION_REQD;
          break;
       }
   }
...
void qcril_uim_lpa_user_consent_ind
(
  const qcril_uim_indication_params_type  * ind_param_ptr
)
{
   if(ind_param_ptr == nullptr)
   {
       return;
   }
   std::shared_ptr<UimLpaIndicationMsg>        ind_msg_ptr = nullptr;

   qcril_uim_lpa_user_consent_ind_type user_consent_ind;

   memset(&user_consent_ind ,0,sizeof(qcril_uim_lpa_user_consent_ind_type));

   switch (ind_param_ptr->ind_data.user_consent_ind.user_consent)
   {
      case QMI_UIM_NO_CONFIRMATION_REQD:
         user_consent_ind.user_consent_type = QCRIL_UIM_LPA_NO_USER_CONFIRMATION_REQD;
      break;
      case QMI_UIM_SIMPLE_CONFIRMATION_REQD:
        user_consent_ind.user_consent_type = QCRIL_UIM_LPA_SIMPLE_USER_CONFIRMATION_REQD;
      break;
      case QMI_UIM_STRONG_CONFIRMATION_REQD:
        user_consent_ind.user_consent_type = QCRIL_UIM_LPA_STRONG_USER_CONFIRMATION_REQD;
      break;
   }
   ind_msg_ptr = std::make_shared<UimLpaIndicationMsg>(UIM_LPA_USER_CONSENT_IND_ID,
                                                      &user_consent_ind);
