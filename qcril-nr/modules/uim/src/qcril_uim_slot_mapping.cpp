...
)
{
  ...
  for (auto it = slot_status.begin(); it != slot_status.end(); it++)
  {
    ...
    if (it->port_info.is_mep) {
      QCRIL_LOG_ERROR(
          "%d is_mep in qcril_uim_util_get_number_of_logically_active_slots ",
          it->port_info.is_mep);
      for (uint8_t i = 0; i < it->port_info.uim_port_status_len; i++) {
        if (it->port_info.uim_port_status[i].port_state ==
            RIL_UIM_PORT_STATE_ACTIVE) {
          QCRIL_LOG_ERROR("%d is_mep in "
                          "qcril_uim_util_get_number_of_logically_active_"
                          "slots ",
                          it->port_info.uim_port_status[i].port_state);
          number_of_active_slots++;
        }
      }
    }
...
)
{
  ...
  if(status_ptr->no_of_slots <= QMI_UIM_MAX_SLOT_COUNT)
  {
    ...
    if (status_ptr->port_information_len > 0) {
      for (uint8_t i = 0; i < status_ptr->port_information_len; i++) {
        slot_status[i].port_information_len = status_ptr->port_information_len;
        slot_status[i].port_info.is_mep =
            status_ptr->port_information[i].is_mep;
        slot_status[i].port_info.mep_mode =
            (RIL_UIM_Mep_mode)status_ptr->port_information[i]
                .negotiated_mep_mode;
        slot_status[i]
            .port_info.max_jointly_supported_lsi_for_enabled_profiles =
            status_ptr->port_information[i]
                .max_jointly_supported_lsi_for_enabled_profiles;
        slot_status[i].port_info.uim_port_status_len =
            status_ptr->port_information[i].uim_port_status_len;
        for (uint8_t j = 0; j < slot_status[i].port_info.uim_port_status_len;
             j++) {
          slot_status[i].port_info.uim_port_status[j].port_state =
              (RIL_UIM_Port_State)status_ptr->port_information[i]
                  .uim_port_status[j]
                  .port_state;
          slot_status[i].port_info.uim_port_status[j].card_protocol =
              (RIL_UIM_Card_protocol)status_ptr->port_information[i]
                  .uim_port_status[j]
                  .card_protocol;
          slot_status[i].port_info.uim_port_status[j].num_app =
              status_ptr->port_information[i].uim_port_status[j].num_app;
          if (slot_status[i].port_info.uim_port_status[j].port_state ==
              RIL_UIM_PORT_STATE_ACTIVE) {
            switch (status_ptr->port_information[i]
                        .uim_port_status[j]
                        .logical_slot) {
            case QMI_UIM_SLOT_1:
              slot_status[i].port_info.uim_port_status[j].logical_slot = 0;
              break;
            case QMI_UIM_SLOT_2:
              slot_status[i].port_info.uim_port_status[j].logical_slot = 1;
              break;
            case QMI_UIM_SLOT_3:
              slot_status[i].port_info.uim_port_status[j].logical_slot = 2;
              break;
            default:
              QCRIL_LOG_ERROR("Invalid input, incorrect logical slot id ");
              QCRIL_ASSERT(0);
           ...
           qcril_uim.card_status.card[slot_status[i].port_info.uim_port_status[j].logical_slot].physical_slot_id = i;
           QCRIL_LOG_INFO("physical_slot_id is %d",qcril_uim.card_status.card[slot_status[i].port_info.uim_port_status[j].logical_slot].physical_slot_id);
          }
          slot_status[i].port_info.uim_port_status[j].iccid_len =
              status_ptr->port_information[i].uim_port_status[j].iccid_len;
          slot_status[i].port_info.uim_port_status[j].iccid.resize(
              slot_status[i].port_info.uim_port_status[j].iccid_len);
          memcpy(&slot_status[i].port_info.uim_port_status[j].iccid[0],
                 status_ptr->port_information[i].uim_port_status[j].iccid,
                 status_ptr->port_information[i].uim_port_status[j].iccid_len);
        }
      }
    }
...
/*=========================================================================
  ...
  FUNCTION:  qcril_uim_switch_slot_port_resp
...
*/
/*=========================================================================*/
void qcril_uim_switch_slot_port_resp(
    const qcril_uim_callback_params_type *const params_ptr,
    const std::shared_ptr<UimSwitchSlotPortRequestMsg> req_ptr) {
  RIL_UIM_Errno ril_err = RIL_UIM_E_SUCCESS;
  std::shared_ptr<RIL_UIM_Errno> rsp_ptr = NULL;
  ...
  if (params_ptr->qmi_rsp_data.qmi_err_code == QMI_UIM_SERVICE_ERR_NO_EFFECT) {
    ...
    ril_err =
        qcril_uim_convert_err_value(params_ptr->qmi_rsp_data.qmi_err_code);
  }
  ...
  rsp_ptr = std::make_shared<RIL_UIM_Errno>(ril_err);
  req_ptr->sendResponse(req_ptr, Message::Callback::Status::SUCCESS, rsp_ptr);
} /* qcril_uim_switch_slot_port_resp */
...
*/
/*=========================================================================*/
void qcril_uim_request_switch_slot_port(
    const std::shared_ptr<UimSwitchSlotPortRequestMsg> req_ptr) {
  qmi_uim_physical_slot_mapping_type params = {};
  RIL_UIM_Errno ril_error = RIL_UIM_E_INTERNAL_ERR;
  std::shared_ptr<RIL_UIM_Errno> rsp_ptr = NULL;
  ...
  const std::vector<RIL_UIM_SwitchSlot> logic_phy_mapping =
      req_ptr->get_slot_mapping();
  params.num_slots = logic_phy_mapping.size();
  ...
  do {
    ...
    for (uint8_t i = 0; i < params.num_slots; i++) {
      /* Find slot info */
      switch (logic_phy_mapping[i].physicalSlotId) {
      case 0:
        params.logical_slot[i] = QMI_UIM_PHY_SLOT_1;
        break;
      case 1:
      ...
      switch (logic_phy_mapping[i].portId) {
      case 0:
        params.port_id[i] = QMI_UIM_PORT_ID_0;
        break;
      case 1:
        params.port_id[i] = QMI_UIM_PORT_ID_1;
        break;
      case 2:
        params.port_id[i] = QMI_UIM_PORT_ID_2;
        break;
      case 3:
        params.port_id[i] = QMI_UIM_PORT_ID_3;
        break;
      default:
        /* Send the response, actual response will be in payload */
        ril_error = RIL_UIM_E_INVALID_ARGUMENTS;
...
)
{
  ...
  if (ind_ptr != NULL)
  {
    Dispatcher::getInstance().dispatchSync(ind_ptr);
  }
