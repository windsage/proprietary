...
)
{
  ...
  *parsed_imsi_len_ptr = *raw_imsi_ptr;
       ...
       src++)
  {
    ...
    QCRIL_LOG_DEBUG( "IMSI[%d] src=%4x, dst=", src, PII(raw_imsi_ptr[src]));
    ...
    if (src > 1)
    {
      ...
      if (imsi_nibble < 0x0a)
      {
        ...
        QCRIL_LOG_DEBUG( "%c", PII(parsed_imsi_ptr[dst],'-'));
        dst++;
      }
    }
...
)
{
  ...
  *parsed_imsi_len_ptr = QCRIL_UIM_IMSI_M_PARSED_SIZE;
  ...
  QCRIL_LOG_DEBUG("mcc %d, mnc %d, min2 %d, thousands %d, second_three %d, last_three %d\n",
                  mcc, mnc, PII(min2), PII(thousands), PII(second_three), PII(last_three));
  ...
  QCRIL_LOG_DEBUG( "Parsed IMSI_M value:[%s]", PII(parsed_imsi_ptr));
...
void qcril_uim_close_logical_ch_session_req(
    const std::shared_ptr<UimSIMCloseChannelSessionReqMsg> req_ptr) {
  qmi_uim_logical_channel_params_type logical_channel_params = {};
  std::shared_ptr<RIL_UIM_Errno> rsp_ptr = NULL;
  uint8_t slot_id = qmi_ril_get_process_instance_id();
  ...
  /* Fill QMI parameter */
  memset(&logical_channel_params, 0,
         sizeof(qmi_uim_logical_channel_params_type));
  logical_channel_params.slot =
      (qmi_uim_slot_type)((uint8_t)QMI_UIM_SLOT_1 + slot_id);
  logical_channel_params.operation_type = QMI_UIM_LOGICAL_CHANNEL_CLOSE;
  logical_channel_params.channel_data.close_channel_info.channel_id =
      req_ptr->get_channel_id();
  logical_channel_params.channel_data.close_channel_info.terminate_app =
      QMI_UIM_FALSE;
  logical_channel_params.isEs10 = req_ptr->get_Es10();
  qcril_uim_remove_select_response_info_entry(
      logical_channel_params.channel_data.close_channel_info.channel_id);
...
)
{
  ...
  isEs10 = request_apdu_ptr->isEs10;
  ...
  send_apdu_params.isEs10 = isEs10;
...
*/
/*=========================================================================*/
void qcril_uim_close_logical_session_channel_resp(
    const qcril_uim_callback_params_type *const params_ptr,
    const std::shared_ptr<UimSIMCloseChannelSessionReqMsg> req_ptr) {
  RIL_UIM_Errno ril_err = RIL_UIM_E_SUCCESS;
  std::shared_ptr<RIL_UIM_Errno> rsp_ptr = NULL;
  ...
  QCRIL_LOG_DEBUG(
      "qcril_uim_close_logical_session_channel_resp: qmi_err_code=%d \n",
      params_ptr->qmi_rsp_data.qmi_err_code);
