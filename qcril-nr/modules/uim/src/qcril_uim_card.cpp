...
uint32_t qcril_temp_unlock_status[QMI_UIM_MAX_CARD_COUNT] = {
    QMI_UIM_UNLOCK_STATUS_UNKNOWN_V01, QMI_UIM_UNLOCK_STATUS_UNKNOWN_V01};
/*===========================================================================
...
)
{
  ...
  if (ril_card_status_ptr->card_state == RIL_UIM_CARDSTATE_PRESENT)
  {
    ...
    if (card_status_ptr->card[slot].card_mep_info.is_mep) {
      QCRIL_LOG_ERROR("is_mep 0x%x",
                      card_status_ptr->card[slot].card_mep_info.is_mep);
      QCRIL_LOG_ERROR("port_id 0x%x",
                      card_status_ptr->card[slot].card_mep_info.port_id);
      ril_card_status_ptr->port_id =
          card_status_ptr->card[slot].card_mep_info.port_id;
      ril_card_status_ptr->negotiated_mep_mode =
          card_status_ptr->card[slot].card_mep_info.negotiated_mep_mode;
    } else {
      ril_card_status_ptr->port_id = 0;
      ril_card_status_ptr->negotiated_mep_mode = 0;
    }
  }
  else {
  ril_card_status_ptr->port_id =
  ...
  QCRIL_LOG_INFO("cardstate is %d porst_id is %d mep_mode is %d",ril_card_status_ptr->card_state,
                           ril_card_status_ptr->port_id,ril_card_status_ptr->negotiated_mep_mode);
  ...
  for (app_index = 0; app_index < max_apps; app_index++)
  {
    ...
    if (card_status_ptr->card[slot].application[app_index].unlock_status_type ==
        QMI_UIM_UNLOCK_STATUS_PERMANENT_UNLOCKED_V01) {
      ril_card_status_ptr->applications[app_index].perso_substate =
          RIL_UIM_PERSOSUBSTATE_SIM_PERMANENT_UNLOCK;
    } else if (card_status_ptr->card[slot]
                   .application[app_index]
                   .unlock_status_type ==
               QMI_UIM_UNLOCK_STATUS_TEMPORARY_UNLOCKED_V01) {
      ril_card_status_ptr->applications[app_index].perso_substate =
          RIL_UIM_PERSOSUBSTATE_SIM_TEMP_UNLOCK;
    }
  ...
  else
  {
    ...
    for (i = 0; i < in_ptr->num_slots && i < QMI_UIM_MAX_CARD_COUNT; i++)
    {
                  ...
                  (j < QMI_UIM_MAX_APP_PER_CARD_COUNT); j++)
      {
        ...
        out_ptr->card[i].application[j].unlock_status_type =
            in_ptr->card[i].application[j].unlock_status_type;
      }
    }
  }
...
)
{
  ...
  QCRIL_LOG_ERROR("Session_type in response is %d",
                   (uim_session_type)rsp_data->rsp_data.change_provision_resp.session_type);
  if ((sub_info_ptr->subs_mode_pref == QCRIL_SUBS_MODE_GW) &&
      (sub_info_ptr->uicc_subs_info.sub_type == UIM_SUBSCRIPTION_1))
  {
    session_type = (uim_session_type)rsp_data->rsp_data.change_provision_resp.session_type;
...
)
{
  ...
  std::shared_ptr<UimSimPersoLockStatusInd> perso_lock_status_ind_ptr = nullptr;
  boolean unlock_status_type = FALSE;
  RIL_UIM_Perso_lock_Status perso_unlock_status;
  QCRIL_LOG_INFO( "%s\n", __FUNCTION__);
  ...
  if (qcril_uim.card_status.card[slot].num_app) {
    for (int i = 0; i < qcril_uim.card_status.card[slot].num_app; i++) {
      if (qcril_uim.card_status.card[slot].application[i].unlock_status_type !=
          QMI_UIM_UNLOCK_STATUS_UNKNOWN_V01) {
        qcril_temp_unlock_status[slot] =
            qcril_uim.card_status.card[slot].application[i].unlock_status_type;
        unlock_status_type = TRUE;
      }
    }
  }
  if (unlock_status_type ||
      qcril_temp_unlock_status[slot] != QMI_UIM_UNLOCK_STATUS_UNKNOWN_V01) {
    for (int i = 0; i < qcril_uim.card_status.card[slot].num_app; i++) {
      perso_unlock_status.applications[i].app_type =
          static_cast<RIL_UIM_AppType>(
              qcril_uim.card_status.card[slot].application[i].app_type);
      perso_unlock_status.applications[i]
          .unlock_status = static_cast<RIL_UIM_unlock_Status>(
          qcril_uim.card_status.card[slot].application[i].unlock_status_type);
    }
    ...
    perso_lock_status_ind_ptr =
        std::make_shared<UimSimPersoLockStatusInd>(perso_unlock_status);
    if (perso_lock_status_ind_ptr != nullptr) {
      perso_lock_status_ind_ptr->broadcast();
    }
  }
...
/*===========================================================================
  ...
  FUNCTION:  qcril_uim_request_get_sim_type

===========================================================================*/
/*!
    @brief
    Handles get_sim_type response request from QCRIL.
...
*/
/*=========================================================================*/
void qcril_uim_request_get_sim_type
(
  const std::shared_ptr<UimGetSimTypeRequestMsg> req_ptr
)
{
  uint8_t                                   slot      = QCRIL_UIM_INVALID_SLOT_INDEX_VALUE;
  ...
  QCRIL_LOG_INFO( "qcril_uim_request_get_sim_type\n");



  if (qcril_uim_send_qmi_async_msg(QCRIL_UIM_REQUEST_GET_SIM_TYPE,
                                   &slot,
                                   req_ptr) >= 0)
  {
    return;
...
} /* qcril_uim_request_get_sim_type */


/*===========================================================================

  FUNCTION:  qcril_uim_request_set_sim_type

===========================================================================*/
/*!
    @brief
    Handles set_sim_type response request from QCRIL.
...
*/
/*=========================================================================*/
void qcril_uim_request_set_sim_type
(
  const std::shared_ptr<UimSetSimTypeRequestMsg> req_ptr
)
{
  qmi_uim_set_sim_type_req                  sim_type_req = {};
  if(req_ptr == NULL)
  {
    QCRIL_LOG_ERROR("Invalid input, cannot process response");
    QCRIL_ASSERT(0);
  ...
  QCRIL_LOG_INFO( "qcril_uim_request_set_sim_type\n");

  const std::vector<uint32_t> sim_slot_type = req_ptr->get_sim_type_mapping();
  sim_type_req.sim_type_len = 	sim_slot_type.size();


  QCRIL_LOG_INFO( "qcril_uim_request_set_sim_type  length %d \n", sim_type_req.sim_type_len);

  for(uint8_t i = 0;i< sim_type_req.sim_type_len;i++)
  {
      sim_type_req.sim_type[i] = (qmi_uim_sim_type)sim_slot_type[i];
      QCRIL_LOG_INFO( "qcril_uim_request_set_sim_type value %d  \n", sim_type_req.sim_type[i]);
  }

  if (qcril_uim_send_qmi_async_msg(QCRIL_UIM_REQUEST_SET_SIM_TYPE,
                                   &sim_type_req,
                                   req_ptr) >= 0)
  {
    return;
...
} /* qcril_uim_request_set_sim_type */

RIL_UIM_Errno qcril_uim_get_perso_lock_status(
    const std::shared_ptr<UimGetPersoLockStatusRequestMsg> req_ptr,
    std::shared_ptr<RIL_UIM_Perso_lock_Status> perso_unlock_status) {
  uint8_t slot;
  slot = qmi_ril_get_process_instance_id();
  for (int i = 0; i < qcril_uim.card_status.card[slot].num_app; i++) {
    perso_unlock_status->applications[i].app_type =
        static_cast<RIL_UIM_AppType>(
            qcril_uim.card_status.card[slot].application[i].app_type);
    perso_unlock_status->applications[i].unlock_status =
        static_cast<RIL_UIM_unlock_Status>(
            qcril_uim.card_status.card[slot].application[i].unlock_status_type);
  }
  return RIL_UIM_E_SUCCESS;
}
