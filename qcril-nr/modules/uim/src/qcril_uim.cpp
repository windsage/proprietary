...
extern std::unordered_map<uint16_t, uint8_t> Prov_Session_type;
/*=========================================================================
...
)
{
  ...
  /* Clean Prov_Session_type hash map globals*/
   auto it = Prov_Session_type.begin();
   Prov_Session_type.erase(it,Prov_Session_type.end());
...
)
{
      ...
      rsp_data.rsp_data.slots_status_rsp.no_of_slots > 0)
  {
    ...
    if(rsp_data.rsp_data.slots_status_rsp.port_information_len)
    {
       for(uint8_t i = 0;i<rsp_data.rsp_data.slots_status_rsp.port_information_len && i < QMI_UIM_MAX_SLOT_COUNT;i++)
       {
         for (uint8_t j = 0;
          j < rsp_data.rsp_data.slots_status_rsp.port_information[i]
                  .uim_port_status_len;j++)
          {
            if(rsp_data.rsp_data.slots_status_rsp.port_information[i].uim_port_status[j].port_state  == QMI_UIM_PORT_STATE_ACTIVE)
            {
              int logical_slot = (int)rsp_data.rsp_data.slots_status_rsp.port_information[i].uim_port_status[j].logical_slot - 1;
              qcril_uim.card_status.card[logical_slot].physical_slot_id = i;
              QCRIL_LOG_INFO("Physicalslot-id during init %d logical_slot is %d",qcril_uim.card_status.card[logical_slot].physical_slot_id,logical_slot);
            }
          }
       }
     }
...
)
{
  ...
  switch(rsp_data_ptr->qmi_rsp_data.rsp_id)
  {
    ...
    case QMI_UIM_SRVC_LOGICAL_CHANNEL_RSP_MSG:
      if (string(UimSIMCloseChannelRequestMsg::MESSAGE_NAME) ==
          req_ptr->get_message_name()) {
        qcril_uim_close_logical_channel_resp(
            rsp_data_ptr,
            std::static_pointer_cast<UimSIMCloseChannelRequestMsg>(req_ptr));
      } else {
        qcril_uim_close_logical_session_channel_resp(
            rsp_data_ptr,
            std::static_pointer_cast<UimSIMCloseChannelSessionReqMsg>(req_ptr));
      }
      break;
    ...
    case QMI_UIM_SRVC_SWITCH_SLOT_EXT_RSP_MSG: {
      if (string(UimSwitchSlotRequestMsg::MESSAGE_NAME) ==
          req_ptr->get_message_name()) {
        qcril_uim_switch_slot_resp(
            rsp_data_ptr,
            std::static_pointer_cast<UimSwitchSlotRequestMsg>(req_ptr));
      }
      if (string(UimSwitchSlotPortRequestMsg::MESSAGE_NAME) ==
          req_ptr->get_message_name()) {
        qcril_uim_switch_slot_port_resp(
            rsp_data_ptr,
            std::static_pointer_cast<UimSwitchSlotPortRequestMsg>(req_ptr));
      }
    } break;
    ...
    case QMI_UIM_SRVC_REMOTE_UNLOCK_RSP_MSG:
      ...
      break;
    case QMI_UIM_SRVC_GET_SIM_TYPE_RSP_MSG:
      qcril_uim_get_sim_type_resp(rsp_data_ptr,
              std::static_pointer_cast<UimGetSimTypeRequestMsg>(req_ptr));
      break;
    case QMI_UIM_SRVC_SET_SIM_TYPE_RSP_MSG:
      qcril_uim_set_sim_type_resp(rsp_data_ptr,
              std::static_pointer_cast<UimSetSimTypeRequestMsg>(req_ptr));
      break;
...
)
{
  ...
  switch(ind_ptr->get_ind_id())
  {
    ...
    case QMI_UIM_SRVC_LPA_APP_PROFILE_IND_MSG:
      ...
      break;
    case QMI_UIM_SRVC_USER_CONSENT_IND_MSG:
      qcril_uim_lpa_user_consent_ind(ind_param_ptr);
      break;
