...
)
{
  ...
  *iccid_len_ptr = file_data_len;
  ...
  {
    QCRIL_LOG_DEBUG("0x%x", PII(iccid_data_ptr[i]));
  }
...
)
{
  ...
  */
  do
  {
    ...
    if (prop_ptr == NULL) {
      QCRIL_LOG_DEBUG("Failed to allocate UimGetRilPropertySyncMsg.");
      break;
    }

    Message::Callback::Status cb_ret = prop_ptr->dispatchSync(respPtr);

    if (cb_ret != Message::Callback::Status::SUCCESS || respPtr == NULL) {
      QCRIL_LOG_DEBUG("CB for UimGetRilPropertySyncMsg failed.");
      break;
    }
...
)
{
  ...
  QCRIL_LOG_INFO("iccid:  %s ", PII(iccid_str));
  return iccid_str;
