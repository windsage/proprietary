...
static bool is_adb_logging_enabled () {
...
static bool is_additional_logging_enabled () {
  bool log_on = false;
  char args[PROPERTY_VALUE_MAX];
  memset(args, 0, sizeof(args));
  property_get(QCRIL_LOG_ADDITIONAL_LOG_ON, args, "0");
  if (!strcmp(args, "1")) {
    log_on = true;
  }
  return log_on;
...
class MutexLoggerInit {
	public:
    MutexLoggerInit() {
        if (is_additional_logging_enabled()) {
...
#ifdef RIL_FOR_MDM_LE
        QtiMutexDefaults::getInstance().setDeadlockDebugLevel(DeadlockDebugLevel::NONE);
#endif
    }
};
...
android_ril_module::android_ril_module()
{
    ...
    if (is_additional_logging_enabled()) {
        qti::ril::logger::Logger::setAdditionalLogOn(true);
    }
#endif
    using std::placeholders::_1;
    mMessageHandler = {
        HANDLER(AndroidRequestRegisterMessage, android_ril_module::handleRegisterRequest),
        HANDLER(RequestRadioStateMessage, android_ril_module::handleRadioStateRequest),
    };
}
...
void android_ril_module::handleRadioStateRequest(std::shared_ptr<RequestRadioStateMessage> msg) {
    if (msg) {
        msg->sendResponse(
            msg,
            Message::Callback::Status::SUCCESS,
            std::make_shared<RIL_RadioState>(currentState(msg->forInstance()))
        );
    }
}
