...
class DeepSleepEventListenerImpl : public deep_sleep::PowerStateHandler::EventListener {
 private:
  bool handleEvent(State state);
 public:
  bool notifyDeepSleepEvent(State state) override;
  bool notifyHibernateEvent(State state) override;
};

bool DeepSleepEventListenerImpl::handleEvent(State state) {
...
bool DeepSleepEventListenerImpl::notifyDeepSleepEvent(State state) {
  bool result = handleEvent(state);
  QCRIL_LOG_DEBUG("state = %s, result = %d", toString(state).c_str(), result);
  return result;
}

bool DeepSleepEventListenerImpl::notifyHibernateEvent(State state) {
  bool result = handleEvent(state);
  QCRIL_LOG_DEBUG("state = %s, result = %d", toString(state).c_str(), result);
  return result;
}
