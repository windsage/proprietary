...
Return<bool> AndroidPowerStateHandler::PowerStateCallbackImpl::notifyDeepSleepEvent(
    V1_0::state state) {
  QCRIL_LOG_FUNC_ENTRY("notifyDeepSleepEvent: state = %d", state);
  bool result = true;
...
Return<bool> AndroidPowerStateHandler::PowerStateCallbackImpl::notifyHibernateEvent(
    V1_0::state state) {
  QCRIL_LOG_FUNC_ENTRY("notifyHibernateEvent: state = %d", state);
  bool result = true;
  EventListener::State eventState;
  if (mOwner.mListener && convertState(eventState, state)) {
    result = mOwner.mListener->notifyHibernateEvent(eventState);
  }
  QCRIL_LOG_FUNC_RETURN("result = %d", result);
  return result;
}
...
void AndroidPowerStateHandler::initPowerStateHal() {
  ...
  if (!mService) {
    ...
    if (mService) {
      ...
      ret = mService->registerHibernateClient(callback, "QCRIL");
      if (!ret) {
        QCRIL_LOG_ERROR("V1_0::IPowerStateService FAILED to registerHibernateClient");
      }
      if (mAcquired) {
        mAcquired = false;
        acquireLock();
