...
struct pbm_cache_info_type {
    ...
    void reset() {
        pbm_status = QCRIL_PBM_STATUS_UNKNOWN;
        pbm_session_type.clear();
        pbm_session_type.shrink_to_fit();
        pbm_capacity.clear();
        pbm_capacity.shrink_to_fit();
        pbm_reg_mask = 0;
        nw_detected_ecc_info = nullptr;
        pbm_ecc_map.clear();
        combined_ecc_map.clear();
    }
};
...
RIL_Errno qmi_ril_send_ecc_list_indication()
{
    ...
    if (pbm_cache_info.nw_detected_ecc_info)
    {
        ...
        alt_ecc->categories = pbm_cache_info.nw_detected_ecc_info->emerg_srv_categ;
        std::string urn{(char *)pbm_cache_info.nw_detected_ecc_info->urn.data(),
            pbm_cache_info.nw_detected_ecc_info->urn.size()};
        alt_ecc->sources = RIL_EccNumberSourceMask::NETWORK_SIGNALING;
        alt_ecc->urns.push_back(urn);
