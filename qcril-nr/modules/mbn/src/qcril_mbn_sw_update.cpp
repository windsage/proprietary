...
typedef enum {
    ...
    QCRIL_MBN_SW_STATE_DEACTIVATE_SELECTED_CONFIGS   = 24,
...
int     qcril_mbn_max_modem_subs = 0;
...
IxErrnoType qcril_mbn_sw_deactivate_selected_configs
(
    uint32_t                 sub_id,
    pdc_config_type_enum_v01 config_type
);
...
)
{
    ...
    else if (state == QCRIL_MBN_SW_STATE_DEACTIVATE_SELECTED_CONFIGS)
    {
        qcril_mbn_cur_instance_id++;
        if (qcril_mbn_cur_instance_id < qcril_mbn_max_modem_subs)
        {
            qcril_mbn_sw_set_cur_state(QCRIL_MBN_SW_STATE_DEACTIVATE_SELECTED_CONFIGS);
            qcril_mbn_sw_deactivate_selected_configs(qcril_mbn_cur_instance_id,
                                     PDC_CONFIG_TYPE_MODEM_SW_V01);
        }
        else
        {
            QCRIL_LOG_DEBUG("Successfully deactivated configs on all SUBs");
            qcril_mbn_sw_send_deactivate_sw_mbn_configs_response(result);
        }
    }
    else
    {
...
)
{
    ...
    if (iccid)
    {
        ...
        {
            QCRIL_LOG_INFO("set iccid_%d to %s in cache", instance, PII(iccid, const_cast<char *>("<hidden>")));
...
void qcril_mbn_sw_send_deactivate_sw_mbn_configs_response(RIL_Errno result) {
  auto msg = getMbnModule().getPendingMessageList().extract(
          QcRilDeactivateSwMbnsRequestMessage::get_class_message_id());

  if (msg) {
    std::shared_ptr<QcRilDeactivateSwMbnsRequestMessage> respMsg =
      std::static_pointer_cast<QcRilDeactivateSwMbnsRequestMessage>(msg);
    if (respMsg) {
      respMsg->sendResponse(respMsg, Message::Callback::Status::SUCCESS,
              std::make_shared<RIL_Errno>(result));
    }
  } else {
    QCRIL_LOG_ERROR("Unexpected: no QcRilDeactivateSwMbnsRequestMessage is pending!");
  }

  //reset MBN states
  qcril_mbn_cur_instance_id = 0;
  qcril_mbn_max_modem_subs = 0;
}
...
)
{
    ...
    QCRIL_LOG_DEBUG("current sub id is %d ",sub_id);
    (void)qcril_event_queue(QCRIL_DEFAULT_INSTANCE_ID,
                            QCRIL_DEFAULT_MODEM_ID,
                            QCRIL_DATA_ON_STACK,
                            QCRIL_EVT_REQUEST_MBN_SW_DEACTIVATE_CONFIG,
...
void qcril_mbn_sw_handle_deactivate_sw_mbn_configs(
  std::shared_ptr<QcRilDeactivateSwMbnsRequestMessage> msg) {
    ...
    do {
      ...
      if (queryMsg) {
        ...
        if (result == Message::Callback::Status::SUCCESS && shared_modem_subs != nullptr) {
          qcril_mbn_max_modem_subs = *shared_modem_subs;
        } else {
      ...
      // set the state as _DEACTIVATING_CURRENT_CONFIG as we need to
      // only deactivate current sw_mbn configs.
      qcril_mbn_cur_instance_id = 0;
      qcril_mbn_sw_set_cur_state(QCRIL_MBN_SW_STATE_DEACTIVATE_SELECTED_CONFIGS);
