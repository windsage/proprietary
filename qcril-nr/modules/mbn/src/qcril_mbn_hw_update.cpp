...
bool qcril_mbn_hw_deactivate_sw_mbn_configs()
{
  std::shared_ptr<QcRilDeactivateSwMbnsRequestMessage> msg =
      std::make_shared<QcRilDeactivateSwMbnsRequestMessage>();
  ...
  if (msg)
  {
    GenericCallback<RIL_Errno> cb(
          ...
          std::shared_ptr<RIL_Errno> rsp) -> void
      {
        ...
        //ir-respective of sw_mbn deactivate status proceed with activating HW MBN
...
)
{
    ...
    {
        //deactivate sw_mbn and proceed with hw_mbn activation
        //in-case of failure
        if (qcril_mbn_hw_deactivate_sw_mbn_configs() == false)
        {
            qcril_mbn_hw_set_cur_state(QCRIL_MBN_HW_STATE_ACTIVATING_CONFIG);
            qcril_mbn_hw_activate_selected_config(0, PDC_CONFIG_TYPE_MODEM_PLATFORM_V01);
        }
