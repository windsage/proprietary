...
/*!
    @brief
    Sends RilUnsolUiccAppsStatusChangedMessage to telephony.
...
)
{
  ...
  do {
    ...
    QCRIL_LOG_INFO("iccid - %s",PII(iccid));
...
RIL_Errno qcril_qmi_prov_evaluate_and_mark_apps_to_deactivate(void)
{
        ...
        ril_card_status->err == RIL_UIM_E_SUCCESS)
    {
        ...
        else
        {
            ...
            if(QCRIL_APP_STATUS_NOT_PROVISIONED != cdma_provision_state)
            {
                qcril_qmi_prov_set_cdma_provision_state(QCRIL_APP_STATUS_NOT_PROVISIONED);
            }
        }
    }
    else
...
)
{
    ...
    if ( params_ptr->data != NULL && params_ptr->datalen > 0 )
    {
        ...
        QCRIL_LOG_INFO("updated iccid - %s", PII(prov_common_cache.prov_info[slot].iccid));
...
)
{
    ...
    do
    {
        ...
        {
            QCRIL_LOG_INFO("iccid %s not present", PII(iccid));
        ...
        else if ( prev_userpref != user_prov_pref )
        {
            ...
            if ( res == RIL_E_SUCCESS )
            {
                ...
                QCRIL_LOG_INFO(" iccid - %s prev user pref - %d, new user pref - %d", PII(iccid), prev_userpref, user_prov_pref);
            }
        }
