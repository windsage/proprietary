...
void qcril_qmi_nas_reset_last_android_mode_pref();
...
void qcril_qmi_nas_request_set_nr_disable_mode
(
    std::shared_ptr<RilRequestSetNrDisableModeMessage> msg
)
{
    ...
    {
        auto qmi_client_error =  qmi_client_nas_send_sync(
        ...
        QCRIL_LOG_INFO("qmi_client_error %d, ril_req_res %d",
          (int) qmi_client_error, (int) ril_req_res );

        if(ril_req_res == RIL_E_SUCCESS)
        {
            QCRIL_LOG_INFO("Set system selection preference request to modem successfully queued");
            QCRIL_LOG_INFO("Waiting for system selection preference indication from QMI-NAS...");
            sendResp = false;   // Response will be sent once indication is received
        } else
        {
            QCRIL_LOG_ERROR("Error setting nr config, QMI Result = %d, error = %d");
        }
    }
    ...
    QCRIL_LOG_FUNC_RETURN();
} // qcril_qmi_nas_request_set_nr_config

void qcril_qmi_nas_request_query_nr_disable_mode
(
    std::shared_ptr<RilRequestQueryNrDisableModeMessage> msg
)
{
    RIL_Errno ril_req_res = RIL_E_SUCCESS;
    ...
    auto payload = std::make_shared<qcril::interfaces::RilQueryNrDisableModeResult_t>(nr_disable_mode);
    if (payload == nullptr) {
...
RIL_Errno qmi_ril_nwreg_request_mode_pref( int android_mode_pref, uint8_t *is_change )
{
    ...
    // If modem is in SDRM mode, return failure to telephony
    if(qcril_qmi_nas_get_sdrm_status()) {
        QCRIL_LOG_ERROR("Invalid state..Device is in sdrm mode");
        res = RIL_E_INVALID_STATE;
    }

    if ( RIL_E_SUCCESS == res && qmi_ril_is_feature_supported( QMI_RIL_FEATURE_SGLTE ) )
    {
...
)
{
  ...
  else
  {
      ...
      if( RIL_E_SUCCESS == ril_req_res )
      {
        ...
        int last_android_mode_pref{0};
        if (qcril_qmi_nas_get_last_android_mode_pref(&last_android_mode_pref))
        {
          if (preferred_network_type != last_android_mode_pref)
          {
            qcril_qmi_nas_reset_last_android_mode_pref();
          }
        }
      }
  }
