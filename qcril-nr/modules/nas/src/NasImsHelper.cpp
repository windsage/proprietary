...
static bool sIsImsSubActive = FALSE;
...
bool qcril_qmi_nas_ims_is_sub_active()
{
  QCRIL_LOG_FUNC_RETURN_WITH_RET(sIsImsSubActive);
  return sIsImsSubActive;
}
...
bool qcril_qmi_nas_handle_ims_active_sub_status_change(bool isSubActive)
{
  sIsImsSubActive = isSubActive;
  QCRIL_LOG_INFO(".. isSubActive = %s", isSubActive? "true" : "false");
  if (sIsImsSubActive == false)
  {
    qcril_qmi_nas_cancel_ims_shutdown_timer();
    qcril_qmi_nas_modem_shutdown_state_machine(
      QMI_RIL_NAS_MODEM_SHUTDOWN_EVENT_IMS_SHUTDOWN_COMPLETE);
