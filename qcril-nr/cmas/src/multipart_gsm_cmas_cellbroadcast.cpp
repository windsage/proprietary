...
/*
 * Returns number of GsmCmasCellbroadcast appended to the existing multipart delivery.
 */
size_t MultipartGsmCmasCellbroadcast ::size() const{
    return gsm_cmas_cellbroadcast_list.size();
}
...
void MultipartGsmCmasCellbroadcast ::process_reset_multipart_gsm_cmas_cellbroadcast(
    CallbackType &callback) {
  bool status = false;
  status = this->process(callback);
...
void handle_event_report_ind_cmas_buffer(MultipartGsmCmasCellbroadcast::RadioTech rat,
    ...
    if ((multipart_gsm_cmas_cellbroadcast.size() != 0) &&
        (multipart_gsm_cmas_cellbroadcast.current_msg_serial
         != cmas_cellbroadcast.get_serial_number())) {
      multipart_gsm_cmas_cellbroadcast.process_reset_multipart_gsm_cmas_cellbroadcast(callback);
    }

    multipart_gsm_cmas_cellbroadcast.current_msg_serial = cmas_cellbroadcast.get_serial_number();

    /* All key fields are set, append the GSM broadcast object.
    */
    multipart_gsm_cmas_cellbroadcast.append(cmas_cellbroadcast);
    /* If we receive all the pages then trigger the transcoding.
    */
    if (multipart_gsm_cmas_cellbroadcast.size() == cmas_cellbroadcast.get_page_count()) {
      multipart_gsm_cmas_cellbroadcast.process_reset_multipart_gsm_cmas_cellbroadcast(callback);
    }
}
