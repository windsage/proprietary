...
enum KeepAliveEventEnum : int {
  ...
  KeepAliveStartTcpMonitoringReq,
  KeepAliveStopTcpMonitoringReq,
  KeepAliveReleaseAllHandles,
  KeepAliveCleanUpForClientId,
};
...
struct keep_alive_ind{
  ...
  uint32_t handle;
  std::shared_ptr<TcpKeepAliveIndInfo_t> tcpKaInfo;
};

struct TcpKeepAlive_client_info_t{
  IpTuple tuple;
...
private:
    ...
    std::map<uint32_t,TcpKeepAlive_client_info_t> mKeepAliveHandlesList;
    std::map<uint32_t,TcpKeepAlive_client_info_t> mConfigHandlesList;
    ...
    ResponseError_t  sendKeepAliveStopRequestMessage( uint32_t handle , wds_keep_alive_type_enum_v01 type, bool bSendUnsol);
    ...
    void handleStartTcpMonitoringRequest(std::shared_ptr<Message> msg);
    void handleStopTcpMonitoringRequest(uint32_t config_handle);
    void sendTcpKeepAliveStartRequestMessage( const KeepAliveEventType e);
    ResponseError_t sendTcpKeepAliveStopRequestMessage(uint32_t handle);
    void handleTcpKeepAliveInd(std::shared_ptr<TcpKeepAliveIndInfo_t> tcp_ka_ind);
