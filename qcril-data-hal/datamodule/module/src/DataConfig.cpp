...
bool isProfileUpdateCarrierSet(std::string mcc, std::string mnc) {
  for (auto simInfo : simInfoList) {
    if (mcc == simInfo.mcc && mnc == simInfo.mnc) {
      Log::getInstance().d("SIM matches MCC,MNC list.");
      return true;
    }
  }
  return false;
