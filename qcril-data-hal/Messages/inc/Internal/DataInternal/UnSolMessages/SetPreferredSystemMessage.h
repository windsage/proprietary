...
public:
  ...
  SetPreferredSystemMessage(int32_t cid, string dnnName, HandoffNetworkType_t prefNetwork):
    UnSolicitedMessage(get_class_message_id()), mCid(cid), mDnnName(dnnName), mPrefNetwork(prefNetwork) {
    mName = MESSAGE_NAME;
  }

  int32_t getCid() { return mCid; }
  std::optional<string> getDnnName() {return mDnnName;}
  HandoffNetworkType_t getPrefNetwork() { return mPrefNetwork; }

  string dump(){return mName + " cid=" + std::to_string(mCid) +
                " dnnName="+mDnnName.value_or("") + " network=" + std::to_string((int)mPrefNetwork);}
