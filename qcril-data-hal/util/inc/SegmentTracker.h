...
struct PendingTransaction {
  ...
  std::unordered_map<uint8_t, std::vector<std::shared_ptr<Message>>> fragments;
  std::unordered_map<uint8_t, std::unordered_set<uint8_t>> remainingFragments;
  ...
  void setTotalFragments(uint8_t segIndex, uint8_t totalFrags) {
    for (uint8_t i = 0; i < totalFrags; i++) {
      remainingFragments[segIndex].insert(i);
    }
  }
};
...
public:
  ...
  void addFragment(MessageId_t msgId, uint16_t txId, uint8_t segmentIndex, uint8_t totalSegments,
                   uint8_t fragmentIndex, uint8_t totalFragments, std::shared_ptr<Message> msg);
  void sendErrorResponse(SegmentationStatus_t status, MessageId_t msgId, uint16_t txId);
  void setEndPointName(std::string name) {mEndPointName = name;}
...
private:
  std::string mEndPointName = "UNSPECIFIED";
#ifdef QMI_RIL_UTF
