...
bool AuthManager::readCarrierImsiKeyCache(auth_send_imsi_public_key_ex_req_msg_v01 *req)
{
  ...
  if (!mImsiEncryptionInfoCache.empty()) {
    ...
    for (int i=0 ; i<req->public_keys_len ; i++) {
      ...
      strlcpy(req->public_keys[i].mcc_mnc_info.mcc, keyInfo.mcc.c_str(), MIN(size, QMI_AUTH_MCC_MNC_SIZE_V01+1));

      size = keyInfo.mnc.length()+1;
      strlcpy(req->public_keys[i].mcc_mnc_info.mnc, keyInfo.mnc.c_str(), MIN(size, QMI_AUTH_MCC_MNC_SIZE_V01+1));
