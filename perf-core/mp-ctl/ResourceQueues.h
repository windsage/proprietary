...
class ResourceQueue {
    private:
        ...
        bool display_doze = false;

    private:
        void CopyQnode(q_node *n1, q_node *n2);
        q_node *GetNode(Resource &resObj);
        uint32_t GetResetOptData(uint16_t idx, uint32_t level);
        void PendCurrentRequestsOp(uint16_t idx, q_node *current);
        void LockCurrentStateOp(q_node *current, Request *req);
        void UnlockCurrentStateOp(uint16_t idx, q_node *current, q_node *pending, Request *req);
