...
)
{
  ...
  DSI_LOG_DEBUG( "dsi_embms_enable: ENTRY" );
  ...
  {
    DSI_LOG_ERROR("unable to set extended tech pref ");
    return DSI_ERROR;
  }
  ...
  do
  {
    ...
    {
      DSI_LOG_ERROR("dsi_embms_enable: dsi not inited");
      break;
    }

    if (!(DSI_IS_HNDL_VALID(st_hndl)))
    {
      DSI_LOG_ERROR( "dsi_embms_enable invalid arg, st_hndl [%p]  hash_value [%p]",
                       st_hndl, hash_value);
      break;
    }
    ...
    if (DSI_SUCCESS != ret)
    {
      DSI_LOG_ERROR("dsi_register_embms_ind failed");
      break;
    }
  ...
  if (DSI_SUCCESS == ret)
  {
    DSI_LOG_DEBUG( "dsi_embms_enable: EXIT with suc" );
  }
  else
  {
    DSI_LOG_DEBUG( "dsi_embms_enable: EXIT with err" );
  }
...
)
{
  ...
  DSI_LOG_DEBUG( "dsi_embms_tmgi_activate: ENTRY" );
  ...
  do
  {
    ...
    if (dsi_inited != DSI_TRUE)
    {
      DSI_LOG_ERROR("dsi_embms_tmgi_activate: dsi not inited");
      break;
    }

    if (!(DSI_IS_HNDL_VALID(st_hndl)) || (NULL == tmgi) || (NULL == earfcnlist))
    {
      DSI_LOG_ERROR( "dsi_embms_activate_tmgi invalid arg, st_hndl [%p] for hash_value [%p],"
                     " or tmgi NULL", st_hndl, hash_value);
      break;
    }
  ...
  if (DSI_SUCCESS == ret)
  {
    DSI_LOG_DEBUG( "dsi_embms_tmgi_activate: EXIT with suc" );
  }
  else
  {
    DSI_LOG_DEBUG( "dsi_embms_tmgi_activate: EXIT with err" );
  }
...
)
{
  ...
  DSI_LOG_DEBUG( "dsi_embms_tmgi_deactivate: ENTRY" );
  ...
  do
  {
    ...
    if (dsi_inited != DSI_TRUE)
    {
      DSI_LOG_ERROR("dsi_embms_deactivate_tmgi: dsi not inited");
      break;
    }

    if (!(DSI_IS_HNDL_VALID(st_hndl)) || (NULL == tmgi))
    {
      DSI_LOG_ERROR( "dsi_embms_tmgi_deactivate invalid arg, st_hndl [%p] for hash_value [%p],"
                     " or tmgi NULL", st_hndl, hash_value);
      break;
    }
  ...
  if (DSI_SUCCESS == ret)
  {
    DSI_LOG_DEBUG( "dsi_embms_tmgi_deactivate: EXIT with suc" );
  }
  else
  {
    DSI_LOG_DEBUG( "dsi_embms_tmgi_deactivate: EXIT with err" );
  }
...
)
{
  ...
  DSI_LOG_DEBUG( "dsi_embms_tmgi_activate_deactivate: ENTRY" );
  ...
  do
  {
    ...
    if (dsi_inited != DSI_TRUE)
    {
      DSI_LOG_ERROR("dsi_embms_tmgi_activate_deactivate: dsi not inited");
      break;
    }
    ...
    {
      DSI_LOG_ERROR( "dsi_embms_tmgi_activate_deactivate invalid arg, st_hndl [%p] for hash_value [%p],"
                     " or tmgi NULL", st_hndl, hash_value);
      break;
    }
    ...
    {
      DSI_LOG_ERROR("activate tmgi is same as deactivate tmgi, INCORRECT\n");
      break;
    }
  ...
  if (DSI_SUCCESS == ret)
  {
    DSI_LOG_DEBUG( "dsi_embms_tmgi_activate_deactivate: EXIT with suc" );
  }
  else
  {
    DSI_LOG_DEBUG( "dsi_embms_tmgi_activate_deactivate: EXIT with err" );
  }
...
)
{
  ...
  DSI_LOG_DEBUG("dsi_embms_get_tmgi_deactivate_reason ENTRY");
  DSI_GLOBAL_LOCK;
      ...
      NULL != dsi_tmgi_deactivate_reason)
  {
    switch(status_field)
    {
      ...
      case CHECK_ACTIVATE_DEACTIVATE_STATUS:
        DSI_LOG_DEBUG("returning reason type [%d] on handle [%p] hash_value [%p]",
                      st->priv.embms_tmgi_list_info.activate_status,st, hash_value);
        *dsi_tmgi_deactivate_reason =
          st->priv.embms_tmgi_list_info.activate_status;
        break;
      case CHECK_DEACTIVATE_STATUS:
        DSI_LOG_DEBUG("returning reason type [%d] on handle [%p] hash_value [%p]",
                      st->priv.embms_tmgi_list_info.deactivate_status, st,hash_value);
        *dsi_tmgi_deactivate_reason =
    ...
    DSI_GLOBAL_UNLOCK;
    DSI_LOG_DEBUG("dsi_embms_get_tmgi_deactivate_reason EXIT success");
    return DSI_SUCCESS;
  }

  DSI_LOG_DEBUG("dsi_embms_get_tmgi_deactivate_reason EXIT error");
  DSI_GLOBAL_UNLOCK;
...
)
{
  ...
  DSI_LOG_DEBUG( "dsi_embms_tmgi_list_query: ENTRY" );

  /* validate input parameter */
  if( NULL == list_info)
  {
    DSI_LOG_ERROR("invalid input parameter");
    return ret;
  }
  ...
  do
  {
    ...
    if (dsi_inited != DSI_TRUE)
    {
      DSI_LOG_ERROR("dsi_embms_tmgi_list_query: dsi not inited");
      break;
    }

    if (!(DSI_IS_HNDL_VALID(st_hndl)))
    {
      DSI_LOG_ERROR( "dsi_embms_tmgi_list_query invalid arg, st_hndl [%p] hash_value[%p]" ,
                     st_hndl,hash_value);
      break;
    }
  ...
  if (DSI_SUCCESS == ret)
  {
    DSI_LOG_DEBUG( "dsi_embms_tmgi_list_query: EXIT with suc" );
  }
  else
  {
    DSI_LOG_DEBUG( "dsi_embms_tmgi_list_query: EXIT with err" );
  }
...
)
{
  ...
  DSI_LOG_DEBUG( "dsi_embms_content_desc_update: ENTRY" );
  ...
  do
  {
    ...
    if (dsi_inited != DSI_TRUE)
    {
      DSI_LOG_ERROR("dsi_embms_content_desc_update: dsi not inited");
      break;
    }

    if (!(DSI_IS_HNDL_VALID(st_hndl)))
    {
      DSI_LOG_ERROR( "dsi_embms_content_desc_update invalid arg, st_hndl [%p] hash_value[%p]" ,
                     st_hndl, hash_value);
      break;
    }
    ...
    memset(&st_hndl->priv.embms_tmgi_list_info.embms_tmgi_params.content_desc_info, 0,
    ...
    if(content_desc_valid != 0 && content_desc != NULL)
    {
      ...
      {
        DSI_LOG_ERROR("dsi_embms_content_desc_update: allocate memory for tmgi failed\n");
        break;
      }
  ...
  if (DSI_SUCCESS == ret)
  {
    DSI_LOG_DEBUG( "dsi_embms_content_desc_update: EXIT with suc" );
  }
  else
  {
    DSI_LOG_DEBUG( "dsi_embms_content_desc_update: EXIT with err" );
  }
...
)
{
  ...
  DSI_LOG_DEBUG( "dsi_embms_svc_interest_list: ENTRY" );
  ...
  do
  {
    ...
    if (dsi_inited != DSI_TRUE)
    {
      DSI_LOG_ERROR("dsi_embms_svc_interest_list: dsi not inited");
      break;
    }

    if (!(DSI_IS_HNDL_VALID(st_hndl)))
    {
      DSI_LOG_ERROR( "dsi_embms_svc_interest_list invalid arg, st_hndl [%p] hash_value [%p]" ,
                     st_hndl, hash_value);
      break;
    }
  ...
  if (DSI_SUCCESS == ret)
  {
    DSI_LOG_DEBUG( "dsi_embms_svc_interest_list: EXIT with suc" );
  }
  else
  {
    DSI_LOG_DEBUG( "dsi_embms_svc_interest_list: EXIT with err" );
  }
