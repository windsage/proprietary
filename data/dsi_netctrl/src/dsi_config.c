...
/*===========================================================================
  Copyright (c) 2013-2014, 2017, 2019-2023 Qualcomm Technologies, Inc.
  All Rights Reserved.
...
)
{
  ...
  snprintf(prop_name, DSI_CFGDB_STRVAL_MAX, "%s%s%s", "dsi_config.", target, ".disable_netmgrd");
  ret = configdb_get_parameter(prop_name, CFGDB_TYPE_INT, &disable_netmgrd, sizeof(disable_netmgrd));
  if (ret != CFGDB_SUCCESS)
  {
    DSI_LOG_ERROR("Error reading property [%s] Err[%d]", prop_name, ret);
  }
  dsi_config.disable_netmgrd = disable_netmgrd;
