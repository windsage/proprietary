...
)
{
  ...
  DSI_LOG_DEBUG( "dsi_process_stop_nw_async_rsp: ENTRY" );
  ...
  do
  {
    ...
    if(NULL == rsp_data)
    {
      DSI_LOG_ERROR("received NULL rsp_data");
      break;
    }
    ...
    if (TRUE == send_no_net)
    {
      ...
      for (j = 0; j < DSI_MAX_DATA_CALLS; j++)
      {
           ...
           ((int)hndl->priv.start_nw_params.ip_family_preference == ip_family))
        {
          ...
          hash_value = dsi_get_hash_from_str_ptr(hndl);
          if( NULL != hndl->net_ev_cb  && hash_value > 0 )
          {
            DSI_LOG_DEBUG( "notifying user of st_hndl [%p] hash_value [%p] of event NET_NO_NET for [%d] ip_family",
                           hndl, hash_value, ip_family );
            hndl->net_ev_cb( (dsi_hndl_t *)hash_value,
                           hndl->user_data,
                           DSI_EVT_NET_NO_NET,
                           NULL );
          }
          ...
          {
            DSI_LOG_ERROR( "callback on store pointer [%p] is NULL or INVALID hash_value=%p", hndl, hash_value );
          }
        }
      }
    }
    ...
    if (TRUE == release_iface)
    {
      ...
      DSI_LOG_DEBUG( "st_hndl [%p] detached from interface [%d] hash_value [%p]",
                     st, i, hash_value);
  ...
  if (DSI_SUCCESS == ret)
  {
    DSI_LOG_DEBUG( "dsi_process_stop_nw_async_rsp: " \
                   "EXIT with suc" );
  }
  else
  {
...
)
{
  ...
  {
    DSI_LOG_VERBOSE("dsi_query_if_addr_from_qdi: valid client not found");
    return DSI_ERROR;
  }
  ...
  switch (ip_version)
  {
    case DSI_IP_VERSION_4:
      DSI_LOG_VERBOSE("IPv4 call - querying addresses from QDI");
      if (QDI_SUCCESS != qdi_get_addr_info(DSI_GET_QDI_HNDL(iface_id),
                                           ...
                                           tech_name))
      {
        DSI_LOG_ERROR("failed to get IPv4 addr info from QDI");
        ret = DSI_ERROR;
      }
      else
      {
        /* This is only used on the PRIMARY PDU */
        dsi_addr_info_ex_t *addrinfo;

        addrinfo = dsi_get_addrinfo_ex(iface_id, DSI_IP_FAMILY_V4,
                                       PRIMARY_PDU, DSI_ADDR_TYPE_DEFAULT);
        addrinfo->pdu_id = 0;
        addrinfo->primary = TRUE;
        addrinfo->deprecateTime = 0xFFFFFFFF;
        addrinfo->expiryTime = 0xFFFFFFFF;
        addrinfo->address_type = DSI_ADDR_TYPE_INVALID;
      }
      break;
    ...
    case DSI_IP_VERSION_6:
    {
      ...
      DSI_LOG_VERBOSE("IPv6 call - querying addresses from QDI");
      ...
      {
        DSI_LOG_ERROR("failed to get IPv6 addr info from QDI");
        ret = DSI_ERROR;
      }
      else
      {
        ...
        /* This is only used on the PRIMARY PDU */
        addrinfo = dsi_get_addrinfo_ex(iface_id, DSI_IP_FAMILY_V6,
                                       PRIMARY_PDU, DSI_ADDR_TYPE_DEFAULT);
        addrinfo->pdu_id = 0;
        addrinfo->primary = TRUE;
        addrinfo->deprecateTime = 0xFFFFFFFF;
        addrinfo->expiryTime = 0xFFFFFFFF;
        addrinfo->address_type = DSI_ADDR_TYPE_DEFAULT;
      }
    }
      break;
    ...
    case DSI_IP_VERSION_4_6:
    {
      ...
      DSI_LOG_VERBOSE("IPv4v6 call - querying addresses from QDI");
      ...
      {
        DSI_LOG_ERROR("Returning Retry dsi_query_if_addr_from_qdi");
        ret = DSI_RETRY;
      }
      break;
    }
...
)
{
  ...
  {
    DSI_LOG_VERBOSE("dsi_query_if_tech_from_qdi: valid client not found");
  }
...
/*===========================================================================
  FUNCTION:  dsi_internal_call_end_reason_update
===========================================================================*/
/*!
    @brief
    Updates the call end reason for store pointer for internal failures

    @param
    st       - the store handle pointer
    rsp_data - the response data we got from qmi wds
    qmi_err_code - error code received from WDS

    @return
    None
*/
/*=========================================================================*/
static void dsi_internal_call_end_reason_update
(
  dsi_store_t                 *st,
  qdi_wds_async_rsp_data_type *rsp_data,
  int                         qmi_err_code
)
{
  ...
  if (!st || !rsp_data)
  {
    DSI_LOG_ERROR("dsi_update_call_end_reason: bad parameter(s)");
    return;
  }
  ...
  if(hash_value <= 0)
  {
    DSI_LOG_ERROR("No valid hash_value for st [%p]", st);
    return;
  }

  /* Update IPv4 call end reason */
  if (rsp_data->flags & QDI_RSP_DATA_V4)
  {
    if (QMI_ERR_NO_EFFECT_V01 == qmi_err_code)
    {
      st->priv.ce_reason[DSI_IP_FAMILY_V4].reason_code = DSI_CE_REASON_INTERNAL;
      st->priv.ce_reason[DSI_IP_FAMILY_V4].reason_type = DSI_CE_TYPE_CALL_ALREADY_EXISTS;
    }
    else
    {
      st->priv.ce_reason[DSI_IP_FAMILY_V4].reason_code = DSI_CE_REASON_UNKNOWN;
  ...
  /* Update IPv6 call end reason */
  if (rsp_data->flags & QDI_RSP_DATA_V6)
  {
    if (QMI_ERR_NO_EFFECT_V01 == qmi_err_code)
    {
      st->priv.ce_reason[DSI_IP_FAMILY_V6].reason_code = DSI_CE_REASON_INTERNAL;
      st->priv.ce_reason[DSI_IP_FAMILY_V6].reason_type = DSI_CE_TYPE_CALL_ALREADY_EXISTS;
    }
    else
    {
      st->priv.ce_reason[DSI_IP_FAMILY_V6].reason_code = DSI_CE_REASON_UNKNOWN;
...
)
{
  ...
  if(rsp_data->flags & QDI_RSP_DATA_V4)
  {
   if ( rsp_data->rsp_data_v4.rsp_data.start_nw_rsp.verbose_call_end_reason_valid == TRUE )
   {
    ...
    {
      DSI_LOG_DEBUG("dsi_update_call_end_reason: V4 call end reason invalid on handle [%p] hash_value [%p]", st, hash_value );
      st->priv.ce_reason[DSI_IP_FAMILY_V4].reason_type = DSI_CE_TYPE_INVALID;
    }

    DSI_LOG_DEBUG("set call end reason V4 code/type [%d/%d] on handle [%p] hash_value [%p]",
                  st->priv.ce_reason[DSI_IP_FAMILY_V4].reason_code,
                  st->priv.ce_reason[DSI_IP_FAMILY_V4].reason_type, st , hash_value);
  }
  ...
  if(rsp_data->flags & QDI_RSP_DATA_V6)
  {
   if ( rsp_data->rsp_data_v6.rsp_data.start_nw_rsp.verbose_call_end_reason_valid == TRUE )
    ...
    {
      DSI_LOG_DEBUG("dsi_update_call_end_reason: V6 call end reason invalid on handle [%p] hash_value [%p]", st, hash_value);
      st->priv.ce_reason[DSI_IP_FAMILY_V6].reason_type = DSI_CE_TYPE_INVALID;
    }

    DSI_LOG_DEBUG("set call end reason V6 code/type [%d/%d] on handle [%p] hash_value [%p]",
                  st->priv.ce_reason[DSI_IP_FAMILY_V6].reason_code,
                  st->priv.ce_reason[DSI_IP_FAMILY_V6].reason_type, st, hash_value);
  }
}
...
static boolean
dsi_query_should_incr_ref_count
(
  int sys_err,
...
static boolean
dsi_query_should_incr_v4_ref_count
(
  dsi_store_t *st,
  qdi_wds_async_rsp_data_type *rsp_data
)
{
  ...
  return dsi_query_should_incr_ref_count(rsp_data->rsp_data_v4.sys_err,
                                         rsp_data->rsp_data_v4.qmi_err);
}

static boolean
dsi_query_should_incr_v6_ref_count
(
  dsi_store_t *st,
  qdi_wds_async_rsp_data_type *rsp_data
)
{
  ...
  return dsi_query_should_incr_ref_count(rsp_data->rsp_data_v6.sys_err,
                                         rsp_data->rsp_data_v6.qmi_err);
}
/*===========================================================================
...
)
{
  ...
  if (!st)
  {
    DSI_LOG_ERROR("dsi_query_iface_addr_and_incr_ref_count: invalid param");
    return event;
  }
  ...
  if ( (WDS_TECHNOLOGY_NAME_EMBMS_V01  != dsi_query_if_tech_from_qdi(iface, ip_family)) &&
       ...
       && (0 > (ret = dsi_query_if_addr_from_qdi(iface, st->priv.ip_version, rsp_data)) )
     )
  {
    DSI_LOG_ERROR("No valid address found on iface=%d, ip_version=%d return from qdi_query %d, incrementing count",
                    iface,
                    st->priv.ip_version, ret);
    ...
    if (DSI_TRUE == dsi_query_should_incr_v4_ref_count(st, rsp_data))
    {
    ...
    if (DSI_TRUE == dsi_query_should_incr_v6_ref_count(st, rsp_data))
    {
      DSI_LOCK_MUTEX(DSI_GET_MUTEX(iface));
...
)
{
  ...
  DSI_LOG_DEBUG( "dsi_process_start_nw_async_rsp: ENTRY" );
  ...
  do
  {
    ...
    if(NULL == st)
    {
      DSI_LOG_ERROR("received NULL st");
      break;
    }
    ...
    if (rsp_data->flags == QDI_RSP_DATA_V4_V6)
    {
         ...
         (DSI_GET_WDS_V6_TXN(st) == DSI_INVALID_WDS_TXN))
      {
        ...
        DSI_LOG_ERROR("dsi_qmi_wds_cmd_cb called in invalid state");
        break;
      }
      else
      {
    ...
    if(QMI_NO_ERR == sys_err_code)
    {
      ...
      {
        /* successfull !! */
        DSI_LOG_DEBUG("qmi txn success: rsp= " \
                      "QMI_WDS_START_NETWORK_INTERFACE_RESP_V01");
        DSI_LOG_DEBUG("DEBUG: QDI Callback Resp Data Flags=[%d]",
                        rsp_data->flags);
       /* Increment the V4 ref_count if V4 call resp was successful*/
        if ((DSI_IP_VERSION_4 == st->priv.ip_version || DSI_IP_VERSION_4_6 == st->priv.ip_version)  &&
          ...
          ( rsp_data->flags & QDI_RSP_DATA_V4)
          )
        {
          DSI_LOCK_MUTEX(&(st->priv.mutex));
          DSI_INCR_COUNT(i, DSI_IP_VERSION_4);
          DSI_UNLOCK_MUTEX(&(st->priv.mutex));
        }
       ...
       if ((DSI_IP_VERSION_6 == st->priv.ip_version || DSI_IP_VERSION_4_6 == st->priv.ip_version) &&
          ...
          ( rsp_data->flags & QDI_RSP_DATA_V6)
          )
       {
         DSI_LOCK_MUTEX(&(st->priv.mutex));
         DSI_INCR_COUNT(i, DSI_IP_VERSION_6);
         DSI_UNLOCK_MUTEX(&(st->priv.mutex));
       }
    ...
    else if(QMI_SERVICE_ERR == sys_err_code)
    {
      ...
      case QMI_ERR_NO_EFFECT_V01:
        ...
        /* QMI_ERR_NO_EFFECT is sent by modem to indicate that the same WDS client
           ...
           and since this is not a valid scenario as such we will send
           DSI_EVT_NET_ERR_CALL_EXISTS event to clients */

        dsi_internal_call_end_reason_update(st, rsp_data, qmi_err_code);

        /* Latch on to exisitng call */
        event = dsi_query_iface_addr_and_incr_ref_count(st, i,ip_family, rsp_data);
        if (DSI_EVT_INVALID != event)
        {
          /* Update event and call state to connected */
          DSI_UPDATE_CALL_STATE(st, DSI_STATE_CALL_CONNECTED);
          event = DSI_EVT_NET_ERR_CALL_EXISTS;
          DSI_LOG_DEBUG("notifying client of event=%d", event);
          notify_user = DSI_TRUE;
        }
        else
        {
          DSI_LOG_ERROR("Latching on to existing call failed for iface %d", i);
        }
      ...
      default:
        ...
        DSI_LOG_INFO("%s(): ip_version %d v4_addr_valid %d v6_addr_valid %d v4_txn_valid %d v6_txn_valid %d v4_refcount %d v6_refcount %d",
                     __func__, st->priv.ip_version,
                     dsi_is_iface_addr_valid(st->priv.dsi_iface_id, DSI_IP_FAMILY_V4),
                     dsi_is_iface_addr_valid(st->priv.dsi_iface_id, DSI_IP_FAMILY_V6),
                     !(DSI_GET_WDS_V4_TXN(st) == DSI_INVALID_WDS_TXN),
                     !(DSI_GET_WDS_V6_TXN(st) == DSI_INVALID_WDS_TXN),
        ...
        if (DSI_IP_VERSION_4_6 == st->priv.ip_version)
        {
          if (((!dsi_is_iface_addr_valid(st->priv.dsi_iface_id, DSI_IP_FAMILY_V4)) && (!dsi_iface_tbl[i].v4_ref_count)) &&
              ((!dsi_is_iface_addr_valid(st->priv.dsi_iface_id, DSI_IP_FAMILY_V6)) && (!dsi_iface_tbl[i].v6_ref_count)) &&
              (DSI_GET_WDS_V4_TXN(st) == DSI_INVALID_WDS_TXN) &&
              (DSI_GET_WDS_V6_TXN(st) == DSI_INVALID_WDS_TXN))
          {
            goto detach;
        ...
        /* For a Dual-IP call, if we already have at least one valid address
          ...
          DSI_LOG_INFO( "%s(): Ignoring and setting to Invalid EVT", __func__ );
          event = DSI_EVT_INVALID;
          notify_user = DSI_FALSE;
        }
        else
        {
detach:
  ...
  if (DSI_TRUE == notify_user)
  {
   ...
   /* Release the IFACE clients only if this store does not have any handle sharing the IFACE */
    ...
    {
      DSI_LOG_ERROR("callback on store pointer [%p] is NULL  or INVALID hash_value [%p]",
                    st, hash_value);
    }
  }
  ...
  if (DSI_SUCCESS == ret)
  {
    DSI_LOG_DEBUG( "dsi_process_start_nw_async_rsp: " \
                   "EXIT with suc" );
  }
  else
  {
...
)
{
  ...
  DSI_LOG_VERBOSE("dsi_process_async_wds_rsp: ENTRY");
  ...
  do
  {
    ...
    case QMI_WDS_START_NETWORK_INTERFACE_RESP_V01:
      DSI_LOG_VERBOSE("rcvd QMI_WDS_START_NETWORK_INTERFACE_RESP_V01");
    ...
    case QMI_WDS_STOP_NETWORK_INTERFACE_RESP_V01:
      DSI_LOG_VERBOSE("rcvd QMI_WDS_STOP_NETWORK_INTERFACE_RESP_V01");
  ...
  if (DSI_SUCCESS == ret)
  {
    DSI_LOG_VERBOSE( "dsi_process_async_wds_rsp: EXIT with suc" );
  }
  else
  {
    DSI_LOG_VERBOSE( "dsi_process_async_wds_rsp: EXIT with err" );
  }
...
)
{
  ...
  {
    DSI_LOG_ERROR("dsi_is_valid_client_found_for_iface: invalid dsi iface");
    goto bail;
  }
  ...
  {
    DSI_LOG_VERBOSE("dsi_is_valid_client_found_for_iface: valid client not found");
  }
...
)
{
  ...
  DSI_LOG_VERBOSE("dsi_handle_pkt_srvc_ind: ENTRY");
  ...
  do
  {
    ...
    if (NULL == ind_data)
    {
      DSI_LOG_ERROR("NULL ind data received in wds_ind_cb");
      break;
    }
        ...
        */
               ...
               st->priv.ip_version == DSI_IP_VERSION_4_6))
          {
            ...
            if (NULL != st->net_ev_cb  && hash_value > 0)
            {
              ...
              {
                DSI_LOG_DEBUG("Call disconnect is in progress on handle [%p] hash_value [%p] " \
                              "Sending NET_NO_NET evt instead", st, hash_value);
            ...
            else
            {
              DSI_LOG_ERROR("NULL callback found on store pointer" \
                            "[%p] or INVALID hash_value [%p]", st, hash_value);
            }
          }
        }
      }
  ...
  if (DSI_SUCCESS == ret)
  {
    DSI_LOG_VERBOSE("dsi_handle_pkt_srvc_ind: EXIT with suc");
  }
  else
  {
    DSI_LOG_VERBOSE("dsi_handle_pkt_srvc_ind: EXIT with err");
  }
  return ret;
}
...
)
{
  ...
  DSI_LOG_VERBOSE("dsi_handle_embms_tmgi_activate_status_ind: ENTRY");
  ...
  do
  {
    ...
    if (NULL == ind_data)
    {
      DSI_LOG_ERROR("NULL ind data received in tmgi_activate_status_ind");
      break;
    }
  ...
  if (DSI_SUCCESS == ret)
  {
    DSI_LOG_VERBOSE("dsi_handle_embms_tmgi_activate_status_ind: "
                    "EXIT with suc");
  }
  else
  {
...
)
{
  ...
  DSI_LOG_VERBOSE("dsi_handle_embms_tmgi_deactivate_status_ind: ENTRY");
  ...
  do
  {
    ...
    if (NULL == ind_data)
    {
      DSI_LOG_ERROR("NULL ind data received in tmgi_dactivate_status_ind");
      break;
    }
  ...
  if (DSI_SUCCESS == ret)
  {
    DSI_LOG_VERBOSE("dsi_handle_embms_tmgi_deactivate_status_ind: "
                    "EXIT with suc");
  }
  else
  {
...
)
{
  ...
  DSI_LOG_VERBOSE("dsi_handle_embms_tmgi_act_deactivate_status_ind: ENTRY");
  ...
  do
  {
    ...
    if (NULL == ind_data)
    {
      DSI_LOG_ERROR("NULL ind data received in tmgi_act_deactivate_status_ind");
      break;
    }
  ...
  if (DSI_SUCCESS == ret)
  {
    DSI_LOG_VERBOSE("dsi_handle_embms_tmgi_act_deactivate_status_ind: "
                    "EXIT with suc");
  }
  else
  {
...
)
{
  ...
  DSI_LOG_VERBOSE("dsi_handle_embms_tmgi_list_ind: ENTRY");
  ...
  do
  {
    ...
    if (NULL == ind_data)
    {
      DSI_LOG_ERROR("NULL ind data received in tmgi_list_ind");
      break;
    }
    ...
    {
      DSI_LOG_DEBUG("received tmgi_list_ind with no valid tmgi list data");
    }
  ...
  if (DSI_SUCCESS == ret)
  {
    DSI_LOG_VERBOSE("dsi_handle_embms_tmgi_list_ind: EXIT with suc");
  }
  else
  {
    DSI_LOG_VERBOSE("dsi_handle_embms_tmgi_list_ind: EXIT with err");
  }
  return ret;
}
...
)
{
  ...
  boolean            handle_found = FALSE;
  int temp_index;
  DSI_LOG_VERBOSE("dsi_handle_embms_sai_list_ind: ENTRY");
  uint64_t hash_value = 0;
  ...
  do
  {
    ...
    if (NULL == ind_data)
    {
      DSI_LOG_ERROR("NULL ind data received in sai_list_ind");
      break;
    }
    ...
    {
      DSI_LOG_ERROR("Received SAI list ind with no sai list data ");
      break;
    }
  ...
  if (DSI_SUCCESS == ret)
  {
    DSI_LOG_VERBOSE("dsi_handle_embms_sai_list_ind: EXIT with suc");
  }
  else
  {
    DSI_LOG_VERBOSE("dsi_handle_embms_sai_list_ind: EXIT with err");
  }
  return ret;
}
...
)
{
  ...
  int                count;
  boolean            handle_found = FALSE;
  DSI_LOG_VERBOSE("dsi_handle_embms_content_desc_control_ind: ENTRY");
  uint64_t hash_value = 0 ;
  ...
  do
  {
    ...
    if (NULL == ind_data)
    {
      DSI_LOG_ERROR("NULL ind data received in content_desc_control_ind");
      break;
    }
  ...
  if (DSI_SUCCESS == ret)
  {
    DSI_LOG_VERBOSE("dsi_handle_embms_content_desc_control_ind: EXIT with suc");
  }
  else
  {
    DSI_LOG_VERBOSE("dsi_handle_embms_content_desc_control_ind: EXIT with err");
  }
  return ret;
}
...
)
{
  ...
  boolean            handle_found = FALSE;
  uint64_t hash_value = 0 ;
  DSI_LOG_VERBOSE("dsi_handle_embms_svc_interest_ind: ENTRY");
  ...
  do
  {
    ...
    if (NULL == ind_data)
    {
      DSI_LOG_ERROR("NULL ind data received in embms_svc_interest_ind");
      break;
    }
  ...
  if (DSI_SUCCESS == ret)
  {
    DSI_LOG_VERBOSE("dsi_handle_embms_svc_interest_ind: EXIT with suc");
  }
  else
  {
    DSI_LOG_VERBOSE("dsi_handle_embms_svc_interest_ind: EXIT with err");
  }
  return ret;
}
...
)
{
  ...
  DSI_LOG_VERBOSE("dsi_handle_handoff_ind: ENTRY");
  ...
  do
  {
    ...
    if (NULL == ind_data)
    {
      DSI_LOG_ERROR("NULL ind_data received handoff_ind");
      break;
    }
    ...
    for (count = 0; count < DSI_MAX_DATA_CALLS; count++)
    {
      ...
      if (st != NULL && st->priv.dsi_iface_id == i)
      {
        ...
        if (NULL == st->net_ev_cb || hash_value <= 0)
        {
          DSI_LOG_ERROR("No event callback for index [%d] , hash_value=%p", i, hash_value);
  ...
  if (DSI_SUCCESS == ret)
  {
    DSI_LOG_VERBOSE("dsi_handle_handoff_ind: EXIT with suc");
  }
  else
  {
    DSI_LOG_VERBOSE("dsi_handle_handoff_ind: EXIT with err");
  }
  return ret;
}
...
)
{
  ...
  do
  {
    ...
    if (NULL == ind_data)
    {
      DSI_LOG_ERROR("NULL ind data received in tuneaway_info_ind");
      break;
    }
...
)
{
  ...
  DSI_LOG_VERBOSE("dsi_process_wds_ind: ENTRY");
  ...
  if (DSI_SUCCESS == ret)
  {
    DSI_LOG_VERBOSE("dsi_process_wds_ind: EXIT with suc");
  }
  else
  {
    DSI_LOG_VERBOSE("dsi_process_wds_ind: EXIT with err");
  }
...
)
{
  ...
  case DSI_QMI_SYS_EVENT_MODEM_OUT_OF_SERVICE_IND:
    DSI_LOG_DEBUG("dsi_process_qmi_sys_ind: rcvd DSI_QMI_SYS_EVENT_MODEM_OUT_OF_SERVICE_IND");
  ...
  case DSI_QMI_SYS_EVENT_MODEM_IN_SERVICE_IND:
    DSI_LOG_DEBUG("dsi_process_qmi_sys_ind: rcvd DSI_QMI_SYS_EVENT_MODEM_IN_SERVICE_IND");
...
/*===========================================================================
  FUNCTION:  dsi_process_qmi_wds_extended_ind
===========================================================================*/
/*!
    @brief
    Processes QMI systerm indications in cb thrd context
...
)
{
  ...
  {
    st = (dsi_store_t *)dsi_store_table[count].dsi_store_ptr;
    if (st != NULL && st->priv.oemprxy_data.qmi_wds_hndl == ext_ind_data->wds_hndl) break;
  }

  if(st == NULL || st->priv.oemprxy_data.qmi_wds_hndl != ext_ind_data->wds_hndl)
  {
    DSI_LOG_ERROR("%s(): No valid OEM proxy handle found!", __func__);
    return;
  }

  switch(ext_ind_data->ind_data.status)
  {
    case WDS_CONNECTION_STATUS_CONNECTED_V01:
      /* Store session handle for later use */
      st->priv.oemprxy_data.sess_handle = ext_ind_data->ind_data.session_hndl;
      dsi_oemproxy_common_ind(DSI_EVT_WDS_EXT_IS_CONN, st->priv.oemprxy_data.oemprxy_iface_id);
      break;
    case WDS_CONNECTION_STATUS_DISCONNECTED_V01:
      dsi_oemproxy_common_ind(DSI_EVT_WDS_EXT_NO_NET, st->priv.oemprxy_data.oemprxy_iface_id);
      break;
    case WDS_CONNECTION_STATUS_SUSPENDED_V01:
    case WDS_CONNECTION_STATUS_AUTHENTICATING_V01:
    default:
      DSI_LOG_ERROR("%s(): Unknown indication!",__func__);
      break;
  }
}
