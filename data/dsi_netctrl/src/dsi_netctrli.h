...
#include "Nicm.h"
#include "dsi_netctrl_nicm.h"

#define DSI_NETCRTL_VERSION "0.0.0"
...
#define DSI_L2S_SETUP()                                                      \
  do{                                                                        \
    char tsv[25] = { 0 };                                                    \
    getSyncTimestamp(tsv, sizeof(tsv));                                      \
    l2s_pid = -1;                                                            \
    memset(l2s_proc_name,0, DSI_L2S_PNAME_SIZE);                             \
    l2s_pid = getpid();                                                      \
    ds_get_proc_name(l2s_pid,l2s_proc_name,DSI_L2S_PNAME_SIZE);              \
    DSI_LOG_DEBUG("L2S[pname:%s pid:%d tid:%d time:%s]",                     \
                   l2s_proc_name,l2s_pid, gettid(), tsv);                    \
  }while (0);
...
#define DSI_L2S_ENTRY_PARAMS(fmt,...)                                         \
  do{                                                                         \
    char tsv[25] = { 0 };                                                     \
    getSyncTimestamp(tsv, sizeof(tsv));                                       \
    DSI_LOG_DEBUG("L2S[pname:%s pid:%d tid:%d type:%s fname:%s time:%s params:" fmt "]",\
                  l2s_proc_name,                                              \
                  l2s_pid,                                                    \
                  gettid(),                                                   \
                  "FE",                                                       \
...
#define DSI_L2S_EXIT_WITH_STATUS(fmt,...)                                      \
  do {                                                                         \
    char tsv[25] = { 0 };                                                      \
    getSyncTimestamp(tsv, sizeof(tsv));                                        \
    DSI_LOG_DEBUG("L2S[pname:%s pid:%d tid:%d type:%s fname:%s status:%s time:%s params:"fmt"]",\
                  l2s_proc_name,                                               \
                  l2s_pid,                                                     \
                  gettid(),                                                    \
                  "FX",                                                        \
...
#define DSI_L2S_EXIT()                                                         \
  do {                                                                         \
    char tsv[25] = { 0 };                                                      \
    getSyncTimestamp(tsv, sizeof(tsv));                                        \
    DSI_LOG_DEBUG("L2S[pname:%s pid:%d tid:%d type:%s fname:%s time:%s]",      \
                  l2s_proc_name,                                               \
                  l2s_pid,                                                     \
                  gettid(),                                                    \
                  "FX",                                                        \
...
#define DSI_L2S_MSG(iface_id, msg)                                             \
  do {                                                                         \
    char tsv[25] = { 0 };                                                      \
    getSyncTimestamp(tsv, sizeof(tsv));                                        \
    DSI_LOG_DEBUG("L2S[pname:%s pid:%d tid:%d type:%s fname:%s iface_id:%d time:%s msg:\"%s\"]",\
                  l2s_proc_name,                                               \
                  l2s_pid,                                                     \
                  gettid(),                                                    \
                  "ME",                                                        \
...
typedef struct
{
  ...
  char wlan_iface_name[NICM_IF_NAME_MAX_LEN];
...
typedef struct
{
  ...
  dsiNicmUserCmdData user_cmd_data;
...
extern NicmClient *nicm_clnt_hndl;
/*===========================================================================
...
extern void dsi_fill_addr_info
(
  int dsi_id,
  dsi_ip_family_t ipf,
