...
/*!
    @brief
    Get the local time timestamp so that QXDM based logs and NICM file based
    logs can be synced up.

    @param
    ts - buffer which the timestamp value is stored
...
*/
/*=========================================================================*/
void getSyncTimestamp(char *ts, size_t size)
{
    struct tm *tmstmp;
    uint32_t msec;
    char tm_info[10];
    ...
    if (!tmstmp) {
      /* If localtime fails, we will make the ts string just a null terminator */
      ts = " ";
      return;
    }
    msec = tv.tv_usec / 1000;
    strftime(tm_info, sizeof(tm_info), "%T", tmstmp);
    snprintf(ts, size, "%8s.%03u", tm_info, msec);
}
...
/*===========================================================================
  FUNCTION:  dsiConvertNicmAddrType
===========================================================================*/
/*!
    @brief
    Converts the nicm_addr_type enum into the equivalent dsi_addr_type

    @param
    addr_type - the nicm_addr_type to convert

    @return
    An equivalent dsi_addr_type value
*/
/*=========================================================================*/
dsi_addr_type dsiConvertNicmAddrType
(
  dsiNicmAddrType addr_type
)
{
  ...
  switch (addr_type)
  {
    case DSI_NICM_ADDR_TYPE_DEFAULT:
      dsi_type = DSI_ADDR_TYPE_DEFAULT;
      break;
    case DSI_NICM_ADDR_TYPE_CANDIDATE:
      dsi_type = DSI_ADDR_TYPE_CANDIDATE;
      break;
    case DSI_NICM_ADDR_TYPE_MULTIHOME:
...
)
{
  ...
  {
    DSI_LOG_DEBUG("storing address for pdu[%d] type [%d]", pdu_cnf, addr_type);
...
(
  int dsi_id,
  const dsiNicmNLEventInfo * info_ptr
)
{
  dsi_store_t * st = NULL;
...
/*===========================================================================
  FUNCTION:  dsi_get_lib_version_info
===========================================================================*/
/*!
    @brief
    function to get dsi library version

    @return
    string for dsi version
*/
/*=========================================================================*/
const char * dsi_get_lib_version_info(void)
{
#ifdef DSI_NETCRTL_VERSION
  return DSI_NETCRTL_VERSION;
#else
