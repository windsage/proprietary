...
/*===========================================================================
  FUNCTION IsLAFusionTargetUsingMHIPrime
=============================================================================
  @brief
    checks if fusion target uses MHI prime channels for DPL
  ...
  @sideefects
    none
===========================================================================*/
int IsLAFusionTargetUsingMHIPrime()
{
  DIR *mhi_dir;
  struct dirent *dir;
  int retry_count = 0;
  int ret_val = ADPL_FAILURE;

  while (retry_count < ADPL_FILE_NODE_OPEN_MAX_RETRY)
  {
    mhi_dir = opendir(ADPL_MHI_FILE_PATH);
    if(!mhi_dir)
    {
    ...
    {
      LOG_MSG_INFO1("Successful in opening the MHI dir", 0, 0, 0);
      while((dir = readdir(mhi_dir)) != NULL)
      {
        if( strstr(dir->d_name, MHI_PRIME_CHANNEL_NODE_SUBSTRING) )
        {
          LOG_MSG_INFO1("MHI prime channels present for DPL : %s", dir->d_name, 0, 0);
          ret_val = ADPL_SUCCESS;
          break;
        }
      }
    }
  ...
  {
    LOG_MSG_ERROR("Unable to open the dir even after %d retries", ADPL_FILE_NODE_OPEN_MAX_RETRY, 0, 0);
  }
...
)
{
   ...
   struct sockaddr           buf_addr;
   socklen_t                 addr_len=sizeof(struct sockaddr);
   char                      buffer;

  if ( ( nbytes = recvfrom(fd, buf, ADPL_MAX_BUF_LEN-1 , 0, &buf_addr, &addr_len)) <= 0 )
  {
    if ( nbytes == 0 )
    {
...
-------------------------------------------------------------------------*/
  ...
  if(strncmp(ro_baseband_value, BASEBAND_PROP_VAL, strlen(BASEBAND_PROP_VAL)) == 0)
  {
    ...
    if (IsLAFusionTargetUsingMHIPrime() == ADPL_SUCCESS)
    {
       /*Stopping ADPL daemon in case fusion target uses MHI prime.*/
       LOG_MSG_INFO1("ADPL:It is %s fusion target : %s", esoc_name, "Waipio/Kona/Hana Fusion", 0);
       LOG_MSG_INFO1("ADPL:Exiting from ADPL daemon",0, 0, 0);
       property_set("ctl.stop","vendor.dataadpl");
    }
    else
    {
       LOG_MSG_INFO1("ADPL:It is %s fusion target", esoc_name, 0, 0);
    }
  }
