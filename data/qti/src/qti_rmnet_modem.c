...
int retrieve_mhi_file(char **mhi_file_name)
{
  ...
  {
    /******************************************************************
          cat /sys/bus/esoc/devices/esoc0/esoc_link_info  --> 0308_01.01.00
          ls /sys/bus/mhi/devices/mhi_0308_01.01.00_QMI1/mhi_uci
              File name -----> mhi_0308_01.01.00_pipe_16
    ******************************************************************/
    snprintf(mhi_file_path, MHI_PATH_STRING_LEN, "%s%s%s",
             MDM_FILE_PATH_MHI_START, mhi_esoc_link_info, MDM_FILE_PATH_MHI_END);
    LOG_MSG_INFO1("Directory to read MHI file pipe info: %s", mhi_file_path, 0, 0);
    ...
    while (retry_count < MHI_DIR_OPEN_MAX_RETRY)
    {
      ...
      if(!mhi_dir)
      {
        LOG_MSG_ERROR("Unable to open the dir. Retrying again. Errno %d error msg=%s",
