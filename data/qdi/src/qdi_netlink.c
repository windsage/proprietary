...
typedef struct qdi_nl_getaddr_req_s
{
  ...
  struct rtattr extra_req __attribute__ ((aligned(NLMSG_ALIGNTO)));
  uint32 extra_mask;
...
)
{
  ...
  req.ifm.ifi_family = af_family;
  req.nlh.nlmsg_seq = ++rseq;
  req.extra_req.rta_type = IFLA_EXT_MASK;
  req.extra_req.rta_len = RTA_LENGTH(sizeof(uint32_t));
  req.extra_mask = RTEXT_FILTER_VF;
...
next_msg:
  ...
  QDI_LOG_HIGH("Waiting on recv message ...\n");
  ...
  QDI_LOG_HIGH("received response netlink msg type : nlh->type =%d\n", nlh->nlmsg_type);
  ...
  while (NLMSG_OK(nlh, buflen))
  {
    ...
    QDI_LOG_HIGH("Next Iface Info : netlink msg type : nlh->type =%d\n", nlh->nlmsg_type);
  }
