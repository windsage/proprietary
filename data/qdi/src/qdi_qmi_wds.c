...
)
{
 ...
 /* Lock the client data list mutex */
  ...
  if (req.pkt_data_handle == 0) {
    free(resp);
    QDI_LOG_ERROR("%s(): pkt_data_handle is 0, not sending stop_nw_if req", __func__);
    *bad_handle = TRUE;
    goto bail;
  }
...
/*===========================================================================
    FUNCTION  qdi_qmi_wds_get_bind_subscription
===========================================================================*/
/*!
@brief
  Gets the current subscription for the given configuration
...
/*=========================================================================*/
int
qdi_qmi_wds_get_bind_subscription
(
  qmi_client_type                     clnt_handle,
  wds_bind_subscription_enum_v01      *subs_id
)
{
  int rc;
  int ret = DSI_ERROR;
  wds_get_bind_subscription_req_msg_v01 req;
  wds_get_bind_subscription_resp_msg_v01 resp;

  QDI_LOG_TRACE_ENTRY;

  if (!subs_id)
  {
    QDI_LOG_ERROR("%s(): failed with null subs_id", __func__);
    goto bail;
  }

  *subs_id = WDS_DONT_CARE_SUBS_V01;

  memset(&req, 0, sizeof(req));
  memset(&resp, 0, sizeof(resp));

  rc = qmi_client_send_msg_sync(clnt_handle,
                                QMI_WDS_GET_BIND_SUBSCRIPTION_REQ_V01,
                                (void *)&req,
                                sizeof(req),
                                (void*)&resp,
                                sizeof(resp),
  ...
  if (resp.subscription_valid != TRUE)
  {
    QDI_LOG_ERROR("%s(): failed with resp.subscription_valid=%d",
                  __func__, resp.subscription_valid);
    goto bail;
  }

  *subs_id = resp.subscription;
